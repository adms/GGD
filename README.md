...
**開放（whitelisted）的英雄、技能與商店清單就直接印在下面、預設展開、不用點任何摺疊**。完整的全 113 名 / 全 662 技能 / 全 214 道具搬到 `docs/reference/roster.md`、`abilities.md`、`items.md`（各區塊都有連結），這樣 README 才不會又肥又被 GitHub 折疊。所有數字都從 repo 量出來，權威計數在 `content/manifest.json`（由 `pnpm content:build` 產生）。

| collection | docs | 說明 |
| --- | ---: | --- |
| `content/champions/` | **113** | 全部英雄文件（含未開放的）。其中 **48** 在**這台機器目前的**開放名單裡 |
| `content/abilities/` | **662** | 每英雄每 slot 一份，**一名英雄六個 slot**：天生技 `PASSIVE` **108**（w3x 的 `NN-00`，等級 1 就有；**3 名英雄真的沒有**）、Q/W/E/R 各 113、EX 102（**11 名沒有 EX**） |
| `content/items/` | **214** | 依 `craftRole` 標記（task #70）：**28 件最終合成武器**真能買（`final` 且有效果，23×1200g + 5×300g）+ **2 項服務**；三選一 draft 抽 **13 件 quest**、傳說寶玉抽 **14 件傳說**；其餘是 114 組件 / 8 代幣 / 43 無角色 / 6 件還沒 payload 的 final |
| `content/vfx/` | 435 | 這個數字跟著 VFX 綁定工作一直在動，以 `manifest.json` 為準 |
| `content/models/` | 119 | 目錄下另有 `_index.json` 與 `_standin-overrides.json`（底線開頭＝非 doc，不進 index） |
| 其餘 7 個 collection | 55 | augments 21 / config 11 / projectiles 5 / status-effects 5 / arenas 5 / skins 5 / loot-tables 3 |
| **合計** | **1598** | 這是**此刻**的磁碟實測；權威計數永遠是 `content/manifest.json` |
...
#### 開放名單 OPEN roster（48 名）— 角色 + 六個技能 slot

> 選角畫面看得到、bot 也會抽到的就是這些。這是**營運策展狀態**，不是程式常數：真相在 `data/curation/whitelist.json`，由 platform 的 `GET /api/v1/curation/whitelist` 提供、game-server 在建房時執行。來源：`data/curation/whitelist.json`（updatedAt `2026-07-22T15:49:20.043229Z`）
>
...
> ℹ️ 其中 **黑人牙膏**（`godie-ogld`）只有五條 —— 沒有 `NN-00` 天生技，**這是還原出來的事實，不是漏掉**（全 113 名裡共 5 名，逐一原因見 [`docs/reference/roster.md`](./docs/reference/roster.md)）。

**`godie-e001` 龍宮禮奈**（蟬在叫人壞掉 · fighter · 近戰） — 來自雛見澤的小女孩，喜歡把"好可愛"的東西帶回家。

- **天生·主動** 22-00 嗚鎖打!：爆發殺氣，對附近的敵方地面部隊造成150的傷害，並將他們嚇昏0.5…
- **Q** 22-01 鬼隱之擊：隱形並在一定的時間內提昇50%速度以暗殺目標，當攻擊時隱形術即告失…
- **W** 22-02 染血的柴刀：攻擊時有18%的機率可以使出會心一擊造成1.25倍的傷害。
- **E** 22-03 五吋釘：將一枚充滿詛咒的五吋釘射向敵方目標，造成瞬間350點傷害，並減緩6…
...
- **天生·被動** 20-00 銀色甲胄：Saber魔力化的銀色鎧甲讓Saber有相當良好的魔法抗性，當Sa…
- **Q** 20-02 感知能力：受到物理攻擊時有7%機率可迴避物理攻擊。
- **W** 20-01 風王結界：開啟風王結界，每次攻擊消耗 30法力，對目標造成額外10+力量*1…
- **E** 20-03 約束與勝利之劍：集結了人們的意念而形成的星星的結晶。是一把精鍊的神造兵裝，被譽為「…
- **R** 20-04 Avalon-永恆的理想鄉：Saber手中握著的石中劍的劍鞘，可以發動傳說中EX級寶具Aval…
- **EX** 20-002 解放.約束勝利劍MAX：理想鄉發動期間如果受到傷害，且魔力高於70%時，能給予敵人連續七次…

**`godie-e007` 天地志狼**（龍之子 · marksman · 遠程） — 本是平凡的國中二年級學生，因為母親項鍊的神奇力量來到三國時代，而被當做龍之子，…

- **天生·被動** 12-00 感應意脈：志狼矇眼修行後，領悟到感應人意識流動，神一般的技巧，可以使自身物理…
- **Q** 12-01 鬥仙術：將武道融入仙道而創出來所謂的不敗的武術，以念體攻擊敵人，造成150…
- **W** 12-02 仙氣．採藥：利用身體小周天循環恢復生命持續250點，並且除去身上任何附加法術狀…
- **E** 12-03 破凰之心-徒手空破山：使用破凰之力的志狼，在每次攻擊皆能施展空破山，劃出大氣之刃，造成1…
- **R** 12-04 龍氣爆發：凝聚體內的龍氣造成550點傷害，集氣每秒增加(敏捷*5)傷害，最多…
- **EX** 12-002 仙氣發勁：天地志狼在近身的最後必殺絕技，將身上所有的仙氣集中在手上瞬間爆發造…

**`godie-e008` 夏娜**（火霧戰士 · fighter · 近戰） — 身分為火霧戰士，為了存在感稀薄的人而戰。

- **天生·被動** 21-00 灼眼：灼眼的特殊體質可以讓夏娜看見隱藏在空間夾縫的所有人，即使在晚上仍能…
- **Q** 21-02 拔焰刀：拔抽覆蓋火焰的刀斬擊敵人造成300傷害，同時可以打昏目標0.5秒。…
...
- **天生·被動** 59-00 暴走：當初號機死亡的時候，有30%機率將會啟動暴走的機制保護自我，增加等…
- **Q** 59-01 吞噬：啟動吞噬狀態可讓初號機攻擊力增加20點傷害，攻擊時獲得生命回饋15…
- **W** 59-02 高週波短刀：高週波短刀攻擊時有10%機率增加75點破壞力，並有機會將敵人擊昏0…
- **E** 59-03 AT力場：一個被稱為神的聖域的力場，有40%降低所有施加在初號機身上的傷害3…
- **R** 59-04 野戰型陽電子砲：集中大量能源，以求一擊必殺的陽電子砲,能夠對前方一直線的敵人造成7…
- **EX** 59-001 完全暴走：完全失去控制的初號機，將暴走機率提升至75%，平時提升25%的閃躲…

**`godie-e00w` 櫻綻剎那**（神鳴流劍士 · fighter · 近戰） — 武道四天王之一，是在京都流傳已久的神鳴流劍術高手，也是個精通陰陽道的劍士。烏鴉…

- **天生·主動** 77-00 浮雲-旋一閃：神鳴流的體術招式之一，閃過敵人攻擊，抓住對手後，旋轉拋摔。（每次發…
- **Q** 77-01 百烈櫻華斬：用劍捲起一陣由內往外的旋風，給予複數敵人200點傷害，並暈眩1秒，…
- **W** 77-02 雷鳴劍：攻擊時有10%的機率可以使出會心一擊造成1.5倍的傷害，並且有10…
- **E** 77-03 GLADIARIA ALAT：GLADIARIA ALAT意指有翼的劍士，剎那不輕易展露烏鴉族…
- **R** 77-04 真-雷光劍：神鳴流決戰奧義，聚集大量雷電於劍上予以斬擊，給予擊中範圍500內敵…
- **EX** 77-002 御雷劍：使用從者道具"御雷劍"的剎那，在效果期間，其雷鳴劍發動機率上升至5…

**`godie-edem` 宇智波佐助**（寫輪眼復仇者 · fighter · 近戰） — 在忍者學校以第一名的成績畢業，是有名的"宇智波一族"的後代。復仇信念堅定，一心…

- **天生·被動** 45-00 寫輪眼：宇智波家族的血繼限界，洞察眼能夠看清所有的攻擊，迴避一般攻擊15%…
- **Q** 45-01 火遁-豪火龍之術：比豪火球之術更上一層的火遁忍術，將吐出的火焰轉化為龍形，給予範圍內…
- **W** 45-02 千鳥流：千鳥流就是將全身充滿千鳥的雷電，使周圍敵人受到雷電傷害75點生命值…
- **E** 45-03 千鳥：卡卡西傳授的技術。 原理是把查克拉集中在手上, 再以高速突刺作直線…
- **R** 45-04 哥哥：「我愚蠢的弟弟啊！憎恨吧！怨恨把！帶著你對我的仇恨，醜陋的苟延殘喘…
- **EX** 45-002 天照：擁有萬花筒寫輪眼的佐助，將可發動天照，造成週遭大範圍部隊每秒受到4…

**`godie-emfr` 涅吉。史普林。菲爾德**（魔法老師 · marksman · 遠程） — 英國某間魔法學校修行的首席畢業生涅吉，目前還只是個10歲的少年。他的目標是成為…

- **天生·主動** 15-00 天生法術書：記載多個涅吉可以使用的技能。
- **Q** 15-01 風精召喚：集合3個風的中等精靈幫助召喚者攻擊敵人，每單位風之精靈造成155點…
- **W** 15-02 沉睡之霧：沉睡之霧可以讓周圍施咒範圍內的敵人陷入睡眠狀態，持續3秒。
- **E** 15-03 雷電風暴：用魔法元素創造出來充滿雷元素的風暴攻擊範圍內的敵人給予400+智慧…
- **R** 15-04 千之雷：千之雷是涅吉。史普林，威力最強大的雷系魔法，缺點是念咒時間長，可以…
- **EX** 15-002 風花-武裝解除：涅吉所有招式中最令人臉紅心跳的絕招，可以將目標的衣服爆破(誤)，造…

**`godie-emns` 夜神月**（奇樂 · marksman · 遠程） — 英俊瀟灑，謹慎且機智，雖然極有女人緣但對異性不感興趣。高中三年級時在學校撿到了…

- **天生·主動** 44-00 機警：夜神月的機警，將智慧具現化成魔力護盾，以魔力做為代價，可抵擋90%…
- **Q** 44-01 死神之眼：與死神暫時交換眼睛來鎖定殺人目標，相對地以自己生命75%作交換，需…
- **W** 44-02 死神的規則：夜神月越了解死神界和筆記本的規則，越能掌控全局，將這份知識化為智慧…
- **E** 44-03 火車輾過：被死神之眼鎖定的部隊身旁將會出現莫名其妙的火車撞上，使附近的敵方部…
- **R** 44-04 心臟麻痺：被死神之眼指定的人將會心臟麻痺，短時間內將會行動緩慢以及喪失(目標…
- **EX** 44-002 交換筆記本：置死地而後生的大絕招，將筆記本暫時送給別人，讓自己跟別人現存生命作…

**`godie-etyr` 木乃香**（治癒系公主 · marksman · 遠程） — 父母家在京都的關西咒術協會，父親近衛詠春是關西咒術協會會長，祖父是關東魔法協會…

- **天生·主動** 14-00 召喚式神：召喚出式神跟隨木乃香，此式神周圍將附帶95點傷害，持續25秒。
- **Q** 14-01 東風繪扇、南風末廣：揮動兩把具有神奇魔力的扇子，使我方範圍300內單位解除異常狀態，並…
- **W** 14-03 魔力應援：木乃香強大的魔力使得400範圍內的友軍增加攻擊速度35%和移動速度…
- **E** 14-02 式神炸裂：讓每個召喚出來的式神自爆，使周圍敵人受到150+智慧*3點傷害，每…
...
- **天生·主動** 60-00 聖光盾：一個無法穿越的護盾將會環繞著這個部隊，保護它免於所有的傷害與法術攻…
- **Q** 60-01 科奇利族的迴旋鏢：使用科奇利族的迴旋標攻擊敵人，造成150點傷害。
- **W** 60-02 鎖鏈槍：勾住一個單位，將他拉到身邊來，範圍300，並且給予額外傷害150。
- **E** 60-03 海拉爾之盾的庇護：開啟後將受到的穿刺及法術傷害降低20%，但是會失去50%移動速度。
- **R** 60-04 迴旋斬：使用迴旋斬將附近敵人砍傷450點並且彈開震暈1秒，每彈開一次附加林…
- **EX** 60-002 絕光斬：在生命低於45%的瀕死狀態，林克的絕光斬將自動覺醒，每次攻擊都能產…

**`godie-h01n` 黑崎一護**（開外掛的死神 · fighter · 近戰） — 黑崎一護除了能夠看見靈之外，是個很普通的高中生，但是有一天，出現了一個叫做〔虛…

- **天生·被動** 79-00 靈壓：一護本身具有的強大靈力，初始法力值較一般人高，且此靈力產生的強大靈…
- **Q** 79-01 瞬步：以急快的速度移動至對方身旁並給予兩段式傷害，共計200點傷害。
- **W** 79-02 斬擊：將月牙的能量聚集在斬月上，在攻擊敵人的同時釋放出，給予目標額外10…
- **E** 79-03 月牙天衝：讓斬月吸收靈力的力量，再由刀尖放出超高密度靈壓的斬擊，造成一直線上…
- **R** 79-04 卍解：死神中的奧義，只有踏上死神最高點才可習得的技能，黑崎的卍解是將靈氣…
- **EX** 79-002 虛化：突破死神力量極限的一種技巧，可以藉此獲的更強大的力量，期間黑崎獲得…

**`godie-h01u` 呂布奉先**（亂世癿王者 · fighter · 近戰） — 呂布（公元151年—公元198年），字奉先，五原（今內蒙古包頭市）人。三國時代…

- **天生·被動** 80-00 飛將神弓：身經百戰的呂布，每殺死一名敵人將會在10秒內攻擊力增加10點，基礎…
- **Q** 80-01 天下無雙：亂世中呂布孤傲叛逆的個性，於戰鬥中追求最高戰力，犧牲3點防禦力，增…
- **W** 80-02 弒鬼神：氣貫長槍，以電光石火之速回旋猛刺，其勢威猛，似可弒殺鬼神，造成直線…
- **E** 80-03 鬼神烈戟：武將殺氣凝聚為無堅不摧的巨大畫戟形象，造成範圍500內敵人350+…
- **R** 80-04 赤兔咆哮：招喚赤兔馬人馬合一，於亂世中有一夫當關的氣勢，可以提升自身跑速10…
- **EX** 80-002 戰無不勝：演過古惑仔之戰無不勝的呂布，擁有過人的戰績，可以幫助他在戰場上發揮…

**`godie-h020` 莉娜因巴斯**（黑魔導士 · marksman · 遠程） — 自稱天才美少女魔導士的莉娜，加入愛與和平純粹只是為了可以獲得賞金，正如同她的綽…

- **天生·被動** 04-00 翔封界：警覺的莉娜平時都會開啟翔封界達到一定的魔法防禦效果，可抵擋負性魔法…
- **Q** 04-01 火球術：使用火球來攻擊敵人造成170點傷害，並附帶暈眩1.50秒的效果。
...
- **天生·被動** 89-00 憤怒的門牙：有天看完神奇寶貝的熊貓，領悟了拉達的大絕招，攻擊時有3%的機率可以…
- **Q** 89-01 憤怒的頭槌：當熊貓憤怒到極點的時候將會使出致命殺傷力的頭槌攻擊，但是牠總是忘記…
- **W** 89-02 憤怒的菊花：當敵人攻擊熊貓的時候，有3%機率會讓熊貓想起四川地震牠剛好在撇條的…
- **E** 89-03 憤怒的胸毛：傳說中拔下熊貓的一根胸毛，也就是熊貓胸毛，可以讓胸毛..是熊貓復活…
- **R** 89-04 憤怒的簡諧運動：[我心中有個夢...] 牠這樣說著，雖然牠搞不清楚是是夢遺還是夢三…
- **EX** 89-002 俄羅斯輪盤：在馬戲團打工的熊貓，為了保護自己不被從樹後偷襲養成了把周圍的樹木破…

**`godie-h02r` 妙蛙花**（種子神奇寶貝 · fighter · 近戰）

- **天生·主動** 90-00 寄生種子：將寄生種子丟向敵人，每秒造成50點傷害並吸收生命回饋給施法者，持續…
- **Q** 90-01 飛葉快刀：發出飛快的葉子進行攻擊，每秒對附近的敵人造成100點傷害，持續2秒。
...
**`godie-h02u` 草泥馬**（看似憂鬱的神獸 · fighter · 近戰） — 草泥馬是中國網民惡搞的十大神獸之一，被《紐約時報》等媒體認為是中國網民對於中國…

- **天生·被動** 92-00 憂鬱的眼神：草泥馬天生有著極富感性的憂鬱眼神，令觀者無不動容，讓草泥馬有18%…
- **Q** 92-01 臥草泥馬：這是較高等級的草泥馬才有的稱號，優秀的牠們被允許躺在朝尼族的水槽邊…
- **W** 92-03 狂草泥馬：草泥馬中最上等的一隻，是草泥馬中的馬王，要朝尼族騎術最好的人才能獲…
- **E** 92-02 消化液：羊駝在驚嚇或憤怒的時候，會從嘴巴裡噴出消化液攻擊敵人，造成範圍內敵…
- **R** 92-04 馬勒戈壁：草泥馬將內心的風景投射到戰場上，改變了戰場，每秒奪取周圍英雄的黃金…
- **EX** 92-002 最終戈壁：人人討厭的馬勒戈壁最終進化版，可以同時搶錢、搶糧、搶娘們(誤)，馬…

**`godie-hapm` Berserker**（海克力斯 · fighter · 近戰） — Berserker是希臘神話中著名的英雄海格力斯(Hercules) ，血統為…

- **天生·被動** 52-00 十二道試煉：海格力士克服了十二道試煉，獲得眾神給予不死之身的獎賞，可以使肉身有…
- **Q** 52-01 狂戰士之怒：進入狂戰士的狀態，可增加40點的額外傷害及裝甲4點，但是會降低攻擊…
- **W** 52-02 蹂躪編年史：迅速將目標抓回再暴力的丟出去，使之撞擊前方400距離處的敵人造成3…
- **E** 52-03 無銘斧劍：由希臘神殿石柱打造出來的斧劍，只有BARSERKER的怪力才能揮動…
- **R** 52-04 巨神一擊：燃燒巨斧向前衝刺400距離後揮出致命的一擊，對周圍400內的敵人造…
- **EX** 52-002 射殺百頭：利用手中的無銘戰斧重現最得意的寶具-射殺百頭，對目標連續100下的…

**`godie-hart` 克勞德**（最終幻想 · fighter · 近戰） — Cloud的名字暗示著他神祕，不清楚的過去以及他那不可預知的將來，為了過去奮戰…

- **天生·被動** 01-00 怒斬：攻擊時有15%的機會，造成175點額外傷害，並震昏敵人0.5秒。
- **Q** 01-01 凶斬：使用大刀在敵人身上刻下凶字造成150傷害，並且使人暫時無法行動1秒。
- **W** 01-02 隕石擊：招喚隕石擊落攻擊區域內的敵人造成100傷害並且跳躍到該範圍給予斬殺…
- **E** 01-03 畫龍點睛：快速迴旋巨劍產生龍捲風砍殺敵人造成450點傷害，並降低目標裝甲3點…
...
- **天生·被動** 07-00 獸化心靈：讓獸矛的持有者每殺死8個部隊增加1點靈敏。
- **Q** 07-01 臨、兵、鬥：可抵擋對方負性魔法。
- **W** 07-02 者、皆、陣：以超快的速度衝刺砍殺一直線上的敵人使得血流成河，砍殺造成225+力…
- **E** 07-03 列、在、前：用盡全身力氣跳起落下斬擊使得大地震動，區域內敵人皆受到(力量*2)…
...
- **EX** 28-002 普烏死亡：effects: applyBuff
...
- **E** 02-03 魂飛魄散：桔梗將死魂收集的妖力爆發噴射，造成一直線上的敵人350點傷害。
- **R** 02-04 百鬼夜行：招換6位式神，吸取附近敵軍部隊的生命能量。當他們回到身邊時，他們會…
...
- **Q** 42-01 凍結的大地：以一波冰雪轟擊敵方部隊，造成目標250點的傷害，以及周圍目標150…
- **W** 42-02 吸血祭品：犧牲選定的友方不死部隊，並將它50%的生命點數轉化為吸血鬼的生命力…
- **E** 42-03 暗夜吹雪：依文潔琳的得意技之一，使用高等冰系黑魔法造成一直線敵方500點傷害。
...
- **Q** 57-01 空氣砲：小叮噹的空氣砲可以造成一直線上敵方部隊250傷害並且擊退附近敵人。
- **W** 57-03 複製鏡：未來的法寶，可將鏡前的人或物加以複製，複製出來的人物擁有與本尊相同…
- **E** 57-02 任意門：使用任意門可以傳送到地圖探索過的任何一點，準備時間為3秒。
- **R** 57-04 竹蜻蜓：給目標帶上竹蜻蜓，而四周會產生共12道龍捲風每個龍捲風都有著150…
- **EX** 57-002 時光機：可以穿梭時空回到過去的超先進科技。

**`godie-n00p` 南野秀一**（妖狐藏馬 · marksman · 遠程） — 魔界高級妖魔轉生寄宿為人類，為控制魔界植物的支配者。

- **天生·被動** 18-00 薔薇荊棘之刃：薔薇荊棘之刃可以讓葉片像刀刃一般劃過多個敵人，最多可同時攻擊3個敵…
- **Q** 18-01 風華圓舞陣：周圍內飄逸出10片的花瓣，每片花瓣碰觸到敵方時造成50傷害，持續8…
- **W** 18-02 寄生種子：把魔界凶惡植物寄生在敵人身上造成75點傷害，若干時間後將會侵蝕敵人…
...
**`godie-n01c` 勇者小呆**（傳說的龍騎士 · fighter · 近戰） — 傳說中打敗魔王的勇者，為神魔人混血創造的龍騎士，為了維護世界和平，再度使用高超…

- **天生·被動** 08-00 龍紋記憶：繼承龍紋章後，歷代累積在龍騎士身上的作戰經驗，使得龍騎士能降低穿刺…
- **Q** 08-01 雙龍紋：繼承父親巴藍以及自身的龍紋章，使用雙龍紋將會使小呆的增加50點傷害…
- **W** 08-02 萊丁快速劍：使用萊丁(閃電咒文)複合阿邦快速劍使出的魔法劍術，造成150點傷害…
- **E** 08-03 龍鬥氣砲咒文：龍騎士的得意技之一，發動龍紋章之力使出咒文迫擊砲，造成攻擊線地面部…
- **R** 08-04 阿邦快速劍X：小呆獨自思考和特訓中，所創出的新阿邦式快速劍，將A式(Arrow)…
- **EX** 08-002 龍魔人：變身成為神魔龍融合的最強生物-龍魔人，使得小呆強化全能力15點，防…
...
- **Q** 53-01 獸王牙操彈：借獸王之力施展之咒文，可依施法者思考操縱攻擊，光帶周圍每秒造成25…
- **W** 53-02 強化炸彈陣：施展火焰爆裂魔法傷害敵方部隊，燒傷150點。
- **E** 53-03 破法對咒：使用強大的魔力展開結界承受住範圍500內650點的法術傷害，持續6…
- **R** 53-04 暴爆咒：火系黑魔法的最高等攻擊法術，需要的魔力相當驚人，賢者等級以上才能施…
- **EX** 53-002 恐懼力量：加強吸收恐懼負面情緒的能量，當傑洛士施展暴爆咒時，能額外增加周圍敵…

**`godie-o00x` 悟空**（超級賽亞人 · fighter · 近戰） — 七龍珠中不死的傳奇英雄，每當世界有難的時候總會亂入(!?)。
...
- **E** 58-03 就決定是你了!小智：對著敵方的部隊投出皮卡丘痛恨已久的低能小智缺，造成350傷害之後還…
...
- **Q** 72-01洗刷刷：召喚一陣雨季的暴風來攻擊對方的部隊，在350範圍造成150點的傷害…
- **W** 72-02 黑人牙菌斑：釋放黑人牙菌斑病毒，被感染的部隊會每秒受到20點傷害並減緩攻擊移動…
- **E** 72-03 超亮白：使用超亮白牙膏攻擊目標，帶腐蝕性的牙膏會侵蝕目標6點裝甲，亮白效果…
- **R** 72-04 黑化：黑化後的黑人牙膏將會在短時間內使出敵我皆傷的噴牙膏攻擊，對周圍所有…
- **EX** 72-002 億萬星殞落：黑人牙膏的最終能力，可以大範圍召喚流星進行攻擊，其規模毀天滅地，每…
...
- **E** 30-03 痴漢火焰：讓一個敵方單位身陷痴漢火焰之中，造成每秒16點的持續性的傷害，並使…
- **R** 30-04 電車之狼衝擊：在臭作肛了新幹線車長之時，導致車長興奮過度讓新幹線脫軌衝了出來，造…
- **EX** 30-002 變態紳士：當臭作變態指數達到顛峰之時，攻擊身上有酒精灌腸效果的敵人將額外敵人…

**`godie-osam` 殺生丸**（犬妖 · fighter · 近戰） — 犬夜叉同父異母的哥哥。他是完全的妖怪，能力要強得多，性格非常冷酷殘忍，對付自己…

- **天生·主動** 34-00 靈魂吞噬：身為妖怪中貴公子的殺生丸，喜好汲取敵人靈魂的生命力來恢復自身生命值…
- **Q** 34-01 風華之爪：殺生丸以風華之爪攻擊，導致這一擊之中10%的傷害會擊穿對方，並造成…
- **W** 34-02 合氣斬：攻擊時有10%的機率造成1.5倍的傷害。
- **E** 34-03 爆碎丸：新生的爆碎丸，有15%的機會，對目標敵人造成90點額外傷害，並擊昏…
...
- **天生·主動** 39-00 無名神風流-玄武：提升鬼畜狂刀的攻擊力30點，生命回復12點和防禦8點，來增加戰場耐…
- **Q** 39-01 無名神風流-白虎：村正臨死前教授的真無名神風
- **W** 39-02 無名神風流-朱雀：村正臨死前教授的真無名神風
- **E** 39-03 無名神風流-蛟龍：你聽到神風的清響聲了嗎?
...
- **Q** 74-01 獄門：傳說中刺死愛麗絲的必殺技，雖然命中範圍小，卻具有強大殺傷力，造成小…
- **W** 74-02 八刀一閃：極快的速度衝刺到敵人面前，給予週遭敵人敏捷*3+150傷害。
- **E** 74-03 闇之天使：抽取星球之力轉換為魔晃能量，瞬間爆發的威力造成150點傷害，共8道…
- **R** 74-04 最終殞落星：招喚災難彗星造成地面嚴重傷害，每顆隕石造成650點傷害，總共1顆隕…
- **EX** 74-002 超新星：在八刀一閃施展後瞬間施展獄門，將會招喚超新星造成巨大的範圍1200…

**`godie-u00k` 死之王**（邪惡意念集合體 · marksman · 遠程） — 所有邪惡的聚合體，從上古時代就誕生的惡魔，與飛鼠先生一戰後魂飛魄散，在此次戰役…

- **天生·被動** 71-00 暗夜契約：死之王在夜間時，移動速度將會提升100%，生命回復速度提升30點，…
- **Q** 71-01 死亡隕落：死之王可幻化成代表死亡的隕石造成敵方250點傷害。
...
**`godie-u00l` 拳四郎**（北斗之鼠 · fighter · 近戰） — 北斗神拳的唯一傳人，使用難以置信的秘穴(!?)拳法致敵人於死地。由於北斗星是不…
...
- **E** 25-03 北斗百裂拳：拳四郎得意絕技之一，以連續拳頭攻擊區域內的敵人造成單體攻擊300+…
...
- **Q** 76-01 伸縮自如的橡膠戰斧：利用橡膠果實的力量，將腿拉長，由上而下重擊敵人，造成傷害250點，…
- **W** 76-02 伸縮自如的橡膠火箭砲：藉由橡膠果實的能力，將手臂伸長，長距離給於敵人200點傷害，並且將…
...
- **天生·被動** 78-00 銅皮鐵骨：有如金屬一樣的皮膚，讓刀槍難以傷害他，遭受物理攻擊時有15%機率降…
- **Q** 78-01 斬鐵拳：基廉列克的拳頭在攻擊時有10%機率增加75點破壞力，並有機會將敵人…
- **W** 78-02 地走龍牙破：對付裝甲戰車時發動之必殺技，挖地道到目標上方後突襲，使該範圍受到2…
- **E** 78-03 廬山昇龍破：使數百輛警車和警察仆街的超必殺技，可對附近敵方單位造成450點傷害。
- **R** 78-04 死亡噴射肘擊：在基廉列克發怒的時候，將會使出意外的致命一擊，超快速速飛奔到敵人面…
- **EX** 78-002 加速爆體：暴走的監獄兔接近無敵狀態，將可抵擋50%法術和穿刺傷害，並有機率增…

**`godie-u010` 飛影**（邪眼師 · fighter · 近戰） — 在魔界中有名的盜賊妖怪，除了是一位邪王炎殺拳的高手之外，也是一位用劍的高手。為…

- **天生·主動** 38-00 邪眼全開：將移植到身上的邪眼全部打開，使得妖氣爆增，攻擊速度提升100%和移…
- **Q** 38-01 邪王炎殺劍：招喚魔界火炎盤旋在劍上，使劍在瞬間變得鋒利無比，衝刺砍殺一直線上的…
- **W** 38-02 邪王炎殺煉獄焦：將火燄集中於拳頭上，以連續拳頭攻擊區域內的敵人造成250傷害。
...
**`godie-u01u` 索隆**（三刀流劍士 · fighter · 近戰） — 夢想成為世界第一的大劍客，使用自創的三刀流劍術擊遍天下劍客。為了在戰鬥中追尋武…
...
- **EX** 11-002 武裝色霸氣：能夠提昇個人的防禦力，作用猶如看不見的盔甲；更可演化為攻擊力，進而…
...
- **W** 37-03 災難之牆：釋放一道火牆，對面前所有的敵人每秒造成450點的傷害，火牆持續3秒。
...
> 📖 **完整 113 名英雄**（含 65 名未開放）與逐欄資料（開放旗標、技能 id、攻擊類型…）在 [`docs/reference/roster.md`](./docs/reference/roster.md)。

*由 `pnpm docs:readme` 從 contentVersion `cv_1c68c834dac0` 產生。 開放 48 / 全 113 名。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:roster -->

<!-- BEGIN GENERATED:abilities -->
#### 技能 abilities（全 662 個；開放英雄的 287 個）

> **開放英雄的每一個技能，都已經印在上面的開放名冊裡**（每名英雄六條：天生 ＋ Q/W/E/R/EX，含名稱與一行效果）。這裡不再重印一次，只放全表的統計與連結，讓 README 保持精簡。
>
> 每個英雄每個 slot 一份：天生 PASSIVE 108 · Q 113 · W 113 · E 113 · R 113 · EX 102。**`slot` 有六種**，`PASSIVE`（天生技，w3x 的 `NN-00`）跟 Q/W/E/R/EX 一樣是一個 slot，而且**等級 1 就擁有**；共 108 份（48 純被動 ＋ 60 有冷卻的天生主動），由 champion doc 的 `passiveAbility` 指到 `<championId>.passive`。
>
...
> 📖 **全 662 個技能的逐欄表**（id、名稱、slot、型態、編號、擁有英雄、開放旗標、完整短效果）在 [`docs/reference/abilities.md`](./docs/reference/abilities.md)；互動版在 <http://localhost:39527/#codex>。

*由 `pnpm docs:readme` 從 contentVersion `cv_1c68c834dac0` 產生。 開放英雄技能 287 / 全 662 個。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:abilities -->

<!-- BEGIN GENERATED:items -->
#### 商店貨架 + 抽卡池（能實際取得的道具）

> 全部 214 件道具依 `craftRole` 標記分類（task #70）。**真正能買的只有 28 件最終合成武器＋2 項服務**；三選一 draft 抽 13 件任務道具、傳說寶玉抽 14 件傳說。其餘 157 件是配方組件、代幣、殘件或還沒 payload 的 final，不會單獨出現在商店或抽卡。
>
...
##### 🛒 商店貨架 shop shelf — `craftRole:final` 且有效果（28）

真正能用金幣買的最終合成武器。白名單啟用時可能再縮小，但永遠不會放進非 final 的東西（`shop.ts:110`）。

| id | 名稱 | 價格 | 開放 | 屬性 modifiers | 被動 |
|---|---|---|---|---|---|
| `godie-i00f` | 霸王槍 | 1200g | ✅ | 攻擊力 +30.2 · 生命 +222 | onBasicAttack→damage |
| `godie-i00i` | 炎龍巨弩 | 1200g | — | 護甲 +4.5 · 攻速 +30% · 攻擊力 +37.6 | — |
| `godie-i00j` | 奇門盾甲 | 1200g | ✅ | 生命 +186 · 回血 +3.9 | — |
| `godie-i016` | 晨曦之光 | 300g | — | 生命 +39 · 攻擊力 +1.9 · 魔力 +23 | — |
| `godie-i018` | 朗基努斯之槍 | 1200g | — | 護甲 +3.8 · 攻速 +25.3% · 攻擊力 +12.6 · 生命 +278 | — |
| `godie-i01j` | 靈魂魔石 | 1200g | — | 生命 +217 · 魔力 +136 | — |
| `godie-i01o` | 死神裝束 | 1200g | — | 攻速 +33.3% · 生命 +55 · 攻擊力 +2.8 · 魔力 +33 · 移速 +0.33 | — |
| `godie-i01v` | 螺旋劍 | 1200g | — | 攻擊力 +45.6 · 生命 +54 · 魔力 +32 | — |
| `godie-i027` | 光魔杖 | 1200g | ✅ | 法強 +93.2 · 魔力 +279 · 攻擊力 +23.3 | onBasicAttack→damage |
| `godie-i02e` | 狂暴軒轅劍 | 1200g | — | 攻擊力 +32.2 · 生命 +202 | onBasicAttack→damage |
| `godie-i02r` | 奇蹟之墜 | 1200g | — | 法強 +28.9 · 魔力 +87 · 生命 +174 | — |
| `godie-i031` | 天生牙 | 1200g | — | 攻擊力 +8.2 · 生命 +181 | — |
| `godie-i039` | 幻之匕首 | 1200g | — | 護甲 +5.2 · 攻速 +56.9% | — |
| `godie-i03b` | 真．雅典娜的驚嘆號 | 1200g | — | 法強 +81.6 · 魔力 +245 · 回魔 +81.6% | — |
| `godie-i03d` | 光明虎徹 | 300g | — | 生命 +39 · 攻擊力 +1.9 · 魔力 +23 | — |
| `godie-i03f` | 甘豆腐之袍 | 300g | — | 回魔 +22.3% · 生命 +59 · 護甲 +1.2 | — |
...
| `godie-i04d` | 冰晶虎魄 - 改 | 1200g | — | 攻擊力 +5.5 · 生命 +122 · 法強 +27.7 · 魔力 +83 | onBasicAttack→applyStatus |
| `godie-i04i` | 厄夜鐮刀 | 1200g | — | 回魔 +300% | — |
| `godie-i05h` | 失心匕首 | 1200g | — | 攻擊力 +14.1 · 攻速 +28.2% | — |
| `godie-i05o` | 刺針 | 300g | — | 攻擊力 +5.2 · 生命 +11 · 魔力 +7 | — |
| `godie-i067` | 惡夢魔王碎片 | 1200g | — | 回魔 +229.8% · 魔力 +689 | — |
| `godie-i06d` | 斬龍刀 | 1200g | ✅ | 護甲 +3.6 · 攻速 +23.9% · 暴擊率 +0.18 · 暴擊傷害 +0.45 · 攻擊力… | — |
| `godie-i06f` | 月神槍 | 1200g | ✅ | 法強 +99 · 魔力 +297 · 護甲 +13.2 · 回血 +8.29 | — |
| `godie-i06i` | 炎神弩 | 1200g | ✅ | 攻擊力 +35.1 · 護甲 +5.3 · 攻速 +35.1% | — |
...
| id | 名稱 | 價格 | 開放 | 屬性 modifiers | 被動 |
|---|---|---|---|---|---|
| `godie-i004` | 魔戒 | 抽卡 | — | 生命 +39 · 攻擊力 +1.9 · 魔力 +23 | — |
| `godie-i00z` | 四魂之玉 | 抽卡 | — | 生命 +154 · 攻擊力 +7.7 · 魔力 +93 | — |
| `godie-i01k` | 火焰泰坦腰帶 | 抽卡 | — | 攻擊力 +8 · 生命 +175 · 護甲 +2.1 | — |
| `godie-i01n` | 天堂之劍 | 抽卡 | — | 生命 -500 · 暴擊率 +0.03 · 暴擊傷害 +48.25 | — |
| `godie-i01s` | 仙后座 | 抽卡 | — | — | — |
| `godie-i02h` | 戰旗 | 抽卡 | — | — | — |
| `godie-i02j` | 復仇之袍 | 抽卡 | — | — | — |
| `godie-i02k` | 惡魔吉他 | 抽卡 | — | — | — |
...
| `godie-i06j` | 獸人船長十字鎬 | 抽卡 | — | 攻擊力 +26 | — |
| `godie-i06n` | 老衲的棒子 | 抽卡 | — | 攻擊力 +26 | onBasicAttack→applyStatus |

##### 💎 傳說池 legendary pool（14）

`content/loot-tables/legendary-weapons.json`，等權重。只能從武器三選一或 2400g 傳說寶玉取得。

| id | 名稱 | 價格 | 開放 | 屬性 modifiers | 被動 |
|---|---|---|---|---|---|
| `godie-i000` | 丈八蛇矛 | 抽卡 | — | 攻擊力 +28.7 · 生命 +237 | — |
| `godie-i007` | 妖刀村正 | 抽卡 | ✅ | 吸血 +0.36 · 攻擊力 +43.2 | — |
| `godie-i00l` | 落魂的嗜血劍 | 抽卡 | — | 吸血 +0.23 · 攻擊力 +46.3 | — |
| `godie-i00u` | 名刀-天狼 | 抽卡 | — | 攻速 +123.2% | — |
| `godie-i012` | 熾天使之弓 | 抽卡 | — | 護甲 +16.3 · 攻速 +108.5% | onBasicAttack→damage |
| `godie-i013` | 八取武士刀 | 抽卡 | — | 攻擊力 +32.2 · 生命 +202 | onBasicAttack→damage |
| `godie-i014` | 天叢雲劍 | 抽卡 | — | 攻擊力 +38.8 · 生命 +111 · 魔力 +67 | — |
| `godie-i01d` | 死之王的長槍 | 抽卡 | — | 生命 +308 · 攻擊力 +15.4 · 魔力 +185 | — |
| `godie-i01g` | 貫雷槍 | 抽卡 | — | 護甲 +3.8 · 攻速 +25.3% · 攻擊力 +12.6 · 生命 +278 | — |
| `godie-i02x` | 斬岩刃 | 抽卡 | ✅ | 攻擊力 +30.2 · 生命 +222 | onBasicAttack→damage |
| `godie-i04v` | 正義之杖 | 抽卡 | — | 生命 +308 · 攻擊力 +15.4 · 魔力 +185 | — |
| `godie-i06e` | 月牙魔杖 | 抽卡 | — | 魔抗 +200 | — |
| `godie-i06g` | 殺豬刀 | 抽卡 | — | 護甲 +4.5 · 攻速 +30% · 攻擊力 +37.6 | — |
| `godie-i06s` | 龍騎士之劍 | 抽卡 | ✅ | 護甲 +3.4 · 攻速 +23% · 攻擊力 +34.5 · 暴擊率 +0.17 · 暴擊傷害 +0… | — |

> ⚠️ 另有 **6 件 `final` 沒有 payload**（風行天衣、雷神之鎚、盾甲天書、黑色魔書、嗜血邪書、天地崩裂魔杖）：主動效果 schema 還裝不下（#56），所以商店拒賣。詳見完整表。

> 📖 **全 214 件道具依 craftRole 的完整分類表**（component 107 / token 8 / none 36 …）在 [`docs/reference/items.md`](./docs/reference/items.md)。

*由 `pnpm docs:readme` 從 contentVersion `cv_1c68c834dac0` 產生。 可取得 57 / 全 214 件。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:items -->
