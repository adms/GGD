...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 倍率讀 `content/config/combat-env.json`（version 8）。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 輪替 12 / 全 13 張。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 級距讀 `content/config/*-tiers.json`（7 張表）、母體讀 `docs/平衡錨點量測.md`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:tiers -->
...
- **E** 22-03 五吋釘：將一枚充滿詛咒的五吋釘射向敵方目標，造成瞬間600點傷害，並減緩6…
...
- **Q** 21-02 拔焰刀：拔抽覆蓋火焰的刀斬擊敵人造成600傷害，同時可以打昏目標0.5秒。…
...
- **天生·主動** 14-00 召喚式神：召喚出式神跟隨木乃香，此式神周圍將附帶600點傷害，持續25秒。
...
- **Q** 01-01 凶斬：使用大刀在敵人身上刻下凶字造成600傷害，並且使人暫時無法行動1秒。
- **W** 01-02 隕石擊：招喚隕石擊落攻擊區域內的敵人造成100傷害並且跳躍到該範圍給予斬殺…
- **E** 01-03 畫龍點睛：快速迴旋巨劍產生龍捲風砍殺敵人造成600點傷害，並降低目標裝甲3點…
...
- **天生·主動** 90-00 寄生種子：將寄生種子丟向敵人，每秒造成600點傷害並吸收生命回饋給施法者，持…
- **Q** 90-01 飛葉快刀：發出飛快的葉子進行攻擊，每秒對附近的敵人造成600點傷害，持續2秒。
...
- **Q** 04-01 火球術：使用火球來攻擊敵人造成600點傷害，並附帶暈眩1.50秒的效果。
...
- **W** 07-02 者、皆、陣：以超快的速度衝刺砍殺一直線上的敵人使得血流成河，砍殺造成600+力…
...
- **天生·主動** 48-00 石化之眼：開啟石化之眼，將Rider附近小範圍的部隊予以石化，持續4秒。
...
- **E** 02-03 魂飛魄散：桔梗將死魂收集的妖力爆發噴射，造成一直線上的敵人600點傷害。
...
- **Q** 42-01 凍結的大地：以一波冰雪轟擊敵方部隊，造成目標600點的傷害，以及周圍目標150…
...
- **Q** 57-01 空氣砲：小叮噹的空氣砲可以造成一直線上敵方部隊600傷害並且擊退附近敵人。
- **W** 57-03 複製鏡：未來的法寶，可將鏡前的人或物加以複製，複製出來的人物擁有與本尊相同…
- **E** 57-02 任意門：使用任意門可以傳送到地圖探索過的任何一點，準備時間為3秒。
- **R** 57-04 竹蜻蜓：給目標帶上竹蜻蜓，而四周會產生共12道龍捲風每個龍捲風都有著600…
...
- **W** 08-02 萊丁快速劍：使用萊丁(閃電咒文)複合阿邦快速劍使出的魔法劍術，造成600點傷害…
...
- **W** 18-02 寄生種子：把魔界凶惡植物寄生在敵人身上造成600點傷害，若干時間後將會侵蝕敵…
...
- **Q** 53-01 獸王牙操彈：借獸王之力施展之咒文，可依施法者思考操縱攻擊，光帶周圍每秒造成60…
- **W** 53-02 強化炸彈陣：施展火焰爆裂魔法傷害敵方部隊，燒傷150點。
- **E** 53-03 破法對咒：使用強大的魔力展開結界承受住極小範圍內650點的法術傷害，持續6秒。
...
- **E** 58-03 就決定是你了!小智：對著敵方的部隊投出皮卡丘痛恨已久的低能小智缺，造成600傷害之後還…
...
- **Q** 72-01洗刷刷：召喚一陣雨季的暴風來攻擊對方的部隊，在350範圍造成600點的傷害…
- **W** 72-02 黑人牙菌斑：釋放黑人牙菌斑病毒，被感染的部隊會每秒受到600點傷害並減緩攻擊移…
...
- **E** 30-03 痴漢火焰：讓一個敵方單位身陷痴漢火焰之中，造成每秒600點的持續性的傷害，並…
...
- **E** 74-03 闇之天使：抽取星球之力轉換為魔晃能量，瞬間爆發的威力造成600點傷害，共8道…
...
- **Q** 71-01 死亡隕落：死之王可幻化成代表死亡的隕石造成敵方600點傷害。
...
- **Q** 76-01 伸縮自如的橡膠戰斧：利用橡膠果實的力量，將腿拉長，由上而下重擊敵人，造成傷害600點，…
- **W** 76-02 伸縮自如的橡膠火箭砲：藉由橡膠果實的能力，將手臂伸長，長距離給於敵人600點傷害，並且將…
- **E** 76-03 伸縮自如的槍亂打：利用拳頭迅速的攻擊範圍400內敵人造成暈眩1.5秒及傷害600點。
...
- **E** 78-03 廬山昇龍破：使數百輛警車和警察仆街的超必殺技，可對附近敵方單位造成600點傷害。
...
- **W** 37-03 災難之牆：釋放一道火牆，對面前所有的敵人每秒造成600點的傷害，火牆持續3秒。
...
- **E** 25-03 北斗百裂拳：拳四郎得意絕技之一，以連續拳頭攻擊區域內的敵人造成單體攻擊600+…
...
- **W** 38-02 邪王炎殺煉獄焦：將火燄集中於拳頭上，以連續拳頭攻擊區域內的敵人造成600傷害。
...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 開放 49 / 全 71 名。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:roster -->

<!-- BEGIN GENERATED:abilities -->
#### 技能 abilities（全 420 個；開放英雄的 293 個）
...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 開放英雄技能 293 / 全 420 個。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 可取得 130 / 全 142 件。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:items -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_2c6cf2ba19a0` 產生。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:grail -->
...
|---|--:|---|--:|---|--:|
| `applyBuff` | 108 | `applyStatus` | 94 | `blink` | 15 |
...
### 狀態標籤 —— 103 個（`content/status-effects/*.json` 逐檔數出來）

標籤是**開放**詞彙（自由字串），條件葉 `status` 的類別分支就是查它：

`accuracy-down`×2 `ai-override`×3 `antiheal`×2 `armor-break`×1 `armor-down`×1 `attack-debuff`×2 `attack-denied`×8 `auto-target`×1 `bankai`×1 `banked`×2 `berserk`×1 `blind`×1 `buff`×14 `burn`×1 `burnstun`×1 `cast-denied`×7 `cc`×21 `channel`×1 `charmed`×1 `combo`×1 `confusion`×1 `cooldown`×1 `counter`×2 `curse`×1 `damage-bank`×2 `debuff`×26 `devour-cooldown`×1 `disable`×11 `dot`×1 `elemental`×1 `empowered`×1 `empowered-next`×1 `fang-stun`×1 `fear`×1 `fire`×2 `flee`×1 `form`×2 `frenzy`×1 `friendly-fire`×1 `generic`×1 `grail-strengthened-projection`×1 `grail-wish`×1 `grief-seed-charge`×1 `grievous-wounds`×1 `hard-cc`×6 `haste`×2 `heal-block`×1 `heal-down`×2 `immobilize`×1 `ingredient`×1 `internal-cooldown`×1 `lifesteal-down`×2 `lifesteal-up`×1 `light-wand-banked`×1 `magic-break`×1 `magic-resist-down`×1 `magical`×1 `mana-banked`×2 `marker`×2 `mechanism-on-card`×4 `millennium-plot-armor`×1 `miss`×2 `moon-combo`×1 `move-denied`×9 `move-speed-down`×7 `named-variant`×6 `nen-banked`×1 `next-attack`×1 `no-heal`×1 `no-stat-change`×1 `numbness`×1 `omnislash-lock`×1 `omnislash-perform`×1 `paralysis`×1 `physical`×1 `projectile`×1 `rage`×1 `red-comet`×1 `regen-down`×2 `resist-down`×2 `root`×1 `self`×1 `self-lock`×1 `shred`×2 `slow`×7 `slow20`×1 `slow25`×1 `slow30`×1 `slow35`×1 `slow40`×1 `slow50`×1 `slow60`×1 `soft-cc`×9 `stat-down`×9 `stat-up`×1 `stun`×7 `timed-window`×2 `trial-stun`×1 `triforce-courage`×1 `uncontrollable`×3 `united-states-of-smash`×1 `witch-form`×1 `wound`×2
