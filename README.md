...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 倍率讀 `content/config/combat-env.json`（version 9）。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 輪替 12 / 全 13 張。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 級距與 `appliesTo` 讀 `content/config/stat-normalization.json`、上限讀 `stat-caps.json`、成長現況現場數 `content/champions/`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 級距讀 `content/config/*-tiers.json`（9 張表）、母體讀 `docs/平衡錨點量測.md`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:tiers -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 開放 49 / 全 71 名。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 開放英雄技能 293 / 全 421 個。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 可取得 130 / 全 142 件。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:items -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_ee068f3903f5` 產生。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:grail -->
...
|---|--:|---|--:|---|--:|
| `applyBuff` | 61 | `applyStatus` | 65 | `blink` | 13 |
| `carry` | 0 | `chainLightning` | 2 | `championForm` | 12 |
| `comboStrikes` | 1 | `consumeStatus` | 0 | `convertTeam` | 0 |
| `cycleBuff` | 1 | `damage` | 85 | `damageArea` | 50 |
| `damageLine` | 19 | `dash` | 8 | `delayed` | 27 |
| `devour` | 4 | `dispel` | 12 | `dot` | 8 |
| `evasion` | 0 | `eventValueConversion` | 5 | `extendBuff` | 1 |
| `floatingText` | 9 | `grantAttribute` | 5 | `grantGold` | 2 |
| `grantXp` | 1 | `heal` | 2 | `interruptCast` | 0 |
| `invulnerable` | 13 | `knockback` | 17 | `leap` | 5 |
| `manaBarrier` | 1 | `modifyCooldown` | 13 | `proxyCast` | 6 |
| `pull` | 0 | `randomArea` | 7 | `restore` | 22 |
| `revive` | 1 | `screenFlash` | 6 | `screenShake` | 17 |
| `shield` | 11 | `shieldBreak` | 2 | `spawnModelFx` | 55 |
| `spawnProjectile` | 4 | `spawnVfx` | 54 | `spendHealth` | 0 |
| `spendMana` | 4 | `summon` | 2 | `swapResource` | 1 |
| `taunt` | 2 | `timeStop` | 0 | `trap` | 0 |
| `weightedBranch` | 10 |  |  |  |  |
