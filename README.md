...
#### 全域倍率表 `combat-env`（38 項 · 21 項不是 1.0）
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 倍率讀 `content/config/combat-env.json`（version 9）。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 輪替 12 / 全 13 張。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 級距與 `appliesTo` 讀 `content/config/stat-normalization.json`、上限讀 `stat-caps.json`、成長現況現場數 `content/champions/`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 級距讀 `content/config/*-tiers.json`（7 張表）、母體讀 `docs/平衡錨點量測.md`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:tiers -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 開放 49 / 全 71 名。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 開放英雄技能 293 / 全 421 個。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 可取得 130 / 全 142 件。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:items -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_e6a687452570` 產生。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:grail -->
...
|---|--:|---|--:|---|--:|
| `applyBuff` | 105 | `applyStatus` | 98 | `blink` | 15 |
| `carry` | 0 | `chainLightning` | 2 | `championForm` | 23 |
| `comboStrikes` | 1 | `convertTeam` | 0 | `cycleBuff` | 1 |
| `damage` | 163 | `damageArea` | 40 | `damageLine` | 12 |
| `dash` | 8 | `delayed` | 14 | `devour` | 3 |
| `dispel` | 8 | `dot` | 3 | `evasion` | 0 |
| `eventValueConversion` | 5 | `extendBuff` | 1 | `floatingText` | 5 |
| `grantAttribute` | 5 | `grantGold` | 1 | `heal` | 10 |
| `invulnerable` | 16 | `knockback` | 12 | `leap` | 8 |
| `manaBarrier` | 1 | `modifyCooldown` | 13 | `proxyCast` | 6 |
| `pull` | 0 | `randomArea` | 7 | `restore` | 18 |
| `revive` | 1 | `screenFlash` | 3 | `screenShake` | 11 |
| `shield` | 10 | `shieldBreak` | 2 | `spawnModelFx` | 14 |
| `spawnProjectile` | 24 | `spawnVfx` | 26 | `spendMana` | 4 |
