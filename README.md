...
**開放（whitelisted）的英雄、技能與商店清單就直接印在下面、預設展開、不用點任何摺疊**。完整的全 113 名 / 全 554 技能 / 全 214 道具搬到 `docs/reference/roster.md`、`abilities.md`、`items.md`（各區塊都有連結），這樣 README 才不會又肥又被 GitHub 折疊。所有數字都從 repo 量出來，權威計數在 `content/manifest.json`（由 `pnpm content:build` 產生）。
...
`pnpm docs:readme` **一次寫兩個目標**：README 裡的**開放**清單，以及 `docs/reference/roster.md` / `abilities.md` / `items.md` 裡的**完整**全表（全 113 / 554 / 214）。兩者讀同一棵 `content/`、同一個 `build_context()`，所以永遠不會互相矛盾。（`pnpm docs:reference` 只重寫 docs 那三個檔，是 CI 用的子集。）
...
> **README 印開放的、預設展開；完整的搬到 docs。** 下面三段（`roster` / `abilities` / `items` 標記之間）只放**開放名單**的英雄、技能與商店貨架，且**不包在 `<details>` 裡**——GitHub 對 `<details>` 預設折疊，之前就是這個原因讓人以為清單不見了。全 113 名 / 554 技能 / 214 道具的逐欄表在 `docs/reference/*.md`（各區塊有連結）。**描述類欄位為了可讀性被截斷**：英雄「一句話說明」40 字、技能效果 34 字、道具 modifiers 52 字、被動 28 字（`gen_readme_lists.py` 頂端的 `LIMIT_*`）；**結尾的 `…` 是產生器加的**。要完整逐字請開 `#codex` 或讀 `content/<collection>/<id>.json`。
...
**技能的 `slot` 只有 Q/W/E/R/EX 五種。** **被動不是一個 slot** —— 全樹沒有任何 `xx-00` 被動技能文件（實測 0 份）。被動掛在某個 QWER 技能的 `passive` 區塊裡（在 `docs/reference/abilities.md` 的 `型態` 欄標「被動」的那些）。README 的開放名冊把每名英雄的 Q/W/E/R/EX 直接列成五條「技能名稱＋一行效果」；全 554 技能的逐欄表在 `docs/reference/abilities.md`。
...
以下三段是機器產生的**開放名單**：開放英雄＋技能、商店貨架＋抽卡池。全 113／554／214 的完整表在各段結尾連到 `docs/reference/*.md`。

<!-- BEGIN GENERATED:roster -->
#### 開放名單 OPEN roster（48 名）— 角色 + 技能
...
>
> 每名英雄一格：**`id` 全名**（稱號 · 職業 · 攻擊）— 一句話說明，底下五條是 Q/W/E/R/EX 的**技能名稱＋一行效果**。效果截斷到 34 字、說明截斷到 40 字，結尾的 `…` 是產生器加的。完整逐字內容在 [`docs/reference/abilities.md`](./docs/reference/abilities.md) 或 <http://localhost:39527/#codex>。
...
*由 `pnpm docs:readme` 從 contentVersion `cv_08c54d5701fc` 產生。 開放 48 / 全 113 名。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:roster -->

<!-- BEGIN GENERATED:abilities -->
#### 技能 abilities（全 554 個；開放英雄的 240 個）

> **開放英雄的每一個技能，都已經印在上面的開放名冊裡**（每名英雄 Q/W/E/R/EX 五條，含名稱與一行效果）。這裡不再重印一次，只放全表的統計與連結，讓 README 保持精簡。
>
> 每個英雄每個 slot 一份：Q 113 · W 113 · E 113 · R 113 · EX 102。`slot` 只有 Q/W/E/R/EX 五種 —— **被動不是一個 slot**，它掛在某個 QWER 技能上（`型態` 欄標「被動」的就是）；全樹沒有任何 `xx-00` 被動技能文件。
>
> 數值是 `content/` 的**原始值**，未套用 `combat-env` 全域倍率 —— 遊戲內顯示的一律是乘算後的最終值，所以畫面上的冷卻／傷害跟表格不會相同。那是預期行為。

> 📖 **全 554 個技能的逐欄表**（id、名稱、slot、型態、編號、擁有英雄、開放旗標、完整短效果）在 [`docs/reference/abilities.md`](./docs/reference/abilities.md)；互動版在 <http://localhost:39527/#codex>。

*由 `pnpm docs:readme` 從 contentVersion `cv_08c54d5701fc` 產生。 開放英雄技能 240 / 全 554 個。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_08c54d5701fc` 產生。 可取得 57 / 全 214 件。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:items -->
