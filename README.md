...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 倍率讀 `content/config/combat-env.json`（version 9）。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 輪替 12 / 全 13 張。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
目前英雄、技能、開放名單與待上架數量由下方產生區塊即時計算；各 collection 的完整計數讀 `content/manifest.json`。已取得素材庫的來源身份與候選則讀中央素材索引，由同一支產生器同步列出。
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 級距與 `appliesTo` 讀 `content/config/stat-normalization.json`、上限讀 `stat-caps.json`、成長現況現場數 `content/champions/`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 級距讀 `content/config/*-tiers.json`（9 張表）、母體讀 `docs/平衡錨點量測.md`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:tiers -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 開放 167 / 全 190 名。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 全量 190 名，其中開放 167 名。另有 8 名待上架（卡還沒進 repo）。 完整清單另見 `docs/全英雄列表.md`。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 開放英雄技能 1001 / 全 1129 個。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 可取得 130 / 全 142 件。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:items -->
...
*由 `pnpm docs:readme` 從 contentVersion `cv_24b299192e58` 產生。 這三段標記之間的任何字都會在下次重新產生時被覆蓋。*
<!-- END GENERATED:grail -->
