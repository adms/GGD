...
版本：1.3 · 2026-09-09 · 適用：GGD 社群英雄、六槽技能、機制、特效、模型與動作的生成及修正。
...
批次角色測試通過後、任何模型上傳或服務建包前，另跑 `prepare-published-handoff.mts --scenario-report <新報告路徑>`。此步以完整本機基線重跑投稿器的預設六槽與固定順序情境；第一批實測可提早抓出相同的 5 名拒絕。缺召喚物、前置命中或資源的拒絕仍須處理，不能把角色因果測試的通過抄成投稿已通過，也不能刪掉使用條件或直接塞資源放行。跨生成器建包需明確採用新版本並追加 revision／receipt；發布脚本須先經模型原檔 API 存檔，且讀取本站的實際配額與模型 ZIP 上限。
