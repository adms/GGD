...
版本：1.4 · 2026-09-09 · 適用：GGD 社群英雄、六槽技能、機制、特效、模型與動作的生成及修正。

本文件供其他生成 AI 直接引用。目標是把完整原設計轉成可以實際操作、驗證、投稿與版本回復的英雄。**能通過 schema、編譯、基本模擬或 ZIP 往返，只證明相應階段通過，不能宣稱英雄原設計完成。**

## 當前核准的設計範圍（2026-09-09）

使用者已允許第一批改成有角色辨識度的惡搞玩法，並要求簡化、複用既有機制。當前規則見 [版本化設計政策](materials/community-hero-forge/design-policy.json)：只選既有 enabled 模板與合法參數，不新增引擎功能、模板定義或全域機制標籤；從完整目錄組合定位、簡單連招與反制，各英雄核心組合必須不同。改名、換色或只改數值不算新特色。

原名與原稿留作歷史，核准改編另記差異及理由。以下忠實還原限制仍用來禁止「未經說明偷換機制」；不阻擋此次明確允許的簡化。不能把舊原稿改掉，或以編譯／結構不重複宣稱創意、行為、畫面都已完成。第二批既有設計應先重用並核對版本，不另起一套重做。

新增批次實作入口：[第一批設計與驗證來源](tools/community-hero-forge/parody/)、[37 名玩法表](materials/community-hero-forge/refinements/parody-review.md)。改模板實例的來源，再由生成器產生微調與作品；保留原稿及舊版快照。共用低階效果允許重用，各英雄必須有不同的資源／準備／選擇／反制組合。

驗證連動時，使用有／無前置與有／無連動的四條等長真實時間線，消除前置技能本身的傷害差；每個主動槽還要與移除實際效果的對照比較，被動由真實事件觸發。不要只檢查模板 ID、浮字、施法成功或非空效果樹。原稿 requiredRefinement 留作歷史，核准改編的每槽用途與差異另記。

注意 GGD 最終數值與效果輸入不同：基礎魔抗在倍率之後加回，`override:0` 不代表最終魔抗零；`damage.flat:0` 仍可能帶通用技能係數。要無傷害移動時直接選無傷害效果；最終零防禦須依版本化配置反算並以真實戰鬥確認。不可為這些內容擴充引擎。

批次角色測試通過後、任何模型上傳或服務建包前，另跑 `prepare-published-handoff.mts --scenario-report <新報告路徑>`。此步以完整本機基線重跑與投稿器、Editor 共用的六槽及整套情境；需要前置時，必須透過真實施法、自然恢復、迴避交鋒與普攻取得，並保留操作紀錄。第一批修正後 37 名皆通過；空資源、移除取得來源等原始拒絕反例仍須保留。缺召喚物、前置命中或資源的拒絕仍須處理，不能把角色因果測試的通過抄成投稿已通過，也不能刪掉使用條件或直接塞資源放行。跨生成器建包需明確採用新版本並追加 revision／receipt；發布脚本須先經模型原檔 API 存檔，且讀取本站的實際配額與模型 ZIP 上限。
...
5. 數值採現行 GGD 級距、規則與可調設定；可調整數值和演出尺度，沒有明確改編授權時，不能把機制種類、對象或事件條件換掉；有授權則記錄核准改編及可驗證的連動。
...
[炭治郎 v2](materials/community-hero-forge/refinements/35.json) 使用既有具名標記、事件被動與效果序列模板，沒有新引擎特判。水火 Q 共用同一實例與冷卻、同耗一層呼吸；火式較強但短暫降速。E 靜止調息較快，移動／受擊較慢，R 蓄勢可打斷，敵人能趁喘氣拉開距離。它示範已核准的簡化，不宣稱原稿較高火式資源成本仍然存在。

驗證包含真實迴避、HP／盾連接命中、施法者來源隔離、牆阻、資源不足、移動對照、到期／死亡／回合清除，以及 [Editor／匯入／舊版獨立性](packages/shared/src/content/heroForge/communityRefinements/tanjiroAcceptance.test.ts)。[收據](materials/community-hero-forge/refinements/tanjiro-verification.json) 將行為、模型綁定、既有 VFX 事件與尚未完成的新畫面／發布分開記錄。

### 本批最終驗收範圍

依 Owner 最後裁決，只有技能、特效或機制失效等重大問題阻擋功能交付。鏡頭、短暫遮擋及美術微調只記錄，不因此反覆改造預覽工具或擴大角色設計。仍須如實區分實際施放／行為測試、畫面收集、人工美術判讀、隔離發布及正式部署；未看過的圖片不能標為美術通過，預覽診斷失敗也不能直接推定為遊戲技能失效。
