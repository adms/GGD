...
/**
 * ⭐ 卡面「有沒有告訴玩家這是普攻觸發」的用詞表。
 *
 * ⚠️ 2026-09-12：在此之前這條漏掉 **「普通攻擊」** —— ⭐ 而那是卡面上**最自然的寫法**。
 * ⇒ 5 支卡面**明明說了**卻被判成沒說（`b2-guts.passive` 逐字：
 *   「自身生命低於一半時，**普通攻擊**額外造成極小物理傷害；每 2 秒最多一次」）。
 *
 * ⛔ 這種假陽性不是無害的：它把一條真的普查推到棘輪上限之上，
 * ⭐ 於是**真正的 29 支**（`onBasicAttack` 被當成別的 hook 的替身，GH#1239）
 * 被埋在同一個數字裡 —— ⇒ 而修法完全不同（那 29 支要的是**新 hook**，⛔ 不是改文案）。
 *
 * ⚠️ 2026-09-15（GH#1239 收尾）：再補 **「攻擊身上」**。92-04 馬勒戈壁逐字
 *   「(**攻擊身上有** [致盲] 標記的敵人將額外附加⋯)」、30-002 變態紳士逐字「**攻擊身上有**酒精灌腸效果的敵人⋯」
 *   —— ⭐ 兩張卡都說了「攻擊（帶某狀態的敵人）時」，JSON 也正是 `onBasicAttack` ＋ `condition.status`。
 *   ⛔ 只收「攻擊身上」這個片語，⛔ 不是「攻擊」一個字（那會把技能命中也算進來）。
 */
const ON_BASIC_ATTACK_SAID =
  /普通攻擊|基本攻擊|攻擊時|攻擊附帶|攻擊命中|每次攻擊|攻擊後|攻擊身上|普攻|平\s?A/;
const rule4 = docs
  .filter(
    (d) =>
