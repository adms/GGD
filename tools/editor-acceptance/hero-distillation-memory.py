...
def tail_value_and_grad(mx, nn, model, prefix, targets, *, tail_layers, block_size=256, loss_block_size=128, observer=None):
    ...
    with gemma_attention_policy(mx,block_size=block_size,observer=observer), \
        ...
        loss=evaluated_leaf(mx,loss)
        answer_gradient=evaluated_leaf(mx,answer_gradient)
        ...
        gradient=evaluated_leaf(mx,norm_gradients[0])
        ...
        for index in range(len(body.layers)-1,split-1,-1):
            ...
            try:
                ...
                mx.eval(values)
                values=[evaluated_leaf(mx,value) for value in values]
            finally:
...
def frozen_mlp_policy(mx,layers,*,block_size,reference=False,observer=None):
    """Block supplied dense frozen MLPs, not attention projections or norms."""
...
def make_attention(mx, *, scale, window=None, block_size=256, observer=None):
    ...
    def attention_vjp(primals, cotangent, output):
        q, k, v = primals
        # Host FP32 accumulation removes repeated whole-K/V device copies
        # and trace-owned updates. Physical host RAM remains supervised.
        ...
        for begin in range(0, q.shape[-2], block_size):
            ...
            qb=q[:,:,begin:end].astype(mx.float32).reshape(batch,kv_heads,groups,end-begin,width)
            kb=k[:,:,None,lo:hi,:].astype(mx.float32);vb=v[:,:,None,lo:hi,:].astype(mx.float32)
            ...
            dq[:,:,begin:end]=np.asarray(gq.astype(mx.float32))
            np.add(dk[:,:,lo:hi],np.asarray(gk.astype(mx.float32)),out=dk[:,:,lo:hi])
            np.add(dv[:,:,lo:hi],np.asarray(gv.astype(mx.float32)),out=dv[:,:,lo:hi])
            del qb,kb,vb,dy,scores,probabilities,dp,ds,gq,gk,gv
            if observer: observer('attention-backward-block', begin=begin, end=end)
        return mx.array(dq).astype(q.dtype), mx.array(dk).astype(k.dtype), mx.array(dv).astype(v.dtype)
...
def gemma_attention_policy(mx, *, block_size=256, reference=False, observer=None):
    ...
    def attention(q, k, v, *, cache, scale, mask, sinks=None):
        ...
        # Retain source BF16 primals, cast only the current score block to
        # FP32. The VJP accumulates ALL K/V contributions in host FP32 before
        # its single final source-dtype cast, matching the dense cast boundary.
        # Forward casts per-token outputs back before concatenation (equivalent
        # to concatenating FP32 then casting), avoiding full FP32 duplicates.
        dtype = q.dtype
        if reference:
            q, k, v = (item.astype(mx.float32) for item in (q, k, v))
            return mx.fast.scaled_dot_product_attention(q, k, v, scale=scale, mask=mask).astype(dtype)
