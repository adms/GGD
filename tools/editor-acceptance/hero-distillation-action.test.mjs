...
test('action queue derives every cursor from previous shapes',()=>{
  ...
  assert(actions.length>1);assert(actions.every(item=>JSON.stringify(item.answer).length<=600));assert(actions.filter(item=>item.answer.op==='value').every(item=>item.answer.value===null||['string','number','boolean'].includes(typeof item.answer.value)));assert.deepEqual(replayActions(actions),value);
});
...
test('action projection retains 74-hero split and has no oversize response',()=>{
  ...
  assert.equal(report.counts.all.heroes,74);assert(report.counts.all.tasks>518);assert.equal(report.actionProtocol,'scalar-leaves@1');assert(report.maxActionsPerSequence<=report.maximumActionSequenceLimit);assert(report.maxOutputChars<=600);assert.equal(manifest.schema,'ggd-distillation-action-frozen-data@1');
});
