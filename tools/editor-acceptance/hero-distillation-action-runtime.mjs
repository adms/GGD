...
function replayAnswers(answers){
  let state=newActionState();for(const answer of answers)state=applyAction(state,answer);return completeActionState(state);
}
...
export function assembleActionHero({identity,slotSelections,slotCores,coreActions,productActions,decisionSpace,context}){
  ...
  for(const slot of SLOTS){
    ...
    const base=replayAnswers(coreActions[slot]);exact(base,['name','abilityOverrides','tuning'],'ACTION_CORE_VALUE_KEYS');assert(Array.isArray(productActions[slot])&&productActions[slot].length===core.productTemplates.length,'ACTION_PRODUCT_ACTION_COUNT');
    const products=core.productTemplates.map((ref,index)=>({template:{ref,params:replayAnswers(productActions[slot][index])}}));
