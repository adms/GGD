...
function sourcePriority(name){return /(jump\s*force|jump.*大亂鬥|the king of fighters|\bkof\b|infinity\s*strash|達伊的大冒險)/i.test(name)}
...
function renderGames(){let box=document.querySelector('#games');box.textContent='';steamGroups.forEach((g,i)=>{let l=document.createElement('label'),check=document.createElement('input'),title=document.createElement('span'),drive=document.createElement('span'),details=document.createElement('span');check.type='checkbox';check.dataset.index=String(i);check.checked=true;title.className='game-title';title.textContent=' '+(g.manifest?.name||g.dir);drive.className='muted';drive.textContent=` (${g.libraryLabel})`;details.className='details muted';let version=g.manifest?.buildId?`版本 build ${g.manifest.buildId}，`:'';details.textContent=`${version}${g.files.length.toLocaleString()} 個候選素材檔，${fmt(g.bytes)}；${g.kinds.join('、')||'未分類'}`;l.append(check,title,drive);if(g.priority){let badge=document.createElement('span');badge.className='badge priority';badge.textContent='優先來源';l.appendChild(badge)}for(const kind of g.kinds){let badge=document.createElement('span');badge.className='badge';badge.textContent=kind;l.appendChild(badge)}l.appendChild(details);box.appendChild(l)});for(const id of ['uploadGames','selectAll','selectPriority'])document.querySelector('#'+id).style.display=steamGroups.length?'inline-block':'none'}
...
document.querySelector('#selectAll').onclick=()=>document.querySelectorAll('#games input').forEach(e=>e.checked=true);
document.querySelector('#selectPriority').onclick=()=>document.querySelectorAll('#games input').forEach(e=>e.checked=steamGroups[Number(e.dataset.index)].priority);
