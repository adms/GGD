...
p=argparse.ArgumentParser();p.add_argument('--workspace',type=Path,required=True);p.add_argument('--out',type=Path,required=True);a=p.parse_args();ws=a.workspace.resolve();out=a.out.resolve();out.mkdir(parents=True,exist_ok=False)
...
for key,c in sources.items():
 ...
 try:
  ...
  doc,binary,report=convert(raw,meshes,list(dict.fromkeys(clips.values())),tex,32);body=encode_glb(doc,binary);(d/'body.glb').write_bytes(body)
