...
import argparse, copy, gzip, json, hashlib, subprocess
...
def source_registered_hero_components(downloads, repo=ROOT):
    """Validate source-owned, policy-compliant hero candidates before indexing."""
    role='complete-hero-body-with-procedural-six-state-motion'
    result=[]
    for source in downloads.get('publicSources',[])+downloads.get('paidSources',[]):
        for candidate in source.get('componentCandidates',[]):
            if not candidate.get('componentReady') or candidate.get('resourceRole')!=role:
                continue
            path=(repo/candidate['gitPath']).resolve()
            if not path.is_relative_to(repo.resolve()) or not path.is_file():
                raise ValueError('Registered hero component is missing: '+candidate['id'])
            payload=path.read_bytes()
            if (len(payload),hashlib.sha256(payload).hexdigest())!=(candidate['bytes'],candidate['sha256']):
                raise ValueError('Registered hero component bytes differ: '+candidate['id'])
            evidence=candidate.get('registrationEvidence',{})
            receipt_path=repo/evidence.get('gitPath','')
            if (not receipt_path.is_file()
                or receipt_path.stat().st_size!=evidence.get('bytes')
                or hashlib.sha256(receipt_path.read_bytes()).hexdigest()!=evidence.get('sha256')):
                raise ValueError('Registered hero component receipt differs: '+candidate['id'])
            receipt=read(receipt_path)
            registration=receipt.get('registration',{})
            model_path=repo/registration.get('modelDocument',{}).get('gitPath','')
            model=read(model_path)
            if (receipt.get('componentId')!=candidate['id']
                or receipt.get('candidate',{}).get('sha256')!=candidate['sha256']
                or receipt.get('validation',{}).get('formalAdoptionTrianglePolicyPassed') is not True
                or registration.get('runtimeSelectable') is not True
                or registration.get('default') is not False
                or model.get('id')!=registration.get('modelKey')
                or model.get('glbPath')!='assets/models/community/'+candidate['sha256']+'.glb'
                or model.get('heroBody') is not True):
                raise ValueError('Registered hero component contract differs: '+candidate['id'])
            result.append(dict(candidate,gitAbsolutePath=str(path),currentPolicyAudit=current_policy_for(candidate,repo)))
    return result


def fateubw_runtime_components(inventory, manifest, registration_receipt, repo=ROOT):
    """Expose every finished FateUBW Git component in the central model index."""
    servants={row['characterId']:row for row in inventory['minecraftCommunity']['servants']}
    manifest_path='materials/hero-model-library/priority-evidence/fateubw-community/runtime-components-v1/manifest.json'
    receipt_path='materials/hero-model-library/priority-evidence/fateubw-community/runtime-components-v1/registration-receipt.json'
    manifest_evidence={
        'gitPath':manifest_path,
        'bytes':(repo/manifest_path).stat().st_size,
        'sha256':hashlib.sha256((repo/manifest_path).read_bytes()).hexdigest(),
    }
    receipt_evidence={
        'gitPath':receipt_path,
        'bytes':(repo/receipt_path).stat().st_size,
        'sha256':hashlib.sha256((repo/receipt_path).read_bytes()).hexdigest(),
        'productionDeploymentVerified':False,
    }
    components=[]
    for row in manifest['components']:
        servant=servants[row['characterId']]
        local=servant['standardizedModelWithNativeMotion']
        mapped=bool(row['heroIds'])
        components.append({
            'id':f"fateubw-{row['characterId']}-runtime-component-v1",
            'candidateId':f"fateubw-{row['characterId']}-runtime-component-v1",
            'label':f"{row['authorLabel']}｜FateUBW 模型＋來源動作",
            'character':row['authorLabel'],
            'library':'Fate/Unlimited Block Works',
            'path':local['absolutePath'],
            'absolutePath':local['absolutePath'],
            'bytes':row['glb']['bytes'],
            'sha256':row['glb']['sha256'],
            'format':'glb',
            'readiness':('backend-dropdown-candidate-registered; production deployment unverified'
                         if mapped else 'git-runtime-component-complete; pending GGD hero definition'),
            'existsLocal':local['liveVerified'],
            'resourceRole':'character-body',
            'sourceId':row['sourceId'],
            'sourceUrl':inventory['minecraftCommunity']['sourceUrl'],
            'sourceWork':'Fate/Unlimited Block Works',
            'heroIds':row['heroIds'],
            'relatedHeroIds':row['heroIds'],
            'defaultEligible':False,
            'automaticEligible':False,
            'componentReady':True,
            'runtimeSelectable':mapped,
            'runtimeDropdownRegistered':mapped,
            'runtimeModelKey':row['modelKey'],
            'gitPath':row['glb']['gitPath'],
            'gitAbsolutePath':str(repo/row['glb']['gitPath']),
            'modelDocumentGitPath':row['modelDocument']['gitPath'],
            'nativeAnimationCount':row['nativeAnimationCount'],
            'animationClipCount':row['nativeAnimationCount'],
            'animationScope':'source-native-converted-library',
            'selectedClips':row['clipMap'],
            'motionSemantics':row['motionSemantics'],
            'characterIdentity':f"fateubw:{row['characterId']}",
            'ggdHeroImplemented':mapped,
            'upstreamLicense':row['upstreamLicense'],
            'projectApproval':row['projectApproval'],
            'validationEvidence':manifest_evidence,
            'registrationEvidence':({**receipt_evidence,'heroIds':row['heroIds']}
                                    if mapped else receipt_evidence),
            'limitations':[
                '六態映射含同來源語意備援，不宣稱每態皆為同名原生動作。',
                '專案主人已核准登記；上游 ARR 權利狀態仍獨立保留。',
                '尚未驗證 Main 合併後正式站部署。',
            ],
        })
    return components
...
def apply_s3_preparation_split(value, manifest):
    """Replace Git claims for #1252 preparation files with verified S3 pointers."""
    by_path={row['repoPath']:row for row in manifest['files']}
    ...
    def visit(node):
        ...
        if split:
            ...
            node['storageClass']='s3-legacy-preparation'
            node['s3Uri']=manifest['s3Uri']
            node['s3ArchiveMember']=split['archiveMember']
            node['s3ManifestUri']=manifest['manifestUri']
            if split['storageRole']=='unreferenced-model-component':
                node['runtimeSelectable']=False
                node['runtimeDropdownRegistered']=False
                node['defaultEligible']=False
                node['automaticEligible']=False
        for item in list(node.values()):visit(item)
...
def verify_git_contents(entries, repo=ROOT):
    ...
    if split_path.is_file():
        ...
        split_by_path={row['repoPath']:row for row in split.get('files',[])}
    for entry in entries:
        path = entry['gitPath']
        split_entry=split_by_path.get(path)
        if split_entry:
            if entry.get('sha256')!=split_entry['sha256']:
                raise ValueError('S3 split declaration differs from catalogued digest: '+path)
            if entry.get('bytes') not in (None,split_entry['bytes']):
                raise ValueError('S3 split declaration differs from catalogued byte count: '+path)
            continue
...
def build(git_link_root=ROOT):
    ...
    for key in ('files','animationClips'):
        ...
        try:
            verify_git_contents([entry])
        except ValueError as error:
            raise ValueError('Refresh 300/MBA unused asset inventory: '+entry['gitPath']) from error
    ...
    fate_runtime_components_path=base/'priority-evidence/fateubw-community/runtime-components-v1/manifest.json'
    fate_runtime_registration_path=base/'priority-evidence/fateubw-community/runtime-components-v1/registration-receipt.json'
    fate_asset=read(fate_asset_path)
    fate_asset_entry=read(fate_asset_entry_path)
    fate_runtime_manifest=read(fate_runtime_components_path)
    fate_runtime_registration=read(fate_runtime_registration_path)
    ...
    if (fate_asset.get('schema')!='ggd.fate-platform-separated-asset-inventory@1'
        ...
        or fate_asset.get('summary',{}).get('runtimeSelectable')!=4
        or fate_asset.get('summary',{}).get('backendHeroOptions')!=5
        or fate_asset.get('summary',{}).get('gitRuntimeComponents')!=14
    ...
    for path,key in (
        ...
        (fate_runtime_components_path,'runtimeComponentsSha256'),
        (fate_runtime_registration_path,'runtimeRegistrationSha256'),
    ):
    ...
    for row in palworld_approved_manifest.get('cries',[]):
        product=row['gitProduct']
        runtime_product=row['runtimeProduct'];runtime_path=ROOT/runtime_product['gitPath']
        try:
            verify_git_contents([product])
        except ValueError as error:
            raise ValueError('Palworld approved cry preparation product is absent or changed: '
                             +row.get('candidateId','unknown')) from error
        if (not runtime_path.is_file() or runtime_path.stat().st_size!=runtime_product['bytes']
    ...
    components.extend(fateubw_runtime_components(
        fate_asset,
        fate_runtime_manifest,
        fate_runtime_registration,
        ROOT,
    ))
    ...
    # #1252 intentionally removes unreferenced preparation GLBs from Git.  The
    # last generated index retains their measured metadata, while the immutable
    # split manifest pins the exact archived bytes.  Exclude those candidates
    # from live file validators and restore only the matching catalog records.
    split_manifest_path=ROOT/'materials/asset-library/pr1152-s3-split.json'
    split_manifest=read(split_manifest_path)
    split_models={row['repoPath']:row for row in split_manifest['files']
                  if row['storageRole']=='unreferenced-model-component'}
    previous_current=read(ROOT/'materials/asset-library/current-resources.json')
    archived_components=[]
    for component in previous_current.get('modelComponents',[]):
        archived=split_models.get(component.get('gitPath'))
        if not archived:continue
        if (component.get('sha256')!=archived['sha256']
            or component.get('bytes')!=archived['bytes']):
            raise ValueError('Archived model component differs from #1252 split: '+component.get('id','unknown'))
        preserved=copy.deepcopy(component)
        preserved['currentPolicyAudit']=current_policy_for(preserved,ROOT)
        archived_components.append(preserved)
    archived_ready_paths={candidate['gitPath']
        for source in component_sources.get('publicSources',[])+component_sources.get('paidSources',[])
        for candidate in source.get('componentCandidates',[])
        if candidate.get('componentReady') and candidate.get('gitPath') in split_models}
    if not {row['gitPath'] for row in archived_components}.issubset(archived_ready_paths):
        raise ValueError('Archived component catalog does not match #1252 split-ready sources')
    component_sources=copy.deepcopy(component_sources)
    for source in component_sources.get('publicSources',[])+component_sources.get('paidSources',[]):
        source['componentCandidates']=[candidate for candidate in source.get('componentCandidates',[])
                                       if candidate.get('gitPath') not in split_models]
    ...
    components.extend(source_registered_hero_components(component_sources,ROOT))
    ...
    if approved_derivative_audit_path.exists():
        ...
        result['approvedDerivativeAudit']=dict(
            ...
            productionDeploymentVerified=False)
    s3_split_path=ROOT/'materials/asset-library/pr1152-s3-split.json'
    if s3_split_path.exists():
        s3_split=read(s3_split_path)
        ...
        result['s3PreparationSplit']={
            'gitPath':str(s3_split_path.relative_to(ROOT)),
            'sha256':hashlib.sha256(s3_split_path.read_bytes()).hexdigest(),
            's3Uri':s3_split['s3Uri'],
            'manifestUri':s3_split['manifestUri'],
            'archiveSha256':s3_split['archiveSha256'],
            'archiveBytes':s3_split['archiveBytes'],
            'summary':s3_split['summary'],
...
def main():
    ...
    if args.check_git:
        if 's3PreparationSplit' in result:
            verify_git_contents([result['s3PreparationSplit']])
        ...
        verify_git_contents(
            ...
            + [
                ...
                *[
                ...
                {
                    'gitPath': result['fateAssetInventory']['runtimeComponentsGitPath'],
                    'sha256': result['fateAssetInventory']['runtimeComponentsSha256'],
                },
                {
                    'gitPath': result['fateAssetInventory']['runtimeRegistrationGitPath'],
                    'sha256': result['fateAssetInventory']['runtimeRegistrationSha256'],
                },
                {
