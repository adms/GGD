...
def is_model_source(source):
    ...
    return source.get('resourceRole') not in {
        ...
        'validation-evidence', 'research-evidence', 'game-resource-archive'}
...
def render_sources(data, policy, source_hero_overrides=None, candidate_overrides=None):
    ...
    lines += ['## 第二守則：預設模型選用順序', '',
        ...
        '**原始檔、半成品、轉換檔、抓取器、解析器、轉換程式及產生器都要另留一份 S3 備份，附逐檔 SHA-256 與完整讀回驗證。** 程式與成品的 Git 共編版本仍保留；Git 資產與工具快照位於 `legacy/git-asset-snapshots/<commit>/`，由 `tools/hero-model-library/backup_git_assets.py` 建立。原始／半成品另依來源與轉換批次保存，不能用 Git 快照冒稱涵蓋未追蹤的本機材料。', '',
