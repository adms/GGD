"""Render one recovered GLB in isolated headless Chrome and preserve three review views."""
...
parser.add_argument('--repo',type=Path,required=True);parser.add_argument('--candidate-id',required=True);parser.add_argument('--idle-clip',required=True)
args=parser.parse_args();source=args.source.resolve();out=args.output.resolve();out.mkdir(parents=True,exist_ok=False);root=args.repo.resolve();done=threading.Event()
meta={'candidateId':args.candidate_id,'idleClip':args.idle_clip,'sha256':hashlib.sha256(source.read_bytes()).hexdigest()}
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/meta.json':data=(json.dumps(meta)+'\n').encode();mime='application/json'
 ...
 def do_POST(self):
  ...
  obj=json.loads(self.rfile.read(length));data=base64.b64decode(obj['png'],validate=True) if name.endswith('.png') else (json.dumps(obj,indent=2)+'\n').encode()
  with (out/name).open('xb') as stream:stream.write(data)
  self.send_response(200);self.end_headers()
server=ThreadingHTTPServer(('127.0.0.1',0),Handler);threading.Thread(target=server.serve_forever,daemon=True).start()
chrome='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';profile=Path(tempfile.mkdtemp(prefix='ggd-historical-model-chrome-'))
...
receipt={'candidateId':args.candidate_id,'source':str(source),'sourceSha256':meta['sha256'],'complete':complete,'proofExists':(out/'proof.json').is_file(),'errorExists':(out/'error.json').is_file(),'images':len(list(out.glob('*.png')))}
