...
def build():
    ...
    for audit in audits:
        for raw in audit['characters']:
            ...
            row['designStatus']=status;rows.append(row)
...
def render(data):
    ...
    labels=[('not-defined','尚未建立對應英雄'),('definitions-incomplete','已有定義，需補查或實作'),('identity-review','來源身份／英雄對應待確認')]
    for status,label in labels:
        selected=[r for r in data['characters'] if r['designStatus']==status]
        lines+=['## '+label,'']
        if not selected:lines+=['目前沒有符合此分類的來源記錄。',''];continue
        lines+=['| 角色 | 作品 | 已取得來源／模型 | 轉換狀態 | 英雄定義／判定 | 本機位置 |','|---|---|---|---|---|---|']
        for r in sorted(selected,key=lambda x:(x.get('work') or '',x.get('name') or '',x['id'])):
            ...
            evidence='；'.join(f"{c['name']}：{c['status']}（{c['availability']}）" for c in checks) if checks else ('未建立對應英雄；保留來源原生 ID' if status=='not-defined' else '身份或同名對應尚未確認，暫不列為確定新英雄')
            lines.append('| '+' | '.join([text(r['name']),text(r.get('work','待核')),text('／'.join(libs)+f' · {len(cs)} 份'),text('／'.join(states)),text(evidence),'<br>'.join(links)])+' |')
        lines+=['']
    lines+=['## 完整性與限制','',
