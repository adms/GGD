...
ROOT=Path('/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT');base=ROOT/'GGD-Asset-Library/intake/public-models-20260910';dst=base/'parallel-community-slayers-lina-hand-review';original=base/'parallel-community-slayers-tales-of-rays/conversion/characters';rows=json.loads((dst/'hand-selection-index.json').read_text());assets={r['id']:Path(r['outputPath']) for r in rows};assets.update({a+'-original':original/(a+'.glb') for a in ['lina00','lina01']});done=threading.Event();run=dst/'evidence/babylon-reviewed';run.mkdir(exist_ok=True)
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/bundle.js':payload=Path('/private/tmp/ggd-lina-render-bundle.js').read_bytes();mime='text/javascript'
...
with (run/'chrome-process.log').open('w') as log:
 ...
 try:
  ...
  print(json.dumps({'complete':complete,'renderFiles':len(list(run.glob('*.png'))),'proofExists':(run/'render-proof.json').exists(),'errorExists':(run/'render-error.json').exists(),'localUrl':url}),flush=True)
 finally:
  proc.terminate()
  try:proc.wait(timeout=10)
  except subprocess.TimeoutExpired:proc.kill();proc.wait(timeout=5)
  server.shutdown();server.server_close()
for f in ['ggd-lina-render-server.py','ggd-lina-render.mjs']:shutil.copyfile('/private/tmp/'+f,dst/'scripts'/f)
