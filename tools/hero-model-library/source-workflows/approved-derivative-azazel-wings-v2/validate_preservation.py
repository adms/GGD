...
STAGE = ROOT.parent / "GGD-Asset-Library/conversions/approved-derivative-azazel-wings-v2"
...
def main() -> None:
    ...
    parser.add_argument("--candidate", type=Path, default=STAGE / "azazel-wings-v2.glb")
    parser.add_argument("--rebuild", type=Path, default=Path(tempfile.gettempdir()) / "azazel-wings-v2-rebuild.glb")
    parser.add_argument("--output", type=Path, default=STAGE / "azazel-wings-v2.preservation.json")
    ...
    assert len(candidate_doc["accessors"]) == len(source_doc["accessors"]) + 10
    preserved = []
    ...
    source_triangles = values(source_doc, source_bin, source_doc["meshes"][0]["primitives"][0]["indices"]).reshape(-1, 3)
    candidate_triangles = np.concatenate([
        values(candidate_doc, candidate_bin, primitive["indices"]).reshape(-1, 3)
        for primitive in candidate_doc["meshes"][0]["primitives"][:3]
    ])
    source_set = sorted(map(tuple, source_triangles.tolist()))
    candidate_set = sorted(map(tuple, candidate_triangles.tolist()))
    assert candidate_set == source_set
    source_attributes = source_doc["meshes"][0]["primitives"][0]["attributes"]
    assert candidate_doc["meshes"][0]["primitives"][0]["attributes"] == source_attributes
    assert candidate_doc["meshes"][0]["primitives"][2]["attributes"] == source_attributes
    head_attributes = candidate_doc["meshes"][0]["primitives"][1]["attributes"]
    assert {key: value for key, value in head_attributes.items() if key != "TEXCOORD_0"} == {
        key: value for key, value in source_attributes.items() if key != "TEXCOORD_0"
    }
    assert head_attributes["TEXCOORD_0"] != source_attributes["TEXCOORD_0"]
    wing = candidate_doc["meshes"][0]["primitives"][3]
    ...
    result = {
        "schema": "ggd.approved-azazel-wings-preservation@2",
        ...
        "sourceBodyAndPantsPrimitiveAttributesUnchanged": True,
        "headUvAccessorDeterministicallyRemapped": True,
        "sourceTriangleUnionUnchanged": True,
        "materialSplitPrimitiveCount": 3, "nodesUnchanged": True,
