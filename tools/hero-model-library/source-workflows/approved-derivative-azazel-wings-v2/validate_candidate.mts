...
const khronos = await validator.validateBytes(candidate, {
  uri: "azazel-wings-v2.glb", maxIssues: 0, writeTimestamp: false,
...
assert.equal(after.meshes, before.meshes + 3);
...
assert.deepEqual(after.json.meshes[0].primitives[0].attributes, before.json.meshes[0].primitives[0].attributes);
assert.equal(after.json.meshes[0].primitives.length, 4);
...
assert.equal(after.json.meshes[0].primitives[3].extras?.binding, "100%-rigid-to-Bip01-Spine1");
...
const result = {
  schema: "ggd.approved-azazel-wings-validation@2",
  ...
  preservation: {
    bodyPrimitiveAttributesUnchanged: true, sourceTriangleUnionPreserved: true, skinJsonUnchanged: true, nodeJsonUnchanged: true,
    animationJsonUnchanged: true, sourceClipInventoryUnchanged: true,
    independentEmbeddedCopy: true, sourceCandidateRetained: true,
  },
  materialSplit: { bodyPrimitive: 0, headPrimitive: 1, pantsPrimitive: 2, geometryChanged: false },
  attachment: { primitive: 3, jointName: "Bip01 Spine1", binding: "100%-rigid-to-chest/back-joint", targetCharacterNativeMotion: false },
