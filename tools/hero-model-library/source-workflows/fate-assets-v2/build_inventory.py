...
def render_md(data: dict) -> str:
    ...
    lines = [
        ...
        f"- 標準化模型：{s['standardizedGlbsVerified']} 顆本機 GLB 通過逐檔 SHA；現行 hard policy {s['hardPolicyPass']}/{s['minecraftServants']}；{s['gitRuntimeComponents']} 顆已釘入 Git 成品庫。上游 ARR 另記，專案候選註冊核准 {s['projectRegistrationApproved']}。",
        f"- 原生來源動作：{s['sourceClips']} 段，已轉 {s['convertedNativeClips']}，另 {s['retainedNoDurationClips']} 段無來源時長；14 名已用同來源片段補齊六態映射，明確標示語意替代。",
        f"- 後台候選：{s['runtimeSelectable']} 名已對應角色、{s['backendHeroOptions']} 個英雄 ID 可選；全部 `automaticEligible=false`，不搶既有預設。其餘 {s['unmappedServants']} 名保留待設計。",
    ...
    for row in data["minecraftCommunity"]["servants"]:
        ...
        lines.append(
            f"| `{row['characterId']}` | {row['authorLabel']} | {mapped} | {m['triangles']}／{m['drawCalls']}／{m['maxTextureEdge']}px／{m['channelsPerFrame']} | {row['convertedNativeClipCount']}/{row['sourceClipCount']}；六態同來源替代 | {missing} | {'後台候選可選' if row['registration']['runtimeSelectable'] else 'Git 成品；待建立英雄定義'} |"
        )
    lines += [
        "",
        "## 平台界線",
        ...
        "## 註冊結論",
        "",
        f"14 顆 Minecraft 候選都通過現行模型 hard policy 與 10,000 面減面觸發規則，並已作為獨立成品與模型文件釘入 Git。已有英雄定義的 {s['runtimeSelectable']} 名建立 {s['backendHeroOptions']} 個候選選項；10 名未杜撰英雄 ID，保留待設計。上游 ARR 與專案註冊核准分欄保存，沒有把專案核准寫成上游授權。",
        "",
...
def build(workspace: Path) -> tuple[dict, str, dict]:
    ...
    runtime_components_path = BASE / "priority-evidence/fateubw-community/runtime-components-v1/manifest.json"
    runtime_registration_path = BASE / "priority-evidence/fateubw-community/runtime-components-v1/registration-receipt.json"
    ...
    inputs = [downloads_path, mapping_path, reserve_path, completion_path, source_backup_path, native_backup_path, derivative_path, derivative_backup_path, platform_path, psp_audit_path, policy_path, runtime_components_path, runtime_registration_path]
    downloads, mapping, reserve, completion = map(read, inputs[:4])
    source_backup, native_backup, derivative, derivative_backup, platform, psp_audit, policy = map(read, inputs[4:11])
    runtime_components, runtime_registration = read(runtime_components_path), read(runtime_registration_path)
    ...
    component_rows = {row["characterId"]: row for row in runtime_components["components"]}
    registrations = {row["heroId"]: row for row in runtime_registration["registrations"]}
    if not all(len(rows) == 14 for rows in (source_candidates, mapping_rows, reserve_rows, completion_rows, policy_rows, component_rows)):
    ...
    for character in sorted(source_candidates):
        ...
        component = component_rows[character]
        semantics = semantic_candidates(converted["convertedClipNames"], character)
        missing = [action for action in REQUIRED_ACTIONS if action not in component["clipMap"]]
        registered_hero_ids = [hero_id for hero_id in mapped["heroIds"] if hero_id in registrations and registrations[hero_id]["sourceModelKey"] == component["modelKey"]]
        blockers = []
        if not mapped["heroIds"]:
            blockers.append("no-existing-GGD-hero-definition")
        elif len(registered_hero_ids) != len(mapped["heroIds"]):
            blockers.append("backend-option-not-registered")
        if missing:
        ...
        servants.append({
            ...
            "runtimeClipMap": component["clipMap"],
            "runtimeMotionSemanticClass": component["motionSemantics"],
            "missingRequiredActions": missing,
            "s3Backup": converted["backup"],
            "gitRuntimeComponent": {"glb": component["glb"], "modelDocument": component["modelDocument"]},
            "registration": {"eligible": bool(mapped["heroIds"]), "blockers": blockers, "registeredHeroIds": registered_hero_ids, "runtimeSelectable": bool(mapped["heroIds"]) and not blockers, "defaultEligible": False, "productionDeploymentVerified": False},
        })
    ...
    summary = {
        ...
        "upstreamRightsApproved": 0, "projectRegistrationApproved": 14, "eventMapComplete": 14,
        "gitRuntimeComponents": 14, "backendHeroOptions": len(runtime_registration["registrations"]),
        "runtimeSelectable": sum(row["registration"]["runtimeSelectable"] for row in servants), "productionDeployed": 0,
    ...
    inventory = {
        ...
        "scope": {"minecraftCommunitySeparateFromFucOriginal": True, "metadataIsNotPayload": True, "policyPassIsNotRegistration": True, "runtimeRegistrationPerformed": True, "productionDeploymentPerformed": False},
        ...
        "minecraftCommunity": {
            ...
            "runtimeComponents": {"gitPath": runtime_components_path.relative_to(ROOT).as_posix(), "sha256": digest(runtime_components_path)},
            "runtimeRegistration": {"gitPath": runtime_registration_path.relative_to(ROOT).as_posix(), "sha256": digest(runtime_registration_path)},
            "servants": servants,
        },
        "fateUnlimitedCodes": {
    ...
    entry = {
        ...
        "sourceId": SOURCE_ID,
        "runtimeComponentsGitPath": runtime_components_path.relative_to(ROOT).as_posix(),
        "runtimeComponentsSha256": digest(runtime_components_path),
        "runtimeRegistrationGitPath": runtime_registration_path.relative_to(ROOT).as_posix(),
        "runtimeRegistrationSha256": digest(runtime_registration_path),
        "status": "14 Minecraft components Git-pinned with six-state fallbacks; 4 mapped characters/5 hero IDs backend selectable candidate-only; 10 await hero definitions; PSP original remains metadata-only",
        "summary": summary,
