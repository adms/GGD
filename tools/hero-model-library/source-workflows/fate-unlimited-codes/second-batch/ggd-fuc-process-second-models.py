...
spec=importlib.util.spec_from_file_location('fuc_outfit_converter','/private/tmp/ggd-fuc-convert-unity-outfit.py'); converter=importlib.util.module_from_spec(spec);spec.loader.exec_module(converter)
...
name=sys.argv[1];root=BASE/('gamebanana-'+name)
...
character={'shirou':'Shirou Emiya / 衛宮士郎','rin':'Rin Tohsaka / 遠坂凜'}[name]
source_id={'shirou':'gamebanana-fuc-shirou-473819','rin':'gamebanana-fuc-rin-474593'}[name]
...
save(root/'validation.json',{'sourceId':source_id,'archive':json.load(open(root/'acquisition.json')),'safeExtraction':'ZIP complete CRC read with path, link, member and total-size guards','sourceObjectCounts':counts,'modelCandidates':len(candidates),'distinctSourceMeshes':counts.get('Mesh',0),'nativeOriginalAnimations':0,'nativeOriginalVFX':0,'audioCount':len(audio),'audioSeconds':sum(x['seconds'] for x in audio),'sourcePlatform':'unknown','packageScope':'MOD selection, not full original game library','khronosValidation':'pending','freezeStatus':'pending-validation'})
