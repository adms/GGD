...
def render():
    ...
    return "\n".join([
        ...
        "",
        f"三名共固定 {summary['distinctSourceSkills']} 個去重來源技能、{summary['sourceSkillMotionCandidates']} 段模型內技能動作候選與 {summary['genericCryCandidates']} 段一般叫聲。獨立原作 VFX {summary['acquiredStandaloneVfx']}、技能專屬 SFX {summary['acquiredSkillSpecificSfx']}、轉換 VFX {summary['convertedVfx']}、核准音訊 {summary['approvedAudio']}、runtime 新增綁定 {summary['runtimeBindingsAdded']}、正式部署 {summary['productionDeployed']}。動作名不能代替特效資產，一般叫聲也不能代替技能音效。",
        "",
