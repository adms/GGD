...
「超過 10,000 面才啟動減面」、貼圖邊長、draw primitive 與動畫通道限制都由 `emit_runtime_policy.mts` 即時 import 正式 runtime 常數。owner 現行規則是減面後不超過 8,000 面；正式 `adoptionPolicy.json` 與 runtime preparer 使用同一個 `<= 8,000` 門檻。
