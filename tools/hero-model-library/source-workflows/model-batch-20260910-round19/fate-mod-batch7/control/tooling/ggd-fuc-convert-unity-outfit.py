...
def convert(bundle, output, root_name, height=1.8, renderer_materials=None):
    import UnityPy
    ...
    for pid, tree in trees.items():
        if readers[pid].type.name != 'SkinnedMeshRenderer' or ref(tree['m_GameObject']) not in go_transform:
            continue
        if renderer_materials is not None:
            if pid not in renderer_materials:
                continue
            tree = dict(tree)
            tree['m_Materials'] = renderer_materials[pid]
            tree['m_Enabled'] = True
    ...
    if renderer_materials is not None and {int(m['rendererPathId']) for m in meshes} != set(renderer_materials):
        raise ValueError('Selected outfit renderer missing from prefab subtree')
    if not meshes:
    ...
    receipt = {'schema': 'ggd-unity-prefab-conversion@1', 'source': {'path': str(bundle),
               ...
               'explicitOutfitRendererMaterials': renderer_materials,
