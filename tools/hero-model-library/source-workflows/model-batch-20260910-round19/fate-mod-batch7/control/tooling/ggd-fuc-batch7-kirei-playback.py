...
ROOT=Path('/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT/GGD-Asset-Library/intake/public-models-20260910/fate-unlimited-codes-mod-models-batch7/kotomine-kirei')
assert not (ROOT.parent/'control/handoff.json').exists()
(ROOT/'preview').mkdir(exist_ok=True)
index=json.loads((ROOT/'motion-index.json').read_text());data=(ROOT/'converted/kotomine-kirei-standard.glb').read_bytes()
...
for ci,clip in enumerate(index['clips']):
 duration=clip['sampleDurationSeconds'];times=[0,duration*.21,duration*.5,duration*.79,duration];samples=[];reference=None;maximum=0.
 for t in times:
  verts=np.concatenate(skinned(transforms(ci,t)));assert np.isfinite(verts).all()
  if reference is None:reference=verts.copy()
  delta=float(np.linalg.norm(verts-reference,axis=1).max());maximum=max(maximum,delta);samples.append({'seconds':t,'sourceFrame':t*clip['fps'],'worldBounds':[verts.min(axis=0).tolist(),verts.max(axis=0).tolist()],'maxVertexDeltaFromStart':delta})
 proofs.append({'glbAnimationIndex':ci,'sourceSequence':clip['sourceSequence'],'blendIndex':clip['blendIndex'],'sourceTimeVarying':clip['hasTimeVaryingTransform'],'sampleCount':len(samples),'maxSkinnedVertexDisplacement':maximum,'sampledGeometryChanges':maximum>1e-6,'samples':samples})
put(ROOT/'analysis/cpu-playback-verification.json',{'modelSha256':hashlib.sha256(data).hexdigest(),'status':'pass-all-finite','method':'Independent glTF accessor decoder, TRS interpolation and CPU skinning; five samples per entry. No game or GGD engine executed.','animationEntries':len(proofs),'sampledFrames':sum(x['sampleCount'] for x in proofs),'entriesWithSampledGeometryChanges':sum(x['sampledGeometryChanges'] for x in proofs),'vertexInstancesPerFrame':sum(len(p['positions']) for p in primitives),'clips':proofs,'caveats':['Sven/GoldSrc motion, not Fate native.','Sequence linear movement, event callbacks and runtime blend-controller behavior are preserved as metadata but not simulated here.','Some multiple-frame entries are static poses.']})
...
chosen=[next(i for i,x in enumerate(index['clips']) if x['sourceLabel']==label) for label in ['look_idle','run2','action_wave']];sheet=Image.new('RGB',(320*4,350*3),'white')
for ri,ci in enumerate(chosen):
 duration=index['clips'][ci]['sampleDurationSeconds'];times=[0,duration/3,duration*2/3,duration]
 ...
 for col,(t,verts) in enumerate(zip(times,frames)):
  im=render(verts,center,span);sheet.paste(im,(col*320,ri*350));ImageDraw.Draw(sheet).text((col*320+10,ri*350+324),f"{index['clips'][ci]['sourceLabel']} t={t:.3f}s",fill=(25,30,40))
sheet.save(ROOT/'preview/kirei-motion-contact-sheet.png')
print(json.dumps({'status':'pass','entries':len(proofs),'frames':len(proofs)*5,'movingSampledEntries':sum(x['sampledGeometryChanges'] for x in proofs),'preview':str(ROOT/'preview/kirei-motion-contact-sheet.png')},indent=2))
