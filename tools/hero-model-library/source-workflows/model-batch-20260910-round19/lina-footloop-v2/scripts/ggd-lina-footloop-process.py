...
def solve_ik(nodes,side,goal,footR):
 hi,ki,fi=[names[f'{name}{side}'] for name in ['Hip','Knee','Foot']];w=globals(nodes);H,K,F=[w[x][:3,3] for x in [hi,ki,fi]];l1=np.linalg.norm(K-H);l2=np.linalg.norm(F-K);delta=goal-H;dist=np.linalg.norm(delta);reach=min(max(dist,abs(l1-l2)+.00001),l1+l2-.00001);u=norm(delta);A=H+u*reach;pole=np.array([0.,0.,1.])-u*u[2]
...
for anim in model.gltf['animations']:
 isidle='bat_idle'in anim['name'];d=lib.duration(model,anim);times=sorted(set(np.linspace(0,d,math.ceil(d*480)+1).tolist())|{float(t) for sm in anim['samplers'] for t in model.accessor_values(sm['input'])});times=np.unique(np.asarray(times,dtype=np.float32)).astype(float).tolist();channels=copy.deepcopy(anim['channels']);values={i:[] for i in range(len(channels))};evidence=[];centers={'R':.10,'L':.62};anchor={}
 if not isidle:
  for s in 'LR':
 ...
 animations.append(output);reports.append({'sourceClip':anim['name'],'outputClip':output['name'],'durationSeconds':d,'outputTimes':times,'samplingHz':480,'durationChanged':False,'nativeAnimation':False,'staticPelvisYAdjustmentMeters':-.02 if isidle else -.06,'loopBlendStartPhase':.82,'loopBlendMethod':'last 18% crossfade to first-pose backward extrapolation, matching first-frame velocity; derived seam processing','contactMethod':'two-bone IK, boot flat orientation, actual skinned-boot ground correction','runContactWindows':None if isidle else {'centers':centers,'coreHalfPhase':.12,'blendPhase':.06},'testActorSpeedMps':0 if isidle else speed,'frames':evidence})
j['animations']=animations;j['buffers']=[{'byteLength':len(saved)}];j['asset']['generator']='GGD 300heroes-to-Rays derived IK footplant and loop processing review';OUT=P/'candidates/lina00-hands-a-300-idle-run-footplant-loop-v2.glb';glb.write(str(OUT),j,bytes(saved));proof={'schema':'ggd-derived-animation-footplant-loop@1','input':str(IN),'inputSha256':hashlib.sha256(IN.read_bytes()).hexdigest(),'output':str(OUT),'outputSha256':hashlib.sha256(OUT.read_bytes()).hexdigest(),'animationAttribution':'300英雄動作重綁到Rays模型，再作落地與循環加工','nativeRaysAnimations':0,'nativeFps':'unknown','speedFit':fits,'testActorSpeedMps':speed,'rootMotion':'GLB remains in-place; optional preview entity world translation at fitted speed tests foot plant; not native root motion','clips':reports,'runtimeReady':False};(P/'processing-proof.json').write_text(json.dumps(proof,ensure_ascii=False,indent=2)+'\n');shutil.copyfile('/private/tmp/ggd-lina-footloop-process.py',P/'scripts/ggd-lina-footloop-process.py');print(json.dumps({'file':str(OUT),'speed':speed,'fits':fits,'clips':[{'name':r['sourceClip'],'keys':len(r['outputTimes']),'minY':min(f['legs'][s]['minBootY'] for f in r['frames'] for s in 'LR'),'maxIKUnreachable':max(f['legs'][s]['ik']['unreachableDistance'] for f in r['frames'] for s in 'LR')} for r in reports]},indent=2))
