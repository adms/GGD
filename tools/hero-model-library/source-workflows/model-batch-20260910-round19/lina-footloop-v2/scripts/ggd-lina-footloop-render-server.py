...
ROOT=Path('/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT');base=ROOT/'GGD-Asset-Library/intake/public-models-20260910';dst=base/'parallel-community-lina-rays-300-retarget-footloop-v2';assets={'prior':base/'parallel-community-lina-rays-300-retarget/candidates/lina00-hands-a-300-idle-run-resampled-120hz.glb','processed':dst/'candidates/lina00-hands-a-300-idle-run-footplant-loop-v2.glb'};done=threading.Event();run=dst/'evidence/playback';run.mkdir(exist_ok=True)
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/bundle.js':payload=Path('/private/tmp/ggd-lina-footloop-render-bundle.js').read_bytes();mime='text/javascript'
...
for f in ['ggd-lina-footloop-render-server.py','ggd-lina-footloop-render.mjs']:shutil.copyfile('/private/tmp/'+f,dst/'scripts'/f)
