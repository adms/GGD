...
root=Path(sys.argv[1]).resolve();variant=sys.argv[2];src=root/'converted'/variant/'body.glb';source_hash=hashlib.sha256(src.read_bytes()).hexdigest();out=root/'previews'/variant;out.mkdir(parents=True,exist_ok=False)
...
coords=[o.matrix_world @ Vector(c) for o in scene.objects if o.type=='MESH' for c in o.bound_box];center=Vector([(min(v[i] for v in coords)+max(v[i] for v in coords))/2 for i in range(3)])
receipts=[]
