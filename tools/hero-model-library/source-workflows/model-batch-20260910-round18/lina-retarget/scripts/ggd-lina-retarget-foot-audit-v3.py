...
source=glb.read(str(p/'candidates/lina00-hands-a-300-idle-run-resampled-120hz.glb'));sj=source.gltf;names={n['name']:i for i,n in enumerate(sj['nodes'])};tree=ast.parse(Path('/private/tmp/ggd-lina-retarget.py').read_text());fn=next(n for n in tree.body if isinstance(n,ast.FunctionDef) and n.name=='sample_animation');code=ast.unparse(fn);line=next(s for s in code.splitlines() if s.strip().startswith('trsrows ='));code=code.replace(line,'    trsrows = copy.deepcopy(sj["nodes"])');exec(code)
