...
j['animations']=all_anims;j['buffers']=[{'byteLength':len(saved_binary)}];j['asset']['generator']='GGD bounded anatomical-axis 300heroes animation retarget prototype';out=p/'candidates/lina00-hands-a-300-idle-run-stable-torso.glb';glb.write(str(out),j,bytes(saved_binary));sha=lambda f:hashlib.sha256(f.read_bytes()).hexdigest();proof={'schema':'ggd-bounded-retarget-prototype@1','sourceModel':str(SRC),'sourceSha256':sha(SRC),'targetModel':str(TGT),'targetSha256':sha(TGT),'output':str(out),'outputSha256':sha(out),'sourceGame':'300英雄','targetSourceGame':'Tales of the Rays','attributionLabel':'300英雄動作重綁到Rays模型','nativeRaysAnimationClips':0,'sourceWalkClipAvailable':False,'sourceClips':['bat_idle','single_run'],'mappedJoints':mapping,'legLengthRatio':ratio,'sourceLegLength':leg_source,'targetLegLengthMeters':leg_target,'targetSceneScale':.01,'clips':clip_proof,'runtimeReady':False,'visualValidationComplete':False,'knownLimits':['Anatomical aim alignment does not transfer limb axial twist; target fingers are rigid mesh alternatives','No toe joint on Rays target; source toe orientation drives target boot orientation','Unmapped hair/cape/accessory bones retain local rest pose','Constant per-clip ground offset does not implement foot-contact IK','Source has idle/run but no separate walk; original run tempo retained']};(p/'retarget-proof-stable-torso.json').write_text(json.dumps(proof,ensure_ascii=False,indent=2)+'\n');shutil.copyfile('/private/tmp/ggd-lina-retarget.py',p/'scripts/ggd-lina-retarget.py');print(json.dumps({'mapped':len(mapping),'output':str(out),'bytes':out.stat().st_size,'clips':[{'name':x['sourceClip'],'ground':x['constantGroundOffsetMeters'],'maxRotationStep':max(x['maxAdjacentRotationStepDegrees'].values())} for x in clip_proof],'legRatio':ratio},ensure_ascii=False))
