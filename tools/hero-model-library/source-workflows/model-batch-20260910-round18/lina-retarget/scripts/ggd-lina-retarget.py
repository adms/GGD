...
for original_name in ['bat_idle','single_run']:
 animation=next(a for a in sj['animations'] if a['name']==original_name);times=sorted({float(t) for sam in animation['samplers'] for t in source.accessor_values(sam['input'])});frames=[];footframes=[]
 for time in times:
  ...
  for t,s,kind,sc,tc in maprows:
   ...
   if kind=='torso':desired[ti]=yframe(vec(sw,sn,s,sc),slateral)@yframe(vec(tw0,tn,t,tc),tlateral).T@r0
 ...
 floor=min(min(x['LminY'],x['RminY']) for x in footframes);ground=-floor;pelvisparent=tw0[tp[tn['Pelvis']]];localGround=(np.linalg.inv(pelvisparent)@np.array([0,ground,0,0]))[:3]
 ...
 ai=add_accessor([f['t'][tn['Pelvis']] for f in frames],'VEC3');output['samplers'].append({'input':inputidx,'output':ai,'interpolation':'LINEAR'});output['channels'].append({'sampler':len(output['samplers'])-1,'target':{'node':tn['Pelvis'],'path':'translation'}});all_anims.append(output);clip_proof.append({'sourceClip':original_name,'outputClip':output['name'],'duration':times[-1]-times[0],'originalKeyTimes':times,'sampling':'union of existing source key times; source LINEAR quaternion/translation evaluation','constantGroundOffsetMeters':ground,'rootMotion':'in-place horizontal root position; source root vertical delta scaled by target/source leg length; no invented trajectory','footSamples':footframes,'maxAdjacentRotationStepDegrees':steps,'nativeRaysAnimation':False})
j['animations']=all_anims;j['buffers']=[{'byteLength':len(saved_binary)}];j['asset']['generator']='GGD bounded anatomical-axis 300heroes animation retarget prototype';out=p/'candidates/lina00-hands-a-300-idle-run-prototype.glb';glb.write(str(out),j,bytes(saved_binary));sha=lambda f:hashlib.sha256(f.read_bytes()).hexdigest();proof={'schema':'ggd-bounded-retarget-prototype@1','sourceModel':str(SRC),'sourceSha256':sha(SRC),'targetModel':str(TGT),'targetSha256':sha(TGT),'output':str(out),'outputSha256':sha(out),'sourceGame':'300英雄','targetSourceGame':'Tales of the Rays','attributionLabel':'300英雄動作重綁到Rays模型','nativeRaysAnimationClips':0,'sourceWalkClipAvailable':False,'sourceClips':['bat_idle','single_run'],'mappedJoints':mapping,'legLengthRatio':ratio,'sourceLegLength':leg_source,'targetLegLengthMeters':leg_target,'targetSceneScale':.01,'clips':clip_proof,'runtimeReady':False,'visualValidationComplete':False,'knownLimits':['Anatomical aim alignment does not transfer limb axial twist; target fingers are rigid mesh alternatives','No toe joint on Rays target; source toe orientation drives target boot orientation','Unmapped hair/cape/accessory bones retain local rest pose','Constant per-clip ground offset does not implement foot-contact IK','Source has idle/run but no separate walk; original run tempo retained']};(p/'retarget-proof.json').write_text(json.dumps(proof,ensure_ascii=False,indent=2)+'\n');shutil.copyfile('/private/tmp/ggd-lina-retarget.py',p/'scripts/ggd-lina-retarget.py');print(json.dumps({'mapped':len(mapping),'output':str(out),'bytes':out.stat().st_size,'clips':[{'name':x['sourceClip'],'ground':x['constantGroundOffsetMeters'],'maxRotationStep':max(x['maxAdjacentRotationStepDegrees'].values())} for x in clip_proof],'legRatio':ratio},ensure_ascii=False))
