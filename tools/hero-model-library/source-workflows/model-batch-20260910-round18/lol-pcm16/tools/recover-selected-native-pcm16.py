...
p=argparse.ArgumentParser();p.add_argument('--input-intake',type=Path,required=True);p.add_argument('--output-root',type=Path,required=True);p.add_argument('--decoder',type=Path,default=Path('/private/tmp/ggd-vgmstream-r2117/cli/vgmstream-cli'));p.add_argument('--native-id',action='append',required=True);a=p.parse_args();root=a.input_intake.resolve();out=a.output_root.resolve();out.mkdir(parents=True,exist_ok=True)
...
for dp in sorted((root/'decoded').glob('*/decoding.json')):
 if dp.parent.name not in a.native_id:continue
