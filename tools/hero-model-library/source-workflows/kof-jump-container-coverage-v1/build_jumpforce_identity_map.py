...
def source_hero_ids(download_sources_path: Path) -> dict[str, list[str]]:
    """Read explicit JForce_/jumpforce- source groups with existing hero IDs."""
    result: dict[str, set[str]] = collections.defaultdict(set)

    def keys(value: dict) -> set[str]:
        source_id = value.get("id")
        if not isinstance(source_id, str):
            return set()
        folded = source_id.casefold()
        if folded.startswith("jforce_"):
            suffix = source_id[len("JForce_"):]
        elif folded.startswith("jumpforce-"):
            suffix = source_id[len("jumpforce-"):].replace("-", " ")
        else:
            return set()
        names = {suffix}
        for field in ("characterName", "name"):
            label = value.get(field)
            if isinstance(label, str) and label:
                names.add(label.split(" / ", 1)[0])
        return {name.strip().replace("_", " ").casefold() for name in names if name.strip()}
        ...
        if isinstance(value, dict):
            ...
            if isinstance(hero_ids, list):
                for key in keys(value):
                    result[key].update(str(hero_id) for hero_id in hero_ids if hero_id)
...
def build(index_path: Path, authority_path: Path, voice_files_path: Path, download_sources_path: Path | None = None) -> dict[str, object]:
    ...
    for identity in named.values():
        identity["heroIds"] = explicit_hero_ids.get(identity["characterName"].replace("_", " ").casefold(), [])
