...
from build_jumpforce_identity_map import build, classify_path, derive_named_audio_identities, source_hero_ids
...
class JumpForceIdentityMapTest(unittest.TestCase):
    ...
    def test_source_hero_ids_accepts_both_jump_group_namings(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "sources.json"
            path.write_text(json.dumps({
                "publicSources": [{
                    "audioGroups": [
                        {"id": "JForce_Dai", "characterName": "Dai", "heroIds": ["dai"]},
                        {"id": "jumpforce-killua", "characterName": "Killua", "heroIds": ["killua"]},
                    ]
                }]
            }))
            self.assertEqual(source_hero_ids(path)["dai"], ["dai"])
            self.assertEqual(source_hero_ids(path)["killua"], ["killua"])
