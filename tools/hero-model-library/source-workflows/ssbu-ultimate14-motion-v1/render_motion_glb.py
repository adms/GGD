"""Render start, middle and end of each pinned Ultimate14 motion in isolated headless Chrome."""
from pathlib import Path
from http.server import ThreadingHTTPServer, BaseHTTPRequestHandler
import argparse,base64,hashlib,json,os,re,subprocess,tempfile,threading

parser=argparse.ArgumentParser();parser.add_argument('source',type=Path);parser.add_argument('output',type=Path);parser.add_argument('--repo',type=Path,required=True);args=parser.parse_args()
source=args.source.resolve();out=args.output.resolve();out.mkdir(parents=True,exist_ok=False)
root=args.repo.resolve();done=threading.Event();env=dict(os.environ,NODE_PATH=str(root/'node_modules/.pnpm/node_modules'))
...
chrome='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';profile=Path(tempfile.mkdtemp(prefix='ggd-ssbu-motion-chrome-'))
