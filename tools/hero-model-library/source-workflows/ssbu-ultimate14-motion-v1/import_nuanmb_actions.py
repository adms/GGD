#!/usr/bin/env python3
"""Import pinned Ultimate14 body motions into the matching frozen SSBU rig.
...
fighter_id = args.fighter_id.lower()
if not fighter_id.replace("_", "").isalnum():
    raise SystemExit("fighter-id must contain only letters, digits or underscores")
if Path(args.output_name).name != args.output_name or not args.output_name.endswith(".blend"):
    raise SystemExit("output-name must be one .blend filename")
...
allowed = {
    ...
    if row["fighterId"] == fighter_id and row["directoryClass"] == "body-motion"
}
motions = []
for path in args.motions:
    ...
    if not any((row["relativePath"], digest_value) in allowed for row in matches):
        raise SystemExit(f"Motion is not a pinned Ultimate14 {fighter_id} body motion: {path}")
...
isolated_root = Path(f"/private/tmp/ggd-ssbu-{fighter_id}-motion-blender-empty")
for key, leaf in (
...
blend_path = output / args.output_name
...
receipt = {
    "schema": "ggd-ssbu-ultimate14-action-import@1",
    ...
    "gaps": [
        f"Ultimate14 contains {len(motions)} distinct {fighter_id} body motion payloads in this batch, not a complete gameplay action set.",
