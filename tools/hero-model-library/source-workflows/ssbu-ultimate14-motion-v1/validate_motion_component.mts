/** Validate an SSBU GLB carrying pinned Ultimate14 native Transform animations. */
...
if (!glbArgument || !blenderReceiptArgument || !importReceiptArgument || !outputArgument) {
  throw new Error('Usage: validate_motion_component.mts <component.glb> <conversion.json> <import-receipt.json> <validation.json>');
}
...
assert.equal(digest(bytes), blenderReceipt.output.sha256, 'GLB differs from Blender receipt');
assert.equal(bytes.length, blenderReceipt.output.bytes, 'GLB size differs from Blender receipt');
assert.equal(blenderReceipt.input.sha256, importReceipt.output.sha256, 'Blender export did not use the pinned animated intermediate');
assert.equal(importReceipt.schema, 'ggd-ssbu-ultimate14-action-import@1');
assert.equal(importReceipt.fighterId, 'sonic');
assert.equal(importReceipt.imports.length, 10, 'Expected exactly ten distinct Sonic body motions');
...
assert.equal(jointIndices.size, 115, 'Expected the complete Sonic c00 skin');
...
for (const animation of document.animations ?? []) {
  ...
  const payload = payloadBySha.get(imported.sha256) as {transformNodeNames: string[]} | undefined;
  ...
  assert.equal(expectedTargetIndices.size, 98, `${animation.name} expected Transform-node count changed`);
  ...
  for (const expected of expectedTargetIndices) assert(targetPaths.has(expected), `${animation.name} omits a pinned NUANMB Transform node`);
  assert.deepEqual(new Set(targetPaths.keys()), jointIndices, `${animation.name} must contain Blender sampled TRS channels for the complete skin`);
  for (const paths of targetPaths.values()) assert.deepEqual([...paths].sort(), ['rotation', 'scale', 'translation']);
  const exporterEvaluatedAdditionalJoints = [...targetPaths.keys()].filter(index => !expectedTargetIndices.has(index));
  assert.equal(exporterEvaluatedAdditionalJoints.length, 17, `${animation.name} exporter evaluated joint count changed`);
  const timeChecks = [];
  ...
  animationChecks.push({name: animation.name, channelCount: animation.channels.length, targetedJointCount: targetPaths.size, pinnedTransformNodeCount: expectedTargetIndices.size, exporterEvaluatedAdditionalJointCount: exporterEvaluatedAdditionalJoints.length, timeAccessors: timeChecks});
}
...
const result = {
  schema: 'ggd-ssbu-ultimate14-motion-validation@1',
  ...
  ggdInspection: {
    ...
    pinnedTransformJointCountPerClip: 98,
    exporterEvaluatedAdditionalJointCountPerClip: 17,
  ...
  limitations: [
    'Ten Ultimate14 community MOD Sonic body motions are preserved; this is not Sonic\'s complete original action set.',
    'Each clip includes its pinned 98 NUANMB Transform nodes; Blender samples evaluated TRS channels for the remaining 17 skin joints during export.',
...
console.log(JSON.stringify({output: outputPath, sha256: result.glb.sha256, khronosErrors: khronos.issues.numErrors, khronosWarnings: khronos.issues.numWarnings, clips: inspection.clips.length, joints: jointIndices.size, animatedJoints: 98, triangles: inspection.triangles, draws: inspection.meshes, textures: inspection.textures, budgetErrors: budget.errors.length, budgetWarnings: budget.warnings}));
