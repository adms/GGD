#!/usr/bin/env python3
"""Render every animation group in a FateUBW GLB through Babylon WebGL."""
from pathlib import Path
from http.server import ThreadingHTTPServer, BaseHTTPRequestHandler
import argparse, base64, hashlib, json, os, re, shutil, signal, subprocess, tempfile, threading
...
subprocess.run([str(esbuild),str(Path(__file__).with_suffix('.mjs')),'--bundle','--format=esm','--outfile='+str(out/'bundle.js')],check=True,env=env)
...
chrome='/Applications/Google Chrome.app/Contents/MacOS/Google Chrome';profile=Path(tempfile.mkdtemp(prefix='ggd-fateubw-native-chrome-'))
