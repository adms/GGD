...
## 2a. 先完成已存在的未加密 Streaming 音訊批次

不需要 PAK AES key 的 43 個 AWB 已解碼為 4,034 個 WAV。以明確角色家族關係登記已有 GGD hero ID，再進行可續跑的逐檔 SHA-256 與 WAV header 驗證：

```bash
python3 tools/hero-model-library/source-workflows/jump-force-full-roster-v1/register_audio_identity_bindings.py --write
python3 tools/hero-model-library/source-workflows/kof-jump-container-coverage-v1/build_jumpforce_identity_map.py --repo . --workspace ..
python3 tools/hero-model-library/source-workflows/jump-force-full-roster-v1/audit_loose_audio_batch.py --write --full-rehash
python3 tools/hero-model-library/source-workflows/jump-force-full-roster-v1/audit_loose_audio_batch.py --check
```

`--check` 預設使用同一 index SHA、檔案大小、預期 SHA 與奈秒 mtime 固定的本機 cache；需要全部重算時加 `--full-rehash`。目前 37 個 loose Streaming 角色群中有 8 組連到 12 個既有 hero ID；連同已保存公開原包後，總計 12 組連到 17 個 hero ID。這批只登記來源角色家族，不核准逐段說話者、語言、事件、技能或 runtime 綁定。
