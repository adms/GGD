...
for(const prop of ['handheld','back']){
 ...
 const after=await inspectModelUpload(normalized.bytes),heroBudget=heroModelBudgetIssues(after);
 // A rigid prop must remain ineligible as a hero body.  Keep that current
 // contract result in the receipt, while applying the shared numeric budget
 // to the component itself.
 const heroModelIneligibilityErrors=heroBudget.errors.filter((message:string)=>message.includes('沒有骨架綁定'));
 const budget={errors:heroBudget.errors.filter((message:string)=>!heroModelIneligibilityErrors.includes(message)),warnings:heroBudget.warnings};
 assert.equal(heroModelIneligibilityErrors.length,1);assert.deepEqual(budget.errors,[]);assert.equal(after.triangles,1854);assert.equal(after.meshes,2);assert.equal(after.clips.length,0);assert.equal(after.json.skins?.length??0,0);assert.equal(after.json.nodes?.length,1);assert.equal(after.textures.length,7);assert.ok(after.textures.every((t:any)=>t.width<=HERO_MODEL_BUDGET.texEdge.limit&&t.height<=HERO_MODEL_BUDGET.texEdge.limit));
