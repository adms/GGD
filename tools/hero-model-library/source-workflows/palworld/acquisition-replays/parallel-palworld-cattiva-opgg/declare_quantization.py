...
root=Path(__file__).resolve().parents[1];src=root/'decoded/cattiva-meshopt-decoded.glb';b=src.read_bytes();n=struct.unpack_from('<I',b,12)[0];g=json.loads(b[20:20+n]);tail=b[20+n:]
fixes=[]
for m in g['meshes']:
...
out=root/'decoded/cattiva-quantization-declared.glb';assert not out.exists() or out.read_bytes()==data;out.write_bytes(data)
report={'schema':'ggd.cattiva.quantization-declaration@1','before':{'path':str(src),'sha256':hashlib.sha256(b).hexdigest()},'after':{'path':str(out),'bytes':len(data),'sha256':hashlib.sha256(data).hexdigest()},'normalAccessors':fixes,'binaryChunksIdentical':True,'change':'Added missing KHR_mesh_quantization declaration for retained normalized SHORT normals; no geometry, rig, animation sample or texture changes.'}
