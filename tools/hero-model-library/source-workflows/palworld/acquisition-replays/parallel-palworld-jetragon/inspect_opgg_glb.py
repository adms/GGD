...
root=Path(__file__).resolve().parents[1];source=root/'original/opgg/model.glb';b=source.read_bytes()
...
# Check every stored accessor, inverse bind matrices and hierarchy references.
for i in range(len(g['accessors'])): acc(i)
parents={child:pi for pi,node in enumerate(g['nodes']) for child in node.get('children',[])}
rig=[]
for si,skin in enumerate(g['skins']):
 ibm=acc(skin['inverseBindMatrices']);assert len(ibm)==len(skin['joints'])
 rig.append({'skin':si,'jointCount':len(skin['joints']),'inverseBindMatricesFinite':True,'inverseBindDecodedSha256':hashlib.sha256(ibm.tobytes()).hexdigest(),'joints':[{'node':i,'name':g['nodes'][i].get('name'),'parentNode':parents.get(i)} for i in skin['joints']]})
signatures={}
for a,c in zip(g['animations'],clips):
 sig=[{'node':r['node'],'path':r['path'],'times':r['timeSha256'],'values':r['valuesSha256'],'interpolation':a['samplers'][ch['sampler']].get('interpolation','LINEAR')} for r,ch in zip(c['tracks'],a['channels'])]
 signature=hashlib.sha256(json.dumps(sorted(sig,key=lambda r:(r['node'],r['path'])),sort_keys=True,separators=(',',':')).encode()).hexdigest()
 c['decodedTrackSignatureSha256']=signature;signatures.setdefault(signature,[]).append(c['name'])
...
out=root/'decoded/jetragon-meshopt-decoded.glb';out.parent.mkdir(exist_ok=True);payload=struct.pack('<4sII',b'glTF',2,28+len(j)+len(blob))+struct.pack('<II',len(j),0x4e4f534a)+j+struct.pack('<II',len(blob),0x004e4942)+blob
assert not out.exists() or out.read_bytes()==payload;out.write_bytes(payload)
mat=json.loads((root/'original/opgg/materials.json').read_text());assert mat['animations']==[a['name'] for a in g['animations']]
report={'schema':'ggd.jetragon.opgg.static-analysis@1','sourcePath':str(source),'sourceBytes':len(b),'sourceSha256':hashlib.sha256(b).hexdigest(),'asset':g['asset'],'allAccessorCount':len(g['accessors']),'allDecodedAccessorValuesFinite':True,'skeletons':rig,'uniqueDecodedTrackSignatureCount':len(signatures),'duplicateClipContentGroups':[v for v in signatures.values() if len(v)>1],'meshes':meshes,'meshCount':len(g['meshes']),'skinCount':len(g['skins']),'uniqueJointNodes':len({i for s in g['skins'] for i in s['joints']}),'animationCount':len(clips),'sourceManifestMatchesClipNames':True,'animations':clips,'sumClipDurations':sum(c['durationSeconds'] for c in clips),'sourceTextureCount':len({v for x in mat['materials'].values() for k,v in x.items() if k.endswith('Texture') and isinstance(v,str)}),'imageCountInSourceGlb':len(g.get('images',[])),'nativeAudioCount':0,'vfxCount':0,'decodedModel':{'path':str(out),'bytes':len(payload),'sha256':hashlib.sha256(payload).hexdigest(),'method':'meshopt view decompression only; same geometry accessor bytes/rig/animation values; no retarget/resampling'},'readiness':'pending-material-binding-and-GGD-standardization','limitations':['Original model.glb has no images. All eight source material images and material parameters preserved separately; no claim this raw body is textured in GGD.','Source shader subsurface/clearcoat render profile is not fully translated.','29 stored game-named clips verified with decoded samples; raw Unreal animation files not acquired.','No new procedural animation. No original Wwise bank or VFX in this model source.']}
(root/'analysis/opgg-static-analysis.json').write_text(json.dumps(report,indent=2)+'\n');print(json.dumps({'meshCount':report['meshCount'],'jointCount':report['uniqueJointNodes'],'animations':[{k:c[k] for k in ['name','durationSeconds','varyingChannels']} for c in clips],'decoded':report['decodedModel']},indent=2))
