"""Freeze acquired PalDB settings source intake and verify authorized S3 PutObject via full GetObject.
...
control={'schema':'ggd.palworld-settings.delivery@1','entries':[json.loads((ROOT/'source-entry.json').read_text())]}
delivery=BASE/'GGD-Asset-Library/deliveries/palworld-three-character-settings-20260911';delivery.mkdir(parents=True,exist_ok=True)
archiveRoot=BASE/'GGD-Asset-Library/archives/palworld-three-character-settings-20260911';archiveRoot.mkdir(parents=True,exist_ok=True)
...
for e in control['entries']:
 root=BASE/e['localPath']
 if Path(__file__).resolve()!=(root/'tools/backup.py').resolve():shutil.copy2(__file__,root/'tools/backup.py')
 rows=[]
 ...
 digest=sha(archive);key='legacy/character-settings/'+e['id']+'-20260911/'+digest+'.tar.gz'
...
write(delivery/'source-entries.json',control);write(Path('/private/tmp/ggd-palworld-settings-final-delivery.json'),{'deliveryRoot':str(delivery),'sourceEntriesPath':str(delivery/'source-entries.json'),'sourceEntriesSha256':sha(delivery/'source-entries.json'),'receipts':receipts})
