"""Local-only Chromium preview; new output directory; never runs source scripts."""
from pathlib import Path
from http.server import ThreadingHTTPServer, BaseHTTPRequestHandler
import argparse, base64, json, re, subprocess, tempfile, threading
...
a=p.parse_args()
root=a.result.resolve(); run=a.output.resolve(); run.mkdir(parents=True,exist_ok=False)
tool=Path(__file__).resolve().parent
bundle=run/'render-bundle.js'
subprocess.run(['node',str(tool/'bundle.cjs'),str(a.repo.resolve()),str(tool/'render.mjs'),str(bundle)],check=True)
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/bundle.js':payload=bundle.read_bytes();mime='text/javascript'
  elif self.path=='/asset/body':payload=(root/'body.glb').read_bytes();mime='model/gltf-binary'
...
with tempfile.TemporaryDirectory(prefix='ggd-cattiva-headless-',dir='/private/tmp') as profile:
