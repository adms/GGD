#!/usr/bin/env python3
"""Register validated Worldblender c00 components in the canonical source indexes."""
...
CANDIDATES = REPO / "materials/hero-model-library/priority-evidence/ssbu-worldblender-c00-batch-v1/candidate-rows.json"
...
IDENTITIES = {
    "ssbu-mario-c00-static-skinned-v2": {"id": "community:ssbu-mario-c00-standardized-v2", "aliases": ["Mario", "瑪利歐", "fighter/mario"], "heroes": []},
    "ssbu-link-c00-static-skinned-v1": {"id": "community:ssbu-link-c00-standardized-v1", "aliases": ["Link", "林克", "fighter/link"], "heroes": ["godie-h00l"]},
    "ssbu-sonic-c00-static-skinned-v1": {"id": "community:ssbu-sonic-c00-standardized-v1", "aliases": ["Sonic", "索尼克", "fighter/sonic"], "heroes": []},
}
...
def main() -> int:
    ...
    require({row.get("id") for row in candidates} == set(IDENTITIES), "Candidate set differs from the fixed three-character batch")
    ...
    for candidate in candidates:
        ...
        for field in ("deliveryEvidence", "validationEvidence", "acceptanceEvidence", "visualEvidence", "webglProofEvidence", "sourceRebuildEvidence", "s3BackupEvidence", "alphaAuditEvidence", "materialBakeEvidence"):
            pin = candidate[field]
        ...
        supplemental_row = {
            ...
            "modelCandidates": [{
                "id": candidate["id"], "label": candidate["label"], "library": "community", "sourceId": candidate["sourceId"],
                ...
                "alphaAuditEvidence": candidate["alphaAuditEvidence"],
                "materialBakeEvidence": candidate["materialBakeEvidence"],
                **({"alphaAtlasFinalizationEvidence": candidate["alphaAtlasFinalizationEvidence"]} if candidate.get("alphaAtlasFinalizationEvidence") else {}),
