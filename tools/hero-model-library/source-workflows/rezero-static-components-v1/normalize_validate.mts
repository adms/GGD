/** Normalize and validate one static Re:Zero Unity prefab component. */
...
assert.equal(conversion.sourceUpAxis, "z", "Re:Zero component must use its explicit Z-up source orientation");
...
assert.equal(before.meshes, 1, "Expected one source draw primitive");
assert.equal(before.skins, 1, "Expected one source skin");
...
assert.equal(before.textures[0]!.height, 2048, "Unexpected source texture height");
...
assert.deepEqual(normalized.report.drawCalls, { before: 1, after: 1 });
...
assert.equal(after.textures[0]!.height, 256, "Normalized texture height differs");
...
const toolPaths = [
  "tools/hero-model-library/convert_unity_prefab.py",
