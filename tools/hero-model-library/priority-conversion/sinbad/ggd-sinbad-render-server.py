...
run=root/'control/render-v3b';run.mkdir(exist_ok=False);done=threading.Event()
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/bundle.js':payload=Path('/private/tmp/ggd-sinbad-render-bundle.js').read_bytes();mime='text/javascript'
  elif self.path=='/asset/body':payload=(root/'runtime-v3/body.glb').read_bytes();mime='model/gltf-binary'
...
for f in ['ggd-sinbad-render-server.py','ggd-sinbad-render.mjs']:shutil.copyfile('/private/tmp/'+f,root/'tools'/f)
