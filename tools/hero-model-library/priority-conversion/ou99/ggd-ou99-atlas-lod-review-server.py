...
W=Path('/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT');source=W/'outputs/priority-ou99-standards-v2-20260910';run=W/'outputs/priority-ou99-atlas-lod-review-20260910';run.mkdir(exist_ok=False);(run/'tools').mkdir();done=threading.Event()
rows=[r for r in json.loads((source/'manifest.json').read_text())if r['atlas'] or r['geometryLod'] or r['textureLod']];assets={};brief=[]
for r in rows:
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/bundle.js':payload=Path('/private/tmp/ggd-ou99-atlas-lod-review-bundle.js').read_bytes();mime='text/javascript'
...
for f in ['ggd-ou99-atlas-lod-review.mjs','ggd-ou99-atlas-lod-review-server.py']:shutil.copy2('/private/tmp/'+f,run/'tools'/f)
