...
const workspace='/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT',repo=join(workspace,'GGD-hero-model-options'),out=join(workspace,'outputs/priority-ou99-standards-v2-20260910');
...
const manifest=read(join(out,'budget-rescue-input.json')),workflow=read(join(repo,'materials/hero-model-library/workflow-model-options.json'));
const stage=join(out,'backend-prepare-content-budget-v3');mkdirSync(stage);for(const sub of ['champions','models'])mkdirSync(join(stage,sub));
...
for(const row of manifest.models){
 ...
 try{
  ...
  const candidate=new Uint8Array(readFileSync(row.candidatePath)),inspection=await inspectModelUpload(candidate),doc=read(join(directory,'original/model.json'));
  ...
  const state=service.state(row.runtimeHeroId),command=zModelVersionCommand.parse({action:'register',expectedHash:state.expectedHash,sourceModelKey:document.id,label:option.label+'（標準化 v2）',source:option.source,automaticEligible:option.source.kind!=='style-proxy'});
...
put(join(out,'backend-validation-budget-v3.json'),{schema:'ggd-ou99-backend-validated@2',sourceCommit:manifest.sourceCommit,localRoot:out,passed:results.filter(r=>r.status==='backend-prepare-passed').length,blocked:results.filter(r=>r.status==='blocked').length,models:results});copyFileSync('/private/tmp/ggd-ou99-verify-budget-v3.mts',join(out,'tools/ggd-ou99-verify-budget-v3.mts'));
