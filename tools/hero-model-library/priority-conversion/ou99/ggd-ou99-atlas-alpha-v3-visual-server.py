...
W=Path('/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT');source=W/'outputs/priority-ou99-standards-v3-20260910';run=source/'visual-audit';run.mkdir(exist_ok=False);(run/'tools').mkdir();done=threading.Event()
rows=json.loads((source/'backend-validation-manifest.json').read_text())['models'];assets={};brief=[]
for r in rows:
 key=r['originalModelKey'].replace('.','-');brief.append({'key':key,'clipMap':r['clipMap']});assets[key+'/source']=Path(r['directory'])/'control/v2-body.glb';assets[key+'/candidate']=Path(r['runtime'])/'body.glb'
...
class Handler(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  ...
  elif self.path=='/bundle.js':payload=Path('/private/tmp/ggd-ou99-atlas-alpha-v3-visual-bundle.js').read_bytes();mime='text/javascript'
...
for f in ['ggd-ou99-atlas-alpha-v3-visual.mjs','ggd-ou99-atlas-alpha-v3-visual-server.py']:shutil.copy2('/private/tmp/'+f,run/'tools'/f)
