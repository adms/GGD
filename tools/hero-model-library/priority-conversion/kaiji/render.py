"""Render a local Kaiji GLB in isolated headless Chrome; preserve every run."""
...
root=Path(__file__).resolve().parents[4];done=threading.Event()
...
profile=Path(tempfile.mkdtemp(prefix='ggd-kaiji-chrome-'))
cmd=[chrome,'--headless=new','--no-first-run','--disable-extensions','--disable-background-networking','--disable-component-update','--disable-sync','--no-default-browser-check','--use-gl=angle','--use-angle=swiftshader','--enable-unsafe-swiftshader','--user-data-dir='+str(profile),f'http://127.0.0.1:{server.server_address[1]}/']
