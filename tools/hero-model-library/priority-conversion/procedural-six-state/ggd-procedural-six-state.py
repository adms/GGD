...
roleSigns{role:[x,y,z]}, roleRotationMatrices{role:[[...],[...],[...]]},
basePoseWorldDegrees{node:[x,y,z]},
durations{state:seconds}, unitsPerMeter, yawOffsetDeg,
coordinateSpace ("local" or "world-rest").  "world-rest" converts Y-up world-space
gesture rotations and translations through each source bone's rest transform, so it
does not assume that every rig uses the same local bone axes.  Bone mapping and the
resulting gestures remain source-specific and require visual acceptance.
...
def qconj(q):
    q=np.asarray(q,float);return np.array([-q[0],-q[1],-q[2],q[3]])/np.linalg.norm(q)
...
def rotation_from_matrix(m):
    linear=np.asarray(m,float)[:3,:3];sc=np.linalg.norm(linear,axis=0)
    if np.any(sc<1e-12):raise ValueError('Rest transform has a zero scale axis')
    rot=linear/sc
    if np.linalg.det(rot)<0:rot[:,0]*=-1
    if float(np.max(np.abs(rot.T@rot-np.eye(3))))>2e-5:
        raise ValueError('Rest transform has shear, cannot derive a stable world rotation')
    return matrix_quat(rot)

def world_delta_to_local(rest_world,delta_degrees):
    """Conjugate a world-space rest-pose delta into the bone's local axes."""
    qworld=rotation_from_matrix(rest_world)
    return qmul(qmul(qconj(qworld),euler(delta_degrees)),qworld)
...
def build(source,config,output):
    ...
    coordinate_space=cfg.get('coordinateSpace','local')
    if coordinate_space not in ('local','world-rest'):raise ValueError('coordinateSpace must be local or world-rest')
    base=cfg.get('basePoseDegrees',{});base_world=cfg.get('basePoseWorldDegrees',{});target=set(roles.values())
    if coordinate_space!='world-rest' and base_world:raise ValueError('basePoseWorldDegrees requires coordinateSpace world-rest')
    if set(base)&set(base_world):raise ValueError('A node cannot use both local and world base-pose rotation')
    for name in set(base)|set(base_world):
    ...
    for state in STATES:
        ...
        for p in u:
            ...
            if state=='idle':
                ...
                a=min(1,p/.8);a=a*a*(3-2*a);setr('pelvis',[0,0,84*a]);setr('spine',[8*a,0,4*a]);setr('head',[-10*a,0,0]);setr('upperArmL',[0,0,18*a]);setr('upperArmR',[0,0,-8*a])
                dy=-float(cfg.get('deathDropMeters',.42))*units*a
            offsets={i:np.zeros(3) for i in target}
            for role,delta in d.items():
                signed=delta*np.array(cfg.get('roleSigns',{}).get(role,[1,1,1]),float)
                matrix=np.array(cfg.get('roleRotationMatrices',{}).get(role,np.eye(3)),float)
                if matrix.shape!=(3,3) or not np.isfinite(matrix).all():raise ValueError('Invalid roleRotationMatrices entry for '+role)
                offsets[roles[role]]+=matrix@signed
            for i in rotations:
                name=g['nodes'][i]['name']
                q=np.array(rest[i]['rotation'])
                if coordinate_space=='world-rest':
                    # In world space the base pose is applied first, followed by
                    # the sampled gesture.  Conjugate the combined delta once.
                    combined=qmul(euler(offsets[i]),euler(base_world.get(name,[0,0,0])))
                    q=qmul(q,qmul(qmul(qconj(rotation_from_matrix(rest_world[i])),combined),rotation_from_matrix(rest_world[i])))
                    q=qmul(q,euler(base.get(name,[0,0,0])))
                else:
                    q=qmul(q,euler(base.get(name,[0,0,0])))
                    q=qmul(q,euler(offsets[i]))
                if rotations[i] and np.dot(q,rotations[i][-1])<0:q=-q
                rotations[i].append(q.tolist())
            world_translation=np.array([0,dy,dz])
            if coordinate_space=='world-rest':
                parent_world=rest_world[parents[roles['pelvis']]] if roles['pelvis'] in parents else np.eye(4)
                local_translation=np.linalg.solve(parent_world[:3,:3],world_translation)
            else:local_translation=world_translation
            translations.append((np.array(rest[roles['pelvis']]['translation'])+local_translation).tolist())
    ...
    g.setdefault('extras',{})['ggdProceduralFallback']={'schema':'ggd-procedural-six-state@1','nativeAnimationCount':original_clip_count,'sourceMotionPreservedElsewhere':True,'timeUnit':'seconds','authoringSampleRateHz':30,'notOriginalGameFrameRate':True,'coordinateSpace':coordinate_space}
    ...
    report={'schema':'ggd-library-model-preparation@1','asset':cfg['asset'],'source':record(source),'output':record(target),'stateClips':stateclips,'yawOffsetDeg':cfg.get('yawOffsetDeg',0),'animationProvenance':{'kind':'GGD generated procedural fallback','native':False,'retargeted':False,'nativeAnimationCount':0,'timeUnit':'seconds','authoringSampleRateHz':30,'coordinateSpace':coordinate_space},'clipDetails':summaries,'boneMapping':cfg['bones'],'basePoseDegrees':base,'basePoseWorldDegrees':base_world,'config':cfg,'matrixToTrsMaxError':max(decomp.values(),default=0),'limitations':['Six clips are procedural fallback authored by GGD, not source-game motions.','Bone mapping, rest-space conversion and visual gesture quality require character-specific visual acceptance.','No source voice, SFX, VFX or gameplay timing is created or implied.','Shared upload and runtime sampling must be run separately.']}
