...
try{
 ...
 for(const group of container.animationGroups){
  ...
  // Quarter samples are required for cyclic clips: a sine-driven run pose is
  // identical at 0, 0.5 and 1.0 and would otherwise evade visual review.
  for(const [label,fraction] of [['start',0],['quarter',.25],['middle',.5],['threequarter',.75],['end',1]]){
   ...
   visualRoot.position.setAll(0);
   // Front view: Y is up and the camera looks toward the model from +Z.
   camera.orthoLeft=-half;camera.orthoRight=half;camera.orthoTop=half;camera.orthoBottom=-half;
   for(const view of [
    {name:'front',position:new Vector3(center.x,center.y,center.z+20),positionAxis:'+Z'},
    {name:'side',position:new Vector3(center.x+20,center.y,center.z),positionAxis:'+X'},
   ]){
    camera.position.copyFrom(view.position);camera.setTarget(new Vector3(center.x,center.y,center.z));
    camera.getViewMatrix(true);camera.getProjectionMatrix(true);
    scene.render();await new Promise(r=>requestAnimationFrame(r));scene.render();
    const file=safe(group.name)+'-'+label+'-'+view.name+'.png';
    await save(file,{png:canvas.toDataURL('image/png').split(',')[1]});
    samples.push({group:group.name,label,fraction,frame,view:view.name,positionAxis:view.positionAxis,file,renderCenterTranslation:visualRoot.position.asArray(),worldSkinnedBounds:{min:b.min.asArray(),max:b.max.asArray(),extent:b.extent.asArray()}});
   }
  }
  group.stop();
 }
 await save('proof.json',{schema:'ggd.procedural-motion-front-webgl@1',babylonVersion:Engine.Version,
  renderer:'actual Babylon WebGL glTF loader; front camera on +Z with Y up; each animation sampled at start, middle and end with CPU-skinned finite world bounds',
  cameraEvidence:{views:[{view:'front',positionAxis:'+Z'},{view:'side',positionAxis:'+X'}],upAxis:'+Y'},
