...
class AssetPipelineTest(unittest.TestCase):
    ...
    def test_manifest_is_fail_closed_and_acyclic(self):
        ...
        self.assertEqual(self.manifest["scope"]["runtimeRegistrationAllowed"], "mapped-qualified-candidate-only")
        self.assertEqual(
    ...
    def test_core_selection_includes_source_dependencies(self):
        ...
        self.assertIn("fate-backend-options", ids)

    def test_fate_registration_is_candidate_only_and_ordered(self):
        backend = next(row for row in self.manifest["stages"] if row["id"] == "fate-backend-options")
        self.assertEqual(backend["dependsOn"], ["fate-runtime-catalog"])
        self.assertEqual(backend["checkCommand"][-1], "--check")
