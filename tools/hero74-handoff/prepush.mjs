...
function sourceHashes(){const out={};const walk=dir=>{for(const entry of readdirSync(dir,{withFileTypes:true})){const path=resolve(dir,entry.name);if(entry.isDirectory()&&entry.name!=='__pycache__')walk(path);else if(entry.isFile())out[relative(root,path)]=sha(readFileSync(path));}};walk(sourceDir);for(const file of ['materials/hero74-handoff/frozen/inventory.json','materials/hero74-handoff/frozen/archive/manifest.json'])out[file]=sha(readFileSync(resolve(root,file)));return out;}
const startSources=sourceHashes();
const receipt={schema:'ggd-hero74-prepush-receipt@1',scope:'Repository gates at the recorded source snapshot; not a replacement for final frozen-dataset verification.',startedAt:new Date().toISOString(),head:git(['rev-parse','HEAD']),branch:git(['branch','--show-current']),worktreeStatus:git(['status','--short']),sourceHashes:startSources,commands:[]};
