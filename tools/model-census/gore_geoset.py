...
EXEMPT: dict[tuple[str, int], str] = {
    ...
    #   ⇒ 4 處 `confirmed`（gore-joint + floor-slab 互相佐證）這一輪已經填進 `hiddenPrimitives`。
    ("doraemon-cat.glb", 14): (
        "⭐ 訊號只有 `offset-floor` 一條（105 頂點）。⚠️ C 條（又扁又貼地又比身體寬的板子）單獨成立時只給 `suspect`，"
        "⛔ 本工具刻意不自動填 —— 檔頭逐字：「列進報告讓 owner 決定」。"
        "⇒ ⭐ 一塊貼地的板子也可能是**影子、法陣、底座**。⭐ 反駁方式：同上，GH#1218 逐處看一眼再決定。"
    ),
    ("doraemon-cat.glb", 15): (
        "⭐ 訊號只有 `second-skeleton` 一條（124 頂點）。⚠️ 本工具的檔頭逐字寫著：「`Sword` 全都是伸在身體外面的**武器**，"
        "六個全部只有這一個訊號 ⇒ 本工具額外要求『另一具骨架根』，⛔ 否則這條判準會叫人去藏掉英雄的劍」。"
        "⭐ 而它從 `confirmed` 掉到 `suspect` 正是因為**沒有第二個訊號互相佐證**。"
        "⇒ ⭐ 看過那一塊是什麼之前不藏（藏錯 = 英雄缺一塊，比屍體更嚴重 —— 同一份檔頭的原話）。"
        "⭐ 反駁方式：GH#1218 逐處看一眼 ⇒ 是屍體就填進 `hiddenPrimitives` 並刪掉這一列；是武器／掛件就把理由改寫成「它是什麼」。"
    ),
    ("ou99_474258.glb", 1): (
        "⭐ 訊號只有 `second-skeleton` 一條（597 頂點）。⚠️ 本工具的檔頭逐字寫著：「`Sword` 全都是伸在身體外面的**武器**，"
        "六個全部只有這一個訊號 ⇒ 本工具額外要求『另一具骨架根』，⛔ 否則這條判準會叫人去藏掉英雄的劍」。"
        "⭐ 而它從 `confirmed` 掉到 `suspect` 正是因為**沒有第二個訊號互相佐證**。"
        "⇒ ⭐ 看過那一塊是什麼之前不藏（藏錯 = 英雄缺一塊，比屍體更嚴重 —— 同一份檔頭的原話）。"
        "⭐ 反駁方式：GH#1218 逐處看一眼 ⇒ 是屍體就填進 `hiddenPrimitives` 並刪掉這一列；是武器／掛件就把理由改寫成「它是什麼」。"
    ...
    ("4bdc23a090527b2ac7e2bc6ebb779a0241b7a2c4764d39fd33268899d2cf3954.glb", 1): (
        ...
        "⇒ ⭐ 看過那一塊是什麼之前不藏（藏錯 = 英雄缺一塊，比屍體更嚴重 —— 同一份檔頭的原話）。"
        "⭐ 反駁方式：GH#1218 逐處看一眼 ⇒ 是屍體就填進 `hiddenPrimitives` 並刪掉這一列；是武器／掛件就把理由改寫成「它是什麼」。"
        "⚠️ 它是 `ou99_474258.glb` 的**凍結副本**（同一顆 597 頂點）⇒ 兩列要一起決定。"
    ),
    ("5ca1a1e022461eb2dd5e241082223c76683ff4d9a44d24f6fb6f192c73dad29e.glb", 14): (
        "⭐ 訊號只有 `offset-floor` 一條（120 頂點）。⚠️ C 條（又扁又貼地又比身體寬的板子）單獨成立時只給 `suspect`，"
        "⛔ 本工具刻意不自動填 —— 檔頭逐字：「列進報告讓 owner 決定」。"
        "⇒ ⭐ 一塊貼地的板子也可能是**影子、法陣、底座**。⭐ 反駁方式：同上，GH#1218 逐處看一眼再決定。"
    ),
    ("5ca1a1e022461eb2dd5e241082223c76683ff4d9a44d24f6fb6f192c73dad29e.glb", 15): (
        "⭐ 訊號只有 `second-skeleton` 一條（324 頂點）。⚠️ 本工具的檔頭逐字寫著：「`Sword` 全都是伸在身體外面的**武器**，"
        "六個全部只有這一個訊號 ⇒ 本工具額外要求『另一具骨架根』，⛔ 否則這條判準會叫人去藏掉英雄的劍」。"
        "⭐ 而它從 `confirmed` 掉到 `suspect` 正是因為**沒有第二個訊號互相佐證**。"
        "⇒ ⭐ 看過那一塊是什麼之前不藏（藏錯 = 英雄缺一塊，比屍體更嚴重 —— 同一份檔頭的原話）。"
        "⭐ 反駁方式：GH#1218 逐處看一眼 ⇒ 是屍體就填進 `hiddenPrimitives` 並刪掉這一列；是武器／掛件就把理由改寫成「它是什麼」。"
    ...
    ("5ca1a1e022461eb2dd5e241082223c76683ff4d9a44d24f6fb6f192c73dad29e.glb", 16): (
        "⭐ 訊號只有 `second-skeleton` 一條（248 頂點）。⚠️ 本工具的檔頭逐字寫著：「`Sword` 全都是伸在身體外面的**武器**，"
        "六個全部只有這一個訊號 ⇒ 本工具額外要求『另一具骨架根』，⛔ 否則這條判準會叫人去藏掉英雄的劍」。"
        "⭐ 而它從 `confirmed` 掉到 `suspect` 正是因為**沒有第二個訊號互相佐證**。"
        "⇒ ⭐ 看過那一塊是什麼之前不藏（藏錯 = 英雄缺一塊，比屍體更嚴重 —— 同一份檔頭的原話）。"
        "⭐ 反駁方式：GH#1218 逐處看一眼 ⇒ 是屍體就填進 `hiddenPrimitives` 並刪掉這一列；是武器／掛件就把理由改寫成「它是什麼」。"
...
FLY_EXEMPT: dict[tuple[str, int], str] = {
    ("doraemon-cat.glb", 5): (
        "⭐ GH#1218（2026-09-11）—— 哆啦A夢的一塊（144 頂點）在「Spell One」浮在身體上方 0.73 個身高。"
        "⚠️ ⛔ **不能照直覺藏它**：144 頂點是一大塊，⭐ 而 owner 抱怨過的阿福那一次有「64–100% 的頂點座標與本體重合」"
        "可以證明是複本 —— ⛔ 這幾塊**沒有**那個證據。⇒ 一塊在施法動作裡飛到頭頂的大幾何，"
        "同樣可能是**技能本身的演出**（法陣／投擲物／召喚體）。"
        "⭐ 反駁方式：GH#1218 播一次那個動作看一眼 ⇒ 是分身就填進 `_overlay-hidden-geometry.json` 並刪掉這一列；是演出就寫下它是什麼。"
    ),
    ("doraemon-cat.glb", 7): (
        "⭐ GH#1218（2026-09-11）—— 哆啦A夢的一塊（616 頂點）在「Spell One」浮在身體上方 0.93 個身高。"
        "⚠️ ⛔ **不能照直覺藏它**：616 頂點是一大塊，⭐ 而 owner 抱怨過的阿福那一次有「64–100% 的頂點座標與本體重合」"
        "可以證明是複本 —— ⛔ 這幾塊**沒有**那個證據。⇒ 一塊在施法動作裡飛到頭頂的大幾何，"
        "同樣可能是**技能本身的演出**（法陣／投擲物／召喚體）。"
        "⭐ 反駁方式：GH#1218 播一次那個動作看一眼 ⇒ 是分身就填進 `_overlay-hidden-geometry.json` 並刪掉這一列；是演出就寫下它是什麼。"
    ),
    ("ou99_433442.glb", 2): (
        "⭐ GH#1218（2026-09-11）—— ou99.433442 的一塊（1611 頂點）在「attack」浮在身體上方 0.91 個身高。"
        "⚠️ ⛔ **不能照直覺藏它**：1611 頂點是一大塊，⭐ 而 owner 抱怨過的阿福那一次有「64–100% 的頂點座標與本體重合」"
        "可以證明是複本 —— ⛔ 這幾塊**沒有**那個證據。⇒ 一塊在施法動作裡飛到頭頂的大幾何，"
        "同樣可能是**技能本身的演出**（法陣／投擲物／召喚體）。"
        "⭐ 反駁方式：GH#1218 播一次那個動作看一眼 ⇒ 是分身就填進 `_overlay-hidden-geometry.json` 並刪掉這一列；是演出就寫下它是什麼。"
    ),
    ("5ca1a1e022461eb2dd5e241082223c76683ff4d9a44d24f6fb6f192c73dad29e.glb", 5): (
        "⭐ GH#1218（2026-09-11）—— 同一顆的凍結副本（376 頂點）在「Spell One」浮在身體上方 0.72 個身高。"
        "⚠️ ⛔ **不能照直覺藏它**：376 頂點是一大塊，⭐ 而 owner 抱怨過的阿福那一次有「64–100% 的頂點座標與本體重合」"
        "可以證明是複本 —— ⛔ 這幾塊**沒有**那個證據。⇒ 一塊在施法動作裡飛到頭頂的大幾何，"
        "同樣可能是**技能本身的演出**（法陣／投擲物／召喚體）。"
        "⭐ 反駁方式：GH#1218 播一次那個動作看一眼 ⇒ 是分身就填進 `_overlay-hidden-geometry.json` 並刪掉這一列；是演出就寫下它是什麼。"
    ),
    ("5ca1a1e022461eb2dd5e241082223c76683ff4d9a44d24f6fb6f192c73dad29e.glb", 7): (
        "⭐ GH#1218（2026-09-11）—— 同一顆的凍結副本（901 頂點）在「Spell One」浮在身體上方 0.28 個身高。"
        "⚠️ ⛔ **不能照直覺藏它**：901 頂點是一大塊，⭐ 而 owner 抱怨過的阿福那一次有「64–100% 的頂點座標與本體重合」"
        "可以證明是複本 —— ⛔ 這幾塊**沒有**那個證據。⇒ 一塊在施法動作裡飛到頭頂的大幾何，"
        "同樣可能是**技能本身的演出**（法陣／投擲物／召喚體）。"
        "⭐ 反駁方式：GH#1218 播一次那個動作看一眼 ⇒ 是分身就填進 `_overlay-hidden-geometry.json` 並刪掉這一列；是演出就寫下它是什麼。"
    ),
