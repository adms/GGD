...
import {
  checkRosterDeclaration,
  loadRosterWorld,
  rosterDeclarationSummary,
} from "../../packages/shared/testkit/rosterDeclaration";
...
function goStringSlice(src: string, name: string): string[] {
  ...
  // ⛔⛔ **這一行原本是 `/"(godie-[^"]+)"/g`** —— ⭐ 只認 `godie-` 開頭的 id。
  //
  // ⚠️ 2026-09-10 抓到（GH#1165）：37 名社群英雄（`community-review-…`）加進
  // `starterChampions` 之後，這支解析器**結構上看不見它們** ⇒ `seeded` 讀成 49（實際 86）
  // ⇒ ⭐ 兩條稽核同時紅，而它們的訊息說「這 37 張卡**不在名單上**」——
  //   ⛔ **它們在**。⭐ 訊息指著一個**不存在的問題**，而修法（把 id 加進名單）已經做了。
  //
  // ⭐ 修法：抓**所有**帶引號的 id，⛔ 不寫死任何前綴 ——
  //   一個寫死前綴的解析器，會在**下一種命名**出現時安靜地少讀一半。
  //   ⚠️ 而 `[a-z]` 開頭 ＋ 至少一個 `-` 足以排掉註解裡的英文字（那些沒有引號包住）。
  // ⭐ 逐行剝掉 `//` 之後才抓 —— ⛔ 註解裡的 id **不是名單的一員**。
  //   ⚠️ 這不是潔癖：2026-09-10 我用一個沒剝註解的正則量同一份檔案，
  //     得到 **182**（實際 86）—— ⭐ 而那個數字讀起來完全合理。
  //   ⛔ 這條迴圈的界線（停在單獨一個 `}`）今天剛好讓它是對的，
  //     ⭐ 而「今天剛好對」正是下一次會安靜地錯掉的形狀。
  const clean = body.map((l) => l.replace(/\/\/.*$/, "")).join("\n");
  const ids = [...clean.matchAll(/"([a-z][a-z0-9]*(?:-[a-z0-9]+)+)"/g)].map((m) => m[1]!);
