...
def main():
    ...
    def walk(rf):
        ...
        return strong_c, weak_c, fams_seen, items_seen, hero_seen, hubs
    ...
    for kind_key in ("dummy_effect_units", "orb_attachments", "loc_oneshot_effects"):
        for row in dmap[kind_key]:
            ...
            strong_c, weak_c, fams_seen, items_seen, hero_seen, hubs = walk_cache[rf]
            cands = strong_c if strong_c else weak_c
            raws = sorted({c["rawcode"] for c in cands})
            hero_units_seen = [
                {"rawcode": c, "name": hero_unit_names.get(c, ""), "depth": dpt}
                for c, dpt in sorted(hero_seen.items(), key=lambda t: (t[1], t[0]))[:3]
            ]
            if hero_units_seen:
                base["hero_units_seen"] = hero_units_seen
            ...
            if len(raws) == 1:
                ...
                if row.get("hero"):
                    ...
                    if len(hero_match) == 1:
                        tie, tie_why = hero_match[0], "hero-match ({})".format(row["hero"])
                if tie is None and hero_seen:
                    seen_champs = {"godie-" + c.lower() for c in hero_seen}
                    unit_match = [rc for rc in raws
                                  if gap_by_raw.get(rc, {}).get("champion") in seen_champs]
                    if len(unit_match) == 1:
                        tie, tie_why = unit_match[0], "hero-unit-type-match"
                if tie:
                ...
                if MAP_INIT_RE.match(rf):
                    ...
                    hs = ", ".join("{} ('{}')".format(hero_unit_names.get(c, ""), c)
                                   for c in sorted(hero_seen, key=hero_seen.get)[:3])
                    reason = "hero-bound trigger, no spell id (unit-type keyed): " + hs
                elif fams_seen:
