...
function structuralCheck(buf) {
    if (buf.readUInt32LE(0) !== 0x46546c67)
        throw new Error("bad GLB magic");
    if (buf.readUInt32LE(4) !== 2)
        throw new Error("bad GLB version");
    const total = buf.readUInt32LE(8);
    if (total !== buf.length)
        throw new Error(`length mismatch ${total} != ${buf.length}`);
    ...
    for (const [i, a] of (json.accessors ?? []).entries()) {
        ...
        if (a.count <= 0)
            throw new Error(`accessor ${i} empty`);
    }
    return json;
}
...
//# sourceMappingURL=validate_glb.mjs.map