...
import { NullEngine, Scene, SceneLoader, Vector3, TransformNode, } from "@babylonjs/core";
...
function computeWorldRecursive(node) {
    node.computeWorldMatrix(true);
    for (const c of node.getChildren()) {
        if (c instanceof TransformNode)
            computeWorldRecursive(c);
    }
}
const engine = new NullEngine();
async function measure(glbPath, source) {
    ...
    try {
        ...
        const container = await SceneLoader.LoadAssetContainerAsync("", "data:base64," + b64, scene, undefined, ".glb");
        container.addAllToScene();
        // force world matrices top-down from root transform nodes
        for (const rn of container.rootNodes) {
            if (rn instanceof TransformNode)
                computeWorldRecursive(rn);
        }
        ...
        if (meshRecs.length === 0)
            throw new Error("no geometry meshes");
        // body = mesh with most verts
        let bodyIdx = 0;
        for (let i = 1; i < meshRecs.length; i++) {
            if (meshRecs[i].verts > meshRecs[bodyIdx].verts)
                bodyIdx = i;
        }
...
for (const r of results) {
    ...
    console.log(`${flag} ${r.model.padEnd(28)} bodyH=${r.bodyH.toFixed(2).padStart(6)} fullH=${r.fullH.toFixed(2).padStart(7)} ratio=${r.ratio.toFixed(2).padStart(6)} anim=${r.animGroups} stray=${strays.length} legit=${legits.length}`);
...
//# sourceMappingURL=mesh_audit.mjs.map