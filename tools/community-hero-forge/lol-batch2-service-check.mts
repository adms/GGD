/** Loopback Editor build/inspect round trip for the eleven editable candidates.
...
const { values } = parseArgs({ options: {
  ...
  "editor-origin": { type: "string", default: "http://127.0.0.1:5211" }, help: { type: "boolean" },
} });
if (values.help) {
  console.log("Usage: node --import tsx tools/community-hero-forge/lol-batch2-service-check.mts --origin http://127.0.0.1:8831 --out /private/tmp/lol-batch2-service-<new-run> [--editor-origin http://127.0.0.1:5211]\nBoth origins must be loopback. Output must be a new directory beneath an existing system temporary directory. No login, submission or publication.");
  process.exit(0);
}
...
const results = COMMUNITY_LOL_BATCH2_EXAMPLES.map((recipe) => ({ id: `lol-${recipe.id}`, name: recipe.name, status: "not-run", errors: [] as { phase: string; message: string }[], facts: {} as Record<string, unknown> }));
const report = { schema: "ggd-lol-batch2-service-check@1", startedAt: new Date().toISOString(), finishedAt: "", status: "running", origin, editorOrigin,
  ...
  recipeSha256: contentSha256(COMMUNITY_LOL_BATCH2_EXAMPLES), target: null as ReturnType<typeof targetOf> | null,
  results, passed: 0, failed: 0, notRun: results.length, error: "" };
try {
  ...
  for (const [index, recipe] of COMMUNITY_LOL_BATCH2_EXAMPLES.entries()) {
    const item = results[index]!; item.status = "running";
    let phase = "source";
    ...
    try {
      const project = createCommunityHeroRecipe(recipe, item.id, templates);
  ...
  report.status = report.error || report.failed || report.notRun ? "failed" : "passed";
  report.finishedAt = new Date().toISOString(); await write("report.json", report);
  if (report.status !== "passed") process.exitCode = 1;
  console.log(JSON.stringify({ output, status: report.status, passed: report.passed, failed: report.failed, notRun: report.notRun, error: report.error }));
}
