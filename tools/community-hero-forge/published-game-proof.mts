 ...
 * Reuses local-game-proof.mts and the first-batch live-match acceptance flow.
 * Reads an existing publication receipt; never submits, reviews or alters heroes.
 * Exercises public account/room APIs and the shipping game socket protocol.
...
import type { EventMessage } from "../../packages/shared/src/protocol/messages";
...
const root = resolve(process.env.GGD_LOCAL_PROOF_RUNTIME_ROOT ?? resolve(import.meta.dirname, "../.."));
const { readTargetProfileFacts } = await import(pathToFileURL(resolve(root, "apps/editor/src/export-center/exportPolicy.ts")).href);
const { ContentLoader } = await import(pathToFileURL(resolve(root, "packages/shared/src/content/loader.ts")).href);
const { FsContentSource } = await import(pathToFileURL(resolve(root, "packages/shared/src/content/node/FsContentSource.ts")).href);
const { OverlayContentSource } = await import(pathToFileURL(resolve(root, "packages/shared/src/content/overlay.ts")).href);
const { registerAll } = await import(pathToFileURL(resolve(root, "packages/shared/src/content/registries.ts")).href);
const { registerSkeletonContent } = await import(pathToFileURL(resolve(root, "packages/shared/src/sim/content/skeleton.ts")).href);
const { buildCommunityRoomContent, captureCommunityContentBase, verifyCommunityRoomManifest } = await import(pathToFileURL(resolve(root, "packages/shared/src/content/communityRoom.ts")).href);
const { unpackEventBatch } = await import(pathToFileURL(resolve(root, "packages/shared/src/protocol/messages.ts")).href);
const { Abilities, Champions } = await import(pathToFileURL(resolve(root, "packages/shared/src/sim/content/registry.ts")).href);
const { withRegistryContext } = await import(pathToFileURL(resolve(root, "packages/shared/src/sim/content/registryContext.ts")).href);
...
const runId = `published-game-${Date.now()}`;
assert(process.env.GGD_LOCAL_PROOF_PUBLICATIONS, "A publication receipt is required");
const audit = JSON.parse(readFileSync(process.env.GGD_LOCAL_PROOF_PUBLICATIONS!, "utf8"));
assert.equal(audit.schema, "ggd-handoff-publication-proof@1");
assert.equal(audit.status, "passed");
assert.equal(audit.origin, platform);
const publications = audit.results;
assert(publications.length > 1 && publications.every((p: any) => p.status === "passed"));
const selected = process.env.GGD_LOCAL_PROOF_HEROES?.split(",");
assert(selected?.length === 2 && new Set(selected).size === 2, "Select two distinct published heroes");
const projects = selected.map((id) => {
 const p = publications.find((row: any) => row.workId === id);
 assert(p, `Hero not in receipt: ${id}`);
 return { projectId: p.workId, name: p.name };
});
const fullMatch = process.env.GGD_LOCAL_PROOF_FULL_MATCH === "1";
assert(fullMatch && process.env.GGD_LOCAL_PROOF_COMBAT === "1", "This proof requires combat and full-match settlement");
...
assert(!existsSync(report), "Use a fresh report; preserve failed attempts");
...
try {
  ...
  assert.deepEqual(target, audit.target, "Published packages must match the live target");
  proof.target = target;
  ...
  const manifest = verifyCommunityRoomManifest(readyA.communityContent); assert.equal(manifest.heroes.length,publications.length);for(const expected of publications){const actual=manifest.heroes.find(p=>p.workId===expected.workId);assert(actual);for(const key of ['submissionId','packageDigest'])assert.equal(actual[key],expected[key]);}
 assert.equal(verifyCommunityRoomManifest(readyB.communityContent).digest, manifest.digest);
  ...
  await until(() => expectedPlayers.every(([accountId, championId]) => [...gameA.state.seats.values()].some((seat: any) => seat.accountId === accountId && seat.championId === championId)), "both test accounts selected their own community champion");
  ...
  proof.reconnection={passed:true,manifestDigest:manifest.digest,championId:projects[0].projectId};log("Actual reconnect retained selected hero and all publication version pins");
  if (process.env.GGD_LOCAL_PROOF_COMBAT === "1") {
    ...
    const casts: unknown[] = [];
    proof.combat = { scope: "Both controlled players cast W through normal progression; this live match does not claim every skill slot was cast.", casts };
    ...
    log("both controlled player seats produced real W cast events");
  }
  if (fullMatch) {
  ...
  proof.productionDeploymentVerified = false;
  proof.visualAcceptanceVerified = false;
