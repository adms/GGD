...
if (process.env.GGD_LOCAL_COMMUNITY_PROOF !== "disposable-local-only" || !process.env.GGD_LOCAL_PROOF_PASSWORD || !process.env.GGD_LOCAL_PROOF_RECORDING || !process.env.GGD_LOCAL_PROOF_REPLAY_DIR) throw new Error("Explicit local proof opt-in, disposable password, recording receipt and replay directory are required.");
const root = resolve(process.env.GGD_LOCAL_PROOF_RUNTIME_ROOT ?? process.cwd());
const { verifyCommunityRoomManifest, buildCommunityRoomContent, captureCommunityContentBase } = await import(pathToFileURL(resolve(root, "packages/shared/src/content/communityRoom.ts")).href);
...
assert(source.completeMatch?.platformRoomDisposed);
...
assert(!existsSync(output), "Use a fresh output and preserve failed attempts");
...
try {
  ...
  const lines=gunzipSync(readFileSync(resolve(process.env.GGD_LOCAL_PROOF_REPLAY_DIR!, `${source.matchId}.jsonl.gz`))).toString().trim().split('\n').map(line=>JSON.parse(line));
