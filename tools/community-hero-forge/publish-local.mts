...
const { values: v } = parseArgs({ options: {
  ...
  "source-platform-port": { type: "string" },
...
const sourcePort = Number(v["source-platform-port"] ?? port);
assert(Number.isInteger(sourcePort) && sourcePort > 0 && sourcePort < 65536);
assert.equal(source.origin, `http://127.0.0.1:${sourcePort}/api/v1`);
