...
組間跨度 **5.1×**。用一把尺量它一定出事 —— 而**最尖銳的證據是「硬輔」這個出身
裡近戰遠程都有**（賈修貝爾 6.4 遠程 vs 另外兩位 1.6 近戰），
任何單一絕對值都會把其中一半打成另一種型別。

⭐ 解法是 \`bandsByAttackType\` —— **兩把階梯，由英雄自己的 \`attackType\` 選**：
...
⇒ 出身給的級距意思變成「**以你的型別而言**算遠還算近」，近戰的「大」是 1.8u
而不是把他變成遠程。⛔ 程式裡沒有任何一行提到 \`range\` —— 是不是雙階梯由**資料**
決定（\`bandsByAttackType\` 有沒有這一項）。

⚠️ 兩把都**不是**等比階梯，而是貼著觀測值鋪：近戰整個區間只有 1.2~2.0，
硬套 ÷2/×2 會產生 0.8 與 3.2 兩個「這個遊戲裡不存在的近戰距離」。
遠程的極大 **12** 也不是挑的 —— 那是 owner 2026-08-12 自己給的卡面上限
（「上限是黑人牙膏 12」），而黑人牙膏正是母體裡唯一的 12.0。

🔴 **為什麼還沒開**：打開會**重寫每一位英雄的射程**，最明顯的是黑人牙膏
12 → 8.2（法師=中）。那是一個設計裁決不是一個預設值，所以留給 owner 決定。
⭐ 要開只要把 \`range\` 加進 \`appliesTo\`，級距表都已經備妥。
...
**「中」= 量出來的母體中位數**（73 位可達英雄，等級 ${N.referenceLevel}；移速那一格是等級 1），
所以改制前後全場的總量不變，只是重新分配。

| 屬性 | 通道 | 極小 | 小 | 中 | 大 | 極大 |
|---|---|--:|--:|--:|--:|--:|
${(Object.keys(N.bands) as (keyof typeof N.bands)[]).flatMap((k) => {
  const two = N.bandsByAttackType[k];
