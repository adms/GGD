...
/**
 * ⭐ 「十一項裡有幾項套用中」**從 `appliesTo` 推導，⛔ 不手打**。
 *
 * ⚠️ 這一段在 2026-08-21 之前是三個手打的字串（「九項套用中」／「尚未套用（2 項）」／
 * 「⭐ 要開只要把 `as` 加回 `appliesTo`」），而 owner 當天已經把 `as` 交回出身表
 * （逐字：「請你照出身表的規劃來設定就好」）。⇒ 這份文件開頭三行同時在說謊，
 * 而 `content:build`、全套測試、以及這支產生器自己**全部是綠的** ——
 * 因為沒有任何一個閘在讀散文。
 * ⛔ 手改成「十一項」只是把過期往後推一次，所以它改成推導。
 */
const NORM_KEYS = Object.keys(STAT_ZH).filter((k) => k in N.bands);
const APPLIES = N.appliesTo as readonly string[];
const APPLIED = NORM_KEYS.filter((k) => APPLIES.includes(k));
const PENDING = NORM_KEYS.filter((k) => !APPLIES.includes(k));
/** 只在那一項**真的還沒開**的時候才印那段解釋。⛔ 開了還印＝文件在說謊。 */
const ifPending = (k: string, body: string) => (PENDING.includes(k) ? body : "");
...
## 一、範圍：**${NORM_KEYS.length} 項屬性，${APPLIED.length} 項套用中**

⚠️ 前兩版只做了三項（移速/魔抗/裝甲），那是我把範圍讀窄了。
owner 2026-08-12：「出身跟定位**是影響所有屬性**不是這幾項而已」。
⭐ owner 2026-08-16 再加第十一項：「你**補充第十一個屬性 攻擊距離**進來」。
⭐ owner 2026-08-21 把**攻速**交回出身表：「請你照出身表的規劃來設定就好」。

⛔ 下面這張表的**項數與名單都是從 \`appliesTo\` 印出來的**，⛔ 不是手打的 ——
它在 2026-08-21 之前手打著「九項套用中／尚未套用 2 項」，而那兩個數字當天就過期了。

| 狀態 | 屬性 | 說明 |
|---|---|---|
| ✅ **套用中（${APPLIED.length} 項）** | ${APPLIED.map((k) => STAT_ZH[k]).join(" · ")} | 由**出身**決定級距（第 2.3 節的表） |
| 🟡 **已定義、尚未套用（${PENDING.length} 項）** | ${PENDING.length ? PENDING.map((k) => `**${STAT_ZH[k]}**`).join(" · ") : "—（⛔ 沒有了，十一項全開）"} | ${PENDING.length ? "級距表都填好了，但**不在 \`appliesTo\`**" : "級距表填好**而且**接上了"} |

⭐ **每級成長 100% 由這張表決定** —— 三圍（力/敏/智）的每級成長在 2026-08-21
**全部歸 0**，所以升一級拿到的每一點都是引擎**反解**出來、去命中第 2.3 節那一格級距值的。
⇒ 卡上的 \`baseStats\` 是**起點（個性）**，出身的級距是**終點（定位）**。
${ifPending("as", `
### 🟡 攻速為什麼定義了卻沒開
