...
export async function checkAdmin(authHeader, opts = {}) {
  // ⚠️ `opts.required` 收的是**布林**（測試/呼叫端），環境變數收的是**字串**。
  //    第一版寫成 `(opts.required ?? env ?? "1") !== "0"` ⇒ 傳 `false` 時
  //    `false !== "0"` 仍然是 true ⇒ ⛔ **一鍵 rollback 根本不會生效**。
  //    （守衛 ④ 當場抓到 —— 那正是它存在的理由。）
  const required =
    typeof opts.required === "boolean"
      ? opts.required
      : (process.env.GGD_REVIEW_REQUIRE_ADMIN ?? "1") !== "0";
