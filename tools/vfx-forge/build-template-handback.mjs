...
const candidates = VFX_FORGE_RECIPES.map((recipe) => {
  ...
  const blockingBrickRequest = recipe.id.startsWith("classic-beam-") || recipe.id === "avalon-counter-chain" ||
    recipe.id === "rider-dash-beam-blue"
  ...
  return {
    ...
    suggestedType: recipe.typeId,
    suggestedVariantKey: `${recipe.familyId}/${recipe.typeId}`,
    ...
    modelKeys: uniq(segments.flatMap((segment) => segment.kind === "modelFx" ? [segment.modelKey] : [])),
...
const familyVariantSuggestions = [...familyMembers.entries()].map(([familyId, members]) => ({
  ...
  naming: `${familyId}/typeN`,
  variants: members.map((candidate) => ({
    typeId: candidate.suggestedType,
...
const mechanicRecommendationTypes = [...mechanicMembers.entries()].map(([afterKind, members]) => ({
  ...
  variants: members.map((variant, index) => ({
    typeId: `type${index + 1}`,
    key: `mechanic-${afterKind}/type${index + 1}`,
    at: variant.at,
    vfxId: variant.vfxId,
...
const payloadWithoutFingerprint = {
  schema: "ggd-editor-vfx-template-handback@1",
  ...
  familyVariantPolicy: "同類候選使用 familyId/type1、type2… 表示可替換變體；編號只描述積木差異，不綁技能。Main 可依既有命名與 Owner 決策重新命名或不收編。",
  designerWorkflow: "設計師先選家族，再選可直接試放的 type 預設；套用後展開為標準積木與時間軸，矩陣／slider 只做最後微調，不要求設計師從零調出每個視覺。",
  preservationReceipt: {
    ...
    rule: "既有調整先收斂成穩定 familyId/typeN；矩陣只修改已選 type 的參數，不作為從零設計入口。",
  },
  ...
  mainBrickRequests: [
    {
      ...
      affectedAbilityIds: [
        ...
        "godie-e002.ex", "godie-e00l.ex", "godie-hvsh.r",
      ],
      ...
      evidence: [
        "apps/client/src/render/modelFxRig.ts",
        "content/models/w3x.stock.revivehuman.json",
        "content/models/w3x.stock.fragdriller.json",
        "docs/_reports/editor-skill-basic-visual-proof/frames/godie-ogrh.r-02-1180ms.webp",
        "docs/_reports/editor-skill-basic-visual-proof/frames/godie-e002.ex-08-1293ms.webp",
        "docs/_reports/editor-skill-basic-visual-proof/frames/godie-hvsh.r-02-1347ms.webp",
      ],
    },
  ],
  candidates,
...
const md = [
  ...
  `- 本次保存 ${payload.preservationReceipt.preservedTypeOutcomes} 個 type 成果：${payload.preservationReceipt.selectablePresetTypes} 個可選完整配方，加上 ${payload.preservationReceipt.automaticMechanicTypes} 個已在 42／46 使用的機制推薦；不把既有成果丟回矩陣重調。`,
  ...
  "## 42／46 已收斂的機制推薦 type",
  "",
  `自動盤點得到 ${mechanicRecommendationTypeCount} 個可重用機制視覺 type；它們已在 Editor 的「依技能自動組裝基本視覺」使用。Owner 對白不參與推論，現有 framebuffer 仍全部等待人工批核。`,
  "",
  "| 機制家族 | type 數 | 已出現技能 |",
  "| --- | ---: | ---: |",
