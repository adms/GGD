...
#:
#: ⛔⛔ **這張表在 2026-08-23 之前指著三份玩家拿不到的抄本**，而那是 GH#450 當場
#: 踩到的洞**在驗收側又長回來一次**：
#:
#: | 舊的 id | 為什麼它不算數 |
#: |---|---|
#: | `godie-h020.e` / `godie-h020.r` | `godie-h020` 逐字寫在 `content/config/roster.json` 的 `retiredChampions` 裡（owner 2026-08-22 GH#552 下架的五個變身態之一）⇒ 量它＝量一份沒有人玩得到的抄本 |
#: | `godie-n01g.r` | 42-04 的**變身態**；`godie-n01g` **不在** `data/curation/whitelist.json` 上。⚠️ 而 2026-08-23 量到本體 `godie-n003.r` 的 `spawnModelFx` 與 `floatingText` 都是 **0** ⇒ 尺量到的「有演出」是變身之後才有的那一份 |
#:
#: ⭐ 所以現在三支**全部是本體**，而且與另一份驗收產物
#: （`apps/client/src/render/vfx/ownerAcceptanceThree.test.ts`）**逐字同一份名單** ——
#: 兩份驗收指著不同的文件時，「驗收過了」這句話沒有意義。
#:
#: owner 2026-08-23 逐字點名的就是這三支：
#: > 「Saber約束勝利之劍(翻滾光束), 依文世界終結(圓周噴發大冰塊), 莉娜龍破斬
#: >  (一直線火球衝擊波後目的地火焰大爆炸) 都是動畫特效」
#:
#: ⚠️ **被拿掉的那兩支知識不會消失**（知識不可以無聲消失）：
#:   · 04-04 神滅斬 的本體是 `godie-hjai.r`
#:   · 01-04 超究武神霸斬 的本體是 `godie-hart.r`（它本來就是本體，只是不在 owner
#:     2026-08-23 點名的這三支裡）
#: 兩支都仍然**逐支被量到** —— 這一格只決定文件 §3 展開哪幾支，⛔ 不是判準。
CALIBRATION = ["godie-e002.e", "godie-n003.r", "godie-hjai.e"]
