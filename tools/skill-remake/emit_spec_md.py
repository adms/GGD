...
def main() -> None:
    ...
    L.append("## 上線狀態")
    L.append("")
    L.append("⛔ **這 90 支目前沒有上線。** 套用後 **17 個測試檔、57 條守衛**變紅（GH#311）。")
    L.append("")
    L.append("⭐ **57 條紅背後只有 8 個缺陷**（15 個診斷 agent + 61 條對抗複驗量出來的）：")
    L.append("")
    L.append("| 類 | 條數 | 收斂後的獨立缺陷 |")
    L.append("|---|---:|---:|")
    L.append("| **A · 產生器缺陷**（規格有、JSON 沒表達出來）＝ **我的 bug** | **35**（61%） | **8** |")
    L.append("| **B · 規格真的改了設計**（測試釘的是舊行為）＝ **要你裁決** | 19（33%） | 5 |")
    L.append("| **C · 測試夾具帳務** | 2（4%） | 1 |")
    L.append("")
    L.append("**不是 90 支各有各的問題，是 8 個模板缺口被複製了 90 次。**")
    L.append("逐支修是 90 輪；修 8 個缺口再重跑產生器是 8 輪（第零守則⑨）。")
    L.append("")
    L.append("→ 完整分析（含逐條真因、修復順序、被複驗推翻的 7 筆）：")
    L.append("[`docs/_skill-remake-batch1-真因分析.md`](_skill-remake-batch1-真因分析.md)")
    L.append("")
    L.append("最高 CP 值的一格：**產生器從來沒跑過 `pnpm content:build`** —— 一行指令消 7 條紅。")
    L.append("")
