...
"""
from common import A, area, dmg, status
...
A("44-01", "44-01 死神之眼", "targeted", [30, 30, 30, 30], [150, 200, 250, 300], 2,
  ...
  #
  # ⚠️ 出貨文件早就有 `rangeTier: "極大"`（GH#414 的級距遷移寫進去的，而
  #    `rangeTier` 不在 SPEC_OWNED，所以 A-6 的 denylist 每次重生成都把它救回來），
  #    而 `resolveRangeTier`（content/rangeTiers.ts:110）明講「`range` 與 `rangeTier`
  #    同時存在 → **級別贏**」⇒ 註冊之後這一支的施法距離是 **12**，
  #    而卡片上寫著「施法距離2」——**卡面說 2、引擎跑 12**（第一·五守則的鏡像：
  #    這一次不是「說了不會發生」，是「發生的比說的多六倍」）。
  # ⇒ 只改卡面那一行。⛔ 不動 `rng=2`：它是級距沒生效時的退路值，改它會讓
  #    `docs/editor-contract/ggd-skill-tiers.md` 的「原始 range ↔ 收進哪一級」
  #    那張普查表換一列，而那是 GH#414/#463 的帳，不是這一輪的。
