...
A("92-04", "92-04 馬勒戈壁", "self", [90, 90, 90], [300, 420, 540], 0,
  # ⭐ GH#1239（2026-09-19）：標籤列補 `[普攻時]`。內文逐字「(**攻擊身上有** [致盲] 標記的
  #    敵人將額外附加⋯)」＋ 下面 `passive.ranks[*].hooks[0].on == "onBasicAttack"`
  #    ⇒ 這支技能**真的**吃普攻，⛔ 而標籤列一個字都沒提。
  #    修法照 owner 2026-08-12 逐字：「這種有衝突的時候都**以內文為主**，
  #    並且**用內文來修正標籤**」⇒ 改標籤，⛔ 不是改內文、⛔ 更不是拿掉 hook。
  #    閘：`tag_gate.MUST_DECLARE_SHAPE` 的反方向（形狀有而標籤沒有）。
