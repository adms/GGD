...
A("77-00", "77-00 浮雲-旋一閃", "self", [30], [0], 0,
  "[被動][機率][迴避][迴避時][旋轉][暈眩][範圍]\n{{cd}}秒冷卻\n有效半徑：{{radius}}\n\n「少女的雙腿就是你的墓穴」\n有10%[機率][迴避]物理攻擊；[迴避]成功後發動，雙腿抓住對手[旋轉]拋摔，對[周圍]敵人造成{{dmg}}+130% [AP]點傷害並[暈眩]2秒。",
  innate="passive",
  passive={"name": "77-00 浮雲-旋一閃", "ranks": [{
      "modifiers": [M("evasion", "flat", 0.10)],
      # ⭐⭐ owner 2026-09-02 逐字：「20-04 Avalon 77-00 浮雲-旋一閃 =>
      #    這兩個是**反彈條件單體**，但**反彈傷害是範圍**喔」
      #    ⇒ ⭐ 觸發條件仍是單體（`onEvade` 打中我的那一發），
      #      ⛔ 而拋摔那一下砸下去是**範圍** ⇒ `area()`，⛔ 不是 `dmg()`。
      #    ⚠️ 卡面同步改（第一·五守則：⛔ 不放任何「說了不會發生」的字）。
      "hooks": [{"on": "onEvade", "target": "event", "internalCooldown": 30.0,
                 "effects": [area("physical", tier="小", dmg_tier="極小", ap=1.3),  # ④ 單發（onEvade + 30 秒 ICD）⇒ 收進級距
