...
function main(): void {
  ...
  const md = [
    ...
    "",
    `正式採用面數規則：來源超過 ${HERO_MODEL_ADOPTION_POLICY.decimateWhenTrianglesAbove.toLocaleString("en-US")} 面時，必須另產生低於 8,000 面的候選（程式上限 ${HERO_MODEL_ADOPTION_POLICY.decimatedTargetTrianglesMax.toLocaleString("en-US")}）並重新完成視覺與骨架驗收。貼圖、mesh、動畫通道等值由程式載入現行 budget，不在本頁另寫一份門檻。`,
    "",
  ].join("\n");
