...
## Owner 範圍限制：額外事項先記錄，確認後才做

Owner 2026-09-09：「你想做額外的事情你可以累積在一份MD 等你做完主任務再讓我確認要不要做就好 不要擅作主張」。

- 開工、續作或接到交接時，先讀 [額外事項待確認](docs/額外事項待確認.md) 的主任務範圍。我的建議、舊待辦、交接摘要及該清單都不構成 Owner 授權。
- Owner 已將原計畫分為 MVP、Phase 2、Phase 3：現在只執行 MVP；後續分期只是保存需求，須 Owner 明確啟動，不因 MVP 完成而自動接續。
- MVP 持續工作時，依主計畫「定期進度留存與 Main 審查」產出進度 MD 並 commit／push 至對應 PR，正常每日約 4～12 次；只記實質變化，不為湊次數新增工作，MVP 交付後停止。
- Owner 要求優先拆分可獨立的 MVP 子任務平行處理；依主計畫分配唯一寫入者與依賴，主線集中整合／發布，不新增範圍或擅派其他 session。
- 每個動作須對應 Owner 已交代的交付結果，或已實際發生、阻止該結果的重大故障。只因「更完整、更保險、更漂亮」提出的工作，一律只記入該 MD。
- 額外事項用現有資訊簡短記錄，不為了完善清單另做調查、估算或驗證；不得自行實作、測試、開票、派平行工作，也不得把它列為主任務的完成門檻。
- 主任務依已授權範圍驗收通過、交付後，才一次請 Owner 確認清單；只有明確同意的項目才能開始。未回覆、同意記錄、或「繼續主任務」都不等於同意額外事項。
- 主任務必要的故障修正仍依既有開票守則處理；範圍限制不是略過必要功能驗收的理由。此規則是 Owner 的工作限制，不另建工具、測試閘或文件系統來驗證它。
