 ...
 *
 * WHERE IT LIVES. It started in `apps/admin/src/testkit/` and moved here on
 * 2026-07-31 when the editor's condition widget and the client's ability/item
 * cards needed the same thing; `apps/admin/src/testkit/headlessUi.ts` is now a
 * re-export so the console's suites are unchanged. Keep that a RE-EXPORT and not
 * a copy — the state below is module-level, and two instances means a test that
 * mocks `react` through one and mounts through the other reads an empty tree.
 *
...
export interface Harness {
  ...
  /** every control carrying `data-field="<name>"`, in document order */
  fields(name: string): HostNode[];
  ...
  /**
   * NODE-TARGETED drivers, for forms where the same control repeats — a list of
   * clause rows all carry 「條件種類」, so `type(name, …)` cannot say WHICH one.
   * The test picks the node (via `fields()`/`hosts()`), these fire its REAL
   * handler with a real-shaped event and settle the render.
   */
  enter(node: HostNode, value: string): void;
  check(node: HostNode, checked: boolean): void;
  press(node: HostNode): void;
  /** let promises resolve, then re-render/flush effects */
  flush(): Promise<void>;
}

/** Shared precondition checks for the node-targeted drivers. */
function handlerOf(node: HostNode, prop: string): (e: unknown) => void {
  const what = String(node.props["data-field"] ?? node.props["aria-label"] ?? node.type);
  if (node.props["disabled"] === true) throw new Error(`control "${what}" is disabled`);
  const fn = node.props[prop];
  if (typeof fn !== "function") throw new Error(`control "${what}" has no ${prop}`);
...
export function mount(element: unknown): Harness {
  ...
  const fieldsOf = (name: string): HostNode[] =>
    hosts().filter((n) => n.props["data-field"] === name);
  const fieldOrNull = (name: string): HostNode | null => fieldsOf(name)[0] ?? null;
  ...
  return {
    ...
    fields: fieldsOf,
    enter(node, value) {
      handlerOf(node, "onChange")({ target: { value }, currentTarget: { value } });
      settle();
    },
    check(node, checked) {
      handlerOf(node, "onChange")({ target: { checked }, currentTarget: { checked } });
      settle();
    },
    press(node) {
