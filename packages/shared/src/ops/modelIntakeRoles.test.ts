 ...
 * ⇒ 同一份壞位元組（貼圖 512），只換它與英雄的**關係**：
 *   ① 它是英雄的預設身體 ⇒ (a) 硬棘輪 **紅**
 *   ② 它只是那位英雄凍結的「原上線模型」 ⇒ (c) 不計、閘綠、並印出理由
 */
...
function scan(badIsDefault: boolean) {
  ...
  const bad = glb(512), sha = createHash("sha256").update(bad).digest("hex");
...
describe("模型入庫棘輪的分母以關係判定（GH#1263）", () => {
  it("① 壞模型是英雄的預設身體 ⇒ (a) 硬棘輪紅並指名它", () => {
    const { code, out } = scan(true);
    expect(out, out).toMatch(/\(a\) 玩家預設拿得到[^\n]*有問題 1/);
    expect(code, out).toBe(1);
    expect(out).toMatch(/⛔⛔ \(a\)[\s\S]*英雄的預設身體 probe[\s\S]*貼圖邊長 512/);
  }, 60_000);

  it("② 同一份位元組只是凍結的原上線模型 ⇒ (c) 不計、閘綠、印出理由", () => {
