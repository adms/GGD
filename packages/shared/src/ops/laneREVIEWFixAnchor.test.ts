...
describe("修復錨與語音漏斗", () => {
  it("① 錨的判準是 diff：訊息說謊也騙不過，⛔ 而看不到的那一面回 null", () => {
    ...
    expect(good.matched.map((m: { path: string }) => m.path)).toContain("content/assets/models/imported/m1.glb");
    expect(
  ...
  it("② 語音進得了漏斗，而門檻讀自 gate JSON（⛔ 不是抄一份數字）", () => {
    ...
    expect(gate.thresholdLadder.rows.map((r) => r.clipsPerChampion), "選中的列要真的在 gate 裡").toContain(
      v.ladderRow,
    );
    // ⛔ 量測過期時一對都選不出來 —— 那要是一個**看得見的字串**，⛔ 不是沉默的 0。
    if (v.status !== "ok") expect(v.reason).toMatch(/\S/);
  });
});
