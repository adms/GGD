...
function routes(): { page: string; label: string; file: string }[] {
  ...
  for (const [, comp = "", f = ""] of src.matchAll(/import \{ (\w+) \} from "\.\/(\w+)"/g)) imp.set(comp, f);
  const rows = [...src.matchAll(/page: "(\w+)", label: "([^"]+)".*Component: (\w+)/g)].map((m) => {
    const [, page = "", label = "", comp = ""] = m;
    const file = imp.get(comp);
    if (file === undefined) throw new Error(`LIVE_ROUTES 的 ${page} 引用了沒 import 的元件 ${comp}`);
    return { page, label, file };
  });
