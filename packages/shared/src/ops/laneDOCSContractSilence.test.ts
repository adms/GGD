 ...
 */
// ⭐ **3 格是刻意留白的**（GH#877，2026-08-29）。
//
// lane 把沉默做到 0，⛔ 而對抗性複驗量到那不是零沉默 —— 是**三句假話**：
//   · `spawnModelFx.scale`（`z.number().max(20)`）拿到 `_shared.ts` 的
//     `zResourcePctTerm.scale`（一個 **enum**）的說明 ⇒ ⭐ 型別欄寫「數字 ≤20」、
//     說明欄叫人填 `"points"` —— **那一列自己跟自己打架**
//   · `extendBuff.basis` / `modifyCooldown.basis` 拿到另一個 `basis` 的說明
//     ⇒ 列舉了**三個這個欄位根本不存在的值**
//
// ⭐ 根因：共用分片被當成**名字對名字**的後備 —— 只驗名詞，⛔ 不問「是不是同一個欄位」。
// ⇒ `gen_spec.ts` 加了兩道關係檢查（型別要對得上 · enum 的說明要提到自己的值），
//   那三格因此**退回 `—`**。
//
// ⚠️ ⭐ **`—` 是正確的結果，⛔ 不是退步**：`—` 說「去看 schema」，
//   而一句錯的說明說「不用看了」（第一·五守則：⛔ 不放任何無效說明）。
// ⇒ 要把它們降到 0，正解是**替那三格寫自己的 TSDoc**，⛔ 不是放寬關係檢查。
const BASELINE = 3;
