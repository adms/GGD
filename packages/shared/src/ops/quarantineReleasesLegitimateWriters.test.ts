 ...
 *
 * ⭐ GH#707 的第 N 次：genguard／hook 從 GH#1096 起已經改問「**這幾個位元組**是不是
 *   產物」，⛔ 而隔離區還在問「這個**檔**是不是產物」⇒ 整份 chmod 444 ⇒「hook 說可以改」
 *   與「檔案是唯讀」**一起是綠的**，只有真的去寫才會發現。2026-09-07 量到兩類誤鎖：
 *   ①**部分產物**（README.md 2,075 行裡只有 9 段是 `docs:readme` 的 · 效果標籤詞彙表v2）
 *   ②**追加式帳本**（`_ledger.tsv` —— `board:build` 只是 `open("a")` 追加一列，⛔ 不是作者）
 *     ⇒ `preserve.sh` 的 `>>` 靜靜失敗而結尾照樣印「✓ 留底 N 份」。
 *
 * ⭐ 兩個方向都要驗，⛔ 只驗「可寫」那一邊對「全部都不鎖」也是綠的。
 * 突變紀錄（2026-09-07 實跑）：`product-quarantine.sh` 的 `is_partial()` 改成永遠
 * `return False`（＝修復前）→ ① 紅並指名部分產物被鎖走。改回來。
 */
