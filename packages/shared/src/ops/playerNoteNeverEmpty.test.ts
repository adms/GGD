...
describe("玩家公告永遠不會是空的（owner 2026-08-30 常設指令）", () => {
  ...
  it("★ 沒有任何票寫玩家那一句時，它仍然產出**一行**公告", () => {
    ...
    let out: string;
    try {
      // ⭐ 2026-09-15：**固定一個空區間**（同一個 tag 到同一個 tag）—— ⛔ 不讀「上一次部署 → 最新 tag」。
      //   在此之前它讀 repo 當下的發版狀態：打了 tag 還沒部署的那段時間（v0.45.0：102 顆玩家面向 commit），
      //   離線路徑正確地拒絕發罐頭句 ⇒ 這條閘紅 ⇒ 而部署又要先過閘 ⇒ ⭐ 死結（形態⑨的反面：一條會被發版狀態卡住的閘）。
      //   ⇒ 它要驗的只有「沒有任何玩家句時仍然出一行」，⭐ 空區間正好是那個情境，而且不隨發版狀態漂。
      out = execFileSync("bash", [SCRIPT, "--since", "v0.44.1", "--until", "v0.44.1"], {
        cwd: REPO,
        encoding: "utf8",
        timeout: 60_000,
