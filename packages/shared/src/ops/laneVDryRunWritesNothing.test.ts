/**
 * GH#769 —— `--dry-run` 不可以留下「看起來已安裝」的轉檔紀錄。缺陷不住在紀錄裡、
 * 也不住在 `.glb` 裡，住在**兩者對不上**（紀錄寫 `verdict: "ok"` 而磁碟一個位元組
 * 沒動），所以分別檢查每一半都是綠的 —— 它已經在 main 上活過一次。兩條斷言各釘一個
 * 驗收條件：①`--dry-run` 之後那兩個目錄逐位元組不變 ②`--check` 綠。
 *
 * ⚠️ ① 刻意**不看離開碼**：沒有 retail MPQ 的機器上轉檔會失敗，而「失敗了也不准
 * 寫檔」正是這條要守的東西。⚠️ 快照**不比 mtime**（`git checkout` 會把它全部重設）。
 */
...
const PY =
  ["python3", "/usr/local/bin/python3", "/opt/homebrew/bin/python3", "/usr/bin/python3"].find((c) => {
    try {
