...
/**
 * 每一行屬於哪一個函式（top-level 就是 `""`）。
 *
 * ⛔⛔ **為什麼需要這個** —— 突變驗證逼出來的，而且逼了**兩次**：
 *   第一版比的是「快照那一行的行號 < 重建那一行的行號」。
 *   ⚠️ 而 bash 有**函式**：`redis_snapshot_before_shutdown` 的**函式體**住在檔案
 *     第 89 行（很前面），於是「檔案裡有一個 :89 的快照呼叫」永遠成立
 *     ⇒ ⭐ **把三個呼叫點全部刪光，守衛照樣是綠的**。
 *   ⇒ 行號順序回答的是「在檔案的哪裡」，⛔ 不是「**在這條路上會不會跑到**」——
 *     而後者才是這條守衛存在的理由。
 */
function fnOf(src: string): string[] {
  const owner: string[] = [];
  let cur = "";
  src.split("\n").forEach((text) => {
    const open = /^(?:function\s+)?([A-Za-z_][\w-]*)\s*\(\)\s*\{/.exec(text);
    if (open) cur = open[1] ?? "";
    owner.push(cur);
    // ⚠️ **單行函式**（`f() { …; }`）要當場關掉 —— ⛔ 否則它後面的每一行都
    //   被歸到它底下（host-deploy.sh:137 的 `accounts_now` 實際造成過）。
    //   ⭐ 一個把 top-level 誤標成某個函式的解析器，會讓覆蓋比對在**錯的群組**裡成立。
    if (/^\}/.test(text) || (open && /\}\s*$/.test(text))) cur = "";
  });
  return owner;
}
...
describe("停 Redis 之前一定先快照 —— 排行榜與 M幣不是快取（GH#860）", () => {
  ...
  it("★ 每一條會重建容器的**路徑**上，快照都要跑在它前面", () => {
    const bad: string[] = [];
    for (const f of scripts) {
      if (f === "redis-snapshot.sh" || f in EXEMPT) continue;
      const raw = readFileSync(join(DIR, f), "utf8");
      const owner = fnOf(raw);
      const lines = codeLines(raw);
      const isCall = (t: string) =>
        /redis-snapshot\.sh|redis_snapshot_before_shutdown/.test(t) &&
        !/^(?:function\s+)?[A-Za-z_][\w-]*\s*\(\)\s*\{/.test(t);
      // ⭐ 哪些函式**自己**就是一次快照（⇒ 呼叫它等於做了快照）
      const snapFns = new Set(
        lines.filter((l) => isCall(l.text)).map((l) => owner[l.n - 1]).filter(Boolean),
      );
      const doesSnapshot = (t: string) =>
        isCall(t) || [...snapFns].some((fn) => new RegExp(`(^|[\\s;&|(])${fn}(\\s|$|;)`).test(t));

      for (const rec of lines.filter((l) => recreatesRedis(l.text))) {
        const scope = owner[rec.n - 1]; // ⭐ 同一條路 = 同一個函式（或同為 top-level）
        const covered = lines.some(
          (l) => l.n < rec.n && owner[l.n - 1] === scope && doesSnapshot(l.text),
        );
        if (!covered)
          bad.push(
            `${f}:${rec.n}${scope ? ` (${scope})` : " (top-level)"} —— ` +
              `⛔ 這條路上重建容器之前沒有快照`,
          );
      }
    }
