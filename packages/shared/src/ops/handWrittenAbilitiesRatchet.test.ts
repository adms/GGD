...
//   ⇒ ⭐ 那兩支要等變身態那一邊也能綁（另一件事）。
// ⛔⛔ ⭐ 2026-09-10（GH#1165 / #1158 之後的**母體變動**）：111 → **159（+48）**。
//   ⚠️ ⭐ **這一次是往上，而棘輪的規矩是只准往下** ⇒ 所以下面逐支列出那 48 支是誰、
//   為什麼它們今天被數進來，⛔ 而不是把數字改一改就算了。
//
//   ⭐ **逐支量過**（`refsOf(template)` 為空 且 `effects` 非空 —— 與 `demandShapes()` 同一條判準；
//   量法：對 `4d1d054c3`（＝這一格還是 111 的那個 commit）與 HEAD 各算一次集合再相減，
//   ⇒ 「變成手刻」48 支、「不再手刻」**0 支**）：
//
//   | 群 | 支數 | 形狀 | 判定 |
//   |---|---:|---|---|
...
//
//   ⭐ ⛔ **那 11 支是誤判，⛔ 不是手刻** —— 它們**有**模板綁定，只是綁法是社群編輯器的
//   **卡片堆疊**（`template.cards[]`），而 `tools/brick-census/gen.ts` 的 `refsOf()`
//   （`gen.ts:142`）只認得 `ref` 與 `stack` 兩種形狀 ⇒ `cards` 一律回空陣列。
//   ⚠️ ⭐ 這是**同一支普查第二次踩同一個坑**：它自己的檔頭（`gen.ts:98`）記著上一次
//   ——「只讀文件級的 `template.ref`／`.stack`」⇒ 8 份模板 67 次採用被記成零採用。
//   ⇒ ⭐ **正解是修 `refsOf()`（讓它認得 `cards`），⛔ 不是把 11 併進基準線**；
//   修好之後 `bricks:build` 會量到 **148**，那時候這一格要跟著降。
//   ⛔ 本 lane 的檔案柵欄不含 `tools/brick-census/` ⇒ ⭐ 只記錄、⛔ 不順手修
//   （第零守則⑧：排序是 owner 的權力）。出處：`docs/_reports/hero-population-gates_temp_*.md`。
//
//   ⭐ 而那 37 支是**真的**第〇·五守則債：37 名社群英雄的 **Q/W/E/R 四格全部有 `template.ref`**
//   （37×4 = 148 支），⛔ **只有 EX 那一格沒有** —— 也就是社群英雄編譯器對 EX 槽
//   **不發模板綁定**。⇒ 那是一個**機制缺口**（擋住 37 支），⛔ 不是 37 次個別的手滑，
//   ⭐ 正好是第〇·五守則說的「按**擋住的支數**做機制」那種形狀。
//
