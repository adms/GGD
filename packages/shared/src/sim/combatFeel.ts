...
export interface StandstillRules {
  ...
  enabled: boolean;
  /**
   * ⚠️ **只有 {@link legacyAbsoluteClosing} 打開時才會被讀**(GH#755,2026-08-27)。
   *
   * 舊規則裡它**一格當兩用**:同時是「有在動」與「正在靠近目標」的門檻,
   * 而那正是本票修掉的東西 —— 新規則把兩者拆成 {@link stillEps} 與
   * {@link closingRatio}。這一格留著是為了讓 rollback 逐位元回得去。
   ...
   *
   * ⭐⭐ 值的**出處**(2026-08-27 補;在此之前它是 0.1,而 0.1 沒有出處 ——
   * 票被退回過一次就是因為這件事)。
   *
   * 這一格問的是「**這具身體這一 tick 有沒有動**」,而 **sim 早就有答案了**:
   * `systems/MovementSystem.ts` 用 **1e-6** 當位移的判準(`d > 1e-6` 決定
   * `moved`;`moved + 1e-6 < stepLen` 決定衝刺撞牆)。⇒ 照抄那一個,
   * ⛔ 不要發明第二個(第〇·四守則:同一個事實不可以有第二個住處)。
   *
   * ⚠️ 而且「雜訊地板」這個說法本身**量過之後不成立**:`t.vel` 是
   * `(pos − before)/dt` 的**精確**位移 ——
   *   · 撞牆/撞邊界 → `moveWithCollision` 整步取消 ⇒ 位移**正好 0**(⛔ 不是「很小」)
   *   · 被 separation 推開 → `separatePair` 只寫 `pos` 不寫 `vel` ⇒ 貢獻 **0**
   *   · 柱子磨蹭 → `attackStandstill.test.ts` 的 ss-obstacle-jitter 逐字記著那是
   *     **接近滿速**的抖動 ⇒ 任何小 epsilon 都擋不住它(擋它的是 {@link closingRatio})
   * ⇒ ⭐ **根本沒有一條「小而假」的速度帶需要跨過**,所以「挑一個夠大的數」
   *   這個問法從一開始就是錯的 —— 挑大了才有代價:0.1 正好夾在
   *   `facingLock.test.ts` 舊夾具的 0.118 底下一點點,⛔ 而那是一次靠**缺陷**才綠的
   *   守衛(見該檔 `movingAttacker` 的檔頭)。
   *
...
export const DEFAULT_STANDSTILL: StandstillRules = Object.freeze({
  ...
  walkEps: 0.5,
  applyToMobs: true,
  // GH#755 —— 兩格門檻各自可調（理由與**出處**寫在宣告上），rollback 預設關。
  //
  // ⭐ `stillEps` = MovementSystem 判斷「這具身體有沒有動」用的同一個 1e-6，
  //   ⛔ 不是一個挑出來的「雜訊地板」。完整推導在 StandstillRules.stillEps 的宣告上。
  stillEps: 1e-6,
  closingRatio: 0.5,
  // ⭐⭐ **出貨預設 = 新規則**（`false`），2026-08-27 第二次評估後定案。
  //
  // ⚠️ 這一格在同一天先被翻成 `true`（rollback），理由記錄為「新規則造成靈魂層
  // 回歸：`sim/facingLock.test.ts` 量到 `alignAtHit = -0.73`，攻擊朝反方向出手」。
  // ⭐ **那個歸因是錯的** —— 逐行複驗之後量到的是兩個**測試自己**的缺陷，
  // ⛔ 不是引擎行為：
  //
  //   ① `facingLock.test.ts` 的 `movingAttacker()` 給攻擊者 `moveSpeedMult: 0.02`
  //      （有效移速 ≈0.118），而那個值**鑽過**舊規則 `lenSq(vel) > walkEps²`
  //      的門檻 ⇒ 站定規則對他**靜默關閉** ⇒ 他才砍得出來。
  //      ⭐ 而他做的事逐字是「**背對敵人後退還揮劍**」—— 站定規則要擋的正是它。
  //      ⇒ 這條靈魂層守衛**過去是靠缺陷 A 才綠的**；#755 補起來它就紅，
  //        那不是回歸，是**前提消失**。
  //   ② 同一支測試的 `w.events.some((e) => e.type === "basicAttack")` ⛔ **沒有問
  //      是誰打的**。對面那位也會自動平砍 ⇒ 攻擊者一次都沒出手的那些世界裡，
  //      守衛照樣認為「看到揮劍了」，然後去量**攻擊者**的 facing ——
  //      ⭐ 那就是 −0.73 的來源（他朝著走路方向 +X，而敵人在 −X）。
  //      ⇒ 失敗形態④：斷言方向跟缺陷無關。
  //
  // ⇒ 修法是把夾具換成**出貨真的會發生**的那一種（壓在邊界上往外走 ⇒ 位移正好 0
  //   ⇒ `vel` 是 0 ⇒ 兩套規則都放行，而 `moved` 仍然 true ⇒ 移動分支照樣搶 facing）
  //   ＋ 事件認 `source`。改完之後 `facingLock` 在**新規則之下**綠。
  //
  // ⚠️ 留著 rollback 的理由沒有變（第〇·六守則：開關是為了**回頭**）：新規則會讓
  //   「被重減速的近戰接近戰」更難出手，而那是要用真的比賽才看得出來的取捨。
  //   後台 `combatFeel.standstill.legacyAbsoluteClosing = true` 一鍵回舊行為。
