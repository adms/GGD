 ...
 *
 * ⭐ 出貨 **true** —— owner 2026-09-08 02:28 的原話是「隨機選寶具的時候 道具欄已滿 怎麼辦」與「A ＋ B 開票」
 * （`docs/_daily/2026-09-08.md:13`）；「B＝讓玩家挑一件丟掉/賣掉再換上」是 Claude 補的定義，
 * 「預設開」是 Claude 依常設指令的推論（⛔ 不是 owner 說的）。它與客戶端的換裝介面（`ui/panels/draftSwapPicker.tsx`）
