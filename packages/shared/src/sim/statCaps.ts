...
export const DEFAULT_STAT_CAPS: StatCapTable = Object.freeze({
  ...
  // 🔴🔴 owner 2026-08-15「移動速度重新設計預設值：極小5/小7/中10/大14/極大18，
  //   一般上限24，解鎖上限30」→ base 24、unlocked 30。
  //
  //   ⚠️⚠️ **這組數字踩過了上一版自己量出來的穿牆警戒線，這裡留著原始推導
  //   讓下一個人看得懂賭注是什麼**（⛔ 不是不同意 owner，是這個數字需要
  //   被看見才算真的被決定過）：
  //
  //     移速二次更正時 owner 曾定「上限是 10」，unlocked 收在 12，理由是
  //     18.0 **正好坐在穿牆的平手線上**：門檻不是「速度 18」而是「每 tick 位移的
  //     法線分量 > 身體半徑」，30 Hz × 0.6(身體半徑) = 18.0，平手值本身就會穿，
  //     走路更是 100% 必穿。12 = 每 tick 0.4u = 半徑的 67%（安全邊際）。
  //
  //   現在極大 = 18（= 舊的平手線本身，14 位鬥士出身英雄**不用任何 buff**就站在
  //   這條線上）、解鎖上限 = 30（每 tick 1.0u = 半徑的 167%，遠在線外）。
  //   ⛔ 沒有任何守衛會攔這個 —— `relax()` 是離散 move-then-resolve
  //   （見 `collision/resolve.ts`），沒有掃描式碰撞，這正是舊註解在警告的洞。
  //   ⚠️ 新地圖的牆是整格厚（GH#324，2 世界單位），比舊注解假設的 0.4 深牆段厚
  //   很多，穿牆的實際機率因此比 2026-08 那次量測時低 —— 但沒有重新量過，
  //   所以這是「風險降低」不是「風險消失」。
  [Stat.MoveSpeed]: Object.freeze({ base: 24, unlocked: 30 }),
});
