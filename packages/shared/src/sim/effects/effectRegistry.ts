 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ⭐ 2026-08-20（#467 ②）—— **一個新 kind 現在是四個新檔 + 兩行**，⛔ 沒有一個
 * 步驟要你去編輯一個幾千行的共用檔：
 *
 *   1. `sim/effects/<name>.ts`          handler：`export const <name>Effect:
 *                                       EffectKindSpec<"<name>"> = { apply(e, ctx) {…} }`
 *   2. `sim/effects/variants/<name>.ts` TS 型別：`export interface <Name>Variant { kind: "<name>"; … }`
 *   3. `content/schema/effects/<name>.ts` Zod：`export const z<Name> = z.object({…}).strict();`
 *                                       （跨欄位規則就 `export const refine`，跟欄位放一起）
 *   4. `effects/effectRegistry.ts`（這一支）ONE import + ONE line in the record below
 *      `sim/effects/effect.ts` 的聯集加一行、`content/schema/effects/index.ts` 加一行
 *
 * 為什麼分成這樣：GH#289 之前十二個 handler 擠在 effectRunner.ts 的一個 500 行
 * `switch` 裡，每一條平行 lane 都撞在同一個 hunk；#467 ② 之前**型別**與 **Zod**
 * 還是各自一個大檔（2,751 / 4,754 行），GH#451 加 `chainLightning` 時另外三條 lane
 * 全被擋住。現在四邊都是「一個 kind 一個檔」。
 *
 * ⚠️ 四邊必須同時到齊，而且**有一條會紅的閘在守**：
 * `content/schema/effects/effectShardWiring.test.ts` 把
 * 「schema 檔案集合 == Zod 聯集 == 這張註冊表的鍵 == variants 檔案集合」四向對起來。
 * 少接一邊就紅，⛔ 不是等到某一份內容載不進去才發現。
 *
