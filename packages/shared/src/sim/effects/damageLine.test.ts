...
describe("damageLine — 前方直線上的敵人真的掉血 (do-damage-line)", () => {
  ...
  // ⭐ GH#1197 威寇茲 W：延遲的第二段。0 = 施放那一刻的正前方；1 = 施法者**橫移**離開原線、轉身之後的正前方。
  //   兩個方向一起驗：aim:"cast" 只打 0，aim:"facing"（舊近似）只打 1 —— ⛔ 量尺不是瞎的。
  //   ⭐ 修正輪：橫移 6 格（> 線寬的一半）⇒ 起點沒凍住、只凍方向的話，那條線從新位置往 +x 出發，打不到 0 ⇒ 紅。
  //   （舊夾具只往後退 3 格，線長 10 仍蓋得到 0 ⇒ 起點凍結整個被移除也是綠的。）
  it.each([["cast", 0], ["facing", 1]] as const)("⭐ 延遲直線 aim:%s —— 施法者橫移轉身後打到第 %i 個人", (aim, victim) => {
    const r = rig([[5, 0], [0, 10]]);
    const delaySec = 0.5;
