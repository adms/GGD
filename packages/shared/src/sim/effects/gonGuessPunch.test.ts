...
function punch(s: Stage, effects: readonly EffectDef[] = Abilities.get(PUNCH).effects ?? []): void {
  ...
  // 傷害走 `world.damageQueue`，在下一個 tick 結算；擊飛（15 tick）此時還在進行中。
...
describe("GH#1020 小傑 06-00 猜猜拳 —— 三個距離變體 ＋ 山形修煉的加強 ＋ 殺意的追加", () => {
  ...
  it("② ⭐ 承重：E 山形修煉-強的階級真的改變了石頭的傷害（350 + 150×lvl ＝ 基礎 × (1 + 3/7×lvl)）", async () => {
    ...
    const at = (rank: number): number => {
      const s = stage(3, 12);
      s.world.abilities.get(s.caster)!.slots.E.rank = rank;
      punch(s);
      return lost(s, s.victim);
    };
    const d0 = at(0);
    const d1 = at(1);
    const d5 = at(5);
    expect(d0).toBeGreaterThan(0);
    expect(d1, "E 的階級沒有進到石頭的傷害 —— 「並增強猜猜拳-石頭的威力」是無效說明").toBeGreaterThan(d0);
    // ⭐ 機制是「基礎 × (1 + coeff × 階級)」⇒ 每一階加**同一個**量：(d5 − d0) ＝ 5 × (d1 − d0)。
    //    AP 係數與減免對三發都是同一個加項／乘數，所以這條線性關係不依賴出生屬性。
    //    ⛔ 不釘出貨數字（第二守則：驗機制，⛔ 不驗數字）。
    expect((d5 - d0) / (d1 - d0), "每階加成不是線性 —— slotRankRatios 沒有照 coeff × 階級 進基礎").toBeCloseTo(5, 1);
  });
