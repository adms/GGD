...
const WORLD_HOOKS: readonly WorldHookRow[] = [
  ...
  { simEvent: "death", hook: "onDeath", scope: "actor", actorKey: "id", targetKey: "killer" },
  // 復活時。持有者是被復活的那個人，不是頂著圈圈的隊友。
  { simEvent: "reviveComplete", hook: "onRevive", scope: "actor", actorKey: "id" },
  ...
  for (const ev of world.events) {
    ...
    for (const row of rows) {
      if (row.scope === "actor") {
        ...
        fireHooks(world, owner, row.hook, idAt(ev.data, row.targetKey));
        continue;
      }
