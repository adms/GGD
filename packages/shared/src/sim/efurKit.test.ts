...
// ═══════════════════════════════════════════════════════════════════════════
// W 13-02 龍頭戲畫。牙突 — 「傷害 ＋ 目標最大生命百分比 ＋ 擊退」（不暈眩）
//
// ⭐ GH#459（owner 2026-08-19）—— **最大生命那一項回來了，而且這一次真的落地**。
//
// ⚠️ 這一段在 2026-08-12 → 08-19 之間期望「差 = 0」，理由寫著 owner 2026-08-12
// 「拿掉最大生命百分比」。⛔ 但那個決定**只落在 effects 上，沒有落在卡片上**：
// `description` 從頭到尾逐字寫著「造成 40/60/80/100 + 目標[最大生命]6/8/10/12%
// 的傷害」。於是這條測試守著的是「卡片說謊」這個狀態本身 —— schema 綠、
// content:build 綠、3,594 條測試綠，而玩家看到的那一項在場上不存在
//（CLAUDE.md 第一·五守則）。產生器那一列的註解自己也把它記成待裁決：
// 「要嘛改 md、要嘛改守衛，⛔ 不可以兩份各說各話」。
//
// GH#459 就是那個裁決，而 owner 選的是**補機制**，⛔ 不是刪那句話。
// ⇒ 期望值再翻一次面：**差 > 0，而且跟著目標的最大生命走**。
//
// ⭐ 斷言仍然**不抄任何出貨數字**（第二守則：驗機制不驗數字）：兩座只差 `maxHp`
// 的隔離夾具相減，問的是「這一項存在嗎、方向對嗎」，⛔ 不是「是不是剛好 6%」——
// 6/8/10/12 是 owner 每週在調的東西，抄進來就是第四個住處。
// 暈眩那一條維持 `toBe(false)`（那一半 owner 沒有推翻）。
// ═══════════════════════════════════════════════════════════════════════════
...
describe("13-02 龍頭戲畫。牙突 (efur-w-hppct / efur-w-knock)", () => {
  it("傷害跟著目標的最大生命走（卡面那一項真的落地），perRank 那一欄也仍然被讀", () => {
    cover("efur-w-hppct");
    ...
    // ⛔ 上面那條只證明「有一項跟最大生命有關」；這兩條擋另外兩種空掉的形狀：
