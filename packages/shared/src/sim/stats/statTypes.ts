...
export enum Stat {
  ...
  /**
   * ⭐ G12（GH#354）—— **單發傷害上限**，以承受者最大生命的比例表示。
   * #52 謎之紙片「單次受到超過最大生命 20% 的傷害時，該傷害最多只能造成 20% 最大生命」。
   *
   * 0 = **沒有上限** = 今天（⛔ 不是「上限 0%」—— 那會讓一個沒填的英雄免疫所有傷害）。
   * 與守衛塔的 `StructureComp.maxHitPctMaxHp` 是**同一個語意、同一個位置**
   *（`combat/damage.ts` 的 `mitigate`，抗性四段之後、護盾之前），
   * ⛔ 但**不是同一格**：塔沒有 `StatsComp`，而英雄的這一格要能被道具/增益/技能
   * 動到。兩處共用「> 0 才生效」這條，所以語意不可能分岔。
   *
   * ⚠️ 掛在**承受者**身上（是「我最多被打掉這麼多」），⛔ 不是攻擊者。
   */
  MaxHitPctMaxHp = "maxHitPctMaxHp", // 0 = 無上限
  /**
   * ⭐ G13（GH#354）—— **這個攻擊者的攻擊有多難被迴避**，0..1。
   * #51 神槍・金剛徹「滿層後普攻**無法被迴避**，仍可被格擋、護盾抵消」。
   *
   * 0 = 不影響 = 今天。1 = 對方的迴避率被完全歸零（＝「無法被迴避」）。
   *
   * ⛔ 它**不是**第三種迴避率，而是**對方那一格的折扣**：讀取端把防禦方算出來的
   * `p` 乘上 `(1 - unavoidable)`。這樣寫的三個好處都是可以驗的：
   *   ① `1` 時 `p` 變 0 → 走既有的 **ZERO GUARANTEE**（不抽 rng、不動狀態），
   *      所以「絕對命中」在**重播位元層**上等於「對方本來就沒有迴避」
   *   ② 中間值有意義（0.5 = 迴避率砍半），一張卡不必二選一
   *   ③ 兩條通道（普攻 `rollEvade` / 技能 `rollEvadeAbility`）共用同一個折扣，
   *      ⛔ 不用第二套 scope 詞彙
   *
   * ⚠️ 它**只**關迴避。格擋、護盾、免疫、傷害減免全部照舊 —— owner 的文案
   * 「仍可被格擋、護盾抵消」講的就是這條界線。
   */
