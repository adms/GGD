 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ③-d ⚠️ **w3x 原始值考古(GH#222)** —— 出貨的 6% / 10 倍**不等於**原作
 *
 * owner 2026-07-30:「天堂之劍爆擊設定錯誤,請你重新參考 w3x 說明」。
 * 逐欄挖出來的原作真值(⛔ 不是抽取器的摘要,是 `OBJECTS.json` 的原始欄位):
 *
 * ```
 * item   I01N 天堂之劍   base=azhr   abilities = [AIrc, A0JU, A110]
 *   A110  base AIcs (Item Critical Strike)   editor_suffix "( 天堂劍,3%50倍 )"
 *         data.1 = 3.0    ← Ocr1 機率 %
 *         data.2 = 50.0   ← Ocr2 傷害倍率
 *   A0JU  base AIlz       editor_suffix "(-500)"      ← 生命 -500
 *   AIrc  Item Reincarnation  data.2 = 5000           ← 「魂藏」死亡復活
 * ```
 *
 * 原作卡面(`description`)逐字:「魂藏 / **3%機率造成50倍傷害** / 生命最大-500」。
 * ⇒ **三個數字全部對得起來**,而且 `A110` 的欄位值與卡面一致 —— 這一件沒有
 * 「JASS 說一套、欄位說一套」的分歧,⛔ 也沒有被抽取器吃掉的欄位。
 *
 * 出貨值是 **6% / 10 倍**(`content/items/godie-i01n.json` 的 `critStrike`),
 * 期望增益 `0.06 × 10 = 0.6` vs 原作 `0.03 × 50 = 1.5` —— **差 2.5 倍**,
 * 而且爆發形狀完全不同(常見的小爆 vs 罕見的一擊)。
 *
 * ⛔ **這裡刻意不改任何數字。** 第〇·六守則:owner 2026-08-01 親手重寫過這張卡
 * (那次拿掉了「魂藏」整段),所以 6%/10 是**第 1 層**;w3x 的 3%/50 是**第 5 層**。
 * 階梯說第 1 層贏。⚠️ 但 GH#222 本身也是 owner 的話,而它指著第 5 層 ——
 * 兩則 owner 指示打架,**這正是守則說「列表拿給 owner 確認」的那一格**,
 * ⛔ 不是我挑一個。要改的話只有兩個欄位:`critStrike.chance` / `.damageMult`。
 *
 * ⭐ 這一段存在的理由是**知識不可以無聲消失**(同 `_w3x-fidelity-superseded.md`):
 * 不論 owner 選哪一邊,⛔ 沒有人需要再解一次 `war3map.w3t`。
 *
 * ─────────────────────────────────────────────────────────────────────────────
