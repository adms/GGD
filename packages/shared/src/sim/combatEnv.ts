...
/**
 * ⛔⛔ **這張表的「系統倍率」那一族是 owner 的人工旋鈕 —— 不是我的。**
 *
 * owner 2026-08-22（逐字，第三次講）：
 *
 * > 「你的傷害要從生命反推我沒意見，但**不能把系統倍率乘進去再反推**啊，
 * >  這樣我用系統倍率就沒意義了」
 * > 「技能↔普攻 跟 生命 之間的相對關係，都跟**系統倍率無關**喔」
 * > 「對 我說過**這是我人工的旋鈕**，並沒有放在公式裡，我們上次已經釐清過」
 *
 * ⚠️ 這句話有**兩半**，而 2026-08-22 之前這個檔只記了第一半：
 *
 * | 半 | 是什麼 | 記在哪 |
 * |---|---|---|
 * | ① **倍率不可以進公式** | 架構規則 | `content/damageTiers.ts::anchorFloorFrom` 的註解 ✅ |
 * | ② ⛔ **倍率不是我能轉的** | **所有權**規則 | ⛔ 在 2026-08-22 之前**沒有任何地方記著** |
 *
 * ⇒ 缺了②的後果（真的發生了）：為了把平衡拉回 owner 說過的「3.5 發」，
 * 我自己把 `damageDealt` 從 1.0 設成 2.5 —— **架構那條規則是綠的**
 * （它是最終縮放、沒有進任何推導），而沒有任何東西說它不是我的。
 * owner 當場抓到：「**我什麼時候提到 damageDealt 1.0→2.5 ?**」
 *
 * ⭐ **判準不是「這一格可不可調」** —— 每一格都可調，那是第一守則的重點。
 *   判準是「**這一次的改動引用得到 owner 的哪一句原話**」。引用不到 ⇒ ⛔ 不要動它。
 *
 * 🔒 **閘（⛔ 不是判準）**：`content/config/owner-knobs.json` 記著每一格的
 * 出貨值＋owner 的**逐字原話**；`packages/shared/src/ops/ownerKnobs.test.ts`
 * 比對它與 `content/config/combat-env.json`，⛔ 沒有原話就改 ⇒ 紅，並指名那一格。
 */
...
export const ATTRIBUTE_ENV_DEFAULTS = {
  ...
  //     所以調它**不偏離原作**。
  // ═══ 2026-08-22 · 那一組**折進基礎層**了（owner:「fix 折進卡片」）═════════════
  //
  //   owner:「技能↔普攻 跟 生命 之間的**相對關係,都跟系統倍率無關**喔」
  //         「⇒ 數字完全不變,但那個全域旋鈕**變乾淨了** —— 以後你調 attackDamage,
  //           它就是『所有人普攻等比變強變弱』,⛔ 而不是偷偷在改『技能 vs 普攻誰比較強』」
  //
  //   ⇒ `attackDamage` 0.6 → **1.0**,那個 0.6 乘進**每一個餵給 Stat.AttackDamage 的來源**:
  //     · 71 張英雄卡的 `baseStats.ad` 與 `growth.ad`   （LV30 中位佔 93.8%）
  //     · `strToAttackDamage` 0.4 → **0.24**            （佔 6.2%）
  //     · ⛔ **道具/增益卡不折** —— 見下面那一段
  //
  //   ⛔⛔ **道具的 `ad` 刻意不折**，而理由揭露了一個更早的缺陷：
  //   `legendary49OwnerText.test.ts` 要求那 49 支的描述與 **owner 交來的原稿逐字相同**，
  //   而原稿寫的是「攻擊力+87」。在 `attackDamage = 0.6` 的世界裡引擎只給 **52.2**
  //   ⇒ **卡片一直在說謊**（第一·五守則），⛔ 那不是這次折算造成的。
  //   ⭐ 倍率設回 1.0 之後 +87 **終於是真的** —— 所以正解是**不折**，讓 owner 寫的數字
  //   真的發生。⚠️ 代價（已知並接受）：道具的 ad 實際效果比 08-13 強 1/0.6 ≈ 1.67 倍。
  //
  //   ⛔ **sela / thorne 不折** —— 它們是內容載入失敗時的骨架保險絲，權威在
  //   `sim/content/skeleton.ts` 的 TS 字面值（`loader.test.ts` 逐欄比對兩邊）。
  //   折它們會讓 JSON 與 TS 分岔，而分岔的那一刻**正是內容壞掉的那一刻** ——
  //   保險絲跟著平衡改動走，就不是保險絲了。
  //
  //   ⚠️ **三個都要折才等價** —— `[Stat.AttackDamage]: [fixed("attackDamage")]` 乘的是
  //   **最終值**,只折卡片會讓 str 那一項與道具那一項各自變強 1.67 倍。
  //   ⭐ 驗證:LV30 中位最終 ad 折前折後都是 **70.77**（差 0.0006 = 浮點）。
  //
  //   ⚠️ `strToAttackDamage` 現在是 **0.24**,而 w3x 原作是 **1.0**
  //   （地圖 `StrAttackBonus=1.0`）—— 偏離原作的**幅度沒有變**,只是換了個住處:
  //   0.4×0.6 與 0.24×1.0 是同一個數。⭐ `_w3x-fidelity-superseded.md` 那一列仍然成立。
  //
