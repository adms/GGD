...
export const DEFAULT_BASE_BONUS: BaseBonusTable = Object.freeze({
  // owner 2026-07-30「目前玩家太容易死了」→ 300 → 650 → **1200**（2026-08-23）。
  // 這個數字 owner 已經動過**四次**,它是後台可調的動態設定 —— 這裡只是
  // 「沒設定過時生效的預設」,不是唯一合法值。
  // ⭐ 2026-08-23 逐字:「幫我調整後台系統倍率:英雄登場初始等級設定為6,
  //   **生命+1200**, 初始魔抗+20%, 生命倍率x12」（同一則四格,見 GH#615）。
  // ⚠️ 它**進得了傷害五級距的推導**（`anchorFloorFrom` 的 `bonus`）——
  //   改它 `anchors:check` 會紅,那是刻意的閘。
  [Stat.MaxHealth]: 1200,
  // ⭐ 2026-08-23 owner:「**初始魔抗+20%**」（同一則四格,GH#615）。
  // ⚠️ **算術**:`mitigationMult = 100 / (100 + 魔抗)` ⇒ 魔抗 20 只減傷 16.7%。
  //   他寫的是「+20**%**」（帶百分號）⇒ 讀成**減傷 20%**,那需要魔抗 **25**
  //   （`100/125 = 0.80`）。⛔ 要改回字面的 20 就是後台一格。
  // ⭐ 它與其他格一樣**不參與倍率**（owner #273「不參與倍率計算」）。
  [Stat.MagicResist]: 25,
