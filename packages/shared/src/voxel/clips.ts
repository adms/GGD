 ...
 *
 * SHARED CORE (task #229). Re-homed from `tools/voxel-gen/clips.ts` into
 * @ggd/shared so the admin studio's animation scrubber plays the EXACT curves
 * the bake writes into the .glb — see the boxman.ts header for the full
 * reconciliation note. `sampleClip` below is the only addition #226 does not
 * have: the bake needs keyframes, a live preview needs the pose BETWEEN them,
 * and interpolating here (rather than in the admin) is what keeps the preview
 * honest instead of merely similar.
 */
...
/**
 * The six logical states, as a list. `zClipMap` is `.strict()` over exactly
 * these keys, so `clipMapFor()` in doc.ts is TOTAL by construction and a
 * generated model can never ship a clipMap that fails validation or a clip
...
/** Clip by name, or null. (`cheer` is reachable this way; it is not a state.) */
export function findClip(name: string): ClipDef | null {
  return CLIPS.find((c) => c.name === name) ?? null;
}

/** One sampled joint pose: euler rotation, plus the hips' translation offset. */
export interface SampledPose {
  readonly rot: Readonly<Record<DrivenJoint, Euler>>;
  readonly hips: readonly [number, number, number];
}

const IDENTITY_ROT: Euler = [0, 0, 0];
...
/**
 * Sample a clip at time `t` seconds with LINEAR interpolation between
 * keyframes — the same interpolation glTF's default `LINEAR` sampler applies
 * to the baked curves, so the studio preview and the in-game playback agree at
 * every frame and not merely at the keys.
 *
 * Looping clips wrap; one-shot clips CLAMP to the final key, which is what
 * `ClipAnimator` does to a corpse (`death` holds its resting pose) and is why
 * the death clip's last frame is authored as the pose it should stick on.
 *
 * Every driven joint is always present in the result. A joint a keyframe omits
 * reads as identity rather than "hold whatever was there" — the #168 defence
 * spelled out in this file's header, enforced here rather than hoped for.
 */
export function sampleClip(clip: ClipDef, t: number): SampledPose {
  const dur = clip.duration;
  const time = clip.loop ? ((t % dur) + dur) % dur : Math.max(0, Math.min(dur, t));
  ...
  const span = b.t - a.t;
  const f = span > 1e-9 ? (time - a.t) / span : 0;

  const rot = {} as Record<DrivenJoint, Euler>;
  for (const joint of DRIVEN_ROTATION_JOINTS) {
    const ra = a.rot[joint] ?? IDENTITY_ROT;
    const rb = b.rot[joint] ?? IDENTITY_ROT;
    rot[joint] = [lerp(ra[0], rb[0], f), lerp(ra[1], rb[1], f), lerp(ra[2], rb[2], f)];
  }
  return {
    rot,
    hips: [
      lerp(a.hips[0], b.hips[0], f),
      lerp(a.hips[1], b.hips[1], f),
      lerp(a.hips[2], b.hips[2], f),
    ],
  };
}
