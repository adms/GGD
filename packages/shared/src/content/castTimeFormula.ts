 ...
 *
 * ⇒ 真源是 `content/config/cast-time-tiers.json`：
 *   `極小 0 · 小 0.1 · 中 0.3 · 大 0.5 · 極大 1.0`
 *   ⭐ 上界 1.0 與 `config.cast-time@1.castTimeMaxSec` **刻意同一個數字**。
 *
 * ⚠️⚠️ ⭐ **而底下那條階梯仍然爬到 `CAST_CAP` = 4.00 秒** ——
 * ⛔ 那是五級距落地**之前**的系統，而它**沒有被退場**。
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *  ⛔ 它已經誤導過人（⭐ 而這段警語就是為此而寫）
 * ═══════════════════════════════════════════════════════════════════════════
 * 2026-09-12：`castTimeCoverage` 拿**這條過期公式**去比**照級距寫的內容**
 * ⇒ 報出 **182 支「不一致」**。⚠️ 而逐項對照之後，內容是 `0.1`（＝**小**）、
 * `0.5`（＝**大**）—— ⭐ **內容一直是對的，說謊的是這支公式。**
 * ⇒ 我因此在 GH#1243 上連開**兩個假缺陷**，而它們都活過了一次自我複驗。
 *
 * ⭐ owner 2026-09-12（逐字）：
 ...
 *  ⭐ 為什麼今天還沒改掉它（⛔ 而不是「忘了」）
 * ═══════════════════════════════════════════════════════════════════════════
 * 把 `ladder()` 直接改成映到那五格 ⇒ 紅燈從 182 變 **586** ——
 * ⭐ 因為 907 支出貨技能裡**只有 185 支**帶 `castTimeTier`，
 * ⛔ 其餘 722 支還帶著這條舊階梯吐出來的值（`0.067` / `0.467` / `1.033`…）。
 * ⇒ ⭐ 遷移**做了 185 支就停了**，而沒有任何東西記錄它停在哪。
 *
 * ⇒ ⭐ **在遷移完成之前，這段警語就是那個「記錄」** ——
 *   ⛔ 不要再拿底下的數字去判斷任何一支技能的 `castTimeSec` 是不是「錯的」。
 *   ⭐ 先看它有沒有 `castTimeTier`：有 ⇒ **級距說了算**，⛔ 這支公式沒有意見。
 *
 * 完整的四步修法與量到的數字在 **GH#1243**。
 *
 * ───────────────────────────────────────────────────────────────────────────
 *
 ...
 */
/**
 * ⛔⛔⛔ **這條 20 階階梯已經被五級距取代了（GH#943），而它還在跑。**
 *
 * owner 2026-09-02（逐字）：
 * > 「吟唱⋯其實這個也可以五級距 **0, 0.1, 0.3, 0.5, 1** 建議也改成這個」
 * ⇒ 唯一真源是 `content/config/cast-time-tiers.json`，⛔ **不是下面這條階梯**。
 *
 * ⚠️⚠️ ⭐ **它會誤導你，而且已經誤導過人**（2026-09-12）：
 * `castTimeCoverage.test.ts` 拿**這條過期公式**去比**照五級距寫的內容**
 * ⇒ 報出 182 支「不一致」。⭐ 而逐項對照之後，內容是 `0.1`（＝小）、`0.5`（＝大）
 * —— ⭐⭐ **內容一直是對的，說謊的是這條公式。**
 * ⇒ 我因此在 GH#1243 上連開**兩個不存在的缺陷**（見那張票的兩則更正）。
 *
 * ⭐ owner 2026-09-12：「**請你更正讓你誤會的 吟唱冷卻公式 以後不要再發生**」
 *
 * ⛔ **為什麼今天還沒換掉**：遷移只做了 **185 / 907** 支
 * （只有 185 支技能有 `castTimeTier`，其餘 722 支還帶著這條階梯吐出來的
 *  `0.067` / `0.467` / `1.033` 這種值）。⇒ 直接換會讓 586 支紅，
 * ⭐ 而那需要一張**只能變短的遷移名單**，⛔ 不是把斷言放寬。工單在 GH#1243。
 *
