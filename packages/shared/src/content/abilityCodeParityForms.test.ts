...
describe("變身對子的技能同步", () => {
  ...
  const CAST_TIME_DRIFT = [
    // ⭐ 2026-09-07（GH#1100）：`77-03` 修好了 —— `godie-e00x.e` 從裸的頂層 `damage`
    //    改成 15-02 疾風迅雷的出貨形狀（`applyBuff` ＋ `onBasicAttack` hook），
    //    castType targeted→self、range 12→0 ⇒ 兩形態的 castTimeSec 不再被推開。
    //    ⛔ 棘輪只能變短，所以它從這張表拿掉，⛔ 不是留著。
    // ⭐⭐ 2026-09-12（GH#1243）：**這張表清空了 —— 16 → 0**。
    //
    // owner 2026-09-12（逐字）：
    ...
    // ⇒ ⭐ 根因不是這 16 個編號各自有問題，是**那條階梯本身**（已搬 legacy）。
    //
    // ⚠️ ⭐ 空表**不等於這條閘沒事做**：它現在守的是「⛔ 不准再長出新的一筆」——
    // 任何一支技能繞過級距去手寫 `castTimeSec`，這裡就會紅。
    // ⭐ 空的 —— 16 個編號全部修好（見上）。⛔ 不要往這裡加，先問「它為什麼繞過級距」。
  ] as const;
