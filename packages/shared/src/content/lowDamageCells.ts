 ...
 *（`damageTiers.tiersFromAnchor()`：極小 × 單體冷卻 ÷ 6）。⇒ 單體那一列**恆等於**
 * 錨點，而 `deficitPct` 只可能在別的形狀上是負的。
 *
 * 量到的（2026-08-20，出貨表）：
 *
 *     單體 五格          191.67/秒   ±0%
 *     範圍・極小  1150×2÷30  =  76.67/秒   **−60%**
 *     範圍・小    2875×2÷45  = 127.78/秒   **−33%**
 *     範圍・中/大/極大               191.67/秒   ±0%
 *     變身        expectedHits 0 ＝ 豁免（回報軸不是傷害）
 *
 * ⭐ 那兩個百分比**逐位元**等於我 2026-08-20 報給 owner 的數字 ——
 * 因為它們是同一條算式，⛔ 不是我抄過來的兩個字面值。
 *
 * ─────────────────────────────────────────────────────────────────────────────
 ...
 * `(1150×15)/15` 這種除不盡的算式偶爾掉出來，而它在數學上是恆等的。
 */
