...
describe("技能在解鎖等級付得起（GH#314）", () => {
  it("⭐ 沒有任何一格的首階 MP 超過持有者當時的魔力池 —— 超過 = 那顆鈕永遠按不下去", async () => {
    ...
    // ⚠️ 這個門檻在 2026-08-21 從 200 降到 100：owner ⑦「**若不是主動傷害技能
    //    就免魔力吧 乾脆點**」把 133 支非傷害主動技的耗魔設成 0，⇒ 帶著 MP 的
    //    技能少了三分之一。⛔ 它擋的是「掃到空集合」，⛔ 不是一個平衡數字 ——
    //    所以留一個離 0 夠遠、離出貨值也夠遠的下界就好。
    expect(checked, "一格帶 manaCost 的技能都沒掃到 —— 這條守衛在測空集合").toBeGreaterThan(100);
    expect(over, "MP 超過魔力池 = 那顆鈕永遠按不下去，而且面板不會說為什麼").toEqual([]);
    // ⛔⛔ GH#979 —— **時鐘，⛔ 不是斷言**。這一條要把出貨的整棵 content 樹
    //   （約 1,900 份文件）從磁碟載入再 `registerAll`。本機 0.68 秒，而 CI 是
    //   2 核共享 runner 且 `pnpm -r --if-present test` 讓多個 workspace 同時在跑
    //   ⇒ 2026-09-04 量到 **5,036 ms**，正好撞穿 vitest 的 5,000 ms 預設。
    //   ⇒ 給一個**明確**的上限。⛔ 這不是放寬斷言（上面兩條一個字都沒動），
    //     ⭐ 是把「這台機器有多快」這個與缺陷無關的變數拿掉。
    //   ⚠️ 60 秒仍然抓得到真的失控（正常是 0.7 秒，差兩個數量級）。
  }, 60_000);
});
