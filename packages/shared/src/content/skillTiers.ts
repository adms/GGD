 ...
 *
 * ⚠️ 「超大」在第四格、「極大」在第五格，是把兩套舊詞彙合併的結果：
 * AoE 舊的第四格叫「超大」（出貨 6 支技能在用），位移舊的第四格叫「極大」
 * （出貨 **0** 支在用）。⇒ 保留使用中的那個字，把沒人用的那個字讓給新的第五格，
 * 於是**沒有任何一支既有技能的級距詞改變意思**。
 */
export const SKILL_TIER_NAMES = ["小", "中", "大", "超大", "極大"] as const;
