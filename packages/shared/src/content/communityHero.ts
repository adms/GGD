...
export const zHeroSnapshot = z.object({ schema: z.literal("ggd-hero-submission@1"), id, workId: id, accountId: id, version: zHeroStoredVersion, inspection: zHeroInspection, allowAttributionRemix: z.boolean(), canonicalTakeover: z.boolean().optional().default(false), source: zHeroSource.optional(), submittedAt: z.string().datetime() }).strict();
...
export const zHeroPublishOperation = z.object({ id, action: z.enum(["publish", "restore", "unpublish", "withdraw"]), inputDigest: digest, submissionId: id, status: z.enum(["publishing", "published", "failed", "stale", "unpublished", "withdrawn"]), expectedPublished: digest, expectedRevision: z.number().int().nonnegative(), guardRevision: z.number().int().nonnegative(), decisionId: z.string().max(128), reason: z.string().max(4000), requestedBy: id, error: z.string().max(12000).optional(), completedAt: z.string().datetime().optional() }).strict();
...
export const HERO_PUBLICATION_STATUSES = ["pending", "approved", "returned", "rejected", "publishing", "publish-failed", "published", "unpublished", "superseded", "withdrawn"] as const;
export const HERO_STATUS_LABELS: Record<typeof HERO_PUBLICATION_STATUSES[number], string> = { pending: "待審", approved: "已審未發布", returned: "退回修改", rejected: "已拒絕", publishing: "發布中", "publish-failed": "已審、發布失敗", published: "已發布", unpublished: "已下架", superseded: "歷史候選", withdrawn: "作者已撤回" };
...
export const zHeroWork = z.object({ schema: z.literal("ggd-hero-work@1"), id, ownerId: id, draftRevision: z.number().int().positive(), draftDigest: digest, draftVersion: digest.optional(), draft: z.unknown(), source: zHeroSource.optional(), createdAt: z.string().datetime(), updatedAt: z.string().datetime() }).strict();
export const zHeroDraftVersion = z.object({ schema: z.literal("ggd-hero-draft-version@1"), versionId: digest, workId: id, ownerId: id, revision: z.number().int().positive(), draftDigest: digest, source: zHeroSource.optional(), createdAt: z.string().datetime(), updatedAt: z.string().datetime(), previousVersion: digest.optional(), restoredFrom: digest.optional() }).strict();
export const zHeroDraftHistory = z.object({ headVersion: digest, revision: z.number().int().positive(), versions: z.array(zHeroDraftVersion).max(50), nextVersion: digest.optional() }).strict();
export type HeroDraftVersion = z.infer<typeof zHeroDraftVersion>;
export type HeroDraftHistory = z.infer<typeof zHeroDraftHistory>;
