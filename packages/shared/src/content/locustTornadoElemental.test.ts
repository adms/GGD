...
describe("蝗蟲群視覺 · TornadoElemental ×9（GH#688 Phase 6）", () => {
  ...
  it("每一個落點都擺得出那具龍捲風（standalone ＋ champion 鏡射），tint 引用 census", () => {
    ...
    for (const row of LANDINGS) {
      ...
      // ⭐ 過出貨那一支驗證器（content:build 用的同一支）—— lane 期間 build 上鎖，
      //    這裡是 spawnModelFx refine 唯一會當場響的地方。
      const coll = row.file.includes("/items/") ? "items" : "abilities";
      const v = validateDoc(coll, standalone);
