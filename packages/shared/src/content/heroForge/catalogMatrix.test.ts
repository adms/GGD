    ...
    for (const [originIndex, origin] of ORIGINS.entries()) {
      ...
      for (const plan of plans) {
        ...
        for (const ability of Object.values(result.draft.abilityDrafts)) {
          const scenario = runHeroAbilityScenario(result.draft.champion, ability, { ticks: 180, relatedAbilities: Object.values(result.draft.abilityDrafts), relatedProjectiles: projectileCatalog, relatedChampions: result.draft.relatedChampions });
        ...
        const kit = runHeroKitScenario(result.draft.champion, result.draft.abilityDrafts, { ticksPerStep: 180, relatedProjectiles: projectileCatalog, relatedChampions: result.draft.relatedChampions });
  ...
  it("compiles and executes every enabled template in its active or passive lane", () => {
    ...
    for (const template of catalog) {
      ...
      for (const rank of [...new Set([1, ability.maxRank])]) {
        const scenario = runHeroAbilityScenario(result.draft.champion, ability, { rank, ticks: 180, relatedAbilities: Object.values(result.draft.abilityDrafts), relatedProjectiles: projectileCatalog, relatedChampions: result.draft.relatedChampions });
  ...
  it("executes every ordered pair of compatible templates with explicit last-wins", () => {
    ...
    for (const slot of ["PASSIVE", "Q"] as const) {
      ...
      for (const first of lane) for (const second of lane) {
        ...
        const scenario = runHeroAbilityScenario(result.draft.champion, result.draft.abilityDrafts[slot], { ticks: 180, relatedAbilities: Object.values(result.draft.abilityDrafts), relatedProjectiles: projectileCatalog, relatedChampions: result.draft.relatedChampions });
