...
const PRESET_FIELDS = [
  ...
  "spinDegPerSec",
  "scale",
  // ⭐ GH#702 —— 非等向縮放。與 `scale` 同一個理由住這張表：「這一族的模型要被
  //    拉多長才讀得成一道光束」是**家族**的性質（那個比例來自 .glb 的包圍盒長寬
  //    比，⛔ 不是逐支技能的選擇）。⚠️ 出貨的模板現在**一份都沒有宣告它**
  //    ⇒ `slotDefault` 回 undefined ⇒ 既有節點逐位元不變 —— 這一行是為了讓
  //    「整族一起變長」有一個住處，⛔ 不是為了現在就動誰。
  "scaleAxis",
