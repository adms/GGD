...
function expectedAbilities(): string[] {
  ...
  return [...out]
    .filter((aid) => existsSync(join(ROOT, `content/abilities/${aid}.json`)))
    .filter((aid) => BLOCKED[aid] === undefined)
    .sort();
}
...
describe("蝗蟲群視覺第一批 · MonsoonBoltTarget (GH#691)", () => {
  ...
  it("每一支證據點名的出貨技能都真的擺得出那具模型（standalone + champion 鏡射）", () => {
    ...
    expect(rejected, "出貨技能文件過不了 schema —— content:build 會在同一處拒絕它").toEqual([]);
