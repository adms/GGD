...
export const zLegendaryShelfConfig = z
  .object({
     ...
     *   （B＝讓玩家挑一件丟掉/賣掉再換上）。⚠️ 在此之前這裡寫「出貨 false，那是 owner 的設計決定、
     *   引用不到原話」—— ⛔ 而上面那一句就是原話（它逐字寫在 GH#1110 最上面）。
 ...
 *
 * ⭐ 出貨 **false**（我挑的，owner 不在時「自己判斷但留開關」）：票文逐字「隊友可**自行決定**是否點燈」，
...
export const DEFAULT_LEGENDARY_SHELF: LegendaryShelfConfig = {
  ...
  priceMultiplier: 3,
  sellRefundPct: 0.4,
  // ⭐ 出貨 true（owner 2026-09-08「A ＋ B 開票」，B＝挑一件丟掉/賣掉再換上）。GH#1110。
  swapWhenFull: true,
