...
export const zConfigCritDoc = z
  .object({
     ...
     * 種子 → 三種模式給出三組不同的總倍率）。
     */
    stackMode: z.enum(["multiply", "max", "add"]).describe(
      "@zh 多條暴擊同時成立時怎麼合成\n" +
      ...
      "@opt multiply multiply 每條獨立骰、倍率相乘（出貨值＝owner 裁決）\n" +
      "@opt max max 只取最高的那一條（2026-08-09 之前的行為）\n" +
      "@opt add add 每條獨立骰、倍率相加",
    ),
    /**
     ...
     */
    maxTotalMult: z.number().min(1).max(1000).describe(
      "@zh 一次攻擊的總倍率上限\n" +
     ...
     */
    sourceCap: z.number().int().min(1).max(16).describe(
      "@zh 同一次攻擊最多算幾條暴擊來源\n" +
