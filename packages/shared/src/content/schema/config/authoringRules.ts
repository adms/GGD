...
export const zConfigAuthoringRulesDoc = z
  .object({
    ...
    editor: z.object({
      // ⭐ 2026-09-08 合併 PR 1118：補上 `@zh` —— ⛔ 少了它後台只能**手寫**一格標籤，
      //   而 `adminFormsHandWrittenRatchet` 的手寫數就會從 37 爬到 38（第〇·四守則：
      //   人話的住處是 Zod 的 `.describe()`，⛔ 不是 admin 的第二份表）。
      autosaveIntervalMs: z.number().int().min(100).max(5000).describe(
        "@zh 本機草稿自動儲存間隔\n"
        + "@note 停止輸入後等待這麼多毫秒，把**原始輸入**保存到本機。⭐ **即使欄位還沒通過驗證也會保存**"
        + "（存的是原始輸入，⛔ 不是驗過的文件）；⛔ 保存不等於投稿 —— 它不會送審。"
        + "⚠️ 調小＝當掉時掉得少但寫得勤；調大＝相反。消費端 `apps/editor/src/drafts/session.ts:88`。",
      ),
