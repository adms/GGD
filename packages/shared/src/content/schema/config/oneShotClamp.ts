 ...
 * ⚠️ B **已不是懸案**（2026-09-15 更正；「結案」是 Claude 依下列原話的推論）：同一晚 Claude 交了 B 的深入了解，
 *    owner 回「系統倍率是我人工旋鈕 不參與公式 不信你查證」（09-06 01:36）、Claude 回「所以 B 不用做了」owner 未反對，
 *    當天定案走旋鈕「好吧 先開票 血量倍率4x, M=15 K=1000」（09-06 12:28；`docs/_daily/2026-09-06.md:27,52`）
 *    ⇒ ⛔ 級距反推空間不動。之後 owner 2026-09-15 對「幾發打死」逐字：「我們已經固定 不需要再乘 頂多是後台試算後顯示 但不干涉也不警示」。
...
export const zConfigOneShotClampDoc = z
  .object({
    ...
    enabled: z.boolean().describe(
      "@zh 夾限總開關\n" +
      "@note ⭐ **出貨開著**（owner 2026-09-06 逐字：「先做 A 但我想深入了解 B」—— A ＝ 翻開這一格，GH#1017；B ＝ 級距反推空間，⛔ 不動：09-06 當天改走旋鈕「好吧 先開票 血量倍率4x, M=15 K=1000」）。⚠️ 開著會改變**每一場**比賽的結果 —— 翻回關閉 ＝ 一鍵 rollback。⭐ 關著時整條夾限逐位元 no-op（⛔ 不是「夾到 100%」）。",
    ),
    /**
