...
export const zConfigVfxCleanupDoc = z
  .object({
    ...
    /**
     * 🖼 平板／低畫質：同時保留幾組。⭐ 這一格存在的理由就是 issue 的那句話 ——
    ...
    groundTextureCacheMaxMobile: z
      ...
      .max(GROUND_TEX_CACHE_BOUNDS.max)
      .optional().describe(
      "@zh 🖼 地面貼圖快取上限：平板／低畫質同時留幾組\n" +
      "@note 同上，但用在畫質判定為 mobile 的裝置（平板，以及 auto 判成低畫質的內顯）。⭐ 這一格單獨存在的理由：那 52 張常駐貼圖在平板／內顯上可能是**致命**的，而畫質的自動降級階梯（粒子密度、陰影、後製）**碰不到貼圖快取** —— 這一格就是把它接進那個階梯。出貨值就是下界（{{出貨值}} 組），也就是「當前這張場地 ＋ 上一張」：上一回合剛打過的那張重播時仍然是零抓取，再往前的就重抓（平板上換圖那幾幀是平色，⛔ 不是黑的）。調大＝平板換圖更順但吃記憶體。",
    ),
  })
  .strict();
...
export const DEFAULT_VFX_CLEANUP: ConfigVfxCleanupDoc = {
  ...
  //   · 平板 4 組（≈22 MB）＝下界：一張場地最多兩個鍵,4 組 = 當前那一張 + 上一張,
