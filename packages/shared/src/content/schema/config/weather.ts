...
export const zConfigWeatherDoc = z
  .object({
    ...
    puddleAlpha: z.number().min(0).max(1).describe(
      "@zh 積水：多不透明\n" +
      "@note 1 = 完全蓋住底下的地板紋理（看起來像油漆不像水）。0.72 = 出貨值：看得到水底下的石頭，但反光是水的。",
    ),
    /** 積水的粗糙度。⭐ 越低越像鏡子；0.02 以下會只剩一顆太陽的亮點。 */
    puddleRoughness: z.number().min(0.01).max(0.6).describe(
      "@zh 積水：多像鏡子\n" +
     ...
     */
    fogDensityAtFull: z.number().min(0).max(0.003).describe(
      "@zh 起霧①空氣：最濃時的濃度\n" +
     ...
     */
    fogBankCount: z.number().int().min(0).max(8).describe(
      "@zh 起霧②霧片：場上同時幾片\n" +
     ...
     * ⛔ 不要因為「閘還沒紅」就把出貨值往上推。
     */
    fogBankAlpha: z.number().min(0).max(0.35).describe(
      "@zh 起霧②霧片：一片有多濃\n" +
     ...
     */
    fogBankLaneFill: z.number().min(0.2).max(1).describe(
      "@zh 起霧②霧片：一片多大（佔車道）\n" +
    ...
    fogBankDriftSec: z.number().min(8).max(600).describe(
      "@zh 起霧②霧片：飄一趟幾秒\n" +
     ...
     */
    fogBankHeight: z.number().min(0.2).max(20).describe(
      "@zh 起霧②霧片：離地多高\n" +
     ...
     * 那是 owner 逐字說**不要**的二元。
     */
    rainEnabled: z.boolean().optional().describe(
      "@zh 🌧️ 下雨總開關（止血閥）\n" +
     ...
     */
    rainChance: z.number().min(0).max(1).optional().describe(
      "@zh 🌧️ 下雨：一場比賽的機率（0–1）\n" +
     ...
     */
    rainDropsAtFull: z.number().int().min(0).max(6000).optional().describe(
      "@zh 🌧️ 下雨：場上同時幾滴（滿載）\n" +
    ...
    rainFallSpeed: z.number().min(1).max(60).optional().describe(
      "@zh 🌧️ 下雨：掉多快\n" +
    ...
    rainTiltDeg: z.number().min(0).max(60).optional().describe(
      "@zh 🌧️ 下雨：斜多少度\n" +
    ...
    rainStreak: z.number().min(1).max(24).optional().describe(
      "@zh 🌧️ 下雨：一滴拉多長\n" +
     ...
     */
    rainAlpha: z.number().min(0).max(0.6).optional().describe(
      "@zh 🌧️ 下雨：一滴多不透明\n" +
