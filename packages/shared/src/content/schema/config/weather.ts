 ...
 * `fogTransmittance()` 是那條界線的算術，而守衛
 * （`render/weatherFog.shipped.test.ts`）拿**出貨場地真的量得到的最遠對戰距離**
 * 去跑它 —— ⛔ 不是抄一個字面距離。
 */
import { z } from "zod";
...
export const zConfigWeatherDoc = z
  .object({
    ...
    /**
     * 霧權重 = 1 時，額外加在空氣漫反射之上的 EXP2 密度。
     *
     * ⚠️ 上界 **0.006** 不是隨手挑的：它是「最遠對戰距離上還留得住
     * {@link FOG_MIN_TRANSMITTANCE}」反解出來的那個數（守衛用出貨場地重算一次）。
     * Zod 的 max 只吃字面值，所以這裡是字面值而**閘在測試**。
     */
    fogDensityAtFull: z.number().min(0).max(0.006),
    /**
