 ...
 * ════════════════════════════════════════════════════════════════════════════
 * Codex 的回報把五張貼圖列成同一種「透明底板 blocker」。⛔ 而逐張量過之後，
 * 那五張**不是同一個病**，而且其中兩張在**它們今天實際被使用的方式下是安全的**：
 *
 ...
 *   · `hasAlphaShape` —— alpha 通道**帶不帶形狀**（`minAlpha` 與 255 有沒有距離）。
 *      ⚠️ `heroeva01effect` 的 alpha 是**平的 253** —— 一個看起來存在、
 *      實際上一格形狀都沒有的 alpha 通道。那比沒有 alpha 更難發現。
 ...
 *      `ribbonblur1` 的平均 59.1 幾乎完全來自**左邊那一條 208.3**（其餘三邊 ≤17.6）
 *      —— 一個平均值會把「一條亮邊」讀成「整體偏亮」，而那是兩種不同的缺陷。
 *
 * ⛔ **這份文件不修貼圖**（那是美術決定，而且需要 owner 授權）。它交付的是
 * **契約與量測**：哪幾張不能用、在什麼條件下不能用、以及**憑什麼這樣說**。
 *
 * ════════════════════════════════════════════════════════════════════════════
        ...
        hasAlphaShape: z
          ...
          .describe(
            "alpha 通道帶不帶形狀 ＝ `alphaRange > 8`。⭐ `false` ＝ alpha 是一個常數" +
              "（255 或 **253**）⇒ **形狀完全住在 RGB 裡** ⇒ 只有 additive 讀得出來。",
          ),
        opaquePct: z
          .number()
          .min(0)
          ...
          .describe(
            "alpha === 255 的像素百分比。⚠️⚠️ **這一格自己會說謊，⛔ 不要單獨讀它**：" +
              "`heroeva01effect` 的 alpha 是常數 **253**，於是這一格算出 **0%** —— " +
              "讀起來像「整張全透明」，⭐ 而它其實是**整張完全不透明**。" +
