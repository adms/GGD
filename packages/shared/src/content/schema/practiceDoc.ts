  ...
  .object({
    ...
    enabled: z
      ...
      .describe(
        ...
        "練習模式的總開關。false = 大廳按了練習模式也只會開出一間普通的房（有對手、時間到就結算、測試碼照舊只在 dev 開）—— 也就是這個功能出現之前的行為，所以它就是一鍵 rollback。⚠️ 它**不影響**任何一場正式比賽：一間房要先被開成練習房才會讀到這份文件。"
      ),
    endlessCombat: z
      .boolean()
      .describe(
        ...
        "練習房的戰鬥階段**永遠不結束**。true（出貨值）＝ 進去就一直待在場上，不會因為時間到或場上只剩一隊而被踢回商店 —— 這是「進入不會有對戰」那句話真正的機制。false ＝ 照正常回合流程跑（沒有對手所以第一 tick 就結算完進中場），適合拿來測商店與三選一的接線。"
      ),
    autoMobWaves: z
      .boolean()
      .describe(
        ...
        "排程的殭屍波要不要自己來。false（出貨值）＝ 場上乾淨，殭屍**只由生怪指令**產生，所以要測一隻特定的怪不會被一整波蓋掉。true ＝ 連波次排程一起開，用來看「一波一波湧進來」在這張場地上長什麼樣子。⚠️ 兩種模式下生怪指令都能用：練習房一律把小怪規則表備妥。"
      ),
    fireRing: z
      .boolean()
      .describe(
        ...
        "火圈要不要點燃。false（出貨值）＝ 不燒，因為練習房沒有「把回合逼到結束」這件事要做，而一個會慢慢燒死你的沙盒沒辦法拿來慢慢看特效。true ＝ 照這張場地的火圈設定走，用來確認縮圈半徑與燒傷節奏。"
      ),
    autoRevive: z
      .boolean()
      .describe(
        ...
        "自己倒下之後要不要自動站起來。true（出貨值）＝ 下一 tick 就滿血復活，所以測到一半被自己召來的殭屍王打死不會讓整場練習卡住。false ＝ 照正常規則（要隊友來復活圈救，而練習房沒有隊友），適合拿來測死亡表演與觀戰畫面。"
      ),
    spawnBatch: z
      .number()
      .int()
      ...
      .describe(
        ...
        "生怪指令**沒有指定數量**時，一次生幾隻。⚠️ 它是預設值不是上限：真正的天花板是小怪波設定裡的「每區同時存活上限」，生怪指令撞到就停，所以這一格調大不會讓練習房被自己生出來的怪淹掉。"
      ),
    dummyCount: z
      .number()
      .int()
      ...
      .describe(
        ...
        "練習房開場站幾個**靶子**（敵方英雄，⛔ 不移動 ⛔ 不普攻 ⛔ 不施放技能）。3（出貨值）＝ owner 明說的「練習模式預設對方三個英雄」。0 ＝ 對面空著，也就是這個功能出現之前的練習房 —— 它就是一鍵 rollback。⚠️ 靶子是**完整的英雄實體**（有血條、有護甲、吃傷害、掛得上狀態），所以量到的傷害數字與正式比賽一致。"
      ),
    dummyFightsBack: z
      .boolean()
      .describe(
        ...
        "靶子要不要**還手**。關掉（出貨值）＝ 站著不動被打不還手，這是「靶子」這兩個字的全部意思。開著＝ 靶子拿的是一般的 bot 大腦，會走位、會普攻、會施放技能 —— 也就是**今天的 vs bot 行為**，所以這一格是「我想要會動的對手」時的切換，⛔ 不是另一種靶子。"
      ),
    dummyChampionId: z
      .string()
      .max(64)
      .describe(
        ...
        "每個靶子固定用**哪一隻**英雄。空字串（出貨值）＝ 每個各自隨機抽（和 bot 同一個池子：白名單 ∩ 有模型）。填一個英雄 id ＝ 全部都用那一隻，用來把「血量／護甲」釘在一個已知的基準上量傷害。⚠️ 填了一個不存在／未上架的 id 會被當成沒填（照樣隨機），⛔ 不會讓練習房開不起來。"
      ),
    dummyRespawnSec: z
      .number()
      .min(0)
      ...
      .describe(
        ...
        "靶子被打死之後幾秒**原地滿血**重生（GH#681，owner 2026-08-24「被打死過五秒會重生」）。5（出貨值）＝ 那句話。0 ＝ 死掉的下一 tick 就站起來（＝這個功能出現之前的行為，一鍵 rollback）。⚠️ 這一格**只管靶子**：玩家自己的自動復活是上面的 autoRevive，而靶子的重生**不吃** autoRevive —— 關掉 autoRevive 測死亡表演時，靶子照樣會回來（不然練到後面沒東西打）。"
      ),
  })
  .strict();
