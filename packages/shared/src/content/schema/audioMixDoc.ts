  ...
  .object({
    othersGain: z
      ...
      .describe(
        ...
        "**其他角色**（敵人／隊友／小怪，也就是除了你自己以外）的語音音量倍率，疊在 #253 的空間化衰減之上。owner 2026-08-17：其他角色語音應該是自己的一半 ⇒ 出貨 {{出貨值}}。1 = 跟自己一樣大聲（＝2026-08-17 之前的行為）。"
      ),
    /**
    ...
    roundEndMachineVoice: z
      ...
      .describe(
        ...
        "GH#527。回合結束那一拍，TTS 機械語音（名言／嘲諷）要不要出聲：off = 只留角色自己的勝利宣言（出貨值，owner 2026-08-22 的原話）；fallback = 只有沒有語音包的英雄才退回機械語音；on = 三支都放（＝2026-08-22 之前的行為）。\n" +
        "@opt off off（只留角色自己的語音 · 出貨值）\n" +
        "@opt fallback fallback（沒有語音包的英雄才用機械語音）\n" +
        "@opt on on（三句都放 ＝ 2026-08-22 之前）"
      ),
    /**
    ...
    roundEndMinGapSec: z
      ...
      .describe(
        ...
        "GH#527 的「重複播放太吵了」那一半。同一位英雄在回合結束拿到一句語音之後，這麼多秒之內不會再拿到第二句（無論是角色語音還是機械語音）。0 = 沒有間隔限制（＝2026-08-22 之前的行為）。"
      ),
  })
  .strict();
