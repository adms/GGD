  ...
  .object({
    ...
    stylePrompt: z
      ...
      .describe(
        ...
        "PASS 2(風格)的**正向**提示詞 —— 這一整段就是「圖長什麼風格」。出貨值落實 owner 2026-08-17 的原話:日本 2D RPG、精緻、⛔ 顏色不要過度花俏(限制色數 · 柔和自然色 · 清楚的輪廓線)。⚠️ ⛔ 不要在這裡描述**畫的是什麼**(角色/物件) —— 那是 PASS 1 的工作,寫進來會把每一張圖都拉向同一個主體,也就是兩階段當初要修的那個缺陷。"
      ),
    negativePrompt: z
      .string()
      .min(1)
      ...
      .describe(
        ...
        "PASS 2 的**負向**提示詞 —— 明著排除掉的東西。除了老三樣(文字/浮水印/邊框/畸形)之外,出貨值特別排除**霓虹 · 過飽和 · 彩虹漸層 · 過度發光 · 雜亂細節**,那幾個字就是 owner 說的「過度花俏複雜」。⚠️ 這裡多寫一個詞的代價是**那個東西整批消失**,所以要排除的是風格不是題材(寫 `fire` 會讓所有火焰技能的圖示一起沒有火)。"
      ),
    ...
    strength: z
      ...
      .describe(
        ...
        "PASS 2 的 img2img **重畫幅度**:模型被允許把 PASS 1 的圖改掉多少。0.58(出貨值)＝保留主體的輪廓與顏色,同時真的把畫風換掉。調高 → 風格更統一,但**主體會開始被塗掉**(0.8 以上幾乎等於重畫一張新圖,也就是兩階段當初要修的缺陷回來了);調低 → 主體很安全,但風格幾乎沒套上去,看起來還是 PASS 1 的半成品。"
      ),
    ...
    pass1Steps: z
      ...
      .describe(
        ...
        "PASS 1(特徵)的取樣步數。這一階決定「看不看得出畫的是什麼」,所以它是**辨識度**的旋鈕。出貨 {{出貨值}}。往下調省時間但輪廓會糊掉;往上調到 40 以上幾乎看不出差別,只是讓全量那一批多跑幾分鐘。"
      ),
    pass1Guidance: z
      .number()
      ...
      .describe(
        ...
        "PASS 1 的 CFG(有多聽話)。出貨 {{出貨值}}。調高 → 更貼特徵描述,但構圖會變僵硬、顏色容易燒;調低 → 更自然,但常常畫出描述以外的東西(那正是「這張到底是哪一招」的來源)。"
      ),
    pass2Steps: z
      .number()
      .int()
      ...
      .describe(
        ...
        "PASS 2(風格)的取樣步數。出貨 {{出貨值}}。⚠️ 它跟 `strength` 相乘才是真正的工作量(img2img 只跑 strength 那一段),所以兩格同時調大,時間是相乘的。"
      ),
    pass2Guidance: z
      .number()
      .min(ICON_GUIDANCE_MIN)
      ...
      .describe(
        ...
        "PASS 2 的 CFG。出貨 7.0。這一格對「顏色會不會太花」最敏感:調高會把畫風推向高飽和的動漫海報(＝ owner 不要的那一種),想更收斂就往 6~7 調。"
      ),
    ...
    size: z
      ...
      .describe(
        ...
        "存檔的圖示邊長(像素,正方形)。出貨 {{出貨值}} —— 全 app 最大的使用面是登入頁跑馬燈的54 CSS px(DPR 2 = 108 裝置像素),128 已經超取樣。⚠️ 模型一律在 512 算完再縮,所以這一格**不影響產圖時間**,只影響檔案大小與放大時的銳利度。"
      ),
  })
  .strict();
