...
export const zModelDoc = z
  .object({
    ...
    /**
     * ⭐⭐ 這一份 .glb 當成特效播出時,**它自己帶著哪幾顆粒子發射器**（`vfx@1` id）。
     *
     * ── 為什麼這一格必須存在（GH#803）───────────────────────────────────────
     * ⭐ **原作的一顆 .mdx 不是一份網格,是「網格 ＋ 它自己的 PRE2 粒子」。**
     * 而 `tools/w3x-import/convert_stock_model.py` **只轉 geoset** ⇒ 出貨的 .glb
     * 是那顆模型的**一半**,⛔ 而它看起來完全正常（只是灰的）。
     *
     * 2026-08-31 直接解 MPQ 的 `.mdx` 量到（⛔ 不是推測）:
     *   `ReviveHuman` 的貼圖 5 張裡 **4 張 sat 0.000–0.066**（灰/白）,
     *   ⭐ 而 `pre2` 第 0 條的 `segment_color[0] = (1.000, 0.890, 0.459)` ⇒ **sat 0.541**。
     * ⇒ ⭐ **顏色不在 mesh 裡,它在粒子裡。** additive 把一疊灰加起來只會更亮的灰,
     *   ⛔ 沒有任何 tint 參數做得出金橘色 —— 那不是參數的事,是**缺了一半的模型**。
     *
     * ── ⭐ 為什麼它住 `model@1`,⛔ 不是住技能 ───────────────────────────────
     * 理由與 `fxLongAxis` / `fxSpawnHeight` **逐字相同**:「這顆模型由哪些零件組成」
     * 是**網格自己的性質**,⛔ 不是引用它的那支技能的選擇。同一份 glb 被兩支技能
     * 引用時,兩邊必須拿到同一顆完整的模型。
     * ⚠️ 反面(把 emitter 寫進每一支技能)＝ 第〇·四守則的第二個住處:
     *   N 支技能 × 每次美術改動 = 一次完整的重新產生鏈。
     *
     * ⚠️ **ABSENT ＝ 不生粒子 ＝ 今天的行為**（逐位元不變）⇒ 純新增,
     * ⛔ 不改任何一支出貨技能的畫面。一鍵 rollback 住
     * `content/config/vfx-tuning.json` 的 `modelFxEmitters.enabled`。
     */
    fxEmitters: z.array(z.string().min(1)).max(8).optional(),
