...
export const zConfigReplayDoc = z
  .object({
     ...
     */
    retainMaxFiles: z.number().int().min(0).max(5_000),
    /**
     ...
     * 預設值唯一的煞車。
     */
    retainMaxAgeDays: z.number().int().min(0).max(3_650),
  })
  .strict();
...
export const zConfigRangeGuideDoc = z
  .object({
    ...
    /**
     * **決策點**：hover 要不要**同時**打開螢幕頂端那條說明橫幅。
     * 出貨 false（只出範圍圈）—— 理由與代價寫在後台那一頁的說明裡。
     */
    hoverOpensBanner: z.boolean(),

    /**
     * **按下**技能鈕／技能鍵時要不要打開頂端說明橫幅。
     *
     * ⭐ owner 2026-08-22（他標為**最高優先**）：
     * 「**天生QWEREX 按按鈕施放時不要一直跳出說明 很亂**」
     * ⇒ 出貨 **false**：按下是**施放**不是**閱讀**，而橫幅蓋在畫面頂端會在
     * 每一次連招時閃好幾下。⛔ 範圍圈仍然出（那是瞄準要用的，GH#367）。
     * ⚠️ **被動技不受這一格影響** —— owner 2026-08-13「被動技的按鈕應該不能被按下」，
     * 而說明正是玩家唯一能對被動做的事，所以那一格永遠開橫幅。
     */
    pressOpensBanner: z.boolean(),
    /** 施法距離圈的顏色 —— 「我打得到多遠」。 */
