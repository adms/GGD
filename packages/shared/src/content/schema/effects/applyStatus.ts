...
z
  .object({
     ...
     *
     * ⭐ 它是**渲染側**的指令，⛔ sim 一格都不讀 —— 那是刻意的：
     * 一個會改變 sim 狀態的動畫指令會讓「畫面」與「勝負」耦在一起，
     * ⛔ 而客戶端的預測世界跑的是同一支 sim。
     *
     * ⚠️ ⭐ **`clipTimeScale: 0` 是「定格」，⛔ 不是「不播」** ——
     * 那兩件事在畫面上差很多：定格是**停在那一格**，不播是**回到 idle**。
     * ⇒ 下界刻意收得下 0。
     *
     * ⚠️ ⭐ 而剪輯**不一定存在** —— `ggd-clip-degradation@1` 量到
     * `guard` 只有 **2.2%** 的角色有、`hurt` **8.2%**
     * ⇒ ⭐ 渲染側要走那份降級契約，⛔ 不是「沒有就不播」。
     *
     * 省略 ＝ 不強制 ＝ 逐位元組同這一格出現之前。
     */
    forceClip: z
      ...
      .optional()
      .describe(
        "強制受害者播這一段動作剪輯（例 `death` / `hurt`）。" +
          "⚠️ 剪輯不存在時走降級契約 —— ⛔ 不是靜靜不播。",
      ),
    /**
     * ⭐ **M4 的另一半** —— 那一段剪輯的播放倍速。
     * ⭐ `0` ＝ **定格在那一格**（⛔ 不是「不播」）· `1` ＝ 正常 · `0.2` ＝ 慢動作。
     */
    clipTimeScale: z
      .number()
      .min(0)
      .max(4)
      .optional()
      .describe("`forceClip` 的播放倍速。⭐ 0 ＝ 定格（⛔ 不是不播）、1 ＝ 正常。"),
    /**
    ...
    /** GH#1197 魅惑：期間內被迫朝施加者走、丟掉自己的指令；解除後恢復。⛔ 不是拉扯＋繳械。 */
