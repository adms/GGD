...
export function buildHeroSourcePackage(raw: unknown, icons: readonly HeroSourceIcon[], target: HeroPackageTarget): EditorImportPackage {
  ...
  return zEditorImportPackage.parse({ schema: "ggd-editor-import@1", manifest, documents: [{ path, document: project }], compiled: [], validation: [], reports: {}, assets: icons.map(({ path, bytes }) => ({ path, bytes })) });
}
