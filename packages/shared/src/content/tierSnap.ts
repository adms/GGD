...
import { DEFAULT_MANA_TIERS, MANA_CAST_ANCHORS } from "./manaTiers";
...
/**
 * ⚠️ 耗魔五格的推導**已經搬到 `manaTiers.ts`**（`config.mana-tiers@1`，2026-08-21）——
 * 它在那裡才是一條**出貨的軸**（有 Zod、有 `resolveManaCostTier`、有後台一頁），
 * 而這一支只是報告在讀它。⛔ 這裡**不再放第二份實作**：兩份會漂移，而漂移的樣子
 * 是「後台改了耗魔級距、報告還在用舊的五格」，兩邊都看起來完全正常。
 */
export { manaTiersFromPool } from "./manaTiers";
export { MANA_CAST_ANCHORS };
