...
/**
 * ⭐ **生成點的「活動空間」門檻**（GH#398）：一個剛生出來的身體，至少要能沿某個
 * 方向直線平移**這麼多個身體半徑**。
 *
 * ⚠️ 它與上面的 `NAV_HEADROOM` 是**兩件事**，⛔ 不要合併：
 * `headroom` 問的是「**這條路**夠不夠寬」（一條通道的性質），
 * 這一格問的是「**這一個點**離不離得開」（一個落點的性質）。
 * 一張通道全部合格的圖，仍然可以把一隻殭屍生在牆與石頭之間的縫裡 ——
 * 出貨的 `arena.dota` 就是（殭屍王 dir0/dir6，只剩 **0.31** 個身體半徑）。
 *
 * ⭐ 出貨 1 ＝「至少動得了自己一個半徑」。這不是挑順眼的：量到的分佈是
 * **雙峰**且中間全空 —— 900 個出貨生成點裡 4 個是 **0.31**、其餘 896 個
 * 全部 ≥ **2.00**。1 落在那個空隙的正中央，兩邊各有 3 倍餘裕。
 *
 * ⚠️ **runtime 用的就是這個常數**（`sim/map/bounds.ts::spotHasRoom` 的預設值），
 * 後台那一格調的是**驗收標準**（守衛會不會擋下一張新圖）—— 與 `nav.headroom`
 * 完全同一個形狀（那一格在 runtime 也是一行都沒讀）。
 * ⛔ 刻意不讓後台把 runtime 的下限調成 0：那等於重新開放「把殭屍生在牆縫裡」。
 */
export const SPAWN_ROOM_BODY_RADII_SHIPPED = 1;
/** 下界 0.25 ＝ 比出貨最糟的 0.31 還鬆，也就是**擋不到任何東西** —— 那是這把尺的關機位。 */
export const SPAWN_ROOM_BODY_RADII_MIN = 0.25;
/** 上界 2 ＝ 出貨 896 個好落點的實測值本身；再高就會把每一張圖都判不合格。 */
export const SPAWN_ROOM_BODY_RADII_MAX = 2;
