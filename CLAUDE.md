...
## 🎯 第〇·三守則：**一致・簡潔・容易維護** —— ⛔ 不是追求極致的正確

owner 2026-09-14（逐字，三則）：

> 「我喜歡 **公式 一致 簡潔** 不要**複雜難懂難維護** 所以我才發明**五級距 整數靠攏 系統倍率**等規則」

> 「你會搞出這一系列問題 就是**沒掌握 一致簡潔容易維護的原則** 導致 **loop 修改 不斷追求中位數 前後規則不一致 數值零碎** 等問題」

> 「**統計是參考** 但是統計數值是零碎的 **對人類無意義 更對好玩沒幫助** 所以才要簡潔 **不是追求極致的正確**」

⭐ 這條排在**所有守則最前面**，因為後面每一條都是它的工具：
五級距（無限的數 ⇒ 5 個值）· 整數靠攏（`73/146/292` ⇒ `75/150/300`）· 系統倍率（散落的調整 ⇒ 1 格旋鈕）。

⚠️ **上面三個括號裡的例子是我（Claude）舉的，⛔ 不是 owner 原話** —— owner 原話只點名了「五級距 整數靠攏 系統倍率」這三個工具。
⭐ 這一行在 2026-09-14 真的誤導過人：一位分析者把「73→75」寫成「owner 自己的例子」，由審查者在 #1260 抓出來。
（CLAUDE.md「我的推測會變成他的需求」那一條的又一個載體：**寫在守則裡、緊貼著 owner 引言的舉例**。）

### ⭐ 目標是**好玩、人看得懂**，⛔ 不是統計上的精確

| ⛔ 追求極致正確 | ⭐ 簡潔 |
|---|---|
| 中位數 `1847.3`、幾何平均校準常數 | 固定的 `1875` —— 玩家與設計者**讀得懂** |
| 算得對、而每上架一批就漂 | 寫死的工整值，⭐ **永遠不漂** |
| 一個公式六個維度 | 查表 |

⚠️ **統計是參考**，⛔ 不是出貨值。量出來的數字拿來**決定梯子怎麼切**，⛔ 不直接當成數值出貨。

### ⛔ 四個症狀 —— 看到任何一個，停手，先問「最簡單的一致形狀是什麼」

owner 點名的四個，⭐ 2026-09-12～14 一個 session 裡四個全中：

| 症狀 | 實例 | 簡單的形狀 |
|---|---|---|
| **loop 修改** | radius 統一 ⇒ 被動跑出施法條 ⇒ 拆兩個概念；吟唱改產物 ⇒ 還原 ⇒ 再改 | 先想清楚再動 |
| **不斷追求中位數** | 逐角色中位 ⇒ 逐出身中位 ⇒ 還是漂 | ⭐ **固定數值**（owner：「以後固定數值 別再取中位數了」） |
| **前後規則不一致** | 20 階吟唱階梯與五級距**並存**；radius 預設 1／0／0 | 被取代的規則搬進 legacy |
| **數值零碎** | 耗魔 `73/146/292`；吟唱 `1.233/1.633/2.433`；AP 六維校準常數 | 五級距 ＋ 整數靠攏 |

### ⭐ 判準（修任何東西之前）

1. **這個修法讓系統變簡單還是變複雜？** ⛔ 變複雜 ⇒ 多半是錯的方向，**即使它技術上更正確**。
2. **修一個壞掉的複雜公式時，⛔ 不要再加機制去撐它** —— 先問它能不能換成更簡單的形狀。
   （2026-09-14：AP 六維校準改了吟唱就差 16%，我第一個反應是「讓校準改成推導的」—— ⛔ 那是再加一層。）
3. **一個人讀得懂這個數字嗎？** 讀不懂 ⇒ 它對好玩沒有幫助。
...
### ⛔⛔ join key **自己漂掉**的時候，跑「照 join key 同步」會**毀資料**

2026-08-27 量到（GH#635 / #764，草泥馬 `godie-h02u`）：

| 層 | 發生了什麼 |
|---|---|
| ⓪ **前提** | 編號是 JASS 對照的 **join key**（`92-02` 永遠是消化液），這條守則上面剛講過 |
| ① **key 漂了** | `h02u` 的 **W/E 兩格編號互換**了 —— 一個**單點**的資料錯誤 |
| ② **照 key 同步** | parity 同步器照「編號配對」跑，於是把 `h02u.e` 配到了 `h02v.w` |
| ③ **三層一起覆蓋** | 效果／說明／語音三層一起被寫過去 ⇒ ⭐ **消化液整支消失、兩格都叫狂草泥馬** |

⇒ ⭐ **一個單點的 key 錯誤，被同步器放大成整支技能的資料毀損。**
⚠️ 而每一層看起來都是對的：編號是 join key（✅ 守則）、同步照 join key（✅ 設計）、
覆蓋是同步的定義（✅ 語意）。⛔ 錯的是**它們的組合**在 key 有錯時沒有任何煞車。

### ⭐ 判準：同步器要先問「**key 本身可信嗎**」，⛔ 不是「兩邊一不一樣」

| 同步器該做的 | ⛔ 違反的樣子 |
|---|---|
| 跑之前先驗 **key 的唯一性與自洽**（同一支英雄的兩格不可以同號、每個號碼在來源裡查得到） | 直接拿 key 去 join |
| key 驗不過 ⇒ **停下來指名那一格**，⛔ 不是「照樣同步，反正兩邊會一致」 | 「一致」了，而兩邊一起是錯的 |
| 覆蓋前比對**被覆蓋的那一份有沒有獨立內容** | 無條件寫過去 |

⚠️ 這與第〇·四守則（值只有一個住處）是**互補**的：那一條說「⛔ 不要有兩份」，
這一條說「⭐ 當你要用一把鑰匙把兩份對起來時，**先驗那把鑰匙**」。
...
2026-08-27 量到（GH#382，貼圖去重）：

> 量尺量的是 `getLoadedTexturesCache().length`。
> ⛔ 而 `NullEngine._releaseTexture(texture) { }` **是空的** —— 清單永遠不變短
> ⇒ ⭐ 「去重生效」與「去重沒生效」**量起來一模一樣**。

⚠️ ⭐ **而校準當時是綠的** —— 因為它只驗了「應該多」那一邊
（關掉去重 ⇒ 量到 3 塊），⛔ 沒驗「應該少」那一邊（打開 ⇒ 量到 1 塊）。

### ⭐ 判準：`calibrate()` 要驗**兩個方向**

| ⛔ 不夠 | ⭐ 要有 |
|---|---|
| 「已知**有**的情境量得到」 | 「已知**有**的量得到 **且** 已知**沒有**的量不到」 |
| 「開啟時數字變了」 | 「開啟時變**大**、關閉時變**小** —— 兩邊都跑過」 |

⚠️ 一把單邊校準的尺會**在它最需要說話的時候沉默**：
它證明得了「東西在」，⛔ 證明不了「東西不在」——
⭐ 而效能/可見性這一族，缺陷通常長成**後者**（沒生效、沒送到、沒畫出來）。

⚠️ 同族前科（本文件記過的三個量尺陷阱）：canvas 背後緩衝 300×150 ·
`readPixels` 讀到上一幀 · 材質沒 ready 靜默跳過。
⭐ 四者都是「量尺在特定方向上是瞎的」，⛔ 而它們的共同症狀是**校準通過**。

### ⛔ 一份報告的**欄位表頭**會靜靜地定義「它量了什麼」—— 而讀的人不會發現

2026-08-27 量到（GH#623，變身態逐對退場）：

> v2 報告有一欄叫「退場會掉什麼」，而它的表頭逐字寫著
> 「（**今天畫面上真的存在的**）」。
> ⇒ ⭐ **它只量了六個視覺軸，從來沒讀過卡面文字。**
...
⭐ 表頭那句括號**不是註解，是判準** —— 而它藏在欄位名裡，
⚠️ 於是後面每一輪讀那張表的人（包括我）都把「🟢」讀成「可以退場」，
⛔ 而它真正的意思是「**在視覺這一個軸上**可以退場」。

### ⭐ 判準：讀一張表之前，先問「**這一欄的分母是什麼**」

| ⛔ 不要問 | ⭐ 要問 |
|---|---|
| 「這一格是綠的嗎？」 | 「這一欄**量的是哪幾個軸**？我要的那個軸在裡面嗎？」 |
| 「表上說 9 個可以」 | 「9 個**在什麼條件下**可以？那個條件寫在哪裡？」 |

⚠️ 而**產生那張表的人**要負的責任是對稱的：
⭐ **限定詞不可以只寫在表頭的括號裡** —— 它要寫在**每一個結論旁邊**，
⛔ 否則第一個把那張表複製到別處的人就會把限定詞丟掉。

⚠️ 同族前科：本文件記過的「一份被散文守著的數字活過了保存期限」、
「被 glob 灌大的統計讀起來跟真的一模一樣」。
⭐ 三者都是**同一件事**：**一個看起來已經量過的東西，量的不是你以為的那個。**

### ⛔ 一張票的 AC 指向「某份文件的某一行狀態」時，**那一行就是這張票的一部分**

2026-08-27 同一天遇到**兩次**（#763 · #759），兩次的形狀一模一樣：

| 票 | 程式做完了 | ⛔ 而它被卡著，因為 |
|---|---|---|
| **#763** 打擊音分層 | `fee9a7ea` 接線＋守衛＋突變全落地 | `combatFeedback.ts:412` 還寫著「SFX **should** pick a tier」 |
| **#759** 觸控保留帶 | `846695eb` 356 → 350 | `docs/todo/latency-visibility.md:115` 還寫著 `in-progress` |

⭐ **兩張票都被自己的一行過期散文卡著。**
⚠️ 而那一行**不會**因為程式做完了就自己更新 —— 它是第三守則的形狀：
...
| 時機 | 要做的 |
|---|---|
| **開票**寫下「某某文件說 X」當成 AC | ⭐ 把**那一行的路徑與行號**寫進 `Files / modules likely affected` |
| **收票**前 | ⭐ 把 AC 逐條**當成清單念一遍** —— 指向文件狀態的那幾條，⛔ 不會被程式的綠燈涵蓋 |

⚠️ ⛔ 不要把這種 AC 當成「文件債」順延 —— 它是**驗收條件**，
⭐ 而一張「程式做完了但 AC 沒滿」的票，下一輪讀到時看起來就是「還沒做」。

### ⛔⛔ **更正也是一個宣稱** —— 它要過同一道證據門檻

2026-08-27 量到（GH#565 / #674 / #809），⭐ **同一個假前提被連續更正了兩次，而兩次都還是假的**：
...
⭐ ②錯在哪：它證明了「檔案在」，⛔ 但沒有問「**機制夠不夠**」。
`spawnVfx.ts:51` 逐字寫著「`at:"bone"` 與 `self` 同路：**錨定單位是施法者**」，
而那 9 支的 JASS 是 `AddSpecialEffectTargetUnitBJ(attach, **GetEnumUnit()**, model)`
—— **掛在受擊者身上**。⚠️ 而票文從來沒記錄那個**第二個參數**。
⇒ 量到的母體：**316 次呼叫中 施法者 124 : 受擊者 124** —— 出貨機制**正好覆蓋一半**。

### ⭐ 判準：更正一個假前提時，⛔ 不要只推翻它，要**走完它原本要走的那條路**

| ⛔ 不夠 | ⭐ 要有 |
|---|---|
| 「那個檔其實存在」 | 「那個檔存在，**而且我照著它把事情做到底了**（或：做到底時撞到 X）」 |
| 「這件事其實做完了」 | 「⋯而我拿**出貨的那條路**跑過一次」 |

⚠️ 一個**只推翻不落地**的更正，會變成下一輪的新假前提 ——
⭐ 而且它更難發現，因為它看起來是**剛剛才查證過的**。

⚠️ 這與「我的推測會變成他的需求」是同一個病的第三個載體：
①票裡的推測 → 需求 · ②模板預設裡的推測 → 「原作就是這樣」 · ③**更正裡的推測 → 「已查證」**。
...
> 票文寫「三支稽核**只在人想到時手動跑**」。
> ⇒ 實際：三支**全部是自動閘** —— 兩支是 `packages/shared` 的普通 vitest
>   （進 `pnpm test` 與 `ship:check`），第三支逐字在 `skills:check` 的 36 支裡。

⭐ **票文把「我沒有在後台看到它」寫成了「它沒有在跑」。**
差別是**執行環境**，⛔ 不是「有沒有做」——
而真缺口在**別的地方**（runtime：後台按白名單那一刻不經過任何 ship 閘）。

⚠️ 這個假前提的代價是**最貴的一種**：它會讓下一個實作者
**再做一次已經存在的東西**，而真缺口原封不動。

### ⭐ 判準（開票與讀票時各問一次）

票文寫「現況是 **X 沒有在跑 / 沒有人用 / 是死的**」時，要問的是：

| ⛔ 不要問 | ⭐ 要問 |
|---|---|
| 「它存不存在？」 | 「**在哪一個環境**它沒有在跑？」（CI / 本機 / 出貨 build / runtime） |
| 「有沒有做？」 | 「**它跑得到的那條路**與**我看的那條路**是不是同一條？」 |

⚠️ 同一族的前科：`/__review` 與 `/__live` 在**本機**活著而**線上**沒有（GH#794）、
那 13 頁在 dev 載入而**正式 build 根本不載入**（同一張票）。
⭐ 三個都是「東西在，但**不在你看的那個環境**」。

### ⭐ 而全票掃描要問的也是這一題

同一條 lane 的量測（值得寫下來）：#300–#529 這 17 張在 **24–48 小時內被逐張讀碼稽核過三次**，
第三次的產出是 **0 張可關**。
⇒ ⭐ **「做完了沒」這一軸的邊際產出已經趨近零。**
下一次掃描直接問：「票文主張的**現況**今天**在哪一個環境**下還成立？」
...
### ⛔⛔ 第三個載體：推測寫進**模板家族預設**，會變成「原作就是這樣」

owner 2026-08-26（他發現我把已經查清楚的事實又推翻回錯的）：

> 「你已經知道是**單個大型光束並非間距排列**，是哪裡走歪讓你又把約束勝利之劍等光束砲家族
>  又變成間距排列？請你**反省根因並修正**」

⭐ **他是對的，而且根因是三段的**（2026-08-26 逐行查證）：
...
| ① **誤讀** | 把**沿線傷害取樣的迴圈**讀成「視覺由 N 具組成」。⭐ 真相：光束是 `CreateNUnitsAtLoc(**1**, …)` ＋ `SetUnitScalePercent` 放大（⚠️ **2026-08-28 更正**：`200,200,400` 在 `Trig_SunFire_Actions`（j:26791），⛔ **不是龜派** —— 龜派 A03S 自己那兩個呼叫（j:31908/31910）三軸都是同一個運算式 `250+15L`／`350+15L`，是**等向**的）。⚠️ 而 20-03 的那條線 GGD **早就表達了** —— `godie-e002.e` 的 `effects[0]` 就是 `damageLine{length:14,width:2}` ⇒ 再映射成 `count/spacing` 等於**把同一條傷害線畫成第二份視覺** |
...
⇒ ⭐ **這是同一個病的第三個載體**：
票裡的推測會變成**需求**（上一節）· 模板預設裡的推測會變成**「原作的結構」** ——
而且它更難發現，因為**逐支覆寫會讓被檢查過的那一支看起來是對的**。

### ⭐ 兩條可以當場檢查的規矩

4. ⭐ **一個參數如果值得逐支覆寫，就要先問「家族預設是誰的量值？」**
   ⛔ 「我幫這一支修好了」不等於「這一格是對的」——
   逐支覆寫**只證明了那一支**，而預設仍在服務其他每一支。
   ⇒ 判準：**寫下逐支覆寫的同一刻**，去看家族預設那一格引用得到什麼。引用不到 ⇒ 那是下一個要修的東西。
5. ⭐ **模板 `params[*].default` 的每一格都要引用得到出處**（`j:行號` / census 量值 / owner 原話 / `inert` 理由）。
   ⛔ 沒有出處的預設 ＝ 一個**會被後來的自己當成證據**的推測。
   閘：`packages/shared/src/content/templateDefaultsHaveOrigin.test.ts`（棘輪只能變短）。
...
### ⛔⛔ 而上面整段的**根方法論**是 owner 一句話給的（2026-08-26，逐字）

> 「你應該做的事情是 **翻譯 JASS to 編輯器JSON**，如果 **JSON 沒支援的標籤或邏輯則去實作**才對阿」

⭐ **翻譯 ≠ 近似。** 我做錯的形狀是：讀 JASS → 推測一個「家族形狀」→ **挑現有的模板參數去逼近它**。
而正確的形狀只有兩步：

| 步 | 做什麼 |
|---:|---|
| **1** | 逐行**翻譯**：JASS 的每一個動詞對到 JSON 的一個標籤（`CreateNUnitsAtLoc` → `spawnModelFx`、`SetUnitScalePercent` → `scale`／`scaleAxis`、`SetUnitTimeScalePercent` → `clipTimeScale`、`GetUnitsInRange+ForGroup` → `damageArea`／`damageLine`…） |
| **2** | 翻不過去就**去實作那個標籤**（第〇·五守則的下半：盤點 → 按擋住幾支排序 → 做機制） |

⛔ **禁止的第三條路**：「用現有參數湊一個看起來像的」。
它的代價這一次量到了 —— `SetUnitScalePercent(200,200,400)`（j:26791 SunFire）表達不了（`scale` 是
`root.scaling.setAll`），我就改用 `count:6 × spacing:2` 去「拼」出粗長的感覺
⇒ 得到一排小光束，而且那個編出來的 6 變成了**七支技能的家族預設**。
⭐ 正解是 2026-08-26 做的那件事：**schema 長出 `scaleAxis`、rig 從 `setAll` 改成三軸 `set`** —— 一次機制，
全家族（JASS 35 個縮放裡 **13 個 37% 是非等向**）都翻得過去了。

⚠️ **判準（當場可檢查）**：寫下一個模板參數的值之前問
「**這個值是從 JASS 的哪一個呼叫翻過來的？**」
· 指得到某一行的某一個參數 ⇒ ⭐ 那是翻譯
· 只能說「這樣看起來比較像」 ⇒ ⛔ 那是近似，停手，去問「缺的標籤是什麼」

⚠️ 順帶一個這次犯了兩遍的算術錯：**WC3 技能等級從 1 起算**（`GetUnitAbilityLevelSwapped`）
⇒ `250 + 15×lvl` 的**最低級是 265%**，⛔ 不是 250%。把**公式的常數項**當成值，
在 20-03（2.5）與 59-04（1.2）上各錯一次。
⚠️ 這一段會在**每一支「沿線傷害」技能**上重複發生，所以它是規矩不是註腳。

⚠️ **順帶兩個這一次量到的事實**（⛔ 不要再憑印象猜）：
· WC3 是 **Z-up** ⇒ `SetUnitScalePercent(200,200,400)`（j:26791）是「柱子變高 2×」，
  而 GGD 把模型橫放（`fxLongAxis`）之後，那一軸才變成**光束的長度**。
· 全 war3map.j 的 35 個 `SetUnitScalePercent` 裡 **13 個（37%）是非等向**
  （⭐ 2026-08-28 複驗過，這個數字是對的 —— ⚠️ **但要用括號配對切參數才數得對**：
  35 個裡有 19 個的三軸是**運算式**（`( 250.00 + I2R(...) )`），一個只吃數字字面值的
  正則會只解析到 16 個而數出「4 個非等向」。⭐ 那一天 CLAUDE.md、一條查證 lane、
  與我自己**各給了一個不同的錯數字**，只有完整解析定案 —— 同族前科：「一個被 glob
  灌大的統計，讀起來跟真的一模一樣」），
...
## 🎨 第一·四守則：資產優先序 —— **有就用**，沒有才 CC0/CC-BY，再沒有才自己生成

owner 2026-08-19（兩則，逐字）：
...
| 階 | 用什麼 | 什麼時候往下一階走 |
|---:|---|---|
| **1** | ⭐ **原作資產**（w3x / MPQ 抽出來的 mdx、貼圖、音效） | 原作**沒有**這個東西 |
| **2** | 外部 **CC0 / CC-BY** | 找不到合用的 |
| **3** | ⭐ **自己生成**（參數化原型 · 程序化 · AI 產） | —— |

⚠️ ⭐ **這條推翻了一個舊裁決**：在此之前有一族特效刻意「用參數化原型，⛔ 不出貨模型」，
而那個理由是**版權**。⇒ 版權解禁之後那個理由消失了，
⛔ 而**理由消失的決定不會自己翻過來** —— 它會以「這是設計選擇」的樣子活下去。

### ⭐ 判準：一個「我們自己畫的」資產，要說得出**它上面兩階為什麼不行**

| ⛔ 不夠 | ⭐ 要有 |
|---|---|
| 「參數化原型比較彈性」 | 「原作**沒有**這一顆（census 查過）⇒ 才自己生成」 |
| 「這樣不用管授權」 | ⛔ 這個理由**已經作廢**（owner 2026-08-19） |

⚠️ 量到的落差（GH#547，#529 的普查）：技能綁**通用原型** `fx.prim.*` 有
**342/384（89%）**，而綁**原作** `fx.w3x.*` 的只有 **2 支**。
⇒ ⭐ 那 89% 裡有多少是「原作真的沒有」、多少是「當年不敢用」，⛔ 今天分不出來 ——
而這條守則就是要讓下一次分得出來。

---

### 🧩 第一·四之零：**匯入模型一定要跑一次入庫檢查** —— 一支指令，⛔ 不是「要記得看」

owner 2026-09-10（兩則，逐字）：

> 「類似這種錯誤 請你**寫成 script 把每個模型都掃過合併並檢查沒問題**吧
>  也寫到守則裡 **匯入模型都要跑一次檢查**」

> 「**後台設定跟編輯器都要自動帶入這個檢查與修正 script**」

```bash
python3 tools/w3x-import/model_intake.py <路徑…>            # 只檢查
python3 tools/w3x-import/model_intake.py <路徑…> --merge    # 順便合併（會先留底）
python3 tools/w3x-import/model_intake.py --all --check      # 閘：有問題回非零
```

⭐ **後台與編輯器兩條匯入路徑自動帶它**（⛔ 不必人記得跑）：
`packages/shared/src/content/modelUpload/normalize.ts` 掛在
`prepareUploadedHeroModel()`（編輯器上傳）與 `ModelVersions.prepare()`（後台下拉選單）上。

### 它問五件事 —— ⛔ 每一件都是 2026-09-10 真的踩到的

| # | 問題 | 量到的 |
|---|---|---|
| ① | **嚴格 glTF 驗證** | 出貨 506 顆裡 **160 顆（32%）**過不了，錯誤集中在 `ACCESSOR_MIN_MISMATCH` / `MAX_MISMATCH` / `ELEMENT_OUT_OF_BOUND` / `VECTOR3_NON_UNIT` |
| ② | **可合併的 draw call** | `imported.doraemon-cat` **22 個 draw 而只有 3 種畫法**；`ou99.472112` **21 個而只有 1 種** |
| ③ | **零長度動作片段** | WC3 modeler 把署名塞進 sequence（「未经允许禁止分享与使用」）⇒ `inspectModelUpload` 擋「動作長度必須大於零」⇒ 整顆註冊不進去（41 顆中 6 顆） |
| ④ | **貼圖整組掉成佔位圖** | BLP 住在子目錄時查不到 ⇒ 靜默退回 8×8（41 顆中 11 顆） |
| ⑤ | **英雄預算** | 三角面／draw call／貼圖邊長，對照 `HERO_MODEL_BUDGET` |

⚠️⚠️ ⭐ **①②③④ 的共同形狀是「壞掉跟正常長得一模一樣」**：
轉檔回 `status: ok`、`missing_textures` 是空的、three.js 與 Babylon 都照畫不誤 ——
⛔ **只有驗證器與這支檢查問得出來**。這正是本文件整章在防的東西，
而它在模型這條路上**一次出現四個載體**。

⭐ **判準：一顆模型「畫得出來」⛔ 不等於「檔案是對的」。**
⛔ 不要用截圖當成入庫證據 —— 截圖只答得了⑤的一半（畫面上有沒有東西），
①②③④它一個都看不出來。

⚠️ **自動修正只做兩件**（合併畫法相同的網格、丟掉零長度片段）——
⛔ 減面／縮圖／貼圖圖集**不做**，因為它們**會改變畫面**：那些走
`tools/model-budget/optimize.ts` 的離線批次，寫到獨立樹、由人審過再採用。
...
owner 2026-09-08 又逐字給了**逐類的歸屬表**（⭐ 這一份比上面那句細，以它為準）：

| 資料 | 存放位置 |
|---|---|
| 原始模型、動畫、**這批大型解析 JSON** | ⭐ **S3** |
| 解析／轉換程式、**英雄與技能設定 JSON**、版本清單、**SHA-256**、文件 | ⭐ **Git** |

⚠️ ⭐ **「JSON」在這張表裡出現兩次，而它們去不同的地方** —— 這是最容易搞錯的一格：

| 是哪一種 JSON | 去哪 | 為什麼 |
|---|---|---|
| **大型解析產物**（w3x 匯出、模型普查、貼圖清冊⋯例：`MODEL_USAGE.json` 105,339 行 · `OBJECTS.json` 96,429 行） | **S3** | 它們是**傾印**：沒有人讀 diff，只有雜湊驗得了 |
| **英雄與技能設定**（`content/champions/*.json` · `content/abilities/*.json` · `content/config/*.json`） | **Git** | 它們是**設計**：每一行都要看得到誰改了什麼 |

⇒ ⭐ 判準沒變，只是被講清楚了：**「一個位元組如果只能靠雜湊驗、不能靠 diff 讀，它就不屬於 git」** ——
⛔ 分界不是副檔名，⛔ 也不是目錄。

| | |
|---|---|
| ⭐ **素材本體** | **S3 `ggd-390630837668-ap-east-2-an`**，⛔ **不進 git** |
| ⭐ **進 git 的** | `manifest.json`（每份檔案的 **SHA-256**）· 可重跑腳本 · 配方／文件 · ⭐ **版本清單** |
...
owner 2026-08-24（逐字，**說了兩次** —— 第二次是因為我第一版的反省不夠深）：
...
#### 五層根因（每一層都要有自己的防線，⛔ 只修最上層必然復發）

| 層 | 問題 | 證據 |
|---|---|---|
| **① 缺陷本身** | UV 退化成全 (0,0)＋漸層形狀住在 alpha 且兩緣是 0 ⇒ 全透明 | 出貨原始碼 32 條弧 / **0 亮像素**；拿掉貼圖 8,947 / 255 |
| **② 為什麼守衛沒抓到** | 四條守衛全在**中間節點**（事件有送/白名單/頂點在/在抖）。⭐ 根因：測試環境（NullEngine）**畫不出像素** ⇒ 我寫的是「**方便驗的東西**」，⛔ 不是「**驗收標準**」 | 四條全綠而玩家零像素 |
| **③ 為什麼我回報「做完」** | ⭐⭐ **「做完」這個詞沒有證據標準。** 我讓它意謂「我的工作鏈完成」（commit＋綠燈），⛔ 不是「使用者看得到的結果被證明」。v0.26.0 的 note 寫了「電弧回歸」——那句話**當時就是假的** | owner 得自己開遊戲才發現 ＝ 人肉測試機 |
| **④ 為什麼第一版的閘防不了 100%** | 見下面「四個洞」 | 每一個洞都有已發生的實例 |
| **⑤ 為什麼會有②③** | 沒有任何東西**區分**「鏈路接通」與「玩家可見」這兩種宣稱 —— 回報、release note、關票註解裡它們長得一模一樣 | 本文件的「我的推測會變成他的需求」是同一個病：**兩種東西長得一樣就會被混用** |

#### 第一版閘（visual-proof.sh v1）的四個洞 —— ⭐ 誠實列出來，逐洞一個防線

| 洞 | 為什麼防不住 | 防線（都已落地） |
|---|---|---|
| **a. 路徑柵欄** | sim/content 側造成的不可見（vfxKey 指到零像素 glb、事件沒發、alpha 寫 0）**不會碰** `apps/client/src/{vfx,render}/` ⇒ 閘綠 | ⭐ **出貨態掃描** `vfxDocsBirthVisibility.test.ts`：不看 diff，掃**全部**出貨 vfx 文件的靜態可判不變量（見下） |
| **b. 標記是自我宣告** | 閘 grep 的是 `@visual-proof` 這個字，⛔ 不是斷言內容 ⇒ 一條空殼測試貼上標記就過（第三守則：註解會說謊） | visual-proof.sh v2：帶標記的測試**必須含可見性斷言詞彙**（readPixels / opacityTexture / UV / alpha / emissive…），否則閘紅並指名該檔 |
| **c. diff 閘看不到已出貨的壞** | 閃電壞了幾週 —— 沒有 diff 碰它就永遠不紅 | 同 a：掃描驗的是**現在出貨的狀態**，⛔ 不是這次改了什麼 |
| **d. 量尺自己會說謊** | 我同一輪犯兩個量測錯（canvas 背後緩衝 300×150；readPixels 讀到上一幀），兩次都得到**相反**結論 | ⭐ **校準寫成程式**：audition 的像素計數器要先量到一個**已知亮**的 control（`calibrate()`），量不到 ⇒ 這台量尺的一切結論**作廢**。掃描測試也內建 sentinel（自造一份必不可見的文件，斷言檢查器抓得到它） |

#### ⭐ 關鍵洞見：這一次的兩個根因都是**靜態可判**的

「畫不出像素」聽起來要 GPU 才驗得到 —— ⛔ 錯。UV 全同一點、additive 疊全黑、出生 alpha 0、
遮罩兩緣取樣 0：**四種都不用渲染就判得出來**。⇒ headless 測試環境不是藉口，
是「我沒有把驗收標準翻成可判的不變量」。

#### 用詞紀律（⭐ 這是人的那一層，⛔ 腳本管不到，所以寫成規定）

對**玩家看得到的功能**，在終端證據存在之前：
- ✅ 只准說：「**鏈路已接上，⛔ 未驗收**」
- ⛔ 不准說：「做完」「已修」「已上線」
- release note 與關票註解裡的「已修」**必須帶證據指標**（`@visual-proof` 測試名或
  `docs/_reports/*_visual-proof_*.md` 路徑）—— 引用不到就不是「已修」。
- ⭐ 這與第一守則的「出貨數值要引用得到 owner 的原話」**同構**：
  宣稱要有出處，出處引用不到 ⇒ 那個宣稱是我編的。

#### 防線總表（哪一支管哪一段）

| 防線 | 管什麼 | 管不到什麼（誠實） |
|---|---|---|
| `scripts/visual-proof.sh`（v2） | 改到畫面層而沒有終端證據 ⇒ 紅；空殼標記 ⇒ 紅 | ⛔ 已出貨的壞（那是掃描的事）；⛔ 「證據品質」（一份亂寫的報告它分不出來） |
| `vfxDocsBirthVisibility.test.ts` | **全部出貨 vfx 文件**的靜態不可見（alpha 0 / additive 全黑 / 尺寸 0） | ⛔ 動態才發生的不可見（被遮擋、鏡頭外、AdaptiveQuality 砍掉）—— 那要 audition |
| `arcGlowRenders.test.ts` | Arc 家族的三個具體病（opacityTexture / 退化 UV / 形狀在 alpha） | 只管這一族 |
| audition + `calibrate()` | 真渲染的 A/B 像素證據；量尺先自證 | ⛔ 真比賽場景的品質階梯／遮擋差異 |
| 用詞紀律 | 「做完」不再免費 | ⛔ 靠自律 —— 所以上面每一層都在，它只是最後一道 |

⭐ **「百分百」的誠實答案**：單一一支閘做不到，**五層疊起來**讓同一個缺陷要同時穿過
五個彼此獨立的洞才能再發生 —— 而這一次它只需要穿過零個。

#### 🧑‍⚖️ 非結構化資產的驗收流水線 —— 機器篩選 → 有邏輯地送 HITL

owner 2026-08-24（逐字，⭐ 這是架構性的補充）：

> 「所有關於**視覺、特效、音效等非結構化資料驗收**，你應該要特別對應的自動化流程，
>  並且把**風險高的、辨識能力差的**額外安插**人類驗收**的步驟，
>  但**避免全部都給人類驗收的極端**，應該還是要**有邏輯的篩選放到 HITL**，
>  並且考慮**批次於一頁網頁瀏覽打勾標記通過與否**的方式，幫助人類快速審查」

⭐ 核心原則：**兩個極端都是錯的** —— 全機器（閃電那次：機器根本不會問「像不像閃電」）
與全人工（owner 變成人肉測試機）。正解是一條**分層漏斗**：

| Tier | 誰驗 | 驗什麼 | 結果 |
|---|---|---|---|
| **0 硬性不變量** | 機器（build/閘） | 靜態可判的「必不可見/必無聲」：alpha 0 · additive 全黑 · 退化 UV · 零像素 glb · 零長度音檔 · 斷掉的引用 | **自動擋下**，⛔ 不進人類佇列 |
| **1 可量測界線** | 機器（閘） | fade ≤0.5s · 粒子/VRAM 預算 · 音量範圍 · 時長 | 界內自動過；超界擋下或降級 |
| **1.5 初審** | ⭐ **Claude 看圖/聽聲** | 接觸表（contact sheet）逐格看：明顯空白/全黑/破圖 → 自動 fail；明顯正常 → 降低優先序 | 縮小人類佇列，⛔ **不得**當高風險項的終審 |
| **2 HITL** | **人類（批次審查頁）** | 機器辨識能力差的語意題：「像不像閃電」「這聲音配這技能對嗎」「戰鬥中讀得出來嗎」 | 一頁網頁、逐格 ✅/❌/備註、鍵盤快捷 |

**什麼東西進 HITL（邏輯篩選，⛔ 不是全部）：**
新資產／內容 hash 變了（舊核准自動過期）· 首次接線（技能第一次引用這顆 key）·
機器信心低（如 sfx 建議表的 medium 檔）· **風險加權**（被多少上架技能引用 × 施放頻率；
曾被 owner 抱怨過的家族置頂）· 從未被人審過的。

**⭐ 三個把人力變成一次性的自動化乘數：**
1. **感知基準線**：人類核准一次 ⇒ 機器存下決定性種子的參考影格 ⇒ 之後每一版重渲染比對
   （perceptual hash），漂移超閾值才**重新**進佇列 —— **人審一次，機器守永遠**。
2. **A/B 並排原作**：有 w3x 來源的，審查頁左右並排「原作 vs GGD」—— 機器判不了相似度，
   ⛔ 但機器可以把並排擺好，這是加速人審最便宜的一招。
3. **接觸表進 release note**：每一版自動渲染「引用最多的前 20 個特效」成一張圖附在 note ——
   owner 十秒掃完，⛔ 不必進遊戲。

**⭐ 功能級的變體：「一頁批次後台驗收」（owner 2026-08-24 逐字定義）**

> 「[一頁批次後台驗收] 代表**先上線成果**，但是在**後台可以一鍵否決還原**，
>  **追加原因的HITL**，但**預設是直接上線**」

⇒ 與資產驗收同族但層級不同：審的是**一批功能成果**（一批技能、一個系列的演出）。
三條不可省的性質：①**先上線**（這頁是事後否決權，⛔ 不是事前審批門）
②否決＝**翻那一批登記的 rollback 開關**（⛔ 不是 revert commit）③否決**必填原因**（進帳本）。
⭐ 閘：一批成果要登記進驗收頁，**必須寫得出自己的 rollback 開關**（config id＋欄位名）——
寫不出來＝違反「留後台開關」常設指令＝不准登記。（機制票 GH#669）

**落地**：`tools/review/`（triage 引擎 + 核准帳本 `docs/_review/approvals.json`，hash 過期制）·
`asset-review.html`（批次審查頁，dev-only，verdict 經 vite middleware 寫回帳本）·
`review:check`（pending 數量進 ship:check 的**警示行**；硬擋是一格可調，⛔ 預設不擋 ——
部署不可以被「人不在」卡死）。
...
### 🟢 一條綠燈有**四種**假的來源 —— 2026-08-27 一天之內四種各中一次

⚠️ 前面七種失敗形態問的是「**功能**壞在哪」。這一節問的是**上一層**：
⭐ **這條綠燈本身是真的嗎？** 四種都不是誰粗心，⛔ 四種都有結構性的成因。

| # | 形狀 | 那一天的實例 | 為什麼看不出來 |
|---|---|---|---|
| ⑨ | ⭐ **一個永遠不會綠的閘** | `build_vfx_census.py` 第一行 `assert len(abilities) >= 600` —— 內容合法縮到 **421** 之後，它在**每一次正確的 checkout 上**都失敗 | ⚠️ 訊息叫人「**去查 checkout**」⇒ ⭐ 錯誤訊息**指著錯方向**，於是每一個人都以為是自己的環境壞了，⛔ 而不是那條閘壞了 |
| ⑩ | ⭐ **守衛是靠缺陷才綠的** | `facingLock.test.ts` 的夾具 `moveSpeedMult:0.02` 正好鑽過 `walkEps²` ⇒ 站定規則對它靜默關閉；⭐ 而夾具做的事逐字是「背對敵人後退還揮劍」——**正是站定規則要擋的東西** | ⭐ **補起洞它就紅，而那不是回歸，是前提消失。** ⚠️ 而紅的當下**看起來就是回歸** ⇒ 上一輪因此把預設翻錯邊 |
| ⑪ | ⭐ **兩條對的守衛，組合是空的** | `liveAuth` 證明 `fetch()` 會帶 token · `reviewAdminGate` 證明伺服器會擋沒 token 的 —— ⭐ 而 `<img src>` 是**瀏覽器的圖片載入**，⛔ 不是 `fetch()` ⇒ **兩條都管不到它** ⇒ 每一張圖 401 | ⚠️ 清單走 `fetch()`（是好的）⇒ 頁面照樣寫著「N 張連續圖片」⇒ ⭐ **壞掉跟正常長得一模一樣** |
| ⑫ | ⭐ **只驗名詞不驗關係的「反方向」** | 隔離區的掃描問「每一個**被宣告**的檔鎖得對嗎」，⛔ 問不出「每一個**被鎖**的檔有人宣告嗎」⇒ 2 份**永久唯讀的孤兒**（沒有 `genrun` 解得開、也沒有產生器會重生成） | ⭐ 它的迴圈是 `for (const [f, owners] of claimants)` —— **沒有人宣告的檔永遠不會進 `claimants`** ⇒ ⛔ **結構上失明** |

#### ⭐ 四條可以當場檢查的問法

1. **⑨**：這條閘**上一次綠**是什麼時候？答不出來 ⇒ 去跑它。
   ⚠️ 一個從來沒人看它綠過的閘，與一個不存在的閘**沒有差別**。
2. **⑩**：這條守衛的夾具，**在出貨的世界裡真的會發生嗎**？
   ⚠️ 夾具用了一個**極端值**（0.02 倍速、0 血、1 個 spawn）⇒ ⭐ 先問「那個值是不是正好落在某個門檻的另一邊」。
3. **⑪**：⭐ **我這兩條守衛，能不能同時是綠的而功能是死的？**
   ⚠️ 兩條各自驗一半的路，⛔ 沒有人驗**接縫**。判準：畫出那條路的**每一段**，指出哪一段沒有守衛站著。
4. **⑫**：這條掃描**從哪一頭走**？⭐ 從「宣告」走 ⇒ 一定漏掉「有實體而無宣告」的；從「實體」走 ⇒ 一定漏掉「有宣告而無實體」的。
   ⇒ ⭐ **兩頭都要走**，⛔ 一頭不算。

⚠️ ⭐ **這四條的共同解法不是更用力地檢查，是「讓閘自己回答一個會變的問題」**：
⑨ 把常數換成從內容推導的分母 · ⑩ 夾具改成出貨真的會發生的那一種 ·
⑪ 掃**出貨原始碼**找繞過接縫的寫法 · ⑫ 反方向再掃一次。
...
### 📋 守則犯錯要**記成一張表** —— 統計頻率與成因，⛔ 不是寫一句反省

owner 2026-08-27（逐字）：

> 「你要把**每次開發守則上規則犯的錯記成一張表 守則犯錯.md**
>  用來**統計每次犯錯的頻率及原因**，日後可以用來**改進反思**，請你記到開發守則」

```bash
bash scripts/rule-slip.sh <守則代號> <成因代號> "一句話：我做了什麼"
bash scripts/rule-slip.sh --check     # 閘：統計區過期／代號打錯 ⇒ 非零
```

⭐ **兩欄，⛔ 不是一句話**：**守則代號**回答「犯的是哪一條」（頻率），
**成因代號**回答「**為什麼**會犯」——⭐ 後者才是改進的抓手。
兩欄都是**封閉詞彙表**（`tools/rule-slip/ledger.py`）：打錯字的代號會讓統計**靜默分裂**成兩列，
而一份會靜默分裂的統計**比沒有統計更糟**（它看起來是對的）。

⚠️ **什麼時候記**：撞到的當下，⛔ 不是「等一下整理」。判準是
**「這件事 CLAUDE.md 已經寫過，而我還是做了」** —— 包含：
別人（守衛／lane／owner）指出來的、我自己中途發現的、以及**同一天重複踩的**。
⛔ 修好了也要記 —— 這張表統計的是**犯錯**，不是**未修的缺陷**。

⭐ **第一次跑就有訊號**（2026-08-27 首日 10 筆）：
最常見的成因是 **`沒有閘`（3 次）** —— 也就是「這條規則當時只是散文」。
⇒ 這張表本身就是元規則「**判準 0/4 全破，只有閘有用**」的量化版：
成因欄若持續由 `沒有閘` 領先，代表**該補的是閘，⛔ 不是更用力地記得**。

⚠️ 這份帳本與 `docs/_daily/*.md`（owner 說了什麼）是**兩張不同的表**：
那一張記**他的輸入**，這一張記**我的失誤**。⛔ 不要合併。
...
### 🎫 開票要帶五件：優先級 tag · 類型 tag · 驗收標準 · 思考策略 · 解決模板 —— 漏寫**當場**有警告

owner 2026-08-24 第 35 條（逐字，同日兩次、第二次補上類型 tag 與「標題及內文」）：
...
| 件 | 是什麼 | 為什麼 owner 要它 |
|---|---|---|
| **優先級 tag** ⭐**標題** | `[緊急]`／`[重要]`／`[優先]`，都不是也要明說 **`[一般]`**（⛔ 不是省略） | 排序是 owner 的權力 —— 他掃的是**標題列表**，tag 不在標題等於沒有 |
| **類型 tag** ⭐**標題** | `[breaking change]`／`[fix]`／`[improve]`／`[feature]` 等開發常用（也收 refactor/perf/docs/test/chore/bug/infra） | 一眼分「壞了要修」與「新東西」；`[breaking change]` 是部署節奏的訊號（⛔ 不可 `--content-only`） |
| **驗收標準** | 「做完」的可檢查條件（`## 驗收` 或 acceptance criteria 一節） | 天譴那次的教訓：「做完」這個詞沒有證據標準就會變成謊話 |
...
| 層 | 什麼時候響 | 指令 |
|---|---|---|
| **開票當下** | `gh issue create` 缺任何一件 ⇒ hook **警告**（⛔ 不擋 —— 一張缺欄的票仍比「算了不開」好） | `scripts/preserve-before-overwrite.py` 的 🎫 段 |
| **事後回補** | 驗一批既有票 / 驗草稿 | `bash scripts/ticket-lint.sh <票號…>` · `--recent [天]` · `--body-file <草稿>`（缺件 exit 1 逐張列出） |

⚠️ gh 連不上時 lint **明說「沒驗到」再跳過** —— 安靜的跳過與全過長得一樣（fail-open 沒錯，靜默才是缺陷）。
...
### 🚫 產生器的產物不准手改 —— 這條**有 hook 會擋**，不是散文

owner 2026-08-24（在同一個錯第 N 次發生之後）：

> 「你已經犯過**數十次**一樣的錯，請你一定要**寫成script擋住先檢查**，並且寫到開發守則」

錯的形狀：直接改產生器的產物 ⇒ 下一次 `skills:sync` 把它打回來，而那個「又紅了」
看起來像**新的**錯 —— 2026-08-23 一晚在 `godie-e002.r` 上中兩次（52 行、44 行各一次）、
另一次 49 個檔。⭐ 而且它會造成**無限迴圈**：`ship:check` 第一件事就是 `skills:sync`
⇒ 每跑一次閘就把修好的東西刪一次 ⇒ 再跑幾次都不會綠。
...
### 🔐 一份資料**兩個寫入端**時：分署不同資料夾，各給一支存取腳本

owner 2026-08-27（逐字）：

> 「請**同步到線上**，並且**線上批核的結果也同步到本機端**，為避免**讀寫混淆**，
>  請將**批核材料跟批核結果分署不同資料夾**，用**特定存取script的特定存取權限**
>  來管理**避免錯改**」

⚠️ 這條與第〇·四守則（值只有一個住處）是**互補**的，⛔ 不是同一條：
第〇·四說「同一個**事實**不可以有兩份」，這一條說
「**兩個不同的事實**如果**寫入端不同**，它們就不可以住同一個檔」。

### 判準：這個檔有幾個**寫入端**？

| 住在一起 | 誰寫 | 下場 |
|---|---|---|
| `feature-verdicts.json` 裡的 `rollback`（登記） | **我**（`review:register`） | ⛔ 登記那一次會**讀出 owner 的裁決欄位再寫回去** |
| 同一個檔裡的 `verdict` / `reason`（裁決） | **owner**（批核頁 / 線上） | ⇒ 任一邊的 bug 或一次併發，吃掉另一邊 |

⭐ **兩個寫入端 ＝ 兩個資料夾。** 落地形狀（`tools/review/stores.mjs` 是範本）：

| | |
|---|---|
| **① 分署** | 各自一個目錄、各自一份 schema，**欄位集合刻意不相交** |
| **② 各一個寫入端** | 兩支函式各自 `filter` 掉不屬於自己的鍵 ⇒ ⭐ 寫錯邊是 **schema 擋**，⛔ 不是紀律 |
| **③ 權限** | 「我寫的」平時 **444**（寫入端自解鎖→寫→重鎖）；「他寫的」保持可寫 |
| **④ 存取腳本** | 一支 `scripts/<x>-access.sh`，含 `status` / `guard` / `lock` / `unlock` |
| **⑤ 閘** | `guard` 逐條驗**關係**（欄位不相交 · 權限 · 兩邊都在），⛔ 不是「目錄在不在」 |

⚠️ **多來源要各自一個檔，⛔ 不是同一個檔名**：本機寫 `local.json`、線上寫 `live.json`。
⭐ 同名 ＝ 同步變成一次**盲目覆蓋**（誰後 rsync 誰贏）。
合併（同一 key 取時間戳新的）是**讀的時候算的**，⛔ 不落地成第三個檔。

⭐ **同步的方向是不對稱的，而那個不對稱就是安全機制**：
材料（我寫的）**走 git** 上線；結果（他按的）用一支只搬**一個檔**的腳本回流。
⛔ 不要 rsync 整棵樹 —— 那正是「讀寫混淆」。
...
6. **不要用 python 盲插 TS。** 用 `Edit`，而且先讀到真的錨點。
   ...
   ⛔⛔ **而真正貴的不是插錯，是「沒插到而以為插到了」**（2026-08-31，一天**三次**）：

   ```bash
   # ⛔ 這一行在字串對不上時：檔案不動、印出「✓ 改好了」、離開碼 0
   python3 -c "s=p.read_text(); p.write_text(s.replace(old,new,1)); print('✓')"
   ```

   ⭐ 三次裡有**兩次**發生在**突變驗證**上 ⇒ 我讀到的「綠」是
   「**那一行根本沒被改壞**」，⛔ 不是「守衛涵蓋不足」——
   ⚠️ 而那正是本文件整章在防的東西（「一條綠燈有四種假的來源」）。

   ⇒ ⭐ **閘**（⛔ 不是「要記得寫 assert」）：

   ```bash
   python3 scripts/edit-or-die.py <檔> --old-file <舊> --new-file <新>
   python3 scripts/edit-or-die.py <檔> --line 119 --new-file <新>      # 整行取代
   ```

   對不上 ⇒ **exit 2 且檔案一個位元組都不動**，並**指出最接近的幾行**
   （⚠️ 對不上八成是**內部**縮排或全形字 —— 前導空白不會擋住子字串比對，
   我第一版的測試夾具就用錯了這一點）。
   守衛 `packages/shared/src/ops/editOrDie.test.ts`（真的把腳本跑起來，⛔ 不是掃字串）。

   ⚠️ **讀離開碼要小心**：`… | head -3; echo "EXIT=$?"` 的 `$?` 是 **head** 的
   （本文件上面那三種變形之一）—— 寫 `… > log 2>&1; echo "EXIT=$?"`。
...
### ⏲️ 第五條：**任何工作流／長指令跑超過 5 分鐘 = 去看它是不是掛了**

owner 2026-08-24（逐字，講了三次，第三次收緊到 5 分鐘）：

> 「[重要] 如果工作流跑超過**5分鐘**，你應該要去看是不會**陷入loop**了，
>  並記錄**如何改善、斷點回報或拆分任務**」

⚠️ 這條是被踩出來的（2026-08-23 一晚兩次）：`packages/shared` 的 vitest 在併行
負載下 **worker 卡死（0% CPU）**，而我等了 11 分鐘才去看；同一晚我自己在
「修紅 → 5 分鐘閘 → mid-run 改動造出新紅」的迴圈裡轉了一小時。
...
⭐ 實際量到（把**兩支**正規化器都歸類正確之後）：**492 份裡 105 份真的有作者，387 份沒有。**

⚠️⚠️ **而我第一次只修了一半。** `skillremake:provenance` 修好之後仍有 **55 份英雄卡**
打不開 —— 因為 **`speedtiers:build` 是完全同一個形狀**（一條 glob 認領
`content/champions/*.json` 71 份，而 `withTierLines()` 只在原始字串上刪/插 tier 那幾行、
⛔ 其餘位元組一個都不碰）。
⭐ **同一個病在同一張表裡有第二個實例，而我修完第一個就以為結束了。**
⇒ 判準：**修好一個 glob 誤判之後，去把那張表裡每一條 glob 都問一次**
...
### ⛔⛔ 被誤導過的東西要**修根因**，⭐ 必要時搬進 legacy —— ⛔ 不是「知道了就好」

owner 2026-09-12（逐字，⭐ 這是常設指令）：

> 「如果我有回答過 請你都要將**讓你誤會的根因資訊做修正 甚至移到 legacy**」

⚠️ 這一條補上了第三守則缺的那一半：第三守則說「**去驗證**」，
⛔ 而驗證完之後那份會說謊的東西**還留在原地** ⇒ ⭐ 下一個人（包括未來的我）會再被騙一次。

| 情況 | 做什麼 |
|---|---|
| 一份**過期的規則／公式**誤導了我，而 owner 已經給過新的 | ⭐ **改掉它**，⛔ 不是在旁邊加一句「已棄用」 |
| 那份東西**整個被取代**了 | ⭐ **搬進 `docs/legacy/`**，並在那裡寫明**它做過什麼**（⛔ 不只是「已退場」） |
| 我把**自己的推測**寫成了 owner 的需求 | ⭐ 查出處（`git log -S` ＋ `asked-before.sh`），⭐ 找不到原話就**整條拿掉** |

⭐ 量到的代價（2026-09-12 一天兩例）：
- **吟唱的 20 階階梯**被五級距取代後留在原地 ⇒ 讓 `castTimeCoverage` 報出 **182 支假的「不一致」**
  ⇒ 我因此開了**兩個假缺陷**，兩個都活過一次自我複驗。⇒ 已搬 `docs/legacy/_cast-time-20-step-ladder.md`。
- **帳本的 15 分鐘合併窗**：我在 #1028 自己發明，然後**當成 owner 的原則引用回來**
  ⇒ owner 逐字否認：「我沒說過 我的原則**一定是詳實記錄不會合併**」⇒ 已整條拿掉。

⇒ ⭐ **判準：我這一次是被什麼騙的？那個東西現在還在原地嗎？**
⛔ 還在 ⇒ 這張票沒做完。
