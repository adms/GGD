...
owner 2026-08-24（逐字，⭐ 這是常設規定）：

> 「你反省一下 以飛鼠先生的天譴為例 為什麼沒做完 你沒完整驗收 你就跟我說你做完？
>  分析其 root cause 寫到開發守則跟 script，**我不想當你的人肉測試機**」

**量到的根因**（⛔ 不是感想）：連鎖閃電這一族**從寫出來的那天起一個像素都沒畫出來過**，
而同時有**四條綠燈**：

| 驗了什麼 | 是什麼 |
|---|---|
| sim 真的發了 `chainLightning` 事件 | 中間節點 |
| 事件真的過了 `eventFanout` 白名單 | 中間節點 |
| 客戶端真的建出了網格頂點 | 中間節點 |
| 那些頂點真的在抖 | 中間節點 |
| ⛔ **螢幕上真的亮起來了嗎** | **終點 —— 一條守衛都沒有** |

⇒ ⭐ **我把「鏈路接上了」當成「做完了」，而 owner 的驗收標準從頭到尾是畫面。**
四條綠燈全部停在中間節點，終點沒有人量 —— 於是「做完」是我說的，
而**唯一真的在驗收的人是 owner**。那正是他說的「人肉測試機」。

⚠️ 這一族的實際缺陷長這樣（兩段，都在終點那一側）：
① `CreateRibbon` 用**全 (0,0,0) 的退化路徑**建 ⇒ Babylon 推出來的 **UV 全是 (0,0)**，
而 `{ instance }` 就地更新**不重算 UV**；
② 那張 1×32 的漸層同時當 `emissiveTexture` 與 `opacityTexture`，形狀住在 **alpha** 且兩緣是 0
⇒ 每條弧都取樣在最外緣 ⇒ **全透明**。

**⇒ 規定（三條，都是可以當場檢查的）：**

1. ⭐ **視覺功能的「完成」定義是一張終端證據**，⛔ 不是「事件有送 / 值算對了」。
   二選一：帶 `@visual-proof` 標記且斷言**可見性**的守衛，
   或一份 `docs/_reports/*_visual-proof_*.md`（要有**功能開/關兩組亮像素數**）。
2. ⭐ **閘**：`bash scripts/visual-proof.sh` —— 改動碰到 `apps/client/src/{vfx,render}/`
   而沒有終端證據 ⇒ **exit 1**。逃生口 `GGD_VISUAL_PROOF_OFF=1`，用了要在 commit 訊息說為什麼。
   （⚠️ 它寫出來的第一件事就是擋下寫它的那一筆 commit —— 那是它有效的證明。）
3. ⚠️ **量畫面的工具自己也要先被驗過。** 同一輪我犯了兩個量測錯誤，兩個都讓我一度得到
   相反的結論：audition 的 canvas 沒 resize（背後緩衝只有 **300×150**，弧比一個像素還細）；
   讀像素前沒有明確 `scene.render()`（讀到上一幀）。
   ⇒ 報「看不見」之前，先證明**你的量尺在同一個場景下量得到別的東西**（例如既有的命中光圈）。

⭐ 這一條與第二守則的失敗形態①（「算出來但畫在畫面外／地板下」）是同一件事，
⛔ 差別是：那裡是一句**判準**（要記得問自己），這裡是一支**會擋下你的腳本**。
而這份文件已經記錄了**五次**判準失效。
