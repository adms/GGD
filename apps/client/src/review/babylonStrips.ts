...
export class StripStudio {
  ...
  async calibrate(): Promise<boolean> {
    ...
    e.b = 1;
    try {
      // audition 的 render loop 自己在跑，等它畫到。⚠️ 6 幀不是 1 幀：Chrome 的
      // KHR_parallel_shader_compile 讓 material 頭幾幀 isReady=false 被跳過
      //（實測：同一 task 連續 render 讀回來全是 clearColor）。
      await waitFrames(6);
  ...
  async vfxStrip(doc: VfxDoc): Promise<HTMLCanvasElement> {
    ...
    try {
      ps.start();
      await waitFrames(2); // 平行 shader 編譯：先讓粒子 shader 就緒再開始計時
...
function normalizeForTurntable(root: TurntableRoot): void {
