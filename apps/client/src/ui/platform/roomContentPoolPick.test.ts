 ...
 *
 * ── 🧬 突變（真的跑過，⛔ 不是打算做）────────────────────────────────────────
 *  ① `store.ts` 的 `presentRoomSettings` 拿掉列舉那一支（＝改回只認數字，也就是
 *     `ROOM_SETTING_KEYS` 的行為）
 *     ⇒ 🔴 `⛔ 房主選了社群池，而它沒有上路: expected undefined to be 'community'`
 *  ② `roomSettingFields` 只吐數字那一族（＝表單改回照 `ROOM_SETTING_KEYS` 產生）
 *     ⇒ 🔴 `⛔ 開房表單裡沒有 contentPool 這一格: expected null to be truthy`
 *  ③ `index.ts` 的 `...forwardRoomSettings(body)` 改回逐格列名的數字
 *     ⇒ 🔴 `⛔ /_internal/matches 沒有照推導轉送房間設定`
 *
 * ⚠️ ①**必須單獨跑**：②把那一格從畫面上拿掉之後，①的斷言連 `<select>` 都找不到
 * ⇒ 它會用**②的訊息**紅。⭐ 兩條同時突變 = 一條在遮蔽另一條，⛔ 而「紅了」看起來
 * 一模一樣（一條綠燈有四種假的來源，紅燈也是）。
 *
 * ⚠️ ⭐ 最後一條**誠實地說是讀原始碼的**（失敗形態⑥的形狀）：`index.ts` 的
 * top-level 會開一台 HTTP server，⛔ import 進測試就是啟一台伺服器。所以它問的
 * 不是「那道門會不會轉送」而是「那道門**是不是照推導轉送的**」—— 而那正是這一次
 * 斷掉的東西（逐格列名 ⇒ 少列一格不會有任何東西紅）。同 package 的
 * `publicEndpoint.test.ts` 讀同一個檔，用的是同一個理由。
 */
...
describe("GH#1025 Scope C —— 開房表單選得到內容池，而且那個選擇到得了對局", () => {
  ...
  it("★ ⭐ `/_internal/matches` **照推導**轉送房間設定，⛔ 不是逐格列名", () => {
    // ⛔ 這一條讀原始碼（見檔頭的理由）。它問的是那道門的**形狀**：
    //   逐格列名 ⇒ 加一格設定而忘了回來加一行 = 靜靜掉在門口，⛔ 沒有東西會紅。
    const src = readFileSync(
      join(__dirname, "../../../../game-server/src/index.ts"),
      "utf8",
    ).replace(/\/\/[^\n]*|\/\*[\s\S]*?\*\//g, ""); // 散文不可以滿足結構斷言
    const open = src.indexOf('matchMaker.createRoom("match", {');
    expect(open, "⛔ 找不到 /_internal/matches 開房的那個呼叫").toBeGreaterThan(-1);
    const options = src.slice(open, src.indexOf("\n  });", open));
    expect(options, "⛔ /_internal/matches 沒有照推導轉送房間設定").toContain(
      "...forwardRoomSettings(body)",
    );
    for (const key of ["champSelectSec", "combatMaxSec", "maxRounds", "contentPool"]) {
      expect(options, `⛔ 房間設定 ${key} 又被逐格列名了（下一格就會掉在門口）`).not.toContain(
        `${key}:`,
      );
    }
  });
