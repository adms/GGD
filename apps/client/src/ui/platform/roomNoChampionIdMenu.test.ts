 ...
 * 沒用的話請拿掉」。那是 RoomView 的英雄預選 `<select>`：把 `catalog.champions`
 * 整份倒出來、label 印**原始 id**（實測 71 筆 `godie-*`），而選了之後只寫進 redis
 * 給一道從這個 UI 永遠觸發不了的持有權閘 —— 從來沒送到遊戲伺服器。
 * 完整拆解在 RoomView.tsx 的檔頭。
 *
 * ⚠️ 刻意**掛真的元件**而不是掃原始碼字串（失敗形態⑥）：掃字串的版本在「有人改用
 * `<datalist>` 或一排按鈕把同一份 id 再倒一次」時照樣是綠的。這裡問的是玩家眼睛
 * 看得到的那件事 —— 房間頁上有沒有出現任何一個英雄 id。
 */
...
const ROOM = {
  room: {
    id: "r_491", name: "probe room", hostId: "acct_1", mapId: "arena.skeleton",
    mode: "PairedDuels", botDifficulty: "normal", status: "open", createdAt: 0,
  },
...
// jsdom 沒有 Element.prototype.scrollTo（聊天視窗捲到底會用到）。jsdom 的缺口,
// 不是產品缺陷 —— 補一顆空的,讓掛載走完真的那條 effect。
...
describe("GH#491 the room page prints no champion ids", () => {
  it("renders the room, and not one catalog id anywhere in it", async () => {
    ...
    // 反空轉：房間真的畫出來了,而且目錄真的有 id 可以被印錯 —— 否則下面那條在
    // RoomView 回 null 或目錄是空的時候會無意義地綠。
    expect(text).toContain(ROOM.room.name);
    expect(appStore.getState().catalog?.champions.length).toBeGreaterThan(0);

    for (const c of appStore.getState().catalog?.champions ?? []) expect(text).not.toContain(c.id);
