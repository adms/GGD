...
/**
 * ⛔ GH#729 的那三格（guardianDamage / guardiansSlain / bountyGold）**不在這裡驗**。
 * 這個檔是純 view-model，而「陣列裡有這一列」證明不了「玩家看得到那個數字」——
 * 那正是失敗形態⑧的形狀。承重守衛搬到 `guardianSettlementRows.test.ts`：它渲染
 * 出貨的 `MatchEndPanel` / `LeaveSettlementOverlay`，讀 markup 裡 label 後面緊接
 * 著的那一格。⛔ 不要在這裡再寫一份「同一件事、另一個角度」的斷言。
 */
