 ...
 * ⭐ 一個機制 ＋ 一張表（第零守則⑨）
 * ═══════════════════════════════════════════════════════════════════════════
 * 「哪一張圖下雨」**沒有新的住處**：仍然是 `config.weather@1.arenas` 那一格級別，
 * 經過 `WEATHER_KIND_WEIGHTS[kind].rain` 這張**共用表**變成 0..1 的權重。
 * ⇒ ⛔ 不可能出現「天氣說晴朗、天上在下雨」，而 owner 想讓某張圖下雨就是改那一列。
 * ⛔ 也**沒有**「雨的類別」與「霧的類別」兩支平行的程式 —— 三層共用同一份政策、
 * 同一個 `weatherLookFor()`、同一格總開關。
 *
 * ⚠️ 出貨 **關著**（`rainEnabled: false`）。這是**新功能**⛔ 不是修復，所以它不套
 * 第〇·六守則的「優先權大的更新後都是預設啟動」—— owner 打開它之前，畫面與上一版
 * 逐像素相同。
 *
 * ═══════════════════════════════════════════════════════════════════════════
