...
describe("#223 hurt / defeat are no longer gated to the local hero (voice-audience-223)", () => {
  ...
  it("the frame's candidates are SORTED before dispatch (no packet-order lottery)", () => {
    cover("voice-audience-223");
    // the flush orders them, and the frame loop calls the flush after the drain
    expect(SRC).toMatch(/for \(const c of orderVoiceCandidates\(this\.frameVoices\)\)/);
    expect(SRC).toMatch(/this\.flushContextualVoices\(\);/);
  ...
  it("distance for a damage line comes from the PACKET, not a stale render pose", () => {
    ...
    expect(SRC).toMatch(/private entityListenerDistance\(id: number\)[\s\S]{0,200}this\.schemaPos\(id\)/);
  });
});
...
describe("#223 the neighbouring categories deliberately left alone", () => {
  ...
  it("abilityCast + crit keep their existing un-gated, immediate dispatch", () => {
    cover("voice-audience-223");
    // already fanned out before #223 — re-routing them would risk double-firing
    expect(SRC).toMatch(/playContextualVoice\(champ, `skill-name\.\$\{slot\.toLowerCase\(\)\}`\)/);
    expect(SRC).toMatch(
      /playContextualVoice\(attacker, Math\.random\(\) < 0\.5 \? "crit" : "attack-heavy"\)/,
    );
  });
