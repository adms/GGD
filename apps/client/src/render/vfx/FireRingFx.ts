 ...
 *
 * WHAT IT DRAWS
 *   1. THE BAND — one flat `MeshBuilder.CreateTorus` on XZ at y ≈ 0.08 with an
 *      additive, UV-scrolling fire material (the `vfxPresets.ShockwaveRing`
 *      recipe: disableLighting + emissiveColor + alpha). ONE draw call, scaled
 *      to `MatchState.fireRingRadius` every frame. It is the readable, always-
...
export class FireRingFx {
  ...
  tick(nowMs: number, dtMs: number, frame: FireRingFrame | null): void {
    ...
    const places = swarmRingPlacements(
      {
        countPerLevel: [want],
        spawnIntervalSec: 0, // all flames burn at once — this is a wall, not a spawn wave
        radiusWc3: 0,
        radiusWorld: r,
        durationSec: 0,
        memberScale: 1,
  ...
  private ensureBand(): Mesh {
    ...
    // unit-diameter torus lying flat on XZ, scaled per frame (ShockwaveRing)
    const mesh = MeshBuilder.CreateTorus(
      "fire-ring-band",
      { diameter: 1, thickness: BAND_THICKNESS, tessellation: BAND_TESSELLATION },
      this.scene,
    );
    mesh.material = mat;
