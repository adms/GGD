...
describe("textureDedup", () => {
  ...
  it("CALIBRATION — 關掉去重時,三份 prop 必須量到 3 塊 GPU 資源(量不到 ⇒ 這把尺作廢)", async () => {
    setTextureDedupEnabled(false);
    const cs = await loadAll(makeScene().scene);
    // ⛔ 這一行紅 = 尺壞了,⛔ 不是功能壞了。
    expect(liveInternals(cs).size).toBe(SHARERS.length);
  ...
  it("內容相同的貼圖共用同一塊 GPU 記憶體,而且什麼都沒有變不見", async () => {
    const cs = await loadAll(makeScene().scene);
    const live = liveInternals(cs);
    expect(live.size).toBe(1); // ⭐ 3 → 1
    expect(textureDedupStats().shared).toBe(SHARERS.length - 1);
    expect(textureDedupStats().bytesSaved).toBeGreaterThan(0);
    // ⭐ 引用計數:正典被三個 container 持有 —— 真引擎才不會把還在用的圖刪掉。
    expect(refsOf([...live][0])).toBe(SHARERS.length);
    ...
    for (const c of cs) {
      for (const t of c.textures) {
        ...
        expect((it?.width ?? 0) * (it?.height ?? 0)).toBeGreaterThan(0);
      }
  ...
  it("被換掉的那一塊引用歸零 —— ⭐ 真引擎在這一刻才真的釋放 GPU 貼圖", async () => {
    setTextureDedupEnabled(false);
    const before = liveInternals(await loadAll(makeScene().scene));
    clearAssetByteCache();
    setTextureDedupEnabled(true);
    const after = liveInternals(await loadAll(makeScene().scene));
    expect(after.size).toBe(1);
    // 去重那一輪自己生出來又被放掉的兩塊:引用數 0 ⇒ `_releaseTexture()` 已經被呼叫。
    expect([...before].every((it) => refsOf(it) === 1)).toBe(true); // 沒去重那輪:各自 1
    expect(refsOf([...after][0])).toBe(SHARERS.length);
  });

  it("⛔ 不跨 Scene —— 第二個 Scene 拿到自己的貼圖(否則第 2 回合整片變黑)", async () => {
    const a = await loadAll(makeScene().scene);
    const b = await loadAll(makeScene().scene);
    const ia = liveInternals(a);
    const ib = liveInternals(b);
    expect(ib.size).toBe(1);
    // b 的那一塊必須是**它自己的**,⛔ 不可以是 a 的(a 的 Scene 一 dispose 就全黑)。
    expect([...ib][0]).not.toBe([...ia][0]);
    for (const c of b) for (const t of c.textures) expect(t.isReady()).toBe(true);
  });
});
