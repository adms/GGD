...
export class FrameSegments {
   ...
   *
   * ⛔⛔ **自證一律用真實時鐘，⛔ 不是 `this.clock`。** 第一版用了注入的那一個，
   * 而守衛注入的是一個**凍住的**假時鐘 ⇒ 忙等的 `while (clock() < until)` 條件
   * 永遠不變 ⇒ **無限迴圈**（2026-08-27 當場踩到，probe 卡死在第二項）。
   * ⭐ 而且它本來就該用真實時鐘：自證要證的是「歸屬邏輯對不對」，
   * 那是**與時鐘無關**的性質，⛔ 拿一個假時鐘去證它等於什麼都沒證。
   * ⚠️ 另外加一道**迭代上限**：一個壞掉的時鐘不可以把遊戲凍住 ——
   * 量尺自己絕對不可以成為 lag 的來源（本檔的存在理由）。
   */
  private selfTest(): void {
    const probe = new FrameSegments(defaultClock);
    probe.armedFlag = true;
    const burn = (ms: number): void => {
      const until = defaultClock() + ms;
      // ⛔ 忙等是刻意的：我們要的是「主執行緒真的被佔住 N ms」，
      //    ⛔ 不是 setTimeout（那會把時間交出去，量尺就量不到它）。
      // ⚠️ 上限 = 一個不會前進的時鐘的逃生口，⛔ 不是效能考量。
      let spins = 0;
      while (defaultClock() < until && spins++ < 5_000_000) {
        /* burn */
      }
    };
    for (let i = 0; i < 3; i++) {
      probe.begin(defaultClock());
      probe.mark("drain");
      probe.mark("draw");
...
/**
 * 自動武裝的門檻（ms）—— 同 `diag.diagWarnings` 的「凍結」門檻，⛔ 不是另訂一個。
 * 一次 ≥ 這麼長的凍結 = 玩家一定感覺得到。
 */
export const AUTO_ARM_STALL_MS = 400;

/**
 * ⭐⭐ **卡過一次就自己打開** —— owner 逐字：「**我不想當你的人肉測試機**」。
 *
 * ⚠️ 一把要玩家先去 F12 打指令才會開的量尺，在「他回報 lag」這個場合等於沒有：
 * 回報的當下現場已經過去了。⇒ 第一次量到 ≥ `AUTO_ARM_STALL_MS` 的凍結就武裝，
 * **下一次**卡頓就有逐段歸屬 —— 而 lag 從來不會只發生一次。
 *
 * ⛔ 為什麼這不是一格後台開關（第一守則）：它**只在遊戲已經在卡的時候**才開，
 * 成本是一次 4ms 自證 + 之後每幀八次減法，而它換到的是「下一次卡頓有兇手」。
 * ⛔ 沒有一個 owner 會想把它關掉，也沒有一個決策點在這裡 —— 這是**耳朵**，
 * 不是功能（同 `perfBus` 那一族計數器：fail-open 沒錯，**靜默**才是缺陷）。
 *
 * @returns 這一次呼叫有沒有真的把它打開（⭐ 給守衛驗機制用）。
 */
export function autoArmOnStall(worstStallMs: number): boolean {
  if (frameSegments.armed) return false;
  if (worstStallMs < AUTO_ARM_STALL_MS) return false;
  frameSegments.arm(true);
  return true;
}
