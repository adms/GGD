...
import { dispatchContextualVoice } from "./game/contextualVoiceDispatch";
import {
...
export class GameApp {
  ...
  private handleDrainedEvent(
    ...
    if (isMarkEvent(ev.type)) {
      ...
      // GH#737 分數／排名（⭐ 伺服器唯一權威，⛔ 客戶端不重算）
      recordRoundSettlement(ev, hudStore.getState().localSeatId ?? null);
  ...
  /** ⭐ 本體搬到 `game/contextualVoiceDispatch.ts`（棘輪：新東西不要塞回這個檔）。 */
  private dispatchContextualVoice(ev: EventMessage, localId: number | null): void {
    dispatchContextualVoice(
      {
        audioTeamOf: this.audioTeamOf,
        entityMaxHp: (id) => this.entityMaxHp(id),
        listenerDistance: (x, z) => this.listenerDistance(x, z),
        noteLocalCombat: () => this.noteLocalCombat(),
        queueVoiceCandidate: (c) => this.queueVoiceCandidate(c),
        voiceWhere: (id) => this.voiceWhere(id),
      },
      ev,
      localId,
    );
