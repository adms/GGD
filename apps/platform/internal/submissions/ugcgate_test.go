...
func ugcRouter(t *testing.T, policy func() SubmitPolicy) *chi.Mux {
	...
	h := NewHandlers(newSvc(t), func(next http.Handler) http.Handler { return next },
		...
		// ⚠️ `WithPromote` 會把 `DigestRecompute` 一起覆寫掉（nil ⇒ 視為 on ⇒ 503）——
		//    ⭐ 這一支量的是 UGC 那幾格，⛔ 不是 digest 重算（那條在 digest_test.go）。
		WithPromote(PromoteDeps{Ugc: policy, DigestRecompute: func() bool { return false }})
	r := chi.NewRouter()
