...
//
// This map is a RATCHET: it should only ever shrink. Each run prints it (see
// the t.Logf below) so an exemption can never become invisible the way a
// missing scope.go rule was.
var coverageExemptions = map[string]string{
	// Found by this gate on 2026-08-28 (GH ticket in the header of
	// docs/_reports/migration-completeness_temp_20260828-2000.md).
	//
	// data/curation/snapshots/<stamp>.json — pre-reset copies of the content
	// whitelist, written by curation.SaveSnapshot so an operator can undo a
	// whitelist reset. export.go's scanCollection SKIPS directories and the
	// `curation` rule is exact(), so this nested collection is enumerated by
	// nothing and refused on import.
	//
	// Why the new host is still correct without it: the LIVE whitelist
	// (curation/whitelist) travels in `core`, so content curation lands intact
	// and the only thing lost is the ability to roll back a reset that was
	// performed on the OLD host.
	//
	// Refute this exemption by: an operator who needs that rollback after the
	// migration. The fix is one `simple(...)`-style rule in Rules() under
	// GroupHistory (or GroupAudit) plus a childDirs-based Enum — at which
	// point DELETE this line.
	"curation/snapshots": "白名單重設前的快照；現行白名單本身走 core，新主機功能完整，" +
		"只失去『回捲舊主機上做過的重設』。要帶走需在 Rules() 補一條巢狀規則。",
}
...
func TestEveryPlatformCollectionIsCarriedOrDeclaredLeftBehind(t *testing.T) {
	...
	// --- standing exemptions are announced, never silent ------------------
	// fail-open is fine; SILENT is the defect. An operator reading -v output
	// before a real migration must see what the archive knowingly leaves.
	exempt := make([]string, 0, len(coverageExemptions))
	for col, reason := range coverageExemptions {
		exempt = append(exempt, col+" — "+reason)
	}
	sort.Strings(exempt)
	for _, line := range exempt {
		t.Logf("KNOWN GAP, not carried by any group: %s", line)
	}
