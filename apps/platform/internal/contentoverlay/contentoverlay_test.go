...
// content-overlay-no-updatedby-leak (#189 security, verify-caught): the
// UNauthenticated public head/bundle endpoints must NOT expose the editing
// admin's account id (UpdatedBy). The durable store keeps it for audit; the
// public wire copy is blanked in the handler.
func TestPublicEndpointsDoNotLeakUpdatedBy(t *testing.T) {
	svc, _, _ := newSvc(t)
	...
	for _, path := range []string{"/content-overlay/head", "/content-overlay/bundle"} {
		req := httptest.NewRequest(http.MethodGet, path, nil)
		w := httptest.NewRecorder()
		r.ServeHTTP(w, req)
		require.Equal(t, http.StatusOK, w.Code, path)
		assert.NotContains(t, w.Body.String(), adminID,
			"%s leaked the editing admin's account id", path)
	}
}
