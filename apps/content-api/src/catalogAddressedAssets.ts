 ...
 *
 * 回頭開關（只有作者／維運會轉 ⇒ 環境變數，⛔ 不進後台）：
 * `GGD_CATALOG_SNAPSHOT_MODE=full` ⇒ 回到全量複製（512 MiB 上限照舊會擋，那是預期）。
 */
...
export function addressedAssetScanner(root: string, mode: CatalogSnapshotMode) {
  ...
  if (mode === "content-addressed") {
    ...
    catch { listing = ""; } // 不在 git 裡 ⇒ 沒有第二個住處 ⇒ 全部照舊複製
...
export function readAddressedAsset(rootPath: string, want: AddressedAssetFact, current: Iterable<AddressedAssetFact> = []): Uint8Array {
  ...
  throw Object.assign(new Error(`此版本的素材只記了雜湊（${want.sha256}），工作樹找不到同雜湊的檔案，未寫入任何資料：${want.path}（可用 git cat-file -p ${want.gitBlob} 從 git 取回）`), { statusCode: 409 });
}
