...
export function makeSourceSandbox(kind: SandboxKind): string {
  ...
  if (kind === "routes") {
    ...
    // ⭐⭐ `common.py::_shipped_number_to_suffix()` 讀的是 **git HEAD** 的 `content/abilities/`
    //   （`git ls-tree` ＋ `git show HEAD:…`，⛔ 刻意不讀工作區 —— 免得一次壞的重跑把錯誤鎖成不動點）
    ...
    const headAbilities = execFileSync(
      "git",
      ["-C", REPO, "archive", "--format=tar", "HEAD", "--", "content/abilities"],
      { maxBuffer: 256 * 1024 * 1024 },
    );
    execFileSync("tar", ["-x", "-C", root], { input: headAbilities });
    git("init", "-q");
    git("add", "--", "content/abilities");
    git(
      "-c", "user.email=sandbox@ggd", "-c", "user.name=sandbox", "-c", "commit.gpgsign=false",
      "commit", "-q", "--no-verify", "-m", "sandbox HEAD = real HEAD content/abilities",
    );
