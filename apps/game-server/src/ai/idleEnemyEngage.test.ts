...
function runScene(enemies: { pos: V.Vec2; hp?: number }[], ticks: number): Scene & { ids: EntityId[] } {
  ...
  // ⚠️ 一顆裸的 `SimWorld` 預設 `combatActive = false`，而 `autoAcquirePass` 第一行就
  //   `return`。bot 不在乎（牠的腦送的是明確的 `attackTarget` 指令），⛔ 但站著不動、
  //   沒有 driver 的人**只有**這條路可以打回去 —— 少了這一行，「它打回去了」永遠是 0，
  //   而那看起來就像 sim 的索敵死了（同 `idleSeekRadius.test.ts` 的 `idleAt`）。
...
describe("GH#999 — a bot goes and hits a standing-still enemy (Tier-0 ENGAGE fallback)", () => {
  ...
  it("a full-HP idle enemy is LAST in line: while a damaged one lives the brain never names the idle one", () => {
    ...
    // must still pick the damaged one, walk past the idle one's side and hit the damaged
    // one — and never once name the full-HP idle one for the whole horizon.
    // ⚠️ 受傷的那個要打不死（2500/5000）：`probe` 沒有英雄卡，死亡那一 tick 的
    //   `recomputeStats` 會 `Champions.get("probe")` 而擲 —— 而「死了之後才輪到站著的人」
    //   本來就是上面那一條在驗的同一個 ENGAGE fallback，⛔ 不必在這裡再驗一次。
    const r = runScene([{ pos: { x: -27, z: -14 }, hp: 2500 }, { pos: { x: -45, z: -22 } }], 450);
    ...
    expect(r.hitsOn.get(damaged) ?? 0, "named the damaged enemy but never reached it").toBeGreaterThan(0);
    expect(
      r.targetedFrames.get(idle) ?? 0,
      "the full-HP idle enemy was named while a damaged one was alive — hp no longer outranks distance",
    ).toBe(0);
  });
});
