 ...
 * 測試 `packages/shared/src/sim/championForm.test.ts:342` 一直是綠的 —— 失敗形態
 * ⑤：被測的不是出貨的那條路），而 `ChampionFormSystem.ts` 的檔頭逐字寫著
 * 「a form is a WITHIN-ROUND state…cannot leak across the intermission」。
 * **那句話是假的，而沒有任何東西會紅**（第三守則：註解會說謊，去驗證）。
 *
 * 出貨內容真的踩得到：8 個 `{"to":"toggle"}` 且沒有 `durationSec` 的形態
 * （`expiresTick = FORM_NEVER_EXPIRES`，其中兩支是 passive，玩家連「再按一次切回
 * 來」都沒有）永遠不會自己到期，而**活著**的人身上沒有任何清除路徑
 * ⇒ 下一回合開場仍然是變身後的模型／技能組／屬性。
 *
 ...
 *
 * ⚠️ 這一支**刻意住在 `apps/game-server`**（票裡原本寫 `packages/shared/src/sim/`）：
 * 它要跑**出貨的** `MatchController`，而 `packages/shared` 的 `tsconfig` `rootDir`
 * 就是它自己 —— 放在那邊 vitest 會綠、`tsc` 會噴 12 個 TS6059（量過）。
 */
...
describe("GH#579 回合結算 → 變身還原", () => {
  it("★ 一個**永不到期**的切換形態，撐不過回合結算", () => {
    ...
    // 出貨 26 對變身的形狀：base 指向一個 `role: "alternate"` 的對手身體。骨架內容
    // 一對都沒有，所以在這裡接一對上去（與 `championForm.test.ts` 的夾具同法）。
