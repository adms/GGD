...
describe("GH#658 一擊佔目標多少血", () => {
  it("真的打一場 → 那一擊的目標最大生命被記下來,而且進得了排行榜 payload", () => {
    ...
    // ⚠️ 條件是「真的**扣到血**」而不是 `heroHits > 0`:命中但傷害是 0 的施放
    // (純控場、被吸收乾淨)也算一次 hit,而它本來就沒有「佔多少血」可言。
    const hits = snap.casts.filter((c) => c.damageToHeroes > 0);
    expect(hits.length, "十二隻 bot 打一整回合不可能一點英雄血都沒扣到").toBeGreaterThan(0);
