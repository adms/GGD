import { resizeImageInBrowser } from "./resizeImage.browser";
...
self.onmessage = async (event: MessageEvent<ModelUploadJob>) => {
  try {
    ...
    else if (job.kind === "prepare") { const prepared = await prepareUploadedHeroModel(job.bytes, job.selections, job.yawOffsetDeg, { resizeImage: resizeImageInBrowser }); result = { summary: prepared.inspected, bytes: prepared.bytes, model: prepared.model, document: prepared.document, warnings: prepared.warnings }; }
