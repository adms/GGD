...
export interface HeroTransferDraft extends HeroDraftPayload { originalIcons: OriginalIcon[]; normalizedIcons: { path: string; base64: string; sha256: string }[] }
...
export async function heroTransferDraft(value: HeroDraftPayload): Promise<HeroTransferDraft> {
  ...
  return { ...draft, originalIconRefs, originalIcons, normalizedIcons };
}
...
export async function restoreHeroDraftAssets(raw: HeroDraftPayload & Partial<HeroTransferDraft>): Promise<HeroDraftPayload> {
  ...
  return { project: raw.project, rawInputs: raw.rawInputs, mode: raw.mode, origin: raw.origin, originalIconRefs: refs, ...(raw.source ? { source: raw.source } : {}), ...(raw.cloud ? { cloud: raw.cloud } : {}) };
}
