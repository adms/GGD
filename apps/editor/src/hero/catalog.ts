...
const simulationFiles = import.meta.glob("../../../../content/{champions,abilities,items,augments,projectiles,status-effects,loot-tables,arenas,config,ability-templates}/*.json", { eager: true, import: "default" });
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<HeroCatalog> => {
      try {
        ...
        return fromDocuments(HERO_SIMULATION_COLLECTIONS.flatMap((collection) => store.all<Record<string, unknown>>(collection).map((document) => [`${collection}/${document.id}`, document] as [string, Record<string, unknown>])), new Set(store.ids("models")), "local-api");
