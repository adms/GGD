/**
 * 變更密碼 · self-service change-password — pure form logic.
 *
 * The console posts `POST /api/v1/account/password` with the CURRENT password
 * alongside the session token: the platform refuses to change a password from a
 * session alone, precisely so a stolen token cannot lock the operator out of
 * their own account. On success the server revokes every refresh token of the
 * account and hands back a fresh pair (api.changePassword swaps it in), so every
 ...
 * network; ui/ChangePasswordDialog.tsx wires it to the real API.
 */
...
export const CHANGE_PASSWORD_SUCCESS =
  "密碼已變更，其他裝置的登入已全部登出 · Password changed. All other sessions were signed out.";
 ...
 * platform's single 401 surface — the console must not become the oracle the
 * API refuses to be.
 */
export const CHANGE_PASSWORD_FAILED =
  "變更失敗，請確認目前密碼後再試一次 · Could not change the password. Check your current password and try again.";

export const CHANGE_PASSWORD_RATE_LIMITED =
  "嘗試次數過多，請稍候再試 · Too many attempts. Please wait a moment and try again.";

/** The platform's password shape rules, mirrored (see the file header). */
 ...
 * 429 throttle is the one exception — telling the operator to wait is useful
 * and leaks nothing).
 */
