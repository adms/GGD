 ...
 *   ⚠️ 這一句講的是 C5 的「幾發打死」，⛔ 不是 B2：同一則訊息裡 owner 還問「B1/B2/B3 => 這三個是啥」
 *     ⇒ 他當時不知道 B2 是什麼。借用的是**形狀**（只顯示、不警示），⛔ 不是 owner 對 B2 的裁決。
 *
 * ⇒ 主 session 2026-09-15 的決定（Claude 的判斷，⛔ 不是 owner 原話，⏸ 待 owner 覆核）：出身表的反解**不改**，
