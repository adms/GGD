...
export const SWAP_WHEN_FULL_LABEL: FieldLabel & { group: "shelf" } = Object.freeze({
  zh: "背包滿時可以換掉一件",
  note:
    "三選一發到手而**背包已經滿**的時候：關著＝卡片變灰寫「道具欄已滿」、點了不換，卡片留著（⛔ 機會不被吃掉）；" +
    "⭐ 開著＝點那張卡會列出背包六格，玩家**指定丟掉哪一件**換上新的，被丟掉的那件依「賣出退款率」退錢。" +
    "⚠️ ⭐ 它**只管三選一**那條路 —— ⛔ 買寶玉那條**行為不變**（仍然是背包滿就拒絕、不扣錢）；" +
    "換裝那一下的賣出與商店賣出守**同一條**「現在可以開商店嗎」規則；電腦座位⛔ 不換裝。" +
    "⚠️ 換裝會結清這一輪的「復原」紀錄（同買寶玉）：換完之後，這一輪先前買的東西不能再復原（照常可以賣）。" +
    "⭐ 出貨**開著**：owner 2026-09-08 裁決「A ＋ B 開票」，預設開是 Claude 依「自己判斷 但是留後台開關可以簡易 rollback」挑的。",
  group: "shelf",
});
