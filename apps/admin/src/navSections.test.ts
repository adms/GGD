...
describe("分類重編一頁都沒有掉", () => {
  it("重編後可到達的頁面集合，與重編前逐一相等（雙向）", () => {
    ...
    const SINCE_BASELINE = new Set<string>([
      ...
      "iconWorkshop",
      // 📤 2026-09-04 GH#966：圖示上傳。⚠️ 這一列是**補登記的** —— 那一頁上線時
      // 沒有連同基準線一起更新 ⇒ 這條守衛從那一刻起就一直紅，而紅的訊息說的是
      // 「多了一頁」而不是「有人漏了一行」⇒ 讀的人會以為是自己這一批造成的。
      "iconUpload",
      // 🧑‍🎨 2026-09-05 GH#991：玩家自製內容（UGC）的開關頁。⭐ 出貨預設 `enabled: false`
      // —— 流水線只有 config 這一段落地，身分／逐份機器閘／配額計數都還沒有
      // ⇒ 出貨開著等於一條**零守衛的公開寫入路**。
      "ugc",
