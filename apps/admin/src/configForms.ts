...
const LOBBY_RALLY_SPEC: ConfigDocSpec = {
  ...
  intro: [
    ...
    "⭐ **同日 owner 反轉了語意**：「**你說的是對的，預設是加入，五秒是讓人按否定的**」⇒ 出貨的行為是 **opt-out**：視窗跳出來，**沒按「不要」就進房**。上面那句「同意後就一起進入」現在由「不反對就一起進入」取代 —— 因為倒數只有 5 秒，而 5 秒對「主動點同意」太短。",
  ...
  fields: [
    ...
    },
    {
      ...
      note: "⭐ owner 2026-08-21 逐字：「**預設是加入，五秒是讓人按否定的**」。**opt-out（出貨值）**＝ 視窗跳出來，**5 秒內沒按「不要」就進房**，主要按鈕是「不要」。**opt-in** ＝ 回到 2026-08-21 早上那一版：要**按下「加入」**才進房，沒按就什麼都不會發生。⛔ 這不是措辭差異：倒數出貨只有 5 秒，5 秒對「主動點同意」太短 —— opt-in 幾乎等於沒有人會加入，而這張票的目的是「拉人進來」。⇒ opt-in 是**一鍵 rollback**，⛔ 不是觀望用的。",
    },
    {
