...
const DISPEL_SPEC: ConfigDocSpec = {
  ...
  intro: [
    ...
    "⭐ **「解除全部負面狀態」現在是真的**（owner 2026-08-18「理論上淨化就是解掉所有負面狀態阿⋯所以提高到 1000 都沒關係」）：下面的「一發淨化每一池最多拔幾層」出貨值是 **1000**，實務上等於不設限。⛔ 這個數字不是「總共只有 26 種可淨化減益」那樣算出來的 —— 引擎是**一筆一筆**算的（同一種減速由 30 隻殭屍各掛一次就是 30 筆），所以筆數沒有種類數那個天花板。",
  ...
  fields: [
    ...
    },
    {
      path: "maxCountCap",
      zh: "一發淨化每一池最多拔幾層",
      note: "全域上限：文件沒寫層數時用它，**文件寫了也夾不過它**。一句話管到底，避免出現兩個會分歧的上限。⭐ **出貨 1000＝實務上「全部」**（owner 2026-08-18「理論上淨化就是解掉所有負面狀態阿」）—— 卡面寫「解除全部負面狀態」的那些技能，靠的就是這一格。填 1＝每發只解一層（很弱但很好懂）。⚠️ **調低它會讓卡面說謊**：好幾張卡的文案寫著「淨化全部可淨化的減益」，而引擎只會拔到你填的這個數字，多的**靜默消失**（⛔ 沒有任何東西會叫，也不會有任何測試變紅）。要做弱淨化請去那一份文件填層數，不要動這一格。",
    },
    {
      path: "defaultOrder",
      zh: "層數不夠時先拔哪一邊",
