...
const MANA_ECONOMY_SPEC: ConfigDocSpec = {
  ...
  intro: [
    ...
    "⭐ **例外清單在 `docs/魔力回復例外清單.md`**（`pnpm mana:audit` 產生，逐隻英雄 × LV30/50/99 三個錨點）。量到的出貨狀態（71 隻裸裝）：**三個錨點各有 70 隻滿魔超過 20 秒**，唯一的例外是 `godie-h020` 黑魔導士-莉娜因巴斯（9.5 / 10.5 / 12.9 秒）。中位滿魔 LV30 **42.1s**・LV50 **38.0s**・LV99 **34.5s**。",
    "⚠️ 分布是**雙峰**的：15–20 秒那一格在三個錨點上**一隻都沒有** —— 莉娜一隻在 10 秒附近，其餘 70 隻全在 22.7 秒以上。⇒ 打開那個開關會**一次動到 70 隻**，⛔ 這不是可以靠微調一個數字擦邊過去的形狀。",
  ...
  fields: [
    ...
    },
    {
      path: "refillSeconds",
      zh: "從空到滿的建議秒數",
      note: "⭐ owner 2026-08-20：「時間是**建議原則** 不是死程式邏輯」⇒ 它是一個**被稽核的目標**，⛔ 不是保證。只有兩個讀者：① 下面那個開關打開時的地板算式（魔力池 ÷ 這個數）② `pnpm mana:audit` 判斷誰超標的門檻。出貨 **15**（owner：「平均回魔不超過 15 秒就可以滿魔再一輪」）。⚠️ 上界 **20** 是 owner 自己給的數字（「最糟的情形也不超過 20 秒」），⛔ 不是防手滑的柵欄。",
    },
    {
      path: "enforceFloor",
      zh: "超標時真的把回魔拉上去",
      note: "⭐ 出貨 **關**。關著 ＝ 上面那個秒數純粹是一條建議，回魔**逐位元**等於屬性管線算出來的 `manaRegen`（中位滿魔 LV30 42.1s／LV50 38.0s／LV99 34.5s）。打開 ＝ 回到 2026-08-19 的硬地板 `每秒回魔 ≥ 魔力池 ÷ 建議秒數`，每一位英雄從空到滿至多那麼多秒，⛔ 與他的智力無關。⚠️ **打開會一次動到 70 隻**（71 隻裡只有 `godie-h020` 莉娜本來就比建議值快）—— 先看 `docs/魔力回復例外清單.md` 再決定。⚠️ 它是**地板不是取代**：本來就回得比它快的英雄一格都不會被動到，否則這條規則會反過來削弱高智力英雄。",
    },
    {
      path: "championsOnly",
      zh: "只套在英雄身上",
