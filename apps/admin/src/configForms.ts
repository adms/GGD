...
const WORLD_CUES_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      ...
      zh: "殭屍王橫幅：淡入秒數",
      note: "殭屍王降臨橫幅與（殭屍王／特殊殭屍）分紅結算面板從透明淡入到全亮要幾秒。⭐ owner 2026-08-24（#642）「太佔螢幕 說明半秒淡出半秒就好」——出貨 0.5。⚠️ **淡入＋淡出就是橫幅在畫面上的全部壽命**（中間沒有停住的一段，太佔螢幕的正是停住那一段）；兩格都調 0＝這兩張橫幅完全不畫（獎金照發、音效照播，只是沒有畫面說明）。",
    },
    {
      path: "hud.mobBossFadeOutSec",
      zh: "殭屍王橫幅：淡出秒數",
      note: "同一對橫幅從全亮淡回透明要幾秒，出貨 0.5（#642）。玩家實際能讀字的時間大約就是淡入的後半＋這一段的前半 —— 覺得結算表閃太快讀不完，調大**這一格**（例如 2）就拿回停留感，不必改程式。",
    },
  ],
  preserved: [],
};
...
const ARENA_RULES_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      ...
      zh: "提前到「現在＋幾秒」點火",
      note: "上面那格成立時，火圈點火時間被夾到「現在＋這格秒數」（出貨 10）。調小＝bot 局收得更快；0＝人類全滅的下一個 tick 就點火。⛔ 它只**提前**點火 —— 已經更早的點火時間不會被往後推，回合硬底線（roundHardCapSec）也一格都不動。",
    },
    // ── 商店 ────────────────────────────────────────────────────────────────
    {
