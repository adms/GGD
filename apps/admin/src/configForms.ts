...
const WEATHER_SPEC: ConfigDocSpec = {
  ...
  consumer:
    "apps/client/src/render/weather.ts 的 weatherPolicy() → ArenaScene.buildArena() 的 buildZoneGround（濕地面／積水材質）與 Lighting.write()（霧濃度／雷擊補光）",
  effect:
    "霧與雷擊補光：玩家**下一次重新整理遊戲頁面**之後**立即**生效（不必重開一場）。濕地面與積水：**下一回合換圖**時生效（它們是建材質當下決定的常數，⛔ 不是每幀寫的東西）。",
  fields: [
    {
      path: "enabled",
      zh: "天氣總開關",
    ...
    },
    {
      ...
      zh: "起霧：最濃時的濃度",
      note: "「起霧」那一級加在空氣漫反射之上的額外濃度（下雨 0.45 倍、雷雨 0.7 倍、室內潮濕 0.3 倍）。⚠️ 上界 0.006 是**反解**出來的：再濃下去，最遠對戰距離上的敵人會塌成一團與霧同色的剪影，而這張地圖是全視野的。",
    },
  ],
  tables: [
    {
