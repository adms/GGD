...
const ARENA_RULES_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      ...
      zh: "只剩 bot 在打就提前縮火圈（GH#643）",
      note: "還在打的場地裡一個活著的人類都不剩（全滅、輪空、或人類的場已分出勝負）時，把火圈點火提前到「現在＋下面那格秒數」，讓 bot 互毆快點收場。owner：「如果現場只剩 bot 存活，回合時間縮減到10秒後就縮火圈 不要平白浪玩家等待」。⚠️ 開著時它**贏過**殭屍王的火圈延後（人都不在了，王的延長沒有觀眾）；關掉＝回到舊行為，bot 互毆也照原點火時間等。⛔ 全 bot 的沙盒房（0 個人類座位）永遠不觸發 —— 沒有玩家在等。",
    },
    {
      path: "botOnlyRingAccelSec",
      zh: "提前到「現在＋幾秒」點火",
      note: "上面那格成立時，火圈點火時間被夾到「現在＋這格秒數」（出貨 {{出貨值}}）。調小＝bot 局收得更快；0＝人類全滅的下一個 tick 就點火。⛔ 它只**提前**點火 —— 已經更早的點火時間不會被往後推，回合硬底線（roundHardCapSec）也一格都不動。",
    },
    // ── 商店 ────────────────────────────────────────────────────────────────
    {
