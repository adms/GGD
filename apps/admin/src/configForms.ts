...
const RANGE_GUIDE_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    {
      path: "pressOpensBanner",
      zh: "按下開說明橫幅",
      note: "**按下**技能鈕／技能鍵時要不要打開畫面頂端那條說明橫幅。出貨 {{出貨值}}。⭐ owner 2026-08-22：「**天生QWEREX 按按鈕施放時不要一直跳出說明 很亂**」—— 按下是**施放**不是**閱讀**，而橫幅在連招時會一秒閃好幾下、蓋住上半個畫面。⛔ 關掉它**不會**關掉範圍圈（那是瞄準要用的，另一格）。⚠️ **被動技不受這一格影響**：被動按不下去，而說明正是玩家唯一能對它做的事，所以那一格永遠開橫幅。想看說明的正確手勢是 **hover**（上面那一格）或技能格自己的 tooltip。",
    },
    {
      path: "rangeColor",
      zh: "施法距離圈的顏色",
