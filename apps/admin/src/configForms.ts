...
const COOLDOWN_RULES_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      path: "hookMinSeconds",
      zh: "被動觸發器最短內部冷卻（秒）",
      note: "一條**觸發器**（英雄被動 / 道具被動 / 增益卡的 hook）真的發動之後，最短隔多久才能再發動。出貨 **{{出貨值}} ＝ 沒有地板**。⚠️ 它與上面那一格是**兩種秒**：技能冷卻填的是**卡面秒**（引擎再乘「技能冷卻時間」倍率，出貨 0.2 ⇒ 60 卡面秒 = 12 實際秒），而觸發器的內部冷卻從來就是**實際秒**、不吃那個倍率 —— 用同一格夾兩種秒就是用同一把尺量兩個空間。⭐ 想一次壓住所有被動的觸發頻率就填 **1.2**（＝冷卻表最便宜的一格：單體·極小 6 卡面秒 × 0.2）；填 0 就是回到今天。⚠️ 出貨值刻意是 0：量到有 **52 條**既有觸發器的內部冷卻低於 1.2 秒（0.5 / 0.6 / 1.0），預設拉高會一次改掉那 52 張卡的手感。⚠️ 它**只夾有填內部冷卻的那些** —— 沒填的（例如相轉移裝甲每 tick 續期的魔免）代表作者明說「每一次事件都算」，夾住它們會讓常駐效果變成閃爍的。",
    },
  ],
  // 三格純量，沒有不編輯的分支要原封帶走。
  preserved: [],
