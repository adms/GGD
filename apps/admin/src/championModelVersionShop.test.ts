/**
 * 🛒 GH#1177 後台「模型版本 → 造型」上架：⛔ 不預選分級；上架寫出一份**出貨 schema 收得下**的 skin@1（只寫 priceTier，
 * ⛔ 沒有 mcoinPrice —— owner 2026-09-15「新模型加購參考 LOL 分級標價」）；下架寫 listed:false、⛔ 不刪檔；
 * ⛔ 啟用中的版本／已有同模型造型的版本不能上架；內容編輯關著（正式 build）⇒ 整段收起、⛔ 不打 API。
 *
 * ⭐ 版本清單讀**出貨的** b2-popp、分級表讀**出貨的** skin-tier-prices.json；只有寫入端換成記憶體樁（寫前先過真的 zSkinDoc）。
...
import { mount, optionLabels, optionValues, textOf } from "@ggd/shared/testkit/headlessUi";
import { zSkinDoc, type SkinDoc } from "@ggd/shared/content/schema/skin";
import { skinPriceTable, zConfigSkinTierPricesDoc } from "@ggd/shared/content/schema/config/skinTierPrices";
...
const tiers = zConfigSkinTierPricesDoc.parse(JSON.parse(readFileSync(join(CONTENT, "config/skin-tier-prices.json"), "utf8")));
...
describe("模型版本商店上架（GH#1177）", () => {
  ...
  it("選了分級才上架；上架＝合法 skin@1 只寫 priceTier；下架＝listed:false 且檔案還在；啟用中／同模型已有造型 ⇒ 擋", async () => {
    ...
    const view = shop({
      ...
      getPriceTiers: async () => ({ doc: tiers, error: null }),
    ...
    expect(listBtn().props.disabled, "⛔ 沒選分級就能上架 —— 分級沒有預設值").toBe(true);
    ...
    const select = view.field(`model-shop-tier:${key}`);
    const [none, tier, ...rest] = optionValues(select);
    expect(none, "第一個選項必須是「未選」").toBe("");
    expect([tier, ...rest].sort(), "下拉選單要列出出貨分級表的每一個分級").toEqual(Object.keys(tiers.tiers).sort());
    expect(optionLabels(select)[1], "選項要帶 LoL 內容標準那一句").toContain(tiers.tiers[tier!]!.standard);

    view.type(`model-shop-tier:${key}`, tier!);
    view.press(listBtn());
    await view.flush();
    expect(saved.at(-1)).toMatchObject({ schema: "skin@1", championId: "b2-popp", modelKey: key, priceTier: tier });
    expect(saved.at(-1)!.mcoinPrice, "⛔ 分級與算好的價不可並存（第〇·四）").toBeUndefined();
    ...
    expect(textOf([view.field(`model-shop-status:${key}`)]), "上架列要顯示分級表解析出來的價").toContain(`${skinPriceTable(tiers)[tier!]} M幣`);

    view.press(view.field(`model-shop-delist:${key}`));
    await view.flush();
    expect(saved.at(-1)).toMatchObject({ id: saved[0]!.id, listed: false, priceTier: tier });
  ...
  it("內容編輯關著（正式 build）⇒ 沒有任何輸入框，只留一句怎麼上架，⛔ 不打 API", async () => {
    ...
    const getPriceTiers = vi.fn();
    const view = shop({ enabled: false, listSkins, getPriceTiers, saveSkin: vi.fn() });
    await view.flush();
    ...
    expect(getPriceTiers).not.toHaveBeenCalled();
  });
});
