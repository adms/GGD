...
export const ARENA_RULES_SPEC: ConfigDocSpec<"arenaTuning"> = {
  ...
  fields: [
    ...
    // 最後一段。標籤在這裡是**必要**的：`configForms.test.ts` 的雙向斷言（少寫紅、
    // 多寫也紅）從 schema 走出來，而 schema 已經有這 21 格了。少了它們，這一頁
    // 會**安靜地少畫 21 個旋鈕**，而畫面上不會有任何錯誤（那正是這條守衛在擋的）。
    // ⚠️ 每一格都不寫 `max` —— round11 的 Zod 每一格都已經有上界，兩邊同時給
    // `boundsFor` 會丟例外（「上下界只有一個來源」）。
    {
      path: "round11.enabled",
      zh: "第十一回合 · 開不開這個模式",
