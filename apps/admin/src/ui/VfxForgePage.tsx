...
export function VfxForgePage(): React.JSX.Element {
  ...
  // ⚡ GH#571 —— 施法電弧的總開關（留白 = 沒說 = 用出貨預設 true）。
  const [castArcs, setCastArcs] = useState("");
    ...
    try {
      ...
      if (parsed) {
        ...
        setGlobals({
          ...
          // #251 —— 同一條 optional 規則：文件沒有就留白，不要幫它填預設。
        ...
        setCastArcs(
          parsed.castArcs === undefined ? "" : parsed.castArcs ? "1" : "0",
        );
        setVfxSound(
  ...
  const pending = useMemo((): ConfigVfxFamiliesDoc | null => {
    ...
    // #251 —— 投射物那兩格數字,同一條規則(留白 = 拿掉 key,不是 0)。
    // `projectileRadiusGain` 尤其不能變成 0:0 的意思是「全部一樣大」,
    ...
    const {
      ...
      projectileRadiusGain: _droppedGain,
    ...
    let next: ConfigVfxFamiliesDoc = {
      ...
      ...(radiusText === "" ? {} : { projectileRadiusGain: Number(radiusText) }),
      ...(flyText === "" ? {} : { projectileFlyHeightY: Number(flyText) }),
  ...
  }, [doc, globals, enabled, castHeight, projArt, famPitch, castArcs, vfxSound, famDrafts, abDrafts, removals]);
