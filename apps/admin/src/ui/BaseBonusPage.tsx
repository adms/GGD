 ...
 *      那一列變 **0**,而旁邊寫著「出貨預設 650」。真正要回到出貨值的按鈕是新的
 *      「還原出貨版」,走平台既有的 revert 端點(整份覆蓋層條目移除)。
 * #279 有上限的六個 stat 會在自己那一列說出來:填的數字會被最終值 clamp 吃掉,
 *      而這一頁的文案保證「填多少就是多多少」。
 * #279 (2026-07-31) 抬頭與還原確認句裡的出貨值改成**算出來的**。之前兩處寫死
 *      「+300」,而出貨值已經是 650 —— 同一個畫面上的三個數字有兩個是錯的,
 *      其中一個還印在沒有 undo 的破壞性按鈕上。見 `shippedBonusNote`。
 */
...
/**
 * 出貨預設那句話。模組層算一次 —— 它只依賴 `DEFAULT_BASE_BONUS`,和頁面狀態無關。
 */
const shippedNote = shippedBonusNote();
...
export function BaseBonusPage(): JSX.Element {
  ...
  const load = async (): Promise<void> => {
    try {
      ...
      //   null —— 一份文件都沒讀到 → 每一格顯示**出貨預設**(生命 650)
      //   {}   —— 讀到一份真的空的文件 → 每一格是 **0**
      // 壓成 `{}` 的話,一台還沒有這份文件的主機會把面板顯示成「全部 0」,
      // 而伺服器仍然在給 650;操作者存下任何一列,就會把那份贈禮真的拿掉 ——
  ...
  return (
    <Panel title="基礎加成">
      {/*
        ⚠️ 出貨預設是**算出來的**,不是打在字串裡的。
        這一行本來寫死「生命上限 +300」,而出貨值在 2026-07-30 被 owner 改成 650
        (`DEFAULT_BASE_BONUS`)—— 頁面於是每一格顯示「出貨預設 650」,抬頭卻同時
        寫著 +300,而「還原出貨版」的確認句也說 300。三個數字裡兩個是謊話。
        `shippedNote` 直接讀 `DEFAULT_BASE_BONUS`,所以下一次 owner 動這個值時,
        文案不會落後 —— CLAUDE.md:「語意改了,舊文案就是謊話,必須一起改」。
      */}
      <p style={{ color: TEXT_DIM, fontSize: 13, lineHeight: 1.7, margin: "0 0 6px" }}>
        每位英雄一開始就多拿的<b style={{ color: TEXT_MAIN }}>固定數值</b>。
        出貨預設是<b style={{ color: GOLD }}>{shippedNote}</b>。
      </p>
      <p style={{ color: TEXT_DIM, fontSize: 13, lineHeight: 1.7, margin: "0 0 6px" }}>
        ⚠️ 這裡填的是<b style={{ color: ACCENT }}>加數,不是倍率</b>,而且
        <b style={{ color: TEXT_MAIN }}>不參與倍率計算</b>——「生命倍率 3.0」放大的是英雄自己
        的血量,不會把這份加成也乘三倍。<b style={{ color: TEXT_MAIN }}>填多少玩家就多多少</b>
        (除非那一列標了上限)。
          ...
          return (
            ...
            >
              ...
              {/*
                ...
                因此變成 **0**。旁邊就寫著「出貨預設 650」,所以「清除」讀起來像
                「回到 650」,實際上是「拿走 650」。名字現在說的是它真正做的事,
      ...
      <div style={{ marginTop: 16, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        {confirmRevert ? (
          <>
            <span style={{ color: WARN, fontSize: 12 }}>
              移除整份覆蓋層條目,回到 repo 出貨版（{shippedNote}）。確定?
            </span>
