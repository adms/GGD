 ...
 *
 * ⛔ 刻意不是「把 textarea 換成一個勾選清單」：那會把一份 71 列的表塞進兩頁裡，而且
 * 貼上／整批替換（操作者真正在做的事）會變得不可能。
 *
 * ⚠️ 名字有**兩個**來源，缺第二個那一頁就答不出多數列：出貨的 `champions` 集合，
 * 以及 `_legacy/champions/`（下架名單 7 個 id 裡有 6 個只住在那裡）。後者沒有
 * `_index.json`，所以是**按 id 直接取**、只對清單上真的出現過的 id 取一次。
 *
 * 邏輯全部在 `../championLabels`（測試住在那邊），這裡只有排版與取資料。
 */
import { useEffect, useMemo, useRef, useState } from "react";
import { ACCENT, DANGER, GOLD, PANEL_BORDER, TEXT_DIM, TEXT_MAIN } from "./theme";
import { loadCollection, loadLegacyChampionRows } from "../content";
...
/**
 * 載入英雄名稱索引。`wanted` 是頁面目前清單上的 id（用來決定要不要去 `_legacy/` 補）。
 *
 * ⚠️ 讀不到不是致命的：頁面照樣編輯、照樣存檔，只是列上少了名字（並且會**說出來**
 * ——⛔ 不是安靜地退回只有 id，那跟修好之前長得一模一樣）。
 */
export function useChampionLabelIndex(wanted: readonly string[]): ChampionLabelIndexState {
  const [rows, setRows] = useState<ContentRow[]>([]);
  const [legacyRows, setLegacyRows] = useState<ContentRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  /** 已經去 `_legacy/` 問過的 id —— 一個 id 只問一次（打字時每一鍵都會重跑）。 */
  ...
  const index = useMemo(() => buildChampionLabelIndex(rows, legacyRows), [rows, legacyRows]);

  // 出貨集合裡沒有的 id → 去 `_legacy/champions/<id>.json` 補一次。
  const wantedKey = [...wanted].sort().join(",");
  useEffect(() => {
    if (loading) return;
    const missing = wantedKey
      .split(",")
      .filter((id) => id !== "" && !index.has(id) && !asked.current.has(id));
    if (missing.length === 0) return;
    for (const id of missing) asked.current.add(id);
    void loadLegacyChampionRows(missing).then((found) => {
      if (found.length > 0) setLegacyRows((prev) => [...prev, ...found]);
    });
  }, [wantedKey, loading, index]);
...
function LabelRow({ label }: { label: ChampionLabel }): JSX.Element {
  ...
  return (
    <li style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "1px 0" }}>
      ...
      {label.legacy && <span style={{ color: GOLD, fontSize: 11 }}>{LEGACY_NOTE}</span>}
    </li>
  );
}
