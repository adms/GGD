...
import { sortModelVersions, modelSourceTier, MODEL_SOURCE_LABELS, MODEL_SOURCE_ORDER, type ChampionModelVersionState, type ModelVersionCommand, type ModelVersionSource } from "@ggd/shared/content/schema/championModelVersions";
...
export function ChampionModelVersions(props: {
  ...
  return <Panel title="上線模型版本">
    <div style={{ display: "grid", gap: 10 }}>
      <label>選擇上線套用的 3D 模型
        <select aria-label="上線模型版本" style={inputStyle} disabled={locked} value={selected} onChange={(event) => setSelected(event.target.value)}>
          ...
          {sortModelVersions(state?.versions ?? []).map((entry) => <option key={entry.modelKey} value={entry.modelKey}>
            {MODEL_SOURCE_LABELS[modelSourceTier(entry.source)]} · {entry.label} · {KIND_LABELS[entry.source.kind]}{entry.modelKey === state?.preferredModelKey ? " · 預設首選" : ""}{entry.modelKey === state?.activeModelKey ? " · 使用中" : ""}
          </option>)}
        </select>
      </label>
      ...
      <div style={{ color: TEXT_DIM, fontSize: 12 }}>預設順序：300英雄 ＞ MBA ＞ 原版 ＞ 借用 W3X。現在為{state?.selectionMode === "manual" ? "手動選用" : "自動選用"}。</div>
