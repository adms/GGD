...
cmd_verify() {
  local snap="${1:-}"
  if [ -z "$snap" ]; then
    # ⛔ `|| true` 不是裝飾：`set -euo pipefail` 下，glob 沒中時 `ls` 非零
    #   ⇒ 命令替換失敗 ⇒ ⭐ **腳本在 die 之前就靜默死掉**（exit 1，零輸出）。
    #   ⚠️ 實測過：一個「沒有備份可驗」的情況，長得跟「腳本壞了」一模一樣。
    snap=$(ls -1dt "$DEST"/redis_* 2>/dev/null | head -1 || true)
