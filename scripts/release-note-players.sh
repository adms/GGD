...
if [ -z "$LINES" ]; then
  ...
  elif [ "${SHIPPED_N:-0}" -gt 0 ] && [ "${GGD_PLAYERNOTE_NO_GH:-0}" = 1 ]; then
    # ⭐ GH#1167 —— **第三個狀態**：離線（沒查票）⇒ ⛔ 既不能發（沒查票就發＝假話），
    #   ⛔ 也不能宣告「沒有玩家可見改動」（那也是假話）⇒ 公告**待補**，這一輪不發、不記帳。
    #   在此之前這條路依定義必 exit 1（NO_GH ⇒ 零票句 ⇒ 撞到下面那個「假話不發」）。
    echo
    echo "⚠️ 離線（GGD_PLAYERNOTE_NO_GH=1）：這一版有 **${SHIPPED_N} 顆玩家面向的 commit**，⛔ 但沒查票 ⇒ 玩家公告**待補**。"
    echo "   ⇒ 這一輪**不發、不記帳**（沒查票就發＝假話；宣告「沒有玩家可見改動」也是假話）。連得上 gh 再跑一次。"
    if [ "$POST" -eq 1 ]; then
      echo "⛔ 離線模式拒絕 --post（exit 4 = 待補，⛔ 不是失敗也不是成功 —— ship-it 會把它記成 discord 沒發）" >&2
      exit 4
    fi
    exit 0
