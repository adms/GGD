...
write)
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      #   ⛔⛔ **在此之前這一行寫的是「沒有玩家看得到的改動就不要填 —— 空白是誠實的答案」**,
      #   ⭐ 而那正是缺陷的來源（2026-09-09）：留空之後,消費端分不出
      #   「**人答過：玩家看不到**」與「**沒有人回答**」——⇒ 玩家公告的閘變成答不出來的,
      #   而 v0.41.0–v0.42.15 **21 版全部退化成罐頭句子**。
      # ⇒ ⭐ 玩家看不到就**明寫**：`--player "無（<為什麼看不到>）"`。
      #
      # ⛔⛔ **而「玩家看得到」⛔ 不只是戰鬥和按鈕**（owner 2026-09-09 逐字舉的例）：
      #   · 動畫更順了嗎          · 網路更快響應了嗎
      #   · 編輯器多支援什麼      · 哪些角色**設計好了正在審查**
      #   · 哪些角色**上架成功**   · 哪些角色**換了造型**
      #
      # ⚠️ ⭐ 為什麼要把這六類寫在這裡：帳本上的歷史句子**本身是偏的** ——
      #   117 列裡 63 列罐頭，54 句真內容中介面 14 · 戰鬥 13，
      #   ⛔ 而「角色上架／造型」只有 **1** 句、編輯器 3、效能 3。
      #   ⇒ ⭐ 讀著它寫下一句的人，會把「玩家看得到」學成「戰鬥或按鈕」——
      #     **我就是這樣把 v0.41.5 的 26 顆玩家面向 commit 發成「系統優化更新」的。**
      #   ⛔ 留空 ＝ 沒有回答,⛔ 不是「答案是沒有」。
