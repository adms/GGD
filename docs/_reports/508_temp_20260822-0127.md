...
## ⛔ 事故紀錄：我的 commit 掃進了 K7 lane 的四個檔

`git add <逐檔>` 是對的，但接下來的 `git commit -F -` **沒有帶 pathspec** ——
git commit 沒有 pathspec 時提交的是**整個 index**，而在我 `git add` 與 `git commit`
之間，#506/K7 lane 把它自己的檔 stage 了進去。結果 `9abccba6` 多帶了四個檔：

```
apps/client/src/ui/ControlLegend.tsx
apps/client/src/ui/controlLegendModel.ts
apps/client/src/ui/controlLegendModel.test.ts
docs/_reports/506_temp_20260822-0128.md
```

⭐ **沒有任何東西遺失** —— 那四個檔的內容完整進了 git，只是掛在錯的 commit 訊息底下。

⛔ **我沒有做 `git reset --soft` 去修**：那會在另一條 lane 可能正在 `git commit` 的
同一秒動 HEAD，而 `HEAD~1` 在那個競態下可能已經是**別人的 commit**。
一個錯的 commit 邊界是紀錄問題；一次搶跑的 reset 是真的損失。⇒ 留給主 session 決定。

⭐ **教訓（給 CLAUDE.md 第零守則⑨/平行工作流那一節）**：
`git add <逐檔>` **不夠**，因為 index 是**全域共用的**。
⇒ commit 一律 `git commit <逐檔列名> -F -`，pathspec 要出現在 **commit** 那一行，
⛔ 不是只出現在 add 那一行。這是今天第四次同型事故（前三次記在 CLAUDE.md ⚡④）。
