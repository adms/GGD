...
| **s2_fuse_fires** | 0 | 19,109 | 54,591 | 保險絲跳（tick **64**）：sim 真的發了 **2** 則「脫困」`floatingText`，出貨 `VfxSystem` 的池裡有 **2** 個活著的字 —— ⛔ **而畫面上一個像素都沒有**（見紅字） |
| s3_phasing_0_5s | 0 | 24,082 | 61,370 | 放行窗內：軟分離對這一對停手，兩具身體開始**互相穿過** |
| s4_passed_through | 0 | 24,082 | 61,370 | 穿過去了：A x=**-38.00**（出發時 B 在 -42.8）、B x=**-47.96**（出發時 A 在 -44）—— 兩人都走到對方的另一邊 |
