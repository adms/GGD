...
**Main 仍依 [#1115](https://github.com/adms/GGD/issues/1115) 接手 M01–M11 整合、G1–G7 驗證與合併。** 替代分支保留原產品提交，以 `9acf806c5`（大型封存加入前）為起點，完整帶回後續文字與工具；尚未合併當前 Main。請先刷新雙方 SHA，將 Main 合併到替代分支，提供可供集中驗收的整合 SHA／服務版本；不能 merge 原 PR #1112 來取得文件，否則大型 blobs 會重新進入歷史。下文 pinned 衝突與遠端查詢紀錄仍是當時診斷，不能當作當前已整合的版本。
