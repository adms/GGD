...
Main 已在 [PR 一般留言](https://github.com/adms/GGD/pull/1112#issuecomment-5578873707) 提出大型材料進 Git 的儲存與部署成本，尚未同意合併；沒有正式 review 紀錄不代表沒有回覆。已請 Main 先在隔離分支完成來源整合，再提供整合 SHA／服務版本供集中驗收。材料保存範圍以 manifest 為準，只含本批選用材料及證據，並未搬入整座外部素材庫；儲存形式如需調整，須保留實際檔案、固定雜湊與還原入口。
