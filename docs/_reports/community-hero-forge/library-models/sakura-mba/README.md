...
為準；本收據保留的是候選量測與當次驗證證據，不另維護門檻副本。
...
候選實測貼圖為 1024×1024，最重單段動畫為 147 通道；舊收據曾依當時門檻把兩者列為效能警示，該歷史門檻不再視為現行契約。採用前須依中央政策重新檢查。Khronos 當次驗證的 20 項警告也保留在 `preparation-and-validation.json`：來自 Assimp 的 skinned mesh 節點變換與父節點用法，須配合畫面確認。
