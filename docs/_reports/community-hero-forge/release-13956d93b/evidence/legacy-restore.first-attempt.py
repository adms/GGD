...
content=root/'legacy-content';assert not content.exists();shutil.copytree(root/'content',content)
...
env.update({'PORT':'8823','HOST':'127.0.0.1','GGD_CONTENT_DIR':str(content),'GGD_CONTENT_BACKUP_DIR':str(root/'legacy-backups'),'GGD_EDITOR_ORIGINS':'http://127.0.0.1:5197','GGD_BUILD_STAMP':report['codeCommit']})
...
try:
 ...
 after=hashes()
 otherHeroes=[f for f in before if ('/champions/' in '/'+f or f.startswith('champions/')) and f.endswith('.json') and not f.endswith('_index.json') and f!=str(heroFile.relative_to(content))]
