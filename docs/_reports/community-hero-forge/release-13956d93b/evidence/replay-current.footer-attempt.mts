...
import { verifyCommunityRoomManifest, buildCommunityRoomContent, captureCommunityContentBase } from "/Users/Takuro/Dropbox/我的 Mac (Moriya.local)/Documents/ABxVFX_EDIT/GGD-community-hero-forge-s3/packages/shared/src/content/communityRoom";
...
try {
  ...
  const loaded = await new ContentLoader(new OverlayContentSource(new FsContentSource(process.env.CONTENT_DIR ?? resolve(root, "content")), currentOverlay)).load({ policy: "fail-closed" });
  ...
  // The recorder labels the result of step N with N, while the transport
  // clock keeps advancing after settlement. Validate the sealed footer instead.
  const footer = detail.summary?.footer;
  assert(footer && !detail.truncated, "complete recording requires a sealed footer");
  assert.equal(lastTick, footer.finalTick);
  assert.equal(footer.faultCount, 0);
  ...
  proof.recordingBoundary = { footer, settlementTick: lastTick + 1, postSettlementTransportTick: source.recordedThroughTick };
