...
const out=path.join(root,'publication-37');await fs.mkdir(out);
let token='';
...
for(const row of submissions.rows){
 ...
 try{
  ...
  const decision=await(await api(`/admin/hero-submissions/${row.submissionId}/decide`,{status:'approved',reason,expectedRevision:view.publication.revision,problems:[]})).json();
  const approved=await(await api(`/admin/hero-submissions/${row.submissionId}`)).json();assert.equal(approved.status,'approved');assert.equal(approved.decision.decidedBy,actor.account.id);
  const control=await(await api(`/admin/hero-submissions/${row.submissionId}/publish`,{operationId:result.operationId,action:'publish',reason:'在授權隔離環境驗證普通正式名單、固定版本與完整對局',expectedRevision:approved.publication.revision})).json();
  ...
  const final=await(await api(`/admin/hero-submissions/${row.submissionId}`)).json();assert.equal(final.status,'published');
  const dir=path.join(out,String(row.number).padStart(2,'0'));await fs.mkdir(dir);await fs.writeFile(path.join(dir,'decision.json'),JSON.stringify(decision,null,2)+'\n');await fs.writeFile(path.join(dir,'publication.json'),JSON.stringify(control,null,2)+'\n');
