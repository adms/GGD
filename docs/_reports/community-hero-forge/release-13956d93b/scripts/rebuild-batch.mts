...
const out=path.join(root,'rebuilt-37-attempt2');
...
for(const row of input.rows){
 ...
 try{
  ...
  assert.match(row.archiveSha256,/^[a-f0-9]{64}$/); assert.equal(await binarySha256(oldBytes),'sha256:'+row.archiveSha256);
