...
已补上同來源詛咒所需的施法者歸屬基礎：`applyStatus`／`applyBuff.sourceScope`、具名狀態條件 `appliedBy`、獨立層數／到期／回放摘要與 Editor 控制項；38 項引擎／回放檢查及完整 Editor 553 項／build 通過。見 [caster-status](caster-status/README.md)。它尚未串成阿薩謝爾 R→EX：接下來仍需原子資源扣除、詛咒移除與成功／缺少狀態的分支，以及反噬、事件與演出。
