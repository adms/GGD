...
- 實作分為五個 commits：`59edef71` 共用核心、`816a6cdb` 正式 importer、`ccdf7fa4` Platform 權限／發布、`fdc97183` Editor／Admin／Desktop、`92d6601a` 遊戲鎖版／回放。較早的測試於同一工作樹提交前執行；基準 commit 不是實作完成 commit。
...
| I1 單一核心 | 唯一 HeroProject、六槽、重複 Product、鎖定、逐級值、原文、演出腳本；完整 Main 模擬基線；可調整的試玩情境；重複 Product 的條件可用真實控制項獨立調整；153 顆 React 表單逐顆操作收據，其中 152 可用 | 最新 Main 相容性、46 文件逐份視覺驗收；`tpl-dragon-shockwave` 仍不可用 |
...
| I6 收斂交付 | 三門檻一起執行並保存失敗；真 Docker build、Helm／Compose render；桌面穩定來源、關閉保存協定、備份與可信更新驗證；macOS universal 未簽署安裝包 | 三門檻全綠、8 組 E2E、受影響 46 文件逐份視覺證據、Windows／macOS 簽署安裝及更新／降級 |
...
這些測試數目有重疊，不能相加成一次完整 suite；來源仍是未提交工作樹。型別檢查及最新三門檻結果另行更新，未完成者不記為通過。
...
CUA 的 in-app browser 已可使用；原生 macOS 視窗操作回覆「Computer Use permissions are not granted」。Windows 實機、簽署安裝與跨版本升降級仍無本次證據。macOS 本機量到 0 個有效簽署 identity；GitHub repository secrets 名稱清單為空，未假設可用的簽署來源。Windows x64 NSIS／portable 交叉建置已成功，但不是 Windows 實機執行證據。

`skillforge:visual-review:check` 或 46 張接觸表 freshness 通過，只證明既有材料仍新鮮，不表示本次已逐份人工視覺驗收。所有發布、跨平台及未完成項持續維持未完成狀態。
...
交付前刷新到 `origin/main@c71941180433d4fa89a68365b169990680d877d7`。相對初始基線有 772 個路徑變動，其中 11 個與本分支實作相交，涉及 resolver、VFX subtype／演出、模板、Hero schema、Admin 表單與 lockfile。不能把舊基線證據當成這些新契約已相容；下一步在本整合分支完成接縫合入與受影響重驗，Main 的分支維持只供審查／合併。
