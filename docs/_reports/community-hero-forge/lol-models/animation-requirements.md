...
後台可從「內容·素材管理 → 模型管理」開啟模型文件；「動畫對應 (clipMap)」表單直接列出這六個核心欄位。
...
以下名稱均存在於實際轉出的 GLB。七位已接到隔離的 GGD 英雄工坊並檢視待機與 Q 施法畫面；正式套件與遊戲對局的模型綁定尚未驗收。
...
`ggd-runtime-candidate/` 保留上表同樣 42 段，總計 14,132,584 bytes。只移除不影響任何可見網格、帶權重骨架及其祖先的節點動畫，並將所有保留片段均相同的節點屬性存成預設值；幾何、骨架綁定與貼圖不變。這份候選採用 GGD 的靜態附著點，不保留 LoL 特效定位骨的完整動畫；原始素材仍在備份。
...
Babylon 實際蒙皮頂點比較涵蓋 714 個片段取樣點、210 組不同片段間的切換；七位均通過，最大頂點差異為初始待機高度的約 0.0324%。這是抽樣姿態比較，尚非逐動作畫面或遊戲內驗收；受擊／格擋／閃避缺項與正式模型綁定仍待完成。原始收據及可重跑腳本存於 `runtime-candidate/`。英雄工坊的 14 張待機／施法畫面及後台核准往返紀錄另存於 `forge-preview/`，未宣稱完整美術核准。
