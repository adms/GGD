...
使用者進一步確認：**模板、生成器及成品都要版本控制，英雄須由模板生成後微調；不能只修成品。** 已確認現有 `heroForge/generator.ts` 與 `planner.ts` 仍保留動態 `inheritDefaults` 路徑。後續須把模板版本固定、生成器來源版本及英雄微調串接到生成／再生成／投稿的同一條流程，不能只以 catalog 還原功能宣稱完成。模板與生成器更新不自動變更其他英雄；採用新版必須生成各英雄自己的版本。

**尚待補齊：模板／生成器／英雄版本來源串接、既有英雄後台還原畫面驗收、正式 Go 覆蓋層版本整合，以及實際對局／重連／回放。** 不增加無關模板來替代上述驗收。
