...
⚠️ 前置更正：#623 的 v2 報告那一欄的表頭逐字寫著「（今天畫面上真的存在的）」
⇒ ⭐ **它只量了視覺軸，從來沒讀過卡面文字**。⛔ 不要照那份報告的顏色做。
下面是**逐對讀完卡面文字**之後的結論。

### 3.1 ⭐ 票文沒說的第一件事：**20 對裡 6 對已經退場了**

`content/config/roster.json` 的 `retiredChampions` 今天有 **12 個 id**，其中 6 個是變身態：
`godie-e007`（12-08）· `godie-h020` · `godie-h02u` · `godie-n01c` · `godie-u010`（#552，08-22）
· ⭐ **`godie-e00x`（剎那）—— commit `21c218fd`，就是今天、就是掛在 #623 底下退的**。

⇒ 「一個變身態都還沒下架」這句票文**已經不成立**。
⇒ ⭐ **退場的動作是改 `roster.json` 的一行**，⛔ 不是搬檔案
（搬檔案會踩 `formPairShipping.halfMigrated` ⇒ `Registry.get()` 在 snapshot builder 裡每秒 throw 30 次）。
⚠️ `content/config/roster.json` **在本 lane 的柵欄外**。

### 3.2 ⭐ 逐對讀卡面之後：**剩下的 14 對裡，今天只有 1 對可以退**

| 判定 | 對數 | 是哪幾對 |
|---|---:|---|
| ✅ **已退場** | 6 | e007 · h020 · h02u · n01c · u010 · e00x |
| ⭐ **可以退（今天）** | **1** | **#6 `godie-h01n` ⇄ `godie-h01o` 一護 卍解** |
| 🔴 退了卡面就變謊話 | 4 | e00n 禮奈 · h02r 妙蛙花 · n01g 依文潔琳 · u01u 索隆 |
| 🔴🔴 **入口變空殼**／**3D model 真的不一樣** | 7 | e00l Saber · e010 卡迪那 · n00p 藏馬 · o02l 皮卡丘 · o00x 悟空 · u034 傑 · u00l 北斗之鼠 |
| 🟡 半張卡會死 | 2 | o030 臭作 · u00o 魯夫 |

### 3.3 ⭐ 唯一可以退的那一對，以及**為什麼只有它**（逐項驗過）

| 證據 | 值 |
|---|---|
| `godie-h01n.r` 的 effects | `championForm(8s)` ＋ **`applyBuff{statusId:"bankai", duration:8}`** ＋ `modifyCooldown` |
| `content/status-effects/bankai.json` 的 tags | `["bankai", **"form"**, "mechanism-on-card", "buff"]` |
| `sim/formGate.ts::inAlternateForm()` | 身體是 index 1 **或** 帶 `FORM_STATUS_TAG("form")` ⇒ **兩條路，OR** |
| `godie-h01n.ex` 的 `whileForm:"alternate"` | ⭐ 退場後仍然成立（走 status 那一條） |
| `godie-h01n.e` / `.w` 的「卍解狀態下追加」 | 走 `condition{kind:"status", statusId:"bankai"}` ⇒ 不受影響 |
| 靈壓光環 | ⭐ **本體自己的 `godie-h01n.passive` 就有**（降敵攻速減半）—— ⛔ 不是只住在變身態上 |

⇒ ⭐ **「退場後那支技能仍然做得到同一件事」對這一對是可證的。**
⚠️ 唯一的損失：變身態那份光環另外帶「移速 −10%」與「初始法力較高」——
⛔ **本體的卡面從來沒宣稱過這兩樣** ⇒ 沒有卡面變謊話，只有一點點強度流失。

**⇒ 建議的一行改動（柵欄外，主 session 執行）：**
`content/config/roster.json` 的 `retiredChampions` 追加 `"godie-h01o"`，然後
`npx vitest run packages/shared/src/ops/transformRetireGate.test.ts packages/shared/src/sim/systems/transformEntryRetired.test.ts`。

### 3.4 ⭐ 為什麼另外 13 對退不了 —— 三個各自獨立的理由

**① 卡面點名了那份文件本身**（最極端，1 對）：
`godie-ogrh.e`（09-03 超級賽亞人）的卡面逐字寫著
「變身期間換成**【超級賽亞人 - 悟空】的身體**：攻擊速度 ×1.58…移動速度 ×1.28…」
—— 而 `【超級賽亞人 - 悟空】` **就是 `content/champions/godie-o00x.json` 的 `name` 欄**。
⇒ 退場 ＝ 那一整句無條件變成謊話。

**② 入口是空殼**（4 對）：`godie-nsjs.e`（妖狐變化）· `godie-ofar.r`（瘋狂皮卡丘）
· `godie-ucrl.r`（傑桑變化）的 `effects` **只有 `championForm` 一格，⛔ 別的什麼都沒有**；
`godie-umal.r`（ChangeDNA）只多一個 `spawnModelFx`。
⇒ 退場後那顆按鈕**逐位元等於沒有效果**，而卡面列著三到四條機制。
（`packages/shared/src/ops/transformRetireGate.test.ts` 正是為這個而存在，它會紅。）

**③ 3D model 真的不一樣**（4 對 —— ⛔ 直接推翻 owner 立 #552 時的前提）：

| 對 | base modelKey | alternate modelKey |
|---|---|---|
| 藏馬 `nsjs`⇄`n00p` | `imported.fox2` | `imported.fox` |
| 皮卡丘 `ofar`⇄`o02l` | `imported.heropikachu` | `imported.picacugy` |
| 傑 `ucrl`⇄`u034` | `champ.thorne` | `imported.herobiggon` |
| 拳四郎 `umal`⇄`u00l` | `champ.skin.barbarian` | `imported.heropikachu` |

⚠️ 另有 3 對 `modelKey` 相同但 `bodyScale` 會變（臭作 無→**3**、妙蛙 0.62→無、卡迪那 1.1→無）
—— 對玩家來說那也是「身體變了」。

### 3.5 ⭐ 另外 2 對（Saber · 卡迪那）有一條**已經被證明過**的解鎖路徑

它們是 repo 裡僅有的、`whileForm:"alternate"` 而**沒有** `form` 狀態的兩對，
而且**整支技能的強度都住在裡面**（`godie-e002.w` 風王結界的 1.4/1.6/1.8/2 倍暴擊、
`godie-e00s.passive` 紮根的 力量+10 與 芬多精友軍光環）。
⇒ 退場 ＝ payload 靜靜蒸發，⛔ 沒有任何東西會紅（`abilityPassives.ts:157` 自己記著這個失敗形態）。

⭐ **解鎖法與一護一模一樣**：出一份 `status-effect@1`、`tags` 含 `"form"`、
由入口技能的 `applyBuff.statusId` 掛上去 ⇒ 那 4+1 個 `whileForm:"alternate"` 區塊
**一個字都不用改**。前例已經有兩份：`bankai.json` 與 `witch-form.json`。
⚠️ `content/status-effects/` **在柵欄外** ⇒ 我沒有做。

### 3.6 ⚠️ 兩個閘的盲區（開票時要知道）

- `abilityCodeParityForms`：`description` 屬於 `COSMETIC_FIELDS` ⇒ ⭐ **單邊的說明漂移它看不見**。
  已量到一筆：`godie-nbbc.ex` 與 `godie-n01c.ex` 的「防禦 ×2」那一條 bullet 內容不同。
- `retiredChampions` **不搬檔案** ⇒ 兩半都還「在架上」⇒ parity 閘會**永遠**要求兩邊一起動。

### 3.7 ⛔ 本 lane 對 #623 的可執行量 ＝ **0**

退場動作住 `content/config/roster.json`；解鎖動作住 `content/status-effects/`。
**兩者都在柵欄外。** ⇒ 我只交結論與證據，⛔ 沒有動任何檔。
