...
full/sliding causal masks are preserved. A custom VJP recomputes each block
using MLX's own autodiff, accumulating K/V gradients in float32. All source and
...
def frozen_mlp_policy(mx,layers,*,block_size,reference=False,observer=None):
    """Patch only supplied dense frozen MLP instances for this synchronous call."""
...
def make_attention(mx, *, scale, window=None, block_size=256, observer=None):
    ...
    def attention_vjp(primals, cotangent, output):
        ...
        for begin in range(0, q.shape[-2], block_size):
            ...
            lo, hi = bounds(begin, end, k.shape[-2])
            def local(qb, kb, vb): return block(qb, kb, vb, begin, lo)
            _, gradients = mx.vjp(local, [q[:, :, begin:end], k[:, :, lo:hi], v[:, :, lo:hi]],
                                  [cotangent[:, :, begin:end]])
            gq, gk, gv = gradients
