...
import {collect} from './r7-final-report-v2.mjs';
...
export function audit(root){
 const final=collect(root),persisted=read(path.join(root,'final-evidence-v1/summary.json'));
 ...
 const names=['test-report.json',...['source-63-regression','previous-source-72','current-main-v4','hero-140','fidelity-114','vfx-118','composition-regression-20','source-clarification-15'].map(n=>'post-v1/'+n+'-report.json')];
...
function render(r){
 const lines=['# R7 結果逐題複核待辦','','本文件由原始結果產生，**不代表每題已親自複核**；不更改分數、答案、門檻或選模。','',`選定step ${r.selectedStep}；${r.newAdapterSelected?'有選中本輪權重':'保留原R3，不能當新微調收益'}。`,'','每組獨立列出對R3的变化；「新增錯誤放行」可能出現在前後都答錯的題，不能只看退步數。','','| 題組 | 修正 | 答對變錯 | 新增錯誤放行 | 仍錯誤放行 | 仍答錯 |','| --- | ---: | ---: | ---: | ---: | ---: |'];
