...
import {collect,render} from './r6-final-report.mjs';
...
const predecessors=[
 ...
 {file:'manual-entry-v3/state.json',done:'complete-research-evidence',pending:['waiting-for-core-and-post','running']},
];
...
async function main(root,runtime){
 ...
 const pinPaths=[self,path.join(path.dirname(self),'r6-final-report.mjs'),policyPath,path.join(root,'dataset-manifest.json')];
