/** One-shot continuation of a live R3 run. No training, checkpoint selection or promotion. */
...
const dir=path.dirname(fileURLToPath(import.meta.url)),out=path.join(root,'post-diagnostics-v1');assert(!fs.existsSync(out),'REFUSE_RESTART');
...
const datasets=['main-diagnostic-v1','fidelity-diagnostic-v2'].map(name=>{
...
const pinFiles=[policyPath,path.join(root,'run-pins.json'),path.join(root,'main-catalog-review.json'),...datasets.flatMap(d=>['requests.json','cases.private.json','manifest.json','token-budget.json'].map(n=>path.join(d.dataRoot,n))),...['worker.py','r3-score.mjs','classification-score.mjs','r3-fidelity-client.mjs','r3-fidelity-diagnostic.mjs','r3-data.mjs','dataset.mjs'].map(n=>path.join(dir,n)),path.join(root,'fidelity-diagnostic-v2/bundle.private.json')];
...
try{
 ...
 fs.writeFileSync(lock,JSON.stringify({pid:process.pid,root:out,kind:'r3-post-diagnostics'}),{flag:'wx'});owned=true;
 ...
 put('comparison.json',{scores,paired:Object.fromEntries(datasets.map(d=>[d.name,{r3VsBase:paired(scores[d.name].base,scores[d.name].r3),r3VsR2:paired(scores[d.name].r2,scores[d.name].r3)}])),releaseQualified:false,automaticActivation:false,checkpointReselected:false,scope:'Supplementary diagnostic only; main version adaptation and full-proposal checklist omissions. Does not replace hero/Owner gold or R3 held-out results.'});
