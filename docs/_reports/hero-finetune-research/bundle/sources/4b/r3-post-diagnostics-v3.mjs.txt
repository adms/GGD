/** Corrected one-shot continuation after receiver and cast-path audits; replaces cancelled v1/v2 diagnostics. No training, checkpoint selection or promotion. */
...
const dir=path.dirname(fileURLToPath(import.meta.url)),out=path.join(root,'post-diagnostics-v3');assert(!fs.existsSync(out),'REFUSE_RESTART');
...
const datasets=['main-hero-diagnostic-v1','fidelity-diagnostic-v2','main-diagnostic-v3'].map(name=>{
 ...
 if(name==='main-diagnostic-v3')assert.equal(manifest.catalogReviewSha256,digest(read(path.join(root,'main-catalog-review-v3.json'))));
 if(name==='main-hero-diagnostic-v1')assert.equal(manifest.inventorySha256,digest(read(path.join(root,'main-hero-inventory-v2.json'))));
 return {name,dataRoot,cases};
});
const pinFiles=[fileURLToPath(import.meta.url),policyPath,path.join(root,'run-pins.json'),path.join(root,'main-catalog-review-v3.json'),path.join(root,'main-pull-throw-probe.json'),path.join(root,'main-data-corrections-v2.json'),path.join(root,'main-growth-receiver-probe.json'),path.join(root,'main-hero-inventory-v2.json'),...datasets.flatMap(d=>['requests.json','cases.private.json','manifest.json','token-budget.json'].map(n=>path.join(d.dataRoot,n))),...['worker.py','r3-score.mjs','classification-score.mjs','r3-fidelity-client.mjs','r3-fidelity-diagnostic.mjs','r3-data.mjs','dataset.mjs'].map(n=>path.join(dir,n)),path.join(root,'fidelity-diagnostic-v2/bundle.private.json')];
...
try{
 ...
 fs.writeFileSync(lock,JSON.stringify({pid:process.pid,root:out,kind:'r3-post-diagnostics-v3'}),{flag:'wx'});owned=true;
