/** Research-only native recipes. v1 failures are retained, not re-scored. */
...
for(const seed of [20260908,20260909]){
  ...
  for(const initialFraction of [0.5,0.99])rows.push(engineProbe(mana.compiled,catalog,
    initialFraction===0.5?'mana-adds-max-fraction-not-sets':'mana-overflow-clamps-at-max',r=>{
      // v1 incorrectly subtracted 10,000 from regen; negative mana regen drains.
      // Use the stat pipeline's existing override and verify the resulting fixture.
      attachSource(r.world,r.caster,{id:'zero-mana-regen',kind:'item',modifiers:[{stat:Stat.ManaRegen,op:ModOp.Override,value:0}]});
      recomputeStats(r.world,r.caster);assert.equal(r.world.stats.get(r.caster).final[Stat.ManaRegen],0);
      ...
      near(h.mana,expected,'MANA_AFTER_ZERO_REGEN');near(r.world.health.get(r.foe).mana,other,'WRONG_TARGET');
      ...
      return {initialFraction,before,after:h.mana,maxMana:h.maxMana,castCost:cost,requested,
        restored:h.mana-before+cost,overflow:restores[0].data.overflow,zeroRegenVia:'stat override, not large negative flat'};
    },seed));
...
const summary={schema:'ggd-research-ir-extension-admission@2',createdAt:new Date().toISOString(),
  ...
  v1Corrections:{priorRunRetained:'extension-admission-v1',priorScore:'4/8',
    mana:'A negative flat regen modifier caused real per-tick mana drain. Immediate v1 restore was already correct; v2 checks immediate value and zero-regen override.',
    ...
    manifest:'The v1 existingEngineSupports field was premature; this version derives per-capability verified flags only from passed probes.'},
