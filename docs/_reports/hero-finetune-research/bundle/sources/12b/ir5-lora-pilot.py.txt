"""Bounded masked full-IR engineering SFT and paired six-control inference.
...
def prepare(out):
    ...
    state = gpu.read(BASE_IR / 'state.json')
    assert state['status'] == 'completed-inference-only' and state['workerPid'] is None
    protocol, requests, prompts = gpu.prepare(BASE_IR)
    verified = gpu.read(HERE / 'ir5-stage-verification-v1/manifest.json')
    dataset = gpu.read(HERE / 'ir5-data-stage-v1/dataset.private.json')
    manifest = gpu.read(HERE / 'ir5-data-stage-v1/manifest.json')
    tm = gpu.read(HERE / 'ir5-mask-tokens-v1/manifest.json')
    all_tokens = gpu.read(HERE / 'ir5-mask-tokens-v1/tokens.private.json')
    assert gpu.json_hash(dataset) == manifest['datasetSha256'] == verified['datasetSha256']
    assert gpu.json_hash(all_tokens) == tm['tokenizedDataSha256'] == verified['tokenDataSha256']
    assert verified['passedTests'] == 66 and verified['supervisedCastFields'] == 168
    for name, expected in manifest['checkerPins'].items():
        assert gpu.digest(HERE / name) == expected, 'DATA_CHECKER_DRIFT'
    ids = {r['id'] for r in dataset if r['split'] == 'engineering-train-candidate'}
    tokens = [r for r in all_tokens if r['id'] in ids]
    assert len(tokens) == 8 and len(ids) == 8
    assert not {r['heroId'] for r in tokens} & {r['id'] for r in requests}
    assert all(len(r['ids']) <= 8192 and r['directLossTokens'] == sum(r['labelMask']) for r in tokens)
    evaluation = {'whole_hero': {'baseRun': BASE_IR.name, 'protocol': protocol, 'requests': requests, 'prompts': prompts}}
    checker_files = sorted(set(protocol['checkerFiles'] + ['semantic_training_loss.py']))
    p = {'schema': 'ggd-ir5-masked-synthetic-lora-pilot@1', 'createdAt': time.time(),
        'purpose': 'bounded engineering experiment on full IR serialization/composition; NOT independent hero generalization',
        'steps': 16, 'saveEvery': 4, 'learningRate': 2e-5, 'numLayers': 2,
        'loraParameters': {'rank': 8, 'scale': 8.0, 'dropout': 0.0, 'keys': ['self_attn.q_proj', 'self_attn.o_proj']},
        'seed': 20260908, 'maxSequenceTokens': 8192, 'metalLimitGiB': 40, 'secondsMaximum': 1800,
        'stepSecondsMaximum': 120, 'modelDirectory': protocol['modelDirectory'], 'model': protocol['model'],
        'revision': protocol['revision'], 'datasetSha256': manifest['datasetSha256'],
        ...
        'guard': protocol['guard'], 'selection': 'fixed final step 16; no selection by loss, dev result or intermediate checkpoint',
        'trainingScope': {'heroes': 8, 'independentFamilies': 1, 'synthetic': True, 'engineeringSFTAdmitted': True},
        'evaluationScope': {'exposedRealControls': 2, 'sameFamilySyntheticDev': 4, 'independentBlind': 0},
        'fullHeroRecipeTrainingAdmitted': False, 'freshBlind': False, 'canPromoteModel': False,
        'knownDataLimitations': protocol['limitations'],
        'stopPolicy': 'checkpoint every four steps; no automatic retry, model promotion or data relabeling'}
    out.mkdir()
    gpu.atomic(out / 'manifest.json', p)
    gpu.atomic(out / 'tokens.private.json', tokens)
    gpu.atomic(out / 'evaluation.private.json', evaluation)
    gpu.atomic(out / 'preparation.json', {'cpuOnly': True, 'heroes': 8, 'tokenCounts': [len(r['ids']) for r in tokens],
        'directLossTokens': [r['directLossTokens'] for r in tokens], 'trainingStarted': False})
    return p
...
def worker(out, token):
    ...
    def loss_fn(net, ids, label_mask):
        return masked_next_token_loss(net, ids, label_mask, mx, nn)
    ...
    def teacher_losses(phase):
        ...
        for row in data:
            ...
            losses.append(loss_fn(model, row['ids'], row['labelMask']).item())
    ...
    for step, index in enumerate(order[:p['steps']], 1):
        ...
        loss, grads = gradient_fn(model, row['ids'], row['labelMask'])
        mx.eval(loss, grads)
    ...
    reset_loss = loss_fn(model, data[0]['ids'], data[0]['labelMask']).item()
    assert abs(reset_loss - baseline_losses[0]) < 1e-5, 'FROZEN_BASE_CHANGED'
    model.load_weights(list(saved.items()), strict=False)
    reload_loss = loss_fn(model, data[0]['ids'], data[0]['labelMask']).item()
    ...
    gpu.atomic(out / 'result.json', {'schema': 'ggd-ir5-masked-lora-pilot-result@1', 'steps': len(trace),
