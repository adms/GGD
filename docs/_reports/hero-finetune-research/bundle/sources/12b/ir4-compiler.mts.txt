/** IR4 research lowering. Legacy operations retain the frozen ir-compiler.mts behavior;
 * extensions compose in original action order. No live engine changes.
 */
...
export function lowerIR4(input: unknown, source: any) {
  const checked = validateIR4(input, source), { ir } = checked;
...
export function compileIR4(input: unknown, source: any, catalog = currentCatalog()) {
  const lowered = lowerIR4(input, source), { ir } = lowered;
