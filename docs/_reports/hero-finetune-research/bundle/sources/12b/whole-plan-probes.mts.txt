...
const shieldTotal=(r:any,id=r.caster)=>r.world.health.get(id).shields.reduce((n:number,s:any)=>n+s.amount,0);
...
export function wholePlanProbes(compiled:any,id:string,catalog:any,onlyNames?:string[]){
  ...
  const probe=(name:string,slot:string,fn:(r:any)=>any)=>{
    ...
    const row=engineProbe(compiled,catalog,name,fn,seed);rows.push({...row,slot});
  };
  for(seed of [20260908,20260909]){
    ...
    if(id==='community7-lux'){
      ...
      probe('W-keep-larger-and-expire','W',r=>{
        ...
        r.step(91);assert.equal(shieldTotal(r),0,'SHIELD_NOT_EXPIRED');return {preserved:greater,expiryTicks:91};
      });
      ...
      probe('EX-self-heal-and-three-second-speed-no-attack-buff','EX',r=>{
        ...
        assert(stat(r,Stat.MoveSpeed)>ms,'NO_SPEED_BUFF');near(stat(r,Stat.AttackSpeed),as,'EXTRA_ATTACK_SPEED');
      ...
      for(const slot of ['Q','R'])probe(`${slot}-finite-source-range`,slot,r=>{
        ...
        const outcome=r.cast(slot,{type:'point',point:{x:p.x+a.range+2,z:p.z}});assert.equal(outcome,'out-of-range');
        return {range:a.range,outcome};
      });
      probe('W-single-ground-explosion-magic','W',r=>{
        ...
        assert.equal(r.hits('W')[0].data.dmgType,'magic');return {hits:1,type:'magic'};
      });
