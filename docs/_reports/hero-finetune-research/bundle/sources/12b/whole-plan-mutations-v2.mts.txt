...
add('lux','EX-wrong-speed-tier',EX,a=>a.EX.effects[0].modifiers[0]={stat:'ms',op:'pctAdd',value:0.2,msBonusTier:'小'});
...
add('xerath','EX-zero-authored-cooldown-with-engine-floor','EX-adds-fifteen-percent-self-mana-and-keeps-cooldown',a=>a.EX.cooldown=[0],'engine-policy-probe-not-proven-source-negative');
...
const rows=definitions.map(d=>{
  ...
  // Probe harness intentionally registers champion with overrideAbilities:true.
  // Reassigned scalar/array fields must be synchronized into embedded mirrors.
  for(const slot of ['Q','W','E','R'])candidate.champion.abilities[slot]=structuredClone(candidate.abilityDrafts[slot]);
  ...
  return {id:d.id,hero:d.hero,probe:d.probe,claim:d.claim,mutationStage:'compiled-runtime-candidate',
    authorViewSchemaValid:true,runtimeResolvedMsValueStrippedInCheckOnlyCopy:true,sourceUnchanged:true,compilerNotReinvokedAfterMutation:true,coreMirrorsSynchronized:true,
...
const checkerFiles=['whole-plan-mutations-v2.mts','whole-plan-probes-v3.mts','whole-plan-seeds-v1.mts','ir-v4.mts','ir4-compiler.mts','probe-harness.mts'];
const manifest={schema:'ggd-whole-plan-mutation-audit@2',createdAt:new Date().toISOString(),
  previousAudit:'whole-plan-mutation-audit-v1',sourcePositiveRun:'whole-plan-candidates-v3',mutants:rows.length,caughtBothSeeds:rows.filter(r=>r.caughtBothSeeds).length,
