...
def prepare(dataset_dir, model_dir, output_dir):
    ...
    result = {'schema': 'ggd-semantic-token-mask-preparation@1', 'cpuOnly': True, 'modelWeightsLoaded': False,
        ...
        'heroes': len(prepared), 'rows': [{k: r[k] for k in ('id', 'promptTokens', 'completionTokens', 'directLossTokens', 'maskedCompletionTokens')} for r in prepared],
        ...
        'limitations': ['Masked values remain in teacher-forcing context, so indirect influence is not eliminated.',
            'Two historically exposed same-family controls are not a training corpus or blind evaluation.',
