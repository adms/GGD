...
[好運姐六槽准入報告](SOURCE_ADMISSION_REPORT.md)：兩個 seed 的 28 項正向情境中 26 項通過，剩餘 2 項均確認指定空地彈幕錯落於施法者周圍。七種錯誤配方皆由具有乾淨對照的探針檢出。候選在訓練前隔離，本批沒有 GPU 推論或訓練；不是模型 26/28 準確率。

前表好運姐 R 的離開／進入測試仍是當時的真實結果，但未涵蓋「沒有實體目標的指定空地」，因此不能將那些局部通過升格成完整 R 實作。本次追加空地反例，不改寫舊分數。
