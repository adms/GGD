...
3. **在 HEAD 無法重現 owner 看到的「零特效只剩文字」**：sim 發事件 ✅、preset 展開 ✅、
   ...
   部署版 alpha 缺陷 A/B 仍可見 ✅、**真比賽連拍全亮 ✅**。
   ⇒ owner 的「零特效」在 HEAD 的每一段都量不到 —— 最可能的解釋按嫌疑排序：
   ① 他玩的是**部署版**（v0.28.5/6 帶 #780 alpha 缺陷 + 未知的部署態差異），
   ② 線上後台 **override**（`config.vfx-cleanup@1` 一族會蓋掉 content 檔 —— 部署前先查），
   ③ 被 stub 的那幾個 sink（views/casts/相機/語音）在**他的**機器上擲例外吃掉批次
   （GH#608 形狀 —— 用詞紀律：這三個都是**假說**，⛔ 不是量到的）。
4. 下一版部署後的煙霧測試應加一條：真比賽施放龍破斬一次、目視光束與爆炸
   （或跑 beam-audition `?ability=godie-h020.e` 量 lit>0）。
