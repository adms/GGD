...
> ⛔⛔ **2026-09-04 更正:上面這一格的後半段是假的。**
> 用**括號配對解析器**（⛔ 不是只吃數字字面值的正則）重數全 war3map.j：
> ⭐ **35 個 `SetUnitScalePercent` 呼叫 · 其中 13 個非等向**（⛔ 不是 2 個）。
> 完整行號：26791 · 30395 · 30397 · 30841 · 31029 · 33122 · 36049 · 36051 · 43918 · 47942 · 49652 · 49890 · 49915。
> ⭐ 而 `200.00, 200.00, 400.00` **就在 war3map.j:26791** —— 只是它屬於
> `Trig_SunFire_Actions`（**90-04 陽光烈焰**），⛔ 不是龜派。
> ⇒ ⭐ CLAUDE.md 第一守則的「13/35 非等向」是**對的**；
>   而「龜派 200,200,400」那半句 CLAUDE.md 自己在 2026-08-28 就更正過了。
> ⚠️ ⭐ **為什麼會數錯**：35 個裡有 **19 個**的三軸是**運算式**（`( 250.00 + I2R(...) )`），
>   一個只解析數字字面值的正則只看得到 16 個 ⇒ 數出「2 個非等向」。
>   ⛔ 這正是 CLAUDE.md 記過的「一個被 glob 灌大／縮小的統計，讀起來跟真的一模一樣」。
