...
|---|---|---|---|---|
| `saber/` 5 張 | Saber（`E002` 亞瑟王） | **一道橫貫畫面的巨大金橘色光束**，由細變粗、末端呈扇形展開；施法者腳邊一顆白藍色核心爆點 | §1.1：`h00S` ReviveHuman `265–310%` ＋ `h008` FragDriller `365–410%` @動畫 15% 速度（⇒ 展開很慢 ⇒ 三張連拍看得到它在長大）；核心爆點 = 32317/32319 的 `NEDeathSmall`＋`NeutralBuildingExplosion` | ✅ **完全一致，且證明是 2 層不是 6 具** |
