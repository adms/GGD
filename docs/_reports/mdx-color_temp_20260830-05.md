...
1. ⛔⛔ **⚠️ 這一條在 2026-08-30 的複驗中被推翻，⛔ 不要照它做。**

   > 原文：「⛔ 不要再照 #803 的 Scope ① 去轉 PRE2 —— `revivehuman` 的 PRE2 是白的，轉了不會變金橘。」

   ⭐ **它是一個 non-sequitur**：「PRE2 的 `segmentColor` 三段全白」⇒ 推不出「顏色不在 PRE2」。

   ⭐ 量到的（我獨立驗過）：
   · 出貨 `content/assets/models/imported/revivehuman.glb` 有 **4 張圖**、⭐ 而有 **5 個材質**
     ⇒ **一個材質沒有貼圖**。
   · 複驗 dump 來源 MDX 的 `TEXS` ⇒ **5 張**，而少掉的正是 `Textures\Yellow_Star.blp`
     —— ⭐ 名字裡就寫著 Yellow。

   ⇒ ⭐ **顏色可能就在那張沒出貨的貼圖上。** PRE2 的 `segmentColor` 是**乘算的染色**，
   它全白只代表「⛔ 不額外染色」，⛔ **不代表底下那張圖沒有顏色**。

   ⚠️ ⭐ 而這一句原本要被下一輪讀成「已查證」——
   那正是 CLAUDE.md 記的第三個載體：**更正裡的推測 → 「已查證」**。

   ⇒ ⭐ 下一步是**去看那張沒出貨的貼圖**，⛔ 不是放棄 PRE2 那條路。
