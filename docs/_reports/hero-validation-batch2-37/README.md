...
| 因果行為 | 37／37 | 74 條連動、296 條配對時間線；每條有真前置／無前置、保留／移除連動四組對照；排除僅證單技防禦的測試 |
| 一般邊界 | 313 項、37 名被動、76 項到期檢查 | 有效盾池、狀態、buff等；條件效果未在孤立施法建立的項目明列 not-observed，不冒稱全覆蓋 |
| 特殊邊界 | 18 cases、170 timelines、340 assertions | 反彈通道與單次消耗、雙門檻、取消排程、卸載附魔、存款消耗、固定24種子分支等 |
...
實際工具測試 17／17（4 個結構診斷測試＋13 個 SimWorld 測試機制測試）。這些通過只證明工具對應行為，不能代替內容品質。所有最新執行碼與 log 見 [pipeline-report.json](data/pipeline-report.json)。
