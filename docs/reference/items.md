...
> 產生自 contentVersion **`cv_e48809fe1c4a`**（`content/manifest.json`；它是 `content/**` 的純函數，改內容就會變）
> 資料列：**142**　·　開放名單來源：`data/curation/whitelist.json`（updatedAt `2026-08-19T20:57:12.386323Z`）
