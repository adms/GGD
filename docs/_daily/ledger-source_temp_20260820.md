...
4個閘都作 
fix Lv18 issue
fix #465, 3 suggestions?

## 20:10

#447 傷害級距（你說「看不懂」）	技能傷害要從「一發打掉血條 3%」拉到「一發打掉 25%」。你 19:03 已裁決用 LV30 錨 = 700 => 這也太扯了 這樣變成四次技能就死了, 你說錯了吧

## 20:13

LV30 中位「有效血量」	13,927	71 隻裸裝走出貨管線量的（pnpm mana:audit 同一條路）。「有效」＝ 已經算進魔法減傷 => 不要計算 HP 系統倍率 以及魔抗減傷 會讓我誤判，這個優先校正，讓我重新判斷思考，你給三個建議

## 20:17

#469 隱藏英雄名單是空的	彩蛋機制做好了，但名單 [] ⇒ 整條路徑等於不存在
=> 預設是死之王、飛鼠先生、喪標麥可、黑人牙膏、黑化Saber(待補)

🅲 保留倍率，但把它從錨點推導裡剝掉（⭐ 我推薦）=> OK，只是技能說明顯示的數值也會變得動態

## 20:27

🅲 保留倍率，但把它從錨點推導裡剝掉（⭐ 我推薦）
=> OK，只是技能說明顯示的數值也會變得動態

#465 相稱性剩下的那一格	你 08-19 手填「範圍·極小要配傷害大」，08-20 你自己給的公式算出來是「小」，差 3 倍	A：照你的公式（小），把手填那一格退休

#445 冷卻 15 格的合理性報告	你要的評估做完了：13 格完全自洽，只有「範圍·極小」和「範圍·小」兩格對範圍技不利（−60% / −33%）
=> 傷害太低要跳出警告清單給我，後台跟 codex 編輯器也同步跳警告

#474 12 位英雄還有推薦出裝	你說過「不需要推薦出裝」，但 12 位還帶著梯子 ⇒ bot 有兩種買法 => 拔乾淨，現在 bot 都是半價購買隨機寶具，但隨機三選一還是會根據自己屬性購買有利的選項 (不會變成法師買近戰暴擊武器)

#482 悟空球體只有忠實尺寸的 77%	註解引用一個repo 裡不存在的數字。正確比值是 0.4161，出貨 0.3221 => 照原著 改成忠實值	0.3221 → 0.4161	悟空的球變大 29%

483 測試種子被重挑第四次	一條測試用寫死的亂數種子追一個機率事件 ⇒ 每次改內容都紅	
=> A：改成掃一段種子範圍

理由：你已經裁決過了（「不需要推薦出裝」），這一題只是把裁決做完。 B 的開關是為了「能回頭」而存在的，但你這一則不是暫時的偏好、是設計方向 —— ⛔ 不需要留回頭路。
