...
### 🆕 2026-08-28 批次（平行批次盤 v3 ＝ GH#852）

owner 當日八則 → 9 張票 · 6 條平行 lane（檔案柵欄互斥，全 lane 禁 build/sync）：

| 票 | 內容 | lane | 狀態 |
|---|---|---|---|
| #845 | 二次施放特效被三秒碼表砍頭（noteVfxRefired 4 通道漏 2） | A | ✅ `8c817f38`（17 通道全掃，無第五個） |
| #843 | 08-04 二連技：直線傷害缺＋第二段缺等級成長 L×7×AGI | B | ✅ 對照報告貼票，級距等 owner 裁 |
| #851 | 六招牌技能模板化稽核 | C | 🔍 |
| #848 | 風王結界太濃太久 | D | 🔧 |
| #849 | Rider/木乃香魔法陣三度沒去背（alphaMode 層） | E | 🔧 |
| #850 | live 資料面 checksum 快取＋8 頁可編輯（#822~834） | F | 🔧 |
| #846 | idle N 秒恢復自動索敵（`idleAutoEngageSec` 出貨 3s，0=關） | 主 | ✅ 已落地 |
| #847 | 練習/離線 ready 即開（early start 讀活座位） | 主 | ✅ 已落地 |
| #844 | 後台每頁 footer copyright | 主 | ✅ 已關 |

部署節奏（owner 逐字）：「都做完並註解 github issues close 才 push note deploy」。
