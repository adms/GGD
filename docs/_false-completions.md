...
| **S15** 🆕 | **同一份來源、兩種讀法，出貨的是舊的那個**<br>兩條管線各自解同一個原始欄位，差一個常數因子；新的那條寫了理由，舊的那條先把資料烘出來了 | 🔴 | `extract_particles.py:216` `radius = width * scale` vs `w3xEmitter.ts:425` `halfExtent = max(width,length)/2 * scale`（**差 2×，且新的那條寫明「矩形發射面 → 外接圓」的理由**）→ 228 份 `godie-*-p*.json` 全部是舊讀法的產物（**已於 2026-08-02 重生，226 份改動**；「全數偏大」實測是錯的，3 份其實偏小，見 P1-E） | 同一個原始欄位若在 >1 處被轉換，**必須共用一支函式，或有一條交叉測試斷言兩者相等**。探針：對每個匯入欄位 `grep -rn "em\.<field>" tools/ apps/` — 出現在兩個 repo 區域就是紅旗 |
...
- ✅ **2026-08-02 結案（GH#110）**：就地重生完成，**226 份 p2 文件改動**（2 份 `l = 2w` 兩式同值不動；54 份 ribbon 一份都沒動）。`extract_emitters.py` 的第三種讀法連同誤導的欄位名一起改掉（`emitterHalfWidth` → `emitterFullWidth` + 新增 `emitterDiscRadius`），EMITTERS.json 已重生。
  - **交辦文與本頁的模型都不完整，實測修正三點**：① 比值**不是**一致 2×——`W≥L` 才是 2×（139 份）；`W<L` 是 `2W/max(W,L)`（10 份）；撞舊 `0.05` 下限的另成一族（79 份）。② **有 3 份是變大不是變小**，`godie-1hswd-01-p2` 0.092 → 0.695（**×7.55**），因為舊式丟掉 `Length=50` 只讀 `Width=3.3`——舊值是**過小**。③「228 份全數偏大」對其中 3 份是假的。
  - **本頁「真正畫錯的只有 2 份」在當時就已經漏了一份**：`godie-bladestorm-swordeffect-p0` 綁在白名單英雄林克的 R（`W3X_ABILITY_ART`），所以是 **7 份可達**沒錯，但可達不等於畫對。
  - **防回歸**：`tools/w3x-import/test/shippedVfxIsCurrent.test.ts`（vitest，會在 CI 跑）比對「出貨的 282 份 vs 現場重生」**逐欄位相等**，失敗訊息報「文件.欄位: 出貨值 → 工具值」——不是只比 radius，因為 S15b 的 `burstCount` 正是「只比一個欄位就漏掉的第二個欄位」。它跑 `--overwrite-tuned` 來 stage，否則被判定為「保留」的文件會跟自己比而永遠相等。`particles_regen.test.ts` 保留 `particles_checks.py`（第 (3) 項已從印橫幅改成**硬失敗**）與 ambient-vfx 那兩條。
  - **腐爛機制本身也修掉了**：舊的 `tuned = shipped != fresh` 分不出「手調」與「過期」（兩者都不一樣），所以它把每個案例都判成保留 —— 它保護的其實是缺陷。改成 `extract_particles.classify_doc` 讀 `out/GoDieEX22s/vfx-provenance.json`（每份文件「產生當下」的雜湊 + 工具指紋），`shipped == recorded` ⇒ 過期可覆寫、`!=` ⇒ 真手調保留並具名列出。兩個方向都有測試（`test/provenance_checks.py`）。
