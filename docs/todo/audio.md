...
| au-32 | SUSTAINED SFX can be STOPPED (task #216, 「回到商店時…還會有火圈聲音」): `playSfx` was fire-and-forget and `AudioSystem` had NO stop path for SFX at all (only `stopBed()` for BGM), so the ~60 s `fireRingLoop` lit at ring ignition and the ~38 s `arenaAmbience` played straight through 結算 into the shop. `SFX_LOOPABLE` now actually drives playback bookkeeping (`isLoopableSfx`): only those events are tracked as voices, `stopSfx(event)` fades + stops them, `stopSustainedSfx()` cuts them all, a per-event epoch cancels a bed still DECODING when combat ended, and transients allocate nothing | audio-sfx-stop | unit | done |
| au-33 | The combat TEARDOWN edge (task #216): `isCombatEnd(prev, next)` is the exact mirror of `isCombatStart` — true on every exit from `combat` (resolution / intermission / matchEnd), false everywhere else, never both on one edge. `AudioDirector` hangs `stopSustainedSfx()` on it, plus a second screen-level teardown for leaving the match mid-combat | audio-combat-teardown-edge | unit | done |
| au-34 | A combat SUSTAINED bed may only START in combat (task #238, 「#216 只修一半：火圈聲音仍會播進商店（被打倒的玩家）」): #216 gave beds a stop path hung on the `isCombatEnd` REACT edge, but `combatSfxKey` runs on the GameApp's rAF `drainEvents` — a different clock — so a `fireRingStart` still queued when the phase committed lit a ~60 s burn bed AFTER the teardown edge was spent, and nothing stopped it until the next round. `combatBedGate` adds a LEVEL gate on the live phase for the three combat-scoped beds (`fireRingLoop` / `arenaAmbience` / `reviveChannel`), leaving transients and the out-of-combat beds (`merchantAmbience` / `legendaryRoll`) untouched; the #132 drift tripwire is gated too, so a straggler cannot false-alarm against the SHOP clock. Proven by driving a REAL `syncHudFromState` phase transition (combat → resolution → intermission → combat) and asserting silence, not by checking a boolean | audio-combat-bed-phase-gate | unit | done |
...
| au-35 | SFX preload LOOKAHEAD + live policy (task #63, second half): warming a scene's set ON its own edge means combat's 46 events / 56 files / 2,747,294 B start fetching the same instant `roundStart` fires. `sfxPreloadPolicy` adds a scene-successor graph (re-derived in test through the real `sceneForMatch`, mirroring the server phase order) and warms `lookahead` hops out, so the combat bucket is requested on the INTERMISSION edge while the login screen — 3 hops from combat, and still short of it at the MAX_LOOKAHEAD ceiling — requests none of it. `enabled` / `lookahead` / `successors` are read at boot from the live-mounted `content/audio-manifests/sfx-preload.json` (clamped BOTH ends; a bad/404 doc keeps the shipped defaults). `enabled:false` fetches nothing up front, core included, and every cue still lazy-loads. Silence (#62) preloads nothing at any depth. Assertions read the ACTUAL requested URLs | audio-sfx-preload-lookahead | integration | done |
| au-36 | Victory-taunt CLIP prefetched across the delay window (task #63): `speak()` assigns `el.src` at the instant the line must be heard, so the 758-clip / 7.7 MB taunt pack was grabbed cold ON the beat. A scheduled taunt now warms its own clip via a plain fetch (never a second `<audio>` element, which would cut a still-playing taunt short), once per file, suppressed under test-mode silence. NOTE the older claim that `loadVictoryTaunts()` has zero production callers is STALE — `ui/useAudio.useAudioScene` warms the script on settlement/victory/defeat since 8094eb42 | client-taunt-clip-warm | unit | done |
