...
| sec-154-18 | ~~One opaque register conflict + timing parity~~ **DONE and insufficient** — both shipped, the enumeration oracle stayed open (pair the value under test with a fresh counterpart, read 201-vs-409). What mitigates it is the #174 invite gate running before the reservation; residual = a caller holding a live code, knob `GGD_BURN_INVITE_ON_CONFLICT`. Remaining work: per-IP register limiter at the edge. **`/auth/login` is the same finding's other half**: it is enumeration-safe only because the argon2 comparison runs BEFORE the ban/approval checks, and that ordering had NO guard until 2026-07-30 — hoisting the status checks left `./internal/{auth,server,admin}` fully green while leaking `403 account_pending` vs `401` to an anonymous caller. Now pinned by `TestLoginRefusesTheSameWayWhateverTheAccountStatusIs`. See the revised F-18. (platform) — enumeration F-18. ⚠️⚠️ ⭐ **2026-09-05 —— 狀態欄從 `partial` 改成 `deferred`，⛔ 不是為了讓閘變綠**：`partial` 不在 `STATUSES`（`tools/todo-check/src/parse.ts:36` 的封閉詞彙表），CI 的 `unit` job 逐字卡在這一列。⭐ 而 `deferred` 是這一列今天**唯一誠實**的格：① 這一列自己寫著 remaining work = per-IP register limiter，⭐ 而那條限流器就是 sec-154-02，它也是 `deferred`；② `grep -rn "sec-154-register-generic-conflict" apps packages tools` **零命中** ⇒ 沒有任何 `cover()` beacon 釘著它 ⇒ 寫 `done` 會讓 `todo:runtime` 直接點名（`check.ts:60`）。⛔ **不要**因為前半段出貨了就把它翻成 `done` —— 那一半的守衛是 Go 測試 `TestLoginRefusesTheSameWayWhateverTheAccountStatusIs`，⛔ 它不發 NDJSON beacon。 | sec-154-register-generic-conflict | security | deferred |
