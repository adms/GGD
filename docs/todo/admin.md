...
| admin-42 | Quick Approval (#242): ONE page listing every decision blocked on the owner, cleared with one submit. Rows are DERIVED at view time — the roster the version-controlled starter set declares minus this deploy's whitelist (forward), the whitelist minus the starter set (reverse: a WARNING, never an approval — it is already enabled), enabled champions missing any of the five `<id>.{q,w,e,r,ex}` slots, and the #126 pending queue — so no answer is ever baked in and a row disappears by itself once the state changes. Every row states 這是什麼/為什麼在等/送出後/風險, champion rows carry a 數值體檢 against the LIVE median of the enabled roster (a rule about numbers, not a named hero) that fails CLOSED when content is unreadable, nothing is pre-ticked, and a failing 體檢 forces a consequence-naming second confirmation. Writes go ONLY through the existing audited seams — `POST /curation/whitelist/bulk` with an always-empty `disable` (union, `curation.bulk`) and `POST /admin/accounts/{id}/approve` (`approval_approved`); `saveWhitelist`/`diffDoc`/`putWhitelist` are un-importable here because their draft diff would delete the operator's 21 non-starter entries in one click | adminui-quick-approval | unit | done |
| admin-43 | Quick Approval SHIPS IN THE PRODUCTION BUNDLE — the inverse of `content-admin-gate`. A source assertion (always on) pins the top-level static `import { QuickApprovalPage }` in App.tsx, proves no `import.meta.env.DEV` guard sits within 400 chars of the page or its nav label, keeps it out of `CONTENT_SUITE_PAGES` and in `SESSION_REQUIRED_PAGES`; the `GGD_BUILD_GATE=1` half runs a real `vite build --mode production` and asserts the emitted bundle CONTAINS the nav label + 一鍵送出確認 + the row prose + the two endpoints, and still contains no `/content-api`, 內容管理, 鑄形工坊 or Babylon. Without this the page could silently dead-fold and be usable only on localhost — precisely the situation the owner asked to fix | adminui-quick-approval-bundle | regression | done |
...
| ID | Item | Test ID | Category | Status |
| --- | --- | --- | --- | --- |
| admin-44 | The online light answers 「1小時內有動作」 and nothing more: the hour threshold is inclusive at the boundary, a missing/unparseable stamp reads as never (not as offline), a live socket outranks the timestamp and names 對戰中 vs 大廳中 separately, and an ABSENT presence field says nothing about connectivity while the last-seen half still works | adminui-players-seen | unit | done |
...
| admin-46 | 經濟與場長脫鉤 (#250)：`wallet/meta.go` 的註解宣稱「一場 ~44 分鐘」，但 `opsenv` 在 LIVE 設定（4 隊 × 20 Team Health）算出 ~11.6 回合 ≈ 21 分鐘。**結論：錯的是註解，不是發放率。**#132/#153 把回合砍短（combatMaxSec 240→100、fireRing 180→60、選角 40s→20s），那個 44 從此沒重推過；owner 看過落差後裁示「藍水晶本來就是獎勵 有人抱怨我們再來改」，所以 240/90/70/60 一毛不動。改的是註解（純註解 diff，常數 byte-identical），並新增 `TestCrystalGrantsAreTheOwnersDecision` 把發放額釘死，讓未來的「重平衡」必須明著推翻 owner 決定才動得了 | opsenv-wallet-playrate | unit | done |
| admin-47 | `TestCombatEnvReplacePublishesInvalidation` 一直紅。**原本的診斷是錯的**——它本來就用 in-process `miniredis.RunT`，從來不需要真的 Redis，所以不是環境問題也不該加 skip 守衛。真因是 7dd31bf「sweep gosec」在加一行無關的 `#nosec` 時，順手刪掉了 `combatenv.Repo.mirror` 裡的 `PublishContentInvalidation`：檔案照存、mirror 照寫、後台照回 200，只有 running shard 收不到失效通知，戰鬥系統改了要等 TTL 才生效。已把 publish 補回，並加 `TestEveryContentKindStillHasAPublisher` 以 AST 掃描確保四種 content kind 都還有非測試的發布點 | combatenv-invalidate-guard | unit | done |
