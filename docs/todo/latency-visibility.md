...
| pc-09 | `enemy-team` 觸控 780×360 侵入下緣保留帶 6px（**#107 既有缺陷**）—— ⭐ **2026-08-27 修好**（GH#759）：逐項算術 `10+44+8+44+8+116+8+44+8+66 = 356` vs 預算 350 ⇒ ⛔ **不是 `enemy-team` 太高，是整條 column 多 6px**；動的是 `minimap` 116→110（`hudLayout.ts:311` 的 `touchHeight/touchWidth: 110`，`Minimap.tsx` 真的拿它當畫布邊長），⛔ 沒有走「把 `enemy-team` 66→60」那條作弊法。⚠️ 這一格在此之前寫著 `in-progress`，而它**自己就是 #759 的最後一個阻塞項** —— 一句在它到期之後還活著的散文（第三守則）。⚠️⚠️ ⭐ **2026-09-05 更正**：狀態欄在此之前寫的是 `**done**`（帶星號），而 `STATUSES` 是**封閉詞彙表** ⇒ `todo:check --static` 逐字判它 invalid，CI 的 `unit` job 卡在這一列。⛔ 星號不是強調，是**一個不存在的狀態**。⚠️ 同時量到：`grep -rn "ping-band-gutter" apps packages tools` **零命中**（EXIT=1）⇒ 這一列的 beacon 不見了（守衛本體還在 `apps/client/src/ui/hud/touchControlsCollision.test.ts`，只是沒有 `cover()`）—— `todo:runtime` 會在 regression job 點名它。 | ping-band-gutter | regression | done |
