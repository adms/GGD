...
**要還原一次失敗的匯入** ⇒ 見下面一整節。這是全篇最容易被誤讀的一段，所以獨立寫。

---

## 5.5 匯錯了怎麼回去

> 給的是半夜、剛把自己家的遊戲弄壞、而且不是 DBA 的那個人看的。
> 照著讀下去就好，不用先懂任何東西。

每次匯入之前，平台都會先自動把這台主機現有的資料備份成
`data/_migration/backups/<UTC 時間>.zip`。旁邊那個同名 `.json` 裡有還原指令、
**這次匯入新增了哪些文件**（`import.addedDocs`），以及下面這兩張表。
...
**`-resolve-collisions=adopt-archive` 不是可選的。** 如果那次壞匯入本身就是走
adopt-archive（§3 的第 2 條解法），它已經把使用者名稱改指到封存裡的帳號；備份裡
那份「指向本機自己帳號」的索引，對試算來說看起來就是一個新的身分衝突，於是
**整包被擋下、一個位元組都不會寫**。你當下已經登不進自己的後台，再吃一個紅色拒絕
是最糟的體驗。反過來說，其他情況根本沒有衝突，這個旗標就是空操作 ——
所以永遠用同一條指令，不需要判斷。

（在還原的方向上，「以封存為準」的封存就是**這台主機自己的備份**，
所以這個旗標的語意正好就是「還原」。）
...
1. **它不會刪東西。** 這次匯入「新增」的帳號、邀請碼、水晶紀錄，還原完都還在 ——
   新帳號甚至還能用它自己的密碼登入。
2. **但你不用猜是哪些。** 匯入完成那一頁會逐筆列出新增了什麼，同一份清單也寫在
   備份旁邊的 `.json` 裡（`import.addedDocs`）。**這次如果沒有新增，那份清單就是空的**，
   代表還原之後就真的乾淨了 —— 這是常見情況，例如你只是把同一包重匯了一次。
3. **照著清單處理。** 多出來的帳號，到後台**「玩家」頁按「婉拒」**，他就登不進來了
   （按錯了再按一次「放行」就好）；多出來的邀請碼，到**「邀請碼」頁按「撤銷」**。
   兩個都會留下稽核紀錄。
4. **清單裡如果有已經被用掉的邀請碼**，撤銷不了（後台會回「已被使用的邀請碼無法撤銷」），
   也不必撤銷 —— 它早就沒有效力了，要處理的是它帶進來的那個帳號。
5. **備份之後才發生的事會一起被蓋掉**：有人打了幾場、改了密碼、後台改了設定，
   全部回到備份當時。所以要還原就趁早。
6. **只增不改的東西不會被重寫**：稽核紀錄、個人戰績履歷、內容覆蓋層歷程 ——
   壞匯入那一行會永遠留在稽核裡，這是刻意的。
7. **備份只涵蓋這次匯入會碰到的資料組**（看 `.json` 的 `groups`）。沒被碰到的資料組
   本來就沒被動過，所以不在備份裡也不影響。

還原本身也是一次匯入，所以**它自己也會先備份一次** —— 連「還原還原錯了」都有退路。

> **為什麼不做「連新增的一起刪掉」的完整還原？** 因為那會是整個產品裡唯一能把
> 35 個真實家人帳號一次抹掉的操作，而它一輩子只會被執行一次：在半夜、由一個
> 已經嚇壞了的非 DBA、面對一個「要刪掉 214 筆文件嗎？」的提示 —— 那個提示他沒有
> 能力稽核。它要清掉的殘留，用現成、可逆、有稽核的「婉拒／撤銷」就能處理完。
> （owner 2026-07-26 決定。）

**這一節的每一句話都有測試在盯：**
第 3 條的兩顆按鈕是用**真的匯入進來的帳號與邀請碼**跑完整條 HTTP 路徑驗證的
（`apps/platform/internal/server/archive_recovery_runbook_test.go`：匯入 → 玩家頁看得到 →
婉拒 → 登入被擋成 `account_denied`；匯入 → 邀請碼頁看得到 → 撤銷 → 拿它註冊被拒），
第 4 條的例外也是。第 2 條那份清單本身由
`apps/platform/internal/platformarchive/addeddocs_test.go` 釘住 ——
它必須逐筆等於試算的判定，否則測試紅。上面「救得回來／救不回來」兩張表的文字
就是 `internal/platformarchive/restore.go` 的 `RestoreRecovers` / `RestoreLimits`，
後台頁面與 CLI 直接印同一份，`apps/admin/src/archiveRestore.test.ts` 會在兩邊漂移時失敗。
