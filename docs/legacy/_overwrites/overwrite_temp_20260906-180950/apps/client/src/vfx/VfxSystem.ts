...
import type { ExpandedVfxScriptDoc, VfxScriptDoc } from "@ggd/shared/content/schema/vfxScript";
// ⭐ GH#990 —— 呼叫段（`{call:{subtype,params}}`）的**唯一**展開器，客戶端與測試／工具共用同一支
...
export class VfxSystem {
  ...
  constructor(
    ...
    // ⭐ GH#990 —— 播放器只吃**展開後**的文件：forge 草稿（`ctx.vfxScriptFor`）與出貨登錄表
    //    都先過 `expandedScript()`（同一支展開器、WeakMap 記憶 ⇒ 同一份文件永遠拿到同一個物件，
    //    `segments.indexOf(seg)` 的段號才穩定）。
    this.scriptPlayer = new VfxScriptPlayer({
      scriptFor: (abilityId) => {
        const draft = this.ctx.vfxScriptFor?.(abilityId);
        return draft ? this.expandedScript(draft) : this.vfxScriptIndex().get(abilityId);
      },
      allScripts: () =>
        (this.ctx.allVfxScripts?.() ?? VfxScripts.all()).map((s) => this.expandedScript(s)),
  ...
  private scriptIndexCache: Map<string, ExpandedVfxScriptDoc> | null = null;
  private projectileIdsCache = new Map<string, ReadonlySet<string>>();
  /** 作者文件 → 展開後文件（identity 記憶）。⚠️ 沒有 call 的文件展開＝同一個物件，⛔ 不複製。 */
  private expandedCache = new WeakMap<VfxScriptDoc, ExpandedVfxScriptDoc>();
  private expandWarned = new Set<string>();

  /**
   * ⭐ GH#990 —— 把一份可能含 `call` 段的 script 展成播放器唯一吃的形狀。
   *
   * 失敗（子模組沒登錄／參數不合法）⇒ **fail-open 但出聲**：印一行指名 script、段、子模組、原因，
   * 然後**丟掉呼叫段**只播 inline 段。⛔ 不是整份丟掉（那會讓一顆打錯的參數把整支技能的演出關掉），
   * ⛔ 也不是靜默（「schema 收得下、畫面上什麼都沒發生」正是第一·五守則要擋的）。
   * ⚠️ 「還沒 registerVfxSubtypes」這一句出現 ⇒ `registerAll()` 少了那一行，⛔ 不是內容壞了。
   */
  private expandedScript(doc: VfxScriptDoc): ExpandedVfxScriptDoc {
    const hit = this.expandedCache.get(doc);
    if (hit) return hit;
    let out: ExpandedVfxScriptDoc;
    try {
      ...
      out = { ...doc, segments: doc.segments.filter((s) => s.kind !== undefined) as ExpandedVfxScriptDoc["segments"] };
    }
    this.expandedCache.set(doc, out);
    return out;
  }

  /** abilityId → 展開後的 script（懶建；forge 熱改後 `invalidateVfxScripts()` 重建）。 */
  private vfxScriptIndex(): Map<string, ExpandedVfxScriptDoc> {
    if (this.scriptIndexCache === null) {
      this.scriptIndexCache = new Map();
      for (const s of VfxScripts.all()) this.scriptIndexCache.set(s.abilityId, this.expandedScript(s));
    }
    return this.scriptIndexCache;
  }

  /** forge 編輯器存檔後叫這一支 —— script 索引、展開快取與彈道歸屬全部重建。 */
