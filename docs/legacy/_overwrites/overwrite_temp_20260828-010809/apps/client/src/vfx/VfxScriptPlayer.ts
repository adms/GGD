...
export class VfxScriptPlayer {
  ...
  /** 由 deps.scriptFor 的資料源列舉 —— 這裡不想再要一個 all()，走注入的列舉器。 */
  allScripts: () => readonly VfxScriptDoc[] = () => [];
  ...
  private fire(seg: VfxScriptSegment, frame: TriggerFrame, nowMs: number): void {
    ...
    switch (seg.kind) {
      ...
      case "vfx": {
        const at = seg.at === "bone" ? "caster" : (seg.at ?? "self");
        const pos = this.anchorPos(at === "self" ? "caster" : (at as "caster" | "target" | "point"), frame);
        if (!pos) return;
