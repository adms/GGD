...
import { recordCacheLoad, recordUncachedLoad, runtimeCacheEnabled } from "./contentCacheHealth";
...
async function loadContent(): Promise<void> {
  ...
  const loadFrom = async (label: string): Promise<boolean> => {
    ...
    const runtimeCache = !withOverlay && runtimeCacheEnabled(process.env);
    const result = runtimeCache
    ...
    // ⚠️ fail-open 沒錯，**靜默**才是缺陷 —— 這一行是那個「說得出來」的地方。
    //    `/healthz` 的 `contentCache` 區塊會指名這一次到底有沒有命中、為什麼沒有。
    if ("cache" in result) recordCacheLoad(result.cache, performance.now() - tLoad);
    else
      recordUncachedLoad(
        withOverlay
