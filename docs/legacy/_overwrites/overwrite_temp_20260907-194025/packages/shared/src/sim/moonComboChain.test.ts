...
function qThenW(rules: Partial<CastTimeRules> = {}): { base: number; inWindow: number; late: number; pressGapTicks: number; resolveGapTicks: number; windowTicks: number } {
  ...
  console.log(
    "MEASURE " +
      JSON.stringify({
        mode: rules.comboWindowFrom ?? "commit(shipped)",
        ...out,
        aBaseCastToDamageTicks: base!.tick - aPress,
        bPressToDamageTicks: hit!.tick - (qTick + pressGapTicks),
        victimMaxHp: a.world.health.get(a.victim)!.maxHp,
        aVictimHpAtHit: a.world.health.get(a.victim)!.hp,
        aVictimAlive: a.world.health.get(a.victim)!.alive,
        bVictimHpAtHit: b.world.health.get(b.victim)!.hp,
        bVictimAlive: b.world.health.get(b.victim)!.alive,
        aHeroAlive: a.world.health.get(a.hero)!.alive,
        bHeroAlive: b.world.health.get(b.hero)!.alive,
      }),
  );
  return out;
}
