 ...
 * 開著 ⇒ 四層都回來；關掉 ⇒ 既不存也不接回。
 *
 * MUTATION LOG（2026-09-06 實測）：把 `autosave.ts` 的 `await store.put(key, record);`
 * 拿掉 → 第一條紅（`restore` 回 `none`，四層一格都沒回來）。用 Edit 改回 → 綠。
 */
...
describe("編輯器草稿 autosave (GH#1023)", () => {
  it("四層各改一格 → 關掉分頁 → 重開 ⇒ 四格都回來，而且畫面說它是草稿", async () => {
    ...
    const before = createAutosave(createMemoryDraftStore(disk), DEFAULT_EDITOR_AUTOSAVE, () => 1_700_000_000_000);
    ...
    expect(getIn(abilityOut.doc, "manaCostTier"), "存的是微調值就該跟著模板走").toBe("高");
  ...
  it("還沒投稿就關頁 ⇒ 攔一次；存不起來 ⇒ 說得出話（⛔ 不是靜默）", async () => {
    const listeners = new Map<string, (event: { preventDefault(): void; returnValue?: unknown }) => void>();
    let dirty = false;
    installUnloadGuard(
      { addEventListener: (t, l) => void listeners.set(t, l), removeEventListener: (t) => void listeners.delete(t) },
      () => dirty,
    );
    const fire = () => {
