 ...
 *
 * 盲區：`CHANGED` 的三個來源（`$BASE..HEAD` / 工作樹 / `--cached`）**全部只列 tracked 檔**，
 * 而 `HAS_REPORT` 正是從它撈 `docs/_reports/*visual-proof*.md`。⇒ 一條被規定
 * 「⛔ 不碰 git 寫入」的 lane 做完了 audition 也**永遠沒辦法讓這支閘變綠**，
 * 而閘的訊息會說「你改了畫面層卻沒有終端證據」——⚠️ 那句話是假的。被忽略的閘等於沒有閘。
 *
 * ⛔ 掃原始碼字串對這個形態永遠是綠的（失敗形態⑥）⇒ 這裡**真的把腳本跑起來**讀離開碼與輸出。
 * 三個情境一起驗，因為「看得見未追蹤證據」與「沒有被放寬」是同一件事的兩半。
 */
...
function run(dir: string) {
  ...
  return spawnSync("bash", [join(dir, "scripts/visual-proof.sh")], { cwd: dir, encoding: "utf8", env });
}
