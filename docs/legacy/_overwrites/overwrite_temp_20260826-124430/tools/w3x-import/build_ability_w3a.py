...
def parse_damage(tips: dict[str, str]) -> dict | None:
    ...
    for lv in levels:
        ...
        if m:
            ...
            continue
        p = _PLAIN_DMG.search(text)
        hits.append(
            {"kind": "flat", "id": None, "coeff": 0.0, "flat": float(p.group(1)), "raw": p.group(0)}
            if p
            else None
        )
    ...
    if dialogue_digits:
        ...
        # before parsing. A0K3-style tooltips put the WHOLE mechanic in quotes.
    ...
    elif len(real) == len(levels) and not out.get("inconsistentAcrossLevels"):
        out["confidence"] = "high"
    else:
        out["confidence"] = "low"
...
def build(meta: dict) -> dict:
    ...
    for table in ("original", "custom"):
        for e in w3a[table]:
            ...
            cols = {int(c): v for c, v in (src.get("data") or {}).items()}
            ...
            for col in sorted(set(cols) | set(sem)):
                ...
                rec["data"][str(col)] = {
                    ...
                    "name": (d or {}).get("name"),
                    "type": (d or {}).get("type"),
                    "values": {k: v for k, v in sorted((cols.get(col) or {}).items())},
                    "authored": col in cols,
                }
                if d is None and col in cols:
