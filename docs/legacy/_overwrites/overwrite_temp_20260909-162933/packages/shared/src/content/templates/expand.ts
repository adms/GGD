...
const FAMILIES: Readonly<Record<string, Family>> = {
      ...
      effects: [
        {
          ...
          maxStacks: num(t, p, "maxStacks"),
          // ⭐ 缺席 ＝ 不自己過期（由消耗側扣掉）—— ⛔ 不編一個秒數。
          ...(has(t, p, "durationSec") ? { duration: num(t, p, "durationSec") } : {}),
        } as unknown as EffectDef,
      ],
  ...
  "spend-resource": (t, p) => {
    ...
    const mode = str(t, p, "spend");
    if (mode !== "all" && mode !== "exact") {
      throw new ExpandError(`template ${t.id}: param "spend"="${mode}" 不是 all/exact`);
    }
    if (mode === "exact" && !has(t, p, "spendCount")) {
      // ⭐ 一個沒有數字的 `exact` 會靜默退化成別的語意 —— ⛔ 那是「說了但不會發生」。
      throw new ExpandError(`template ${t.id}: spend="exact" 必須填 spendCount`);
    }
    const gate = {
      ...
      effects: [
        {
          ...
          subject: "self",
          statusId,
          count: mode === "all" ? "all" : num(t, p, "spendCount"),
          condition: gate,
        } as unknown as EffectDef,
        // ⭐ 這一發真正做的事 —— ⛔ 這一族不規定它打什麼。
        //   ⚠️ 每一顆都掛同一道 `condition`：⛔ 少掛的那一顆會在層數不夠時照樣發生。
        ...(raw(t, p, "effects") as EffectDef[]).map(
          (e) => ({ ...(e as unknown as Record<string, unknown>), condition: gate }) as unknown as EffectDef,
        ),
      ],
    };
  },
