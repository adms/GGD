...
describe("模板展開後的引用，載入期驗得到 (GH#723)", () => {
  ...
  it("② 模板指到一具不存在的模型 ⇒ 載入時就紅，而且訊息說得出它是模板生的", () => {
    ...
    // ⭐ 受害者要**真的有人用**：一個沒有採用者的模板改壞了不會長出任何 edge，
    //   那樣的「綠」只證明了注入沒發生（量尺自證）。
    const adopted = new Set<string>();
    for (const a of templatedAbilities(store)) {
      for (const c of normalizeTemplateBinding((a as unknown as Record<string, unknown>)["template"])
        .cards) {
        adopted.add(c.ref);
      }
    }
    ...
    const victim = store
      .all<TemplateDoc>("ability-templates")
      .find((t) => adopted.has(t.id) && hasModelKeySlot(t));
    expect(victim, "沒有任何**被採用**的模板帶 modelKey docRef 參數 ⇒ 這個注入不成立").toBeDefined();
    const params: Record<string, unknown> = { ...(victim!.params as Record<string, unknown>) };
    params["modelKey"] = {
