...
const VFX_CLEANUP_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      ...
      note: "同上，但用在畫質判定為 mobile 的裝置（手機，以及 auto 判成低畫質的內顯）。⭐ 這一格單獨存在的理由：那 52 張常駐貼圖在手機／內顯上可能是**致命**的，而畫質的自動降級階梯（粒子密度、陰影、後製）**碰不到貼圖快取** —— 這一格就是把它接進那個階梯。出貨 4 組＝下界，也就是「當前這張場地 ＋ 上一張」：上一回合剛打過的那張重播時仍然是零抓取，再往前的就重抓（手機上換圖那幾幀是平色，⛔ 不是黑的）。調大＝手機換圖更順但吃記憶體。",
    },
  ],
  preserved: [
    {
