...
export function externalRows(): NavLink[] {
  const raw =
    (import.meta as unknown as { env?: HubEnv & { PROD?: boolean } }).env ?? {};
  const editor = resolveHubLinks(raw, raw.PROD === true ? "prod" : "dev").find(
    (l) => l.key === "editor",
  );
  return [
    {
  ...
  useEffect(() => {
    ...
    void import("./ContentPage").then(
      (m) => {
        ...
        setLoaded({
          routes,
          render: (page, onNav) => m.renderContentDevPage(page, onNav),
        });
      },
      () => undefined,
    );
...
function useVoiceGenPage(): ContentAdmin | null {
  ...
  useEffect(() => {
    ...
    void import("./VoiceGenPage").then(
      (m) => {
        if (alive)
          setLoaded({ Page: m.VoiceGenPageRoot, nav: { ...m.VOICE_NAV } });
      },
      () => undefined,
    );
    return () => {
...
export function NavRail(props: NavRailProps): React.JSX.Element {
  ...
  const store =
    props.prefStore === undefined ? browserPrefStore() : props.prefStore;
  ...
  return (
    ...
    >
      ...
      <button
        onClick={() =>
          apply(
            everyOpen ? new Set(allSections(rows, SECTION_ORDER)) : new Set(),
          )
        }
        title={
          everyOpen
            ? "把所有分組收起來（目前所在的那一組會留著）"
            : "把所有分組展開"
        }
        style={{
        ...
        return (
          <Fragment key={g.section}>
            ...
            >
              ...
              <span>{g.section}</span>
              <span style={{ opacity: 0.6, letterSpacing: 0 }}>
                {g.rows.length}
              </span>
              ...
              {!open &&
                pendingCount > 0 &&
                g.rows.some((r) => isNavItem(r) && r.page === "approvals") && (
                  <span
...
export function Console(): React.JSX.Element {
  ...
  const withContent =
    contentSuite === null
      ? NAV
      : insertBlockAfter(NAV, "audit", contentSuite.routes);
  const liveSuite = useLiveSuite();
  const navBase =
    voiceAdmin === null
      ? withContent
      : insertBefore(withContent, "curation", {
          ...voiceAdmin.nav,
          section: SEC_CONTENT,
        });
  ...
  const onNavigate = (p: string, _selectId?: string): void =>
    navigate(p as Page);
  ...
  useEffect(() => {
    ...
    resetContentScroll({
      ...
      win:
        typeof w.scrollTo === "function"
          ? { scrollTo: w.scrollTo.bind(w) }
          : null,
    });
  }, [page]);
      ...
      >
        {!narrow && (
          <>
            ...
            <div style={{ fontSize: 11, color: TEXT_DIM, marginBottom: 20 }}>
              operator console
            </div>
          </>
        )}
          ...
          {account ? (
            <>
              ...
              <Btn
                ...
                style={narrow ? undefined : { width: "100%" }}
              >
                Sign out
              </Btn>
            </>
      ...
      >
        {gated ? (
          ...
          <>
            ...
            {page === "navMap" && (
              <NavMapPage rows={rows} onNavigate={onNavigate} />
            )}
            {contentSuite === null && isContentSuitePage(page) && (
              <div style={{ color: TEXT_DIM, padding: 8 }}>
                載入內容·素材管理…
              </div>
            )}
  ...
  return (
    <div style={{ maxWidth: 520 }}>
      <Panel title="需登入 · Operator sign-in required">
        ...
        >
          此頁面為玩家 / 營運管理功能，資料由平台 admin API
          提供，需要管理員登入才能使用。 英雄 / 技能 /
          道具的內容編輯不需登入即可使用。
          <br />
          This page is backed by the platform admin API and requires an operator
          sign-in. Content editing (heroes / skills / items) needs no login.
        </div>
