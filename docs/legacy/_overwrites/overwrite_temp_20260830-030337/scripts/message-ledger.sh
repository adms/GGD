...
def decided(ticket_cell: str) -> bool:
    ...
    ⚠️ 這裡在 2026-08-30 之前寫的是 `re.search(r"#\\d{2,4}", …)` —— **強制要有 `#`**,
    而 `#` 是排版⛔ 不是語意。實測 5 列**已經對到票**的列被誤報成「未對票」
    (2026-08-20 的 `447` × 3、2026-08-28 的 `860` / `863`)。
    ⭐ 之前沒有人發現,是因為這支閘只看**今天** —— 而這一版開始掃已結束的日子,
    那 5 個假紅會**淹掉真紅**(＝這張票要治的病本身)。
    ⛔ 仍然拒絕:留空、`⏸ 未對票`(兩者都沒有數字)。
    """
    s = ticket_cell.strip()
...
def unmapped_rows(day: str):
    ...
    return [(n, c) for n, c in LT.canonical_rows(ledger_of(day)) if not decided(c[-1])]
...
def report(day: str, missing, bad, prefix: str = "⛔") -> None:
    for t, m in missing:
        print(f"{prefix} 漏了 {day} {t}  {re.sub(chr(10), ' ', m)[:70]}…")
    for n, c in bad:
        print(f"{prefix} 未對票 {ledger_of(day)}:{n}  {c[0]}  {c[1][:60]}…")


HOWTO = ("→ 跑 `pnpm msgledger:build` 補列,再把每一列的票號填上"
