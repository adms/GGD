...
export const DEFAULT_AP_COEFFICIENT: ApCoefficientConfig = Object.freeze({
  ...
  //   ⚠️ 被收掉的那 36 條係數**系統性偏高**（多是山形修煉那一族普攻 proc 的 0.9–3.0）
  //   ⇒ 現況幾何平均 0.6780 → 0.6928、公式幾何平均 0.8508 ⇒ 校準比 **1.2282** ⇒ base ÷ 1.2282。
...
export function forEachApRatio(
  ...
  const walk = (o: unknown, anc: Record<string, unknown>[], path: string, collapse: boolean): void => {
    ...
    const next = [...anc, node];
    for (const [k, v] of Object.entries(node)) walk(v, next, `${path}.${k}`, false);
  };
