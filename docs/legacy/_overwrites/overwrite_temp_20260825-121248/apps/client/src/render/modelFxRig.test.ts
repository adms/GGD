...
describe("model fx tint 分流：顏色住哪就乘哪 (@visual-proof)", () => {
  ...
  it("① luma-keyed stock 特效：tint [1,0,0] 把 B 通道消掉（藍→紅）", async () => {
    ...
    expect(after.length, "一份著色過的素材都沒有進場景樹").toBe(before.length);
  ...
  it("② sentinel —— 不透明 body（英雄 glb）的自發光逐位元不變", async () => {
    ...
    const src = before.map((m) => rgb(m.emissiveColor));
    expect(src.every((e) => Math.max(...e) === 0), "前提不成立：這具 body 有自發光").toBe(true);
    after.forEach((m, i) => {
      expect(rgb(m.emissiveColor), "分流塗到了不該塗的 body 材質").toEqual(src[i]);
  ...
  it("③ 近黑退路 —— 全 glow 的 blackhole 染 [0,0,0] 不會整具消失", async () => {
    ...
    after.forEach((m, i) => {
      ...
      ).toEqual(rgb(before[i]!.emissiveColor));
