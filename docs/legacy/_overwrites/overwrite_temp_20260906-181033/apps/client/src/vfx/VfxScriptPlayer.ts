...
// ⭐ GH#990 —— 播放器**只吃展開後**的文件（`call` 段已由 `VfxSystem.expandedScript()` 換成 inline 段）。
//    這裡的 `VfxScriptDoc` 一律指 `ExpandedVfxScriptDoc`；作者形狀（可能含 call）⛔ 不會走到這一檔。
import type { ExpandedVfxScriptDoc as VfxScriptDoc, VfxScriptSegment } from "@ggd/shared/content/schema/vfxScript";
