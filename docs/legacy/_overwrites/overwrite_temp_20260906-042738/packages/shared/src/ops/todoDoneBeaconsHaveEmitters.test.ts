 ...
 *
 * 帳本 `KNOWN_UNEMITTED` 只能變短（GH#996 開出的後續票列著它們）。⛔ 體驗層：不做突變。
 */
import { describe, it, expect } from "vitest";
import { readFileSync, readdirSync } from "node:fs";
import { dirname, join } from "node:path";
import { fileURLToPath } from "node:url";
import { parseTodoMarkdown, type TodoItem } from "../../../../tools/todo-check/src/parse";
...
function loadTodos(): TodoItem[] {
  ...
  return readdirSync(dir)
    ...
    .sort()
    .flatMap((f) => parseTodoMarkdown(`docs/todo/${f}`, readFileSync(join(dir, f), "utf8")).items);
}
...
describe("docs/todo 的每一列 done 都有人發射它的信標（GH#996，反方向）", () => {
  ...
  it("⭐ 哨兵：缺發射器的被指名，有的（字面值／前綴／退回）不被誤判", () => {
    ...
    const bad = audit(parseTodoMarkdown("x.md", md).items, emittersOf(src));
