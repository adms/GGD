...
describe("fx GENERATE.sh ↔ the directory it writes (GH#744)", () => {
  ...
  it("does not encode raw PCM into an .mp3 name", () => {
    ...
    for (const line of SCRIPT.split("\n")) {
      if (!line.includes('"$OUT/')) continue;
      if (/-c:a\s+pcm_/.test(line) || /-c:a\s+pcm_/.test(SCRIPT.split(line)[0]!.split("\n").at(-2) ?? "")) {
        expect(line, "raw PCM written under an .mp3 name").not.toMatch(/"\$OUT\/[^"]+\.mp3"/);
      }
    }
  });
