...
// ⇒ 追蹤票：**GH#1001**。
// ⭐ 2026-09-06 999 → 1000（GH#1022）：`ugc.digestRecompute` 的標籤。UGC_SPEC 是**純手寫** `fields[]`（⛔ 沒走
//   schemaToForm，所以 Zod 上的 @zh 到不了它）—— 搬去 schemaToForm 是 GH#1001 那一批的事，⛔ 不在今晚。
//   同一晚 GH#1019 的 `abilitySelfDamageGuard` 那 2 格**沒有**進來：那格被 configDecorationCensus 判成裝飾，整格拿掉了。
const BASELINE = 1000;
