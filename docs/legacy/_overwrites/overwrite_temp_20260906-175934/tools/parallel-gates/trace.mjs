...
}
{
  ...
  let stale = (() => {
    ...
    try {
      let sb = null;
      try { sb = JSON.parse(readFileSync(STAMP, "utf8")).fingerprint; } catch { sb = depFingerprint(SANDBOX); }
      return sb === fingerprint ? null : "依賴指紋變了（pnpm-lock.yaml／package.json 依賴欄位）⇒ node_modules 要重來";
    } catch {
