...
- 額外（⛔ 不是光束）：32318/32320 在 `LocPoint3` 各放一發 `AddSpecialEffectLocBJ`：
...
| 欄位 | 現在 | 應為 | 出處 |
|---|---|---|---|
| `params.count.default` | **6** | ⭐ **1** | 全部 7 個生成點皆 `CreateNUnitsAtLoc( 1, …)` |
...
- `godie-n01c.e.json` / `godie-nbbc.e.json`（08-03）：⭐ **這一支根本不屬於 `tpl-beam-roll`** ——
  ...
  ⛔ 並拿掉沒有出處的 `scale: 4.5`。
- 03-04 全彈發射（`Hlgr` 鋼彈）：目前 GGD 側**沒有** beam-roll 綁定 —— 若要補，是**三層**（§1.3）。
- 90-04 陽光烈焰（種子神奇寶貝）：同上，1 層 ReviveHuman @200%（⛔ 等 §4.1 驗完再決定 Z 軸）。
- 81-03 DBE（魔砲少女）：3 層，其中兩層是 `MidchilderNanohaAura` 魔法陣（⛔ 不是光束模型）。
