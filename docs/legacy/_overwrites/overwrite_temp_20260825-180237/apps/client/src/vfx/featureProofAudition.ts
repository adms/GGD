...
const CLASSICS: Record<string, ClassicSpec> = {
  ...
  omnislash: {
    ...
    beats: [
      ...
      // ⭐ 收尾那一拍**不是猜的**：`config.combo-strikes@1` 的 superff7 逐字寫著
      //    steps [0,.5,.6,.7,.8,.9] ＋ finisherDelaySec .1（＝ JASS 原文的等待序列）
      //    ⇒ 從連段起算 3.6 秒 ＝ 108 tick，收尾落在 cast resolve 之後第 108 tick。
      { label: "o4_combo_finish", ticks: 78, note: "收尾一擊：warstompcaster ＋ screenFlash ＋ screenShake（superff7 的 steps 0/.5/.6/.7/.8/.9 ＋ finisher .1）" },
      { label: "o5_expired", ticks: 45, note: "到期：3.5 秒鎖定與無敵結束，畫面回到只有替身" },
      // ⚠️ 診斷格（⛔ 不是演出的一部分）：第一次施放時 `imported.herocloudstrife`
      //    （389 KB）還在載，模型節點 0 頂點就到期了 ⇒ 第二次施放走**熱快取**，
      //    這一張回答「那一層是載太慢，還是根本畫不出來」。
      { label: "o6_recast_warm_cache", ticks: 25, note: "熱快取重放：同一支再放一次（glb 已在 AssetManager 快取裡）", act: "cast" },
    ],
  },
  ...
  avalon: {
    ...
    beats: [
      ...
      { label: "a1_avalon_up", ticks: 30, note: "放 20-04 永恆的理想鄉（castType self）：2 秒反彈 buff ＋ tpl-locust-strike 紅柱", act: "cast" },
      { label: "a2_reflect_hit", ticks: 3, note: "敵人打一發魔法傷害 ⇒ 反彈成功 ⇒ EX 的 onReflectSuccess 發動：七彩爆炸長在**被反彈者**身上", act: "reflectProbe" },
      { label: "a3_seven_hits", ticks: 8, note: "連續七斬進行中（delayed count 7 × 0.12 秒，每一段 fx.avalon.reflect-spark）" },
      { label: "a4_finisher", ticks: 19, note: "收尾：約束與勝利之劍 damageLine（前方直線）＋ 第二發七彩爆炸" },
  ...
  dragonslave: {
    ...
    beats: [
      ...
      { label: "g2_resolve", ticks: 10, note: "詠唱解算：tpl-line-blast 的火球出膛（preset 在載入時補齊 modelKey/speed/distance）" },
      { label: "g3_travel", ticks: 6, note: "飛行中：沿施放方向前進（speed 27.5 · distance 12 ⇒ 約 0.44 秒），途中 touch 判定" },
      { label: "g4_blast", ticks: 7, note: "落點爆炸：fx.prim.fire.explosion-lg ＋ damageArea 8 半徑 ＋ 四段 delayed 的後續" },
  ...
  endworld: {
    ...
    beats: [
      ...
      { label: "e2_resolve", ticks: 14, note: "詠唱解算：spawnProjectile（imported.wave）＋ tpl-radial-burst 十二向散開" },
      { label: "e3_burst", ticks: 14, note: "放射中：12 具沿 radial 路徑外擴（speed 6 · distance 4.5），onTouch 減速" },
      { label: "e4_arrive", ticks: 8, note: "到點：fx.prim.ice.nova ＋ 12 記 screenShake ＋ 三段 delayed 的後續傷害" },
