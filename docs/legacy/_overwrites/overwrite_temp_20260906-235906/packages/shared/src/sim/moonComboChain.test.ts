 ...
 *
 * ⇒ 兩個「窗口內 ⇒ 含加成」的方向今天是 **`it.fails`**（量到的缺陷，⛔ 不是規格）：
 *    修好引擎（讓 `recentCast`／`comboBonus` 相對於**這一次施放的提交 tick** 量，或讓紀錄與求值
 *    落在同一種時間點）之後它們會「意外變綠」而紅 ⇒ 把 `it.fails` 改回 `it`。那是 **GH#1086** 的驗收。
...
function stage(seed = 1074): Stage {
...
function qThenW(): { base: number; inWindow: number; late: number; pressGapTicks: number; resolveGapTicks: number; windowTicks: number } {
  // 基準：只放 W。
  const a = stage();
...
function wThenE(): { base: number; inWindow: number; late: number; pressGapTicks: number; windowTicks: number } {
...
describe("GH#1074 Q→W 的 1 秒連擊窗（07-02 卡面「在臨兵鬥發動後 1 秒內施展」）", () => {
  ...
  it.fails("⛔ 量到的缺陷：吟唱一結束就按 W（1 秒內）⇒ 傷害**該**含條件式 AP 項，今天不含", () => {
    const r = qThenW();
    ...
    expect(
      ...
      `窗口內的 W 應該比基準多出 AP 項；量到 press 差 ${r.pressGapTicks} tick、` +
        ...
        `recentCast 是在 W 的解算 tick 量的，W 的吟唱把窗口吃光了`,
...
describe("GH#1074 W→E 的 moon-combo 鏈（07-03 卡面「在者皆陣發動後 1 秒內施展」）", () => {
  ...
  it.fails("⛔ 量到的缺陷：W 解算後立刻按 E（1 秒內）⇒ 落地傷害**該**含 comboBonus，今天不含", () => {
    const r = wThenE();
    ...
    expect(
      ...
      `窗口內的 E 應該比基準多出 comboBonus；量到 press 差 ${r.pressGapTicks} tick（窗口 ${r.windowTicks} tick）—— ` +
        `comboBonus 在 E 的解算 tick 烘焙，E 的吟唱把 moon-combo 的 1 秒吃光了`,
    ).toBeGreaterThan(r.base);
  });
});
