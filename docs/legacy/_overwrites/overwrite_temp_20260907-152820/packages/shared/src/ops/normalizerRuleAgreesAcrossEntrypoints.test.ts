/**
 * 🕳→🔒 **「正規化器 vs 作者」這一條判準，四個執行點必須給同一個答案**（GH#1099）。
 *
 * ── 量到的（2026-09-07，修之前）────────────────────────────────────────────
 ...
 *
 * 突變紀錄（2026-09-07 實跑）：把 `scripts/genguard.sh` 裡新接上的那一行
 * （`normalizer_rules.py` 的呼叫）拿掉 ⇒ ③紅，訊息指名 `skillremake:json`。改回來。
 */
...
for (const s of IO.steps)
  for (const w of s.writes ?? [])
    for (const f of /[*?[]/.test(w) ? globSync(w, { cwd: REPO }) : [w])
      claimants.set(f, [...(claimants.get(f) ?? []), s.name]);
...
describe("正規化器判準：四個執行點只准有一種說法（GH#1099）", () => {
  ...
  it("④ 隔離區：`onlyOutsideOwnWrites` 的那一支在自己 writes 裡就是**作者** ⇒ 鎖", () => {
    ...
    const [own, shared, outside, io, norms] = [
      "own.json",
      "shared.json",
      "outside.json",
      "io.json",
    ...
    execFileSync("bash", ["-c", `printf '{}' | tee ${own} ${shared} ${outside} >/dev/null`]);
    execFileSync("bash", [
      "-c",
      `cat > ${io} <<'J'\n${JSON.stringify({
        steps: [
    ...
    const mode = (f: string): number =>
      Number(execFileSync("stat", ["-f", "%Lp", f], { encoding: "utf8" }).trim());
    expect(mode(own) & 0o200, "在 fake:both 自己的 writes 裡 ⇒ 它是**作者** ⇒ 要鎖").toBe(0);
    expect(mode(shared) & 0o200, "同上（只有 fake:both 認領）").toBe(0);
    expect(mode(outside) & 0o200, "只有純正規化器認領 ⇒ ⛔ 不可以鎖（GH#707 的死路）").not.toBe(0);
    execFileSync("bash", ["-c", `chmod -R u+w ${dir}; rm -rf ${dir}`]);
  });
});
