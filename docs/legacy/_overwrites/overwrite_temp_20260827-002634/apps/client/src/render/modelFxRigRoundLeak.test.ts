...
describe("GH#782 modelFxRig 回合邊界不留殘（含動畫軌）", () => {
  it("逐回合換 modelKey 連打之後，回合邊界清完的場景計數線平；dispose 歸零", async () => {
    ...
    for (const m of scene.materials) tally.set(m.name, (tally.get(m.name) ?? 0) + 1);
    // eslint-disable-next-line no-console
    console.log("[probe] material names", [...tally.entries()].slice(0, 12));
    // eslint-disable-next-line no-console
    console.log("[probe] geometry ids", scene.geometries.slice(0, 6).map((g) => g.id));
