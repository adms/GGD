...
// ⭐ GH#734 —— 指令當下的回饋。決定住那支純函式，⛔ 這裡只呼叫一行。
import {
  orderFeedbackFor,
  orderFeedbackPolicy,
} from "./input/orderFeedback";
...
export class GameApp {
  ...
  /**
   * ⭐⭐ GH#734 —— 指令當下的回饋（環 ＋ 音），**本地零延遲**。
   *
   * ⭐ 決定住 `input/orderFeedback.ts`（一支可單獨跑的純函式）——
   * ⛔ 不在這裡寫 if：`GameApp.ts` 是 4,041 行、每條 lane 都要改一行的檔
   *（第〇·七守則的觸發器②），⭐ 而判斷留在這裡等於只驗得到字串（失敗形態⑥）。
   *
   * ⚠️ ⭐ 環走 `orderFlashUntilMs` 這個**時間戳**，⛔ 不是一個 boolean ——
   * 一個 boolean 需要有人記得把它關掉，⭐ 而時間戳自己會過期。
   */
  private orderFlashEntityId: number | null = null;
  private orderFlashUntilMs = 0;

  /** 指令環閃多久（毫秒）。⭐ 一次心跳的長度 —— ⛔ 不是一個要調的數字。 */
  private static readonly ORDER_FLASH_MS = 320;

  private applyOrderFeedback(order: Order | null | undefined): void {
    const fb = orderFeedbackFor(order, orderFeedbackPolicy());
    if (fb.flashEntityId !== null) {
      this.orderFlashEntityId = fb.flashEntityId;
      this.orderFlashUntilMs = performance.now() + GameApp.ORDER_FLASH_MS;
    }
    if (fb.cue !== null) {
      // ⭐ 選**既有**池（票文逐字：⛔ 不錄新的）。`attack` 用的是 WINDUP 那一支
      //   同一個池 —— ⚠️ 而它自己帶著節流（`playContextualVoice` 的 policy），
      //   ⇒ ⛔ 連點右鍵不會變成一串疊在一起的吼叫。
      const champ = localChampionId();
      playContextualVoice(champ, fb.cue === "attack" ? "attack-light" : `order-${fb.cue}`);
    }
  }

  /** 指令環現在該不該畫（⭐ 過期自己收掉，⛔ 不必有人記得關）。 */
  private orderFlashTarget(nowMs: number): number | null {
    return nowMs < this.orderFlashUntilMs ? this.orderFlashEntityId : null;
  }
  ...
  constructor(
    ...
    this.input = new InputCapture(canvas, {
      ...
      onOrder: (order) => {
        // ⭐⭐ GH#734 —— **按下去的那一刻就答得回來**（owner 2026-07-29：
        //   「點右鍵攻擊會讓目標物閃紅圈圈 並且玩家角色發出攻擊語音」）。
        // ⚠️ 在這一行之前這裡是**直通** `setOrder(order)` —— 零個回饋呼叫，
        //   ⭐ 而目標環那一套 decal 早就存在、⛔ 只有**手把**那條路在用。
        // ⭐ 本地零延遲：⛔ 不等 ack、⛔ 不讀 snapshot（票文逐字的規格）。
        this.applyOrderFeedback(order);
        this.sender.setOrder(order);
      },
  ...
  private renderFrame(nowMs: number): void {
    ...
    // desktop mouse — the ui/abilityHold seam) shows its dashed range + AoE.
    {
      ...
      // ⭐⭐ GH#734 —— 指令環與手把環走**同一個** indicator。
      // ⚠️ ⭐ 兩者不可能同時要畫：手把那條是「按著技能鍵時的軟鎖定」，
      //   指令這條是「剛按下右鍵的那 320ms」——⛔ 而萬一同時有，
      //   **指令環贏**（它是玩家剛剛做的動作，⛔ 手把那個是持續狀態）。
      const orderTarget = this.orderFlashTarget(performance.now());
   ...
   */
  /** 腳步聲的敵我關係 —— 純函式住 `game/appBridges.ts`（4,000 行線）。 */
