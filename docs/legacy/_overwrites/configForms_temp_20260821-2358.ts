...
const COOLDOWN_RULES_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      path: "hookMinSeconds",
      zh: "被動觸發器最短內部冷卻（秒）",
      note: "一條**觸發器**（英雄被動 / 道具被動 / 增益卡的 hook）真的發動之後，最短隔多久才能再發動。出貨 **0 ＝ 沒有地板**。⚠️ 它與上面那一格是**兩種秒**：技能冷卻填的是**卡面秒**（引擎再乘「技能冷卻時間」倍率，出貨 0.2 ⇒ 60 卡面秒 = 12 實際秒），而觸發器的內部冷卻從來就是**實際秒**、不吃那個倍率 —— 用同一格夾兩種秒就是用同一把尺量兩個空間。⭐ 想一次壓住所有被動的觸發頻率就填 **1.2**（＝冷卻表最便宜的一格：單體·極小 6 卡面秒 × 0.2）；填 0 就是回到今天。⚠️ 出貨值刻意是 0：量到有 **52 條**既有觸發器的內部冷卻低於 1.2 秒（0.5 / 0.6 / 1.0），預設拉高會一次改掉那 52 張卡的手感。⚠️ 它**只夾有填內部冷卻的那些** —— 沒填的（例如相轉移裝甲每 tick 續期的魔免）代表作者明說「每一次事件都算」，夾住它們會讓常駐效果變成閃爍的。",
    },
  ],
  // 三格純量，沒有不編輯的分支要原封帶走。
  preserved: [],
...
const REPLAY_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      path: "retainMaxFiles",
      zh: "磁碟上最多留幾份錄影（0 = 不限）",
      note: "**出貨 0 ＝ 份數不限**（GH#498）。填非 0 的話超過的從最舊的開始刪（正在錄的那一場永遠不會被刪）。實測一場 4 分鐘 12 人的比賽壓縮後約 60 KB，所以 200 份約 12 MB。調成 1＝只留最新一場，昨天那一場明天就找不回來了。⚠️ 這是**和下面那格獨立的第二條刪除規則** —— owner 2026-08-21 說的是「超過幾天」，但只把天數設成不刪的話，第 201 場照樣會把第 1 場刪掉，所以兩格一起以「不限」出貨。⭐ 想回到舊行為：這一格填 200。",
    },
    {
      path: "retainMaxAgeDays",
      zh: "超過幾天的錄影一律刪掉（0 = 永不刪）",
      note: "owner 2026-08-21：「對戰錄影 超過幾天的錄影一律刪掉 **預設不刪除**」(GH#498)。**出貨 0 ＝ 不因為年齡刪除任何錄影**。填非 0 的話和上面那格取**先觸發**的。⚠️ 錄影檔帶著每一位玩家的顯示名稱，所以 0 代表**永久保留個資**，那是 owner 的明確選擇。⚠️ 兩格都 0 ＝ 磁碟無限成長，而正式機的 docker 和錄影目錄在**同一顆碟**（塞爆過一次，網站 502）——「對戰回放」那一頁的頁首會印出目前佔用與剩餘空間，那是這個預設值唯一的煞車，請偶爾看一眼。⭐ 想回到舊行為：這一格填 30。",
    },
  ],
  preserved: [],
};
...
const LOBBY_RALLY_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      path: "waitSeconds",
      zh: "倒數幾秒後開打",
      note: "⭐ owner 明說 **10**（2026-08-21：原始規格 10 → 早上「改成五秒」→ 晚上「5->10秒」改回）。這是**期限**不是共識：時間到就開，沒趕上的位子由 BOT 補。調大＝更容易湊到真人，代價是每一場開打前都要空等；調小＝幾乎等於立刻開。⚠️ 上界 120 由 schema 給（伺服器的傳輸柵欄同界），⛔ 那不是政策，是防止一個打錯的數字把主揪關在自己按不掉的畫面裡。",
    },
    {
    ...
    },
    {
      ...
      zh: "自動加入要比開場早幾秒",
      note: "⚠️ 承重，⛔ 不是裝飾：主揪的畫面在倒數**歸零那一刻**按下開始，而一間已經開打的房會把同一刻送出的加入請求拒掉 —— 於是「預設加入」會變成「預設加入失敗」，而畫面上什麼都不會說。這一格就是那趟來回的餘裕（出貨 1.5 秒＝ 5 秒的窗口裡，玩家有 3.5 秒可以按「不要」）。調大＝更保險但可以反悔的時間更短；調小＝網路慢的人會被關在門外。⚠️ 上界 10 大於倒數秒數的下界 3，所以填得出「提前量吃掉整個窗口」—— 程式另外夾住「不得超過窗口的一半」，⛔ 讓「不要」永遠按得到。",
    },
    {
