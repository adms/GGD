 ...
 *
 * ── ⚠️ 判定的一個補丁（⛔ 不是第二套判準）─────────────────────────────────
 * `snapshotChannels()` 今天**看不見召喚**（`world.summon` 不在六個頻道裡，`spawnSummon`
 * 也不掛狀態／buff／事件）⇒ 照它自己的檔頭，那是「缺一個 kind 是**假的 ❌**」。
 * 正解是在 `sim/castabilityVerdict.ts` 補一格 `summons`（GH#1078 報告已列）；在那之前
 * 這裡只多量 `world.summon.size` 這**一根指針**，⭐ 它補上的那天把 {@link EXTRA_CHANNELS} 刪掉。
 *
...
import { classifyCastOutcome, snapshotChannels } from "../../sim/castabilityVerdict";
...
/** 出貨判定看不見、而這裡多量的指針（見檔頭）。 */
const EXTRA_CHANNELS: Record<string, (w: SimWorld) => number> = { summon: (w) => w.summon.size };
...
function probe(t: TemplateDoc): Row {
  ...
  if (def.description?.includes(DEGRADED_ABILITY_NOTE)) {
  ...
  const extraBefore = Object.fromEntries(Object.entries(EXTRA_CHANNELS).map(([k, f]) => [k, f(world)]));
  const events: string[] = [];
  ...
  let out = { verdict: "FAIL" as string, channel: undefined as string | undefined, reason: undefined as string | undefined };
  for (let i = 0; i < window; i++) {
    ...
    const extra = Object.entries(EXTRA_CHANNELS).find(([k, f]) => f(world) > extraBefore[k]!);
    if (extra) { out = { verdict: "PASS", channel: extra[0], reason: undefined }; break; }
  }
...
describe("每一份 enabled 模板的預設展開，在真的 SimWorld 裡施放要有東西動（GH#1078）", () => {
  it("★ 35/35：主動 PASS／被動掛上來源／純演出生得出模型 —— ⛔ 沒有一份是 no-op", () => {
    ...
    // ⭐ GH#1076 的那一格要靠**召喚指針**才看得見（⛔ 不是靠 damage 順便綠）。
    expect(rows.find((r) => r.id === "tpl-summon-agent")?.channel, "tpl-summon-agent 要由召喚指針量到").toBe("summon");
  });
});
