 ...
 * 改回 `if writable:`（＝修復前的行為）→ ①③④ 三條一起紅，①的訊息指名 EACCES。改回來。
 */
...
describe("隔離區 × genguard：同一個檔只准有一種說法", () => {
  ...
  writeFileSync(
    ...
    JSON.stringify({
      steps: [
        ...
        { name: "fake:norm", writes: [normOnly, shared] },
      ],
    }),
  );
  ...
  it("③ 模擬 genrun 的解鎖→跑→重鎖：收工後那些檔還是可寫的", () => {
    ...
    run(["unlock"]);
    for (const f of [author, normOnly, shared]) chmodSync(f, 0o644);
  });
