 ...
 *
 * ① **GH#797 修復錨**：登記的 commit 真的碰過這一批嗎 —— 判準是 **diff**，⛔ 不是訊息。
 *    ⭐ 承重的那一跳是 `modelKey → content/models/*.json → glbPath → .glb`：
 *      少了它，`invprim` 這一批（登記 `e44bf446`，而它動的正是 `revivehuman.glb`）
 *      會被誤判成「錨錯了」。⚠️ 實測 #797 票文提的**寬目錄白名單**在它自己的例子上
 *      得到相反的結論 —— 所以這裡驗的是**推導**，⛔ 不是白名單。
 * ② **GH#756 語音進漏斗**：triage 認得 voice，而門檻**讀自 gate JSON**（⛔ 不抄字面值）。
 *
 * 突變驗證（2026-08-27）：`fix-anchor.mjs` 的 `fixTouchesBatch` 開頭插
 * `return { status: "touches", matched: [], claimed: 0, commits: [], unresolved: [] };`
 * → ① 的「evidence-only／unrelated／判不了」三條全紅（帳本回到會說謊的狀態）。
 */
