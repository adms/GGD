...
describe("蝗蟲群視覺 · QUAD 四小族（GH#688 Phase 6）", () => {
  ...
  it("無落點清單的理由保持成立（不成立的那一天這裡紅 ⇒ 把落點接上）", () => {
    ...
    const dtr = JSON.stringify(combo).includes('"dragontigerready"')
      ? (combo.strikes ?? combo.combos ?? []).find?.((c: any) => c.key === "dragontigerready")
      : null;
    if (dtr) expect(dtr.ownerKind, "A0J2 有了英雄擁有者 —— o019 動地剁該綁上去了").toBe("map-mechanic");
  });
});
