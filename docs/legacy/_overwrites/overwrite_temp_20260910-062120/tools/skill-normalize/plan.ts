...
async function build(): Promise<{ rows: Row[]; meta: Record<string, unknown> }> {
  ...
  for (const def of Abilities.all()) {
    ...
    if (process.env.GGD_DBG_ID && String(raw["id"] ?? "").includes(process.env.GGD_DBG_ID)) {
      console.error(`[dbg] ${String(raw["id"])} agg=${DAMAGE_AGG} headline=${dmg.headline} ceiling=${dmg.ceiling} guaranteed=${dmg.guaranteed} basis=${JSON.stringify(dmg.basis)}`);
    }
    const dmgCol: Column =
      dmgValue > 0
