...
export const PLANNED_CAPABILITIES: readonly CapabilityEntry[] = [
  ...
  // ══ 2026-08-22 —— #551【移動中的模型特效】· #543【螢幕回饋】· #549【特效文字】══
  {
    ...
    caveat:
      "⭐ **兩半都到齊了（2026-08-23，GH#606）**：玩法那一半（路徑求值、`onTouch` 的" +
      "逐段取樣、`onArrive` 的落點結算，班表推進**既有的** `SimWorld.delayed`）在 sim 裡，" +
      "視覺那一半在 `apps/client/src/render/modelFxRig.ts`，事件也在 `eventFanout` 白名單上。" +
      "⚠️ **在 2026-08-23 之前這一格的 caveat 是對的而我們自己不知道**：sim 送 " +
      "`{ caster, modelKey, instances, … }`,客戶端讀 `ev.data.spec` —— 零個寫入端 ⇒ " +
      "消費端第一行就 `break`,整族在畫面上一具模型都沒出現過。" +
      "⭐ 根治不是改欄位名（下一個欄位照樣會歪）,是**兩邊 import 同一個型別** " +
      "`ModelFxSpawnEvent`（住 `sim/effects/spawnModelFx.ts`）。" +
      "⛔ **落點特效不由客戶端排**:寫在技能 JSON 的 `onArrive: [{ kind: \"spawnVfx\" }]`," +
      "它走 sim 的延遲班表 ⇒ 特效與傷害同 tick 同點。客戶端再排一次會差幾幀。" +
      "⚠️ 模型離地高度是 `model@1.fxSpawnHeight`（缺席 = 貼地),⛔ 不是技能的欄位 —— " +
      "「這根東西該飛多高」是網格自己的性質,同 `fxLongAxis`。",
    evidence:
