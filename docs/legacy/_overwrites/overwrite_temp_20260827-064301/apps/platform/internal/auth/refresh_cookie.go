...
// Secure attribute is set when it will actually be honoured and omitted when
// setting it would make the browser DISCARD the cookie (plain-http LAN play).
//
// ⚠️ X-Forwarded-Proto is only consulted for a request that arrived from a
// trusted peer — the same rule httpx.ClientIP applies to X-Real-Ip, and for the
// same reason: a header anyone can send must not decide anything on its own.
// Getting this wrong is not a security hole (the worst case is a cookie without
// Secure on a connection that already had TLS) but it would be a lie.
func requestIsSecure(r *http.Request) bool {
	...
	return strings.EqualFold(strings.TrimSpace(firstProto(r.Header.Get("X-Forwarded-Proto"))), "https")
}

// firstProto takes the left-most entry of a possibly-chained X-Forwarded-Proto.
...
// peerIsTrusted mirrors the trusted-edge test the rate limiter uses. It is a
// thin wrapper so this file does not reach into httpx's internals; loopback and
// the private ranges are where our own edge always is.
...
func refreshTokenFrom(r *http.Request, body string) string {
	...
	return "" // MUTANT: cookie fallback removed
}
