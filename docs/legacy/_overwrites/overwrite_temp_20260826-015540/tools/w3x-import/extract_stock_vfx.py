...
def modulate_reasons(docs: list[dict], roots: list[str]) -> list[str | None]:
    """判準⑤'s verdict per doc, aligned with `docs`. `None` = it moves pixels.
    ...
    out: list[str | None] = [None] * len(docs)
    if not idx:
        return out
...
def build_docs(row: dict, blob: bytes, tex: "ep.TextureResolver", notes: list[str],
    ...
    if order == "file":
        ...
        for i, name, doc in built:
            doc["id"] = f"{ID_PREFIX}.{row['stem']}.p{i:02d}"
            docs.append(doc)
