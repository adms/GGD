...
ELEMENT_CUES = {"fire": "magicFire", "ice": "magicIce"}
...
def build(docs: dict[str, dict], scan: dict) -> dict:
    ...
    for ability in sorted(docs):
        ...
        def skip(cause: str, extra: dict) -> None:
        ...
        # ⭐ 順序就是證據強度，⛔ 不是方便：**原作替這一支綁了什麼**永遠先被回答，
        # 就算答案是「綁了但位元組不在」——那一列比一句家族通用音更該被看見。
        if len(usable) == 1:
            suggestions.append(row("A-jass-direct", usable[0], {"ggSnds": snds}, True))
        elif len(usable) > 1:
            suggestions.append(
                row("B-jass-multi", None, {"ggSnds": snds, "candidates": usable}, False)
            )
        elif snds:
            silent = any(g in reserved_silent for g in snds)
            skip("source-map-silent" if silent else "clip-not-extracted", {"ggSnds": snds})
        elif isinstance(own_launch, str) and own_launch in sfx:
            # 特效層已經在播一個**逐支指名**的 clip ⇒ 施法層寫同一個 key
            suggestions.append(
                row(
                    "A-vfx-owned",
                    own_launch,
            ...
            # ⛔ 它本來就不在通用池上 ⇒ 家族通用音對它不是一個「修好」
            skip("element-whoosh-already", {"elementCue": elem})
        elif (
            isinstance(family_launch, str)
            and family_launch in sfx
            # ⛔ 家族原型就是通用池本身時，「建議」它＝逐字建議今天的行為（第一·五守則）
            and family_launch != (elem or GENERIC_CUE)
        ):
            suggestions.append(
                row(
                    "C-family-generic",
            ...
            if tag:
                skip("stock-label-only", {"base": base, "effectSound": tag})
            else:
                skip("no-source-audio", {"base": base})
...
def run(check: bool) -> str | None:
    """寫（或 --check）建議表。回傳過期的相對路徑，或 None。"""
    text = dumps(build(shipped_docs(), load(SFX_BINDINGS)))
    current = None
...
if __name__ == "__main__":
    stale = run("--check" in sys.argv[1:])
    if stale:
        print(f"ERROR: {stale} 過期了 ⇒ 跑 `pnpm sfxbind:build` 然後 git add", file=sys.stderr)
