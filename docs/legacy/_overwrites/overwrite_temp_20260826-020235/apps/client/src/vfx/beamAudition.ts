...
export async function startBeamAudition(
  ...
  /** render ×2 → readPixels（坑②：不這樣做會讀到上一幀）。 */
  const readRaw = async (): Promise<{ w: number; h: number; buf: Uint8Array }> => {
    scene.render();
    scene.render();
    const w = engine.getRenderWidth();
    const h = engine.getRenderHeight();
    return { w, h, buf: (await engine.readPixels(0, 0, w, h)) as Uint8Array };
  };
  ...
  const probeDocs = async (
    ...
    const { toParticleSystem } = await import("./particleFactory");
