...
describe("陣營轉換：借走一隻殭屍王 (mind-control / 大師球)", () => {
  it("捕獲之後：敵方英雄選得到牠、捕獲者選不到、殭屍打牠、而且那一列快照真的過網了", async () => {
    ...
    // TEMP-PROBE
    {
      const t = w.mob.get(zombie)!.target;
      const tz = w.transform.get(zombie)!.pos, tb = w.transform.get(bossId)!.pos;
      const tt = t == null ? null : w.transform.get(t as never)?.pos;
      const d = (a: {x:number;z:number}, b: {x:number;z:number}) => Math.hypot(a.x-b.x, a.z-b.z).toFixed(2);
      // 量這兩行各自剔掉什麼
      {
        const MT = 255;
        let byTeam = 0, byMob = 0, kept = 0, mobsNotMonster: number[] = [];
        for (const [cid, ct] of w.team) {
          if (ct.teamId === MT) { byTeam++; continue; }
          if (w.mob.has(cid)) { byMob++; mobsNotMonster.push(cid); continue; }
          kept++;
        }
        console.log("PROBE3>> 第一行(MONSTER_TEAM)剔掉", byTeam,
          "| 第二行(world.mob.has)剔掉", byMob, "⇒", mobsNotMonster,
          "| 留下", kept, "| 召喚物在 world.mob 嗎", [...w.summon.keys()].map((id)=>w.mob.has(id)));
      }
      const { isMindControlled } = await import("@ggd/shared/sim/mindControl");
      const { isMobTargetable } = await import("@ggd/shared/sim/targeting");
      console.log("PROBE2>> mindControl.has(boss)", w.mindControl.has(bossId),
        "| isMindControlled", isMindControlled(w, bossId),
        "| isMobTargetable(boss, zombie)", isMobTargetable(w, bossId, zombie),
        "| mob.has(boss)", w.mob.has(bossId));
      console.log("PROBE>> zombie", zombie, "target", t,
        "| boss", bossId, "team", w.team.get(bossId)?.teamId,
        "| target 是英雄?", t != null && w.champion.has(t as never),
        "| target team", t == null ? null : w.team.get(t as never)?.teamId,
        "| dist(zombie,boss)", d(tz, tb),
        "| dist(zombie,target)", tt ? d(tz, tt) : "?",
        "| zombie team", w.team.get(zombie)?.teamId);
    }
