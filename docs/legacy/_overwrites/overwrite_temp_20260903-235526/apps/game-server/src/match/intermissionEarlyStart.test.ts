...
describe("MEASURE", () => {
  it("1 真人 + 11 bot：按 Ready 之後還要幾個 tick 才進戰鬥？", () => {
    ...
    const notReady = [...ctl.seats.values()].filter((s) => s.entityId !== null && !s.ready).length;
    console.log(
      `[MEASURE] phase=${ctl.phase.phase} waitedTicks=${waited} (~${(waited / 30).toFixed(1)}s) ` +
        `humanReady=${seat.ready} seatsNotReady=${notReady} offers=${ctl.offers.size} ` +
        `intermissionTicks=${CFG.intermissionTicks}`,
    );
