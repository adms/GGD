...
describe("觸發接線:施法窗口 → 集氣（隊色）", () => {
  ...
  it("內縮:同一循環內光束單調靠近身體（集氣的靈魂）", () => {
    ...
    const d1 = (fx.update(1000 + cycleMs * 0.15), distToChest(fx, 9, 0));
    const d2 = (fx.update(1000 + cycleMs * 0.45), distToChest(fx, 9, 0));
    const d3 = (fx.update(1000 + cycleMs * 0.8), distToChest(fx, 9, 0));
