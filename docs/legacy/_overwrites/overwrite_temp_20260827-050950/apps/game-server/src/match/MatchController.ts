...
export class MatchController {
  ...
  selectChampion(seatId: SeatId, championId: string): SelectResult {
    ...
    // ⭐ GH#726 ① —— **座位鎖**的權威閘。出貨的客戶端一次點擊就是「選 + 鎖」
    // （`ChampSelectPanel.commit`，鎖定之後名單就凍結），所以**第一次成功的
    // selectChampion 就是鎖定**；再來一次只可能來自改造過的客戶端。
    //
    // ⚠️ 位置在**所有寫入之前**：寫在下面會讓被拒的那一次仍然改掉 `championId`
    //（失敗形態②的鏡像 —— 擋下來了但東西還是變了）。
    //
    // 強制與否是一格後台開關（`match/integrityPolicy.ts`），⛔ 但 `lockedSeats`
    // **不管開關照樣記** —— 開關管的是「拒不拒絕」，⛔ 不是「要不要告訴大家」，
    // 而 `SeatState.locked` 永遠照實反映伺服器知道的事。
