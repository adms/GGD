...
#
# ⛔ THE PER-FILE EXIT CODE USED TO BE DROPPED ON THE FLOOR (#772 — the twin of
# the #749 hole one function down). `sha256sum "$f"` sits in a `while` loop that
# is the TAIL of a pipeline inside a subshell, so a failure printed one stderr
# line and changed nothing: the loop's status is only the LAST iteration's, the
# pipeline's is only the loop's, and `set -eu` therefore saw success unless the
# very last file happened to be the broken one. An unreadable file was simply
# ABSENT from SHIP.sha256 — while count_of() went on counting it. Both sides of
# a later `verify` then skip the same file, so the omission VERIFIES GREEN.
#
...
cmd_manifest() {
  ...
  # Same shape as the bytes_of bail above (#772): fail BEFORE a half-written
  # SHIP.sha256 can be mistaken for a good one. `|| list_ok=0` because under
  # `set -e` a bare failing redirect would abort with no diagnosis at all.
  list_ok=1
  ...
  # TWO NOUNS, ONE RELATION (#772). `files=` is counted by count_of() and the
  # listing is written by listing_of(); checking each half alone is green in
  # exactly the case that matters. This is the only place both exist at once.
  case "${GGD_ASSET_STRICT_READ:-1}" in
    0|no|off|false) ;;
    *)
      ...
      if [ "$listed" != "$files" ]; then
        rm -f "$dir/SHIP.sha256"
        echo "$SELF: FAIL — '$name': SHIP.sha256 lists $listed files but the set contains $files. The listing dropped files silently; a manifest built on it would verify green against the same gap." >&2
        exit 1
      fi
      ;;
  esac
