  ...
  for (let p = 8; p + 8 <= buf.length;) {
    ...
    if (type === "IHDR") {
      ...
      if (bitDepth !== 8 || interlace !== 0) {
        throw new Error(
          `PNG 不在支援範圍（bitDepth=${bitDepth} interlace=${interlace}）`,
        );
      }
 ...
 */
export function modulateMaxDelta(
...
function effectiveColors(doc: Vfxish): Rgba[] {
  if (doc.colorStops && doc.colorStops.length > 0)
    return doc.colorStops.map((s) => s[1]);
 ...
 */
export function birthVisibilityDefects(
  doc: Vfxish,
  tex?: TexStats | null,
): string[] {
  ...
  if (doc.schema === "vfx@1") {
    ...
    if (peakA <= PEAK_ALPHA_MIN) {
      reasons.push(
        `生命內 peak alpha=${peakA} ≤ ${PEAK_ALPHA_MIN} —— 整條生命都是透明的`,
      );
    }
    const sizes =
    ...
    if (doc.blendMode === "additive" || doc.blendMode === "modulate") {
      const peakRgb = Math.max(
        ...colors.map((c) => Math.max(c[0], c[1], c[2])),
      );
    ...
    if (width <= 0)
      reasons.push(`widthAbove+widthBelow=${width} —— 零寬的帶子沒有面積`);
    ...
    if (
      ...
      reasons.push(
        `${doc.blendMode} 疊全黑（max(R,G,B)=${Math.max(c[0], c[1], c[2])}）`,
      );
    }
  }
  return reasons;
  ...
  if (!TEX_CACHE.has(rel)) {
    ...
    try {
      TEX_CACHE.set(
        rel,
        existsSync(p)
          ? texStatsFromRgba(decodePng(readFileSync(p)).rgba)
          : null,
      );
    } catch {
...
describe("👁 vfx 出貨態掃描：每份文件要有可能畫出一個像素", () => {
  it("⭐ 量尺自驗：三份必不可見的 sentinel 文件，檢查器全部抓得到", () => {
    ...
    expect(
      birthVisibilityDefects(alphaZero).join(),
      "alpha 0 應該被抓到",
    ).toMatch(/peak alpha/);
    ...
    expect(
      birthVisibilityDefects(additiveBlack).join(),
      "additive 全黑應該被抓到",
    ).toMatch(/疊全黑/);
    ...
    expect(
      birthVisibilityDefects(sizeZero).join(),
      "size 覆寫成 0 應該被抓到",
    ).toMatch(/size/);
    ...
    expect(
      birthVisibilityDefects(visible),
      "普通文件被誤抓 —— 量尺壞了",
    ).toEqual([]);
    ...
    expect(
      birthVisibilityDefects(flatRibbon).join(),
      "零寬 ribbon 應該被抓到",
    ).toMatch(/零寬/);
  });
  ...
  it("⭐ 量尺自驗（⑤ modulate 恆等）：白貼圖被抓到、真貼圖不被誤抓", () => {
    ...
    const synth = (
      n: number,
      f: (i: number) => [number, number, number, number],
    ): TexStats =>
      texStatsFromRgba(
        Uint8Array.from(Array.from({ length: n }, (_, i) => f(i)).flat()),
      );

    // ① 全白貼圖（RGB=255，alpha 任意）× 白色 doc ⇒ δ ≡ 0 ⇒ 恆等
    const white = synth(256, (i) => [255, 255, 255, i]);
    expect(
      birthVisibilityDefects(modDoc("sentinel-mod-ident"), white).join(),
    ).toMatch(/恆等/);
    ...
    expect(
      birthVisibilityDefects(tinted, white),
      "有色 modulate 被誤抓 —— 量尺壞了",
    ).toEqual([]);
    ...
    expect(modulateMaxDelta([[1, 1, 1, 1]], real!)).toBeGreaterThan(
  ...
  it("⭐ KNOWN_INVISIBLE 過期偵測：修好的（或消失的）要從名單刪掉", () => {
    ...
    for (const id of Object.keys(KNOWN_INVISIBLE)) {
      ...
      if (!doc) {
        ...
        birthVisibilityDefects(doc, texStatsFor(doc.texture)).length === 0
      ) {
        stale.push(`${id}（已經可見了）`);
      }
    }
