...
function readmeLine(inside: boolean): string {
  ...
  for (const line of lines) {
    ...
    else if (depth > 0 === inside && line.length > 40 && text.split(line).length === 2) picked.push(line);
  }
  // ⚠️ 只收**全檔唯一**的行（`split().length === 2`）—— 一個出現兩次的 old_string
  //   會讓「它落在哪一段」變成兩個答案，而那不是這條測試要問的東西。
