...
const SHIPPED = JSON.parse(
  readFileSync(fileURLToPath(new URL("../../../../content/config/model-lod.json", import.meta.url)), "utf8"),
) as typeof DEFAULT_MODEL_LOD;
...
describe("平台政策 (client-platform-policy)", () => {
  ...
  it("⛔ 平板（iPad mini 744×1133，iPadOS 只有 maxTouchPoints 說真話）⇒ 沒有告知，fps 上限吃 config", () => {
    ...
    applyPlatformPolicy({ ...SHIPPED.platformPolicy, tabletFpsCap: 45 });
