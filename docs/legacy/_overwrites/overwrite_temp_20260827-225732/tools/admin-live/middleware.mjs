...
function globHit(glob, path) {
  const re = new RegExp(
    "^" + glob.split("").map((c) => (c === "*" ? "[^/]*" : /[a-zA-Z0-9_/.-￿-]/.test(c) ? c : "[" + c + "]")).join("") + "$",
  );
  return re.test(path);
}

/** JSON pointer 樣式比對：`/entries/*/weight` 的 `*` 命中任一段。 */
...
/** 走到 pointer 的父節點，設值（value===null ⇒ 刪欄位）。中途節點必須已存在。 */
function setAtPointer(doc, pointer, value) {
  const segs = pointer.split("/").filter((s) => s !== "");
  let node = doc;
  for (let i = 0; i < segs.length - 1; i++) {
    node = node?.[Array.isArray(node) ? Number(segs[i]) : segs[i]];
    if (node === null || typeof node !== "object")
      return { error: `路徑走不到 /${segs.slice(0, i + 1).join("/")} —— ⛔ 這個端點不創造中間節點` };
  }
  const last = Array.isArray(node) ? Number(segs[segs.length - 1]) : segs[segs.length - 1];
  if (Array.isArray(node) && !(Number.isInteger(last) && last >= 0 && last < node.length))
    return { error: `陣列索引 ${String(last)} 越界（長度 ${node.length}）` };
  const old = node[last];
  if (value === null) delete node[last];
  else node[last] = value;
  return { old: old === undefined ? null : old };
}
...
async function handleSave(repoRoot, mode, entry, dsName, body, res) {
  ...
  try {
    ...
    return sendJson(res, 500, { error: `讀不動 ${path}：${String(err)}` });
  }
  const applied = setAtPointer(doc, pointer, value);
