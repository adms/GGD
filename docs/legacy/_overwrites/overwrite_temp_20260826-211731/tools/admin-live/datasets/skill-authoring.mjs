...
function scanShipped(repoRoot, effectKinds) {
  const dir = join(repoRoot, "content/abilities");
  let key = "absent";
  try {
    key = `${statSync(join(dir, "_index.json")).mtimeMs}|${statSync(dir).mtimeMs}`;
  } catch {
    /* 目錄缺了就每次重掃（會掃到空） */
  }
  ...
  const usage = new Map(); // kind → count
  const example = new Map(); // kind → { sourceAbility, node, bytes, tierScore }
  const tagVocab = new Map(); // tag → count
  let abilityCount = 0;
  let files = [];
  try {
  ...
  for (const f of files) {
    ...
    for (const node of nodes) {
      ...
      if (
        !prev ||
        tierScore > prev.tierScore ||
        (tierScore === prev.tierScore && json.length < prev.bytes)
      ) {
        example.set(node.kind, { sourceAbility: doc.id, node, bytes: json.length, tierScore });
      }
    }
  }
