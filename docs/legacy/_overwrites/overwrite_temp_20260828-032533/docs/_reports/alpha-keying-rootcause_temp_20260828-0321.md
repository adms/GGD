...
join 得到 filterMode 的 **238** 份＝ 217 份地圖內建（`out/GoDieEX22s{,-src}/raw/*.mdx`）
＋ 21 份 stock（四顆 MPQ，`convert_stock_model.py::STOCK_MODELS`）。
⛔ join 不到的 185 份是 KayKit props / scenery / hex / 體素英雄等**非 w3x 資產**（沒有 filterMode 這個概念）
...
| 能 join 回 WC3 來源拿到 filterMode 的 | **238** | 見 §2.3 的分母說明 |
...
| **⛔ 不可以靜默跳過** | join 不到來源的 .glb 要**逐檔列名**進一張明示的清單（今天 185 份，其中 172 份是非 w3x 資產、13 份是來源 MDX 不在磁碟）。⚠️ 「跳過」與「全過」長得一模一樣 —— fail-open 沒錯，靜默才是缺陷 |
