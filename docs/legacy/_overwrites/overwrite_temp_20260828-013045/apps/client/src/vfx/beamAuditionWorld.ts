...
export async function buildBeamAuditionWorld(
  ...
  function doCast(): void {
      // ⭐ GH#838 —— 槽位由 abilityId 後綴推導（`.e`→E、`.q`→Q…），⛔ 不寫死 R：
      //    studio 要放的三招橫跨 E/R/EX。beam 頁全是 `.r` ⇒ 這一行對它們逐位元不變。
      const suffix = abilityId.slice(abilityId.lastIndexOf(".") + 1).toUpperCase();
      const slot = (["Q", "W", "E", "R", "EX"].includes(suffix) ? suffix : "R") as
        | "Q"
        | "W"
        | "E"
        | "R"
      ...
      if (ab) {
        const s = (ab.slots as Record<string, { cooldownRemainingTicks: number; rank: number }>)[
          slot
        ];
        if (s) {
          s.cooldownRemainingTicks = 0;
          if (s.rank < 1) s.rank = 1; // E/Q/W 在 spawnFighter 可能沒點 —— 抬到 1 才放得出
        }
      }
