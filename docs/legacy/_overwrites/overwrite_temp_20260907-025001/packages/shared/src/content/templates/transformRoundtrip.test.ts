 ...
 *
 * ⚠️ 三條，⛔ 沒有一條靠讀散文：
 *  ① diff=0 roundtrip —— 預設值展開 ≡ 出貨的 `godie-ofar.r`（58-04，最單純的一支）。
...
/** ⭐ 出貨那一支的 params（它已經接上模板了）⇒ 展開結果就是「出貨的 effects」。 */
const SHIPPED = read("content/abilities/godie-ofar.r.json");
const SHIPPED_PARAMS = (SHIPPED["template"] as { ref: string; params: Record<string, unknown> }).params;
...
describe("變身積木（GH#1067）", () => {
  ...
  it("★★ ⭐ **diff=0 roundtrip**：預設展開 ≡ 出貨的 `godie-ofar.r`（58-04）", () => {
    const byDefault = expand({});
    const shipped = expand(SHIPPED_PARAMS);
    expect(
      JSON.stringify(byDefault.effects),
      "⛔ 預設值展開出來的不是出貨那一支 —— 模板在編輯器裡看起來正常，而它產出的技能不是同一個",
    ).toBe(JSON.stringify(shipped.effects));
    expect(byDefault.castType).toBe("self");
    expect(byDefault.effects.map((e) => e.kind)).toEqual(["championForm"]);
  });
