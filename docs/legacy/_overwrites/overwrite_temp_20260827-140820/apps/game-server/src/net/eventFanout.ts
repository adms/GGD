...
export const FANNED_OUT_EVENT_TYPES: ReadonlySet<string> = new Set<string>([
  ...
  // 打完之後就 `world.emit`（`sim/effects/chainLightning.ts` 的檔頭④），而它
  // 非這樣不可：逐跳跨了很多個 tick，整段結束才送一次的話玩家看到的是一排血條
  // 在不同時間掉、畫面上什麼都沒有（＝第②種故障）。`chains`/`hits` 因此恆為 1，
  // `segments` 恆為一段 —— 欄位名是複數，是為了讓「一次送整段」仍然表達得出來。
  //
  // 預算（2026-08-20 量的，⛔ 不是推的）：出貨兩支（65-04 天譴 / 86-04 打雷絕招）
  // 都是 `maxSources 20 · jumps 16 · jumpIntervalSec 0.05`，所以**一次施放最多
  // `min(20×16, maxTotalJumps)` = 320 則**，一則 JSON 約 123 B ⇒ 約 39 KB，
  // 攤在 16×0.05 = 0.8 秒裡。
  // ⭐ 所以「每次施放的事件上限」**不需要第二格後台欄位** —— 事件數恆等於跳數，
  //   而跳數已經被 `maxTotalJumps`（出貨 480，硬上限 `CHAIN_MAX_TOTAL_JUMPS`）
  //   夾住了。⛔ 再加一格會變成兩個描述同一件事的旋鈕，而它們遲早會互相矛盾。
  "chainLightning",
  ...
  "guardianHeirPulse",
  "guardianSlain",
  // ⭐ 戰場任務的陣營塔 (GH#752 mini dota). owner 點名「**任務提示要明顯**」，
  // 而那句話的整條線路就是這兩則：
  //   · `objectiveSpawn`     —— `{ id, zone, teamId, x, z, maxHp, onDestroyed }`。
  //     ⭐ 這是 HUD 立起任務條的**唯一**訊號：⛔ 客戶端不該去掃快照猜「這場有沒有塔」
  //     （那會在塔還沒進 view 的第一秒說謊）。
  //   · `objectiveDestroyed` —— `{ id, zone, teamId, x, z, killerSeatId }`。
  //     ⭐ 它與回合結束是**同一個瞬間**（主機在下一次 `checkCombatEnd` 判掉那一區），
  //     所以演出接在這一則上才對得準；⛔ 接在 `duelWinner` 上會晚一拍且說不出「為什麼」。
  // ⚠️ 兩則的客戶端消費端**還沒接**（本 lane 的檔案柵欄外）—— 見 #752 的接線清單。
  "objectiveSpawn",
  "objectiveDestroyed",
