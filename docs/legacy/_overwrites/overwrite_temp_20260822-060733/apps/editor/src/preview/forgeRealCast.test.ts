/**
 * GH#174 —— 工坊的「即時試放」真的放得出去，而且畫面跟著 sim 走。
 *
 * ⭐ 承重的那一條線是 `castAbility` 裡的 `world.step(intents)`：把它換成空的
 * intent（＝指令沒送出去）之後，下面第一條 `it` 立刻紅 —— 因為 `abilityCast`
 * 不會出現、魔力不會掉、冷卻不會轉。⛔ 這一條刻意**不驗數字**（傷害多少、
 * 冷卻幾秒都是後台在調的），只驗「這一發到底有沒有發生」。
 */
...
describe("GH#174 鑄技工坊的試放走玩家那條路", () => {
  it("送出的是 IntentFrame，答案是 sim 自己喊的 —— 拒絕時也拿得到理由", () => {
    ...
    // 所以拿得到理由本身就證明了這一發**穿過了那些閘**，不是被直接呼叫進去的。
