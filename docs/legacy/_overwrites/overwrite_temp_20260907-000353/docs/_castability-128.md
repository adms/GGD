...
- **格數**：51 英雄 × 7 槽 = **357**
- **✅ PASS：268 / 357**（75.1%）　🟣 PASSIVE：65　🟡 只有特效：0　❌ FAIL：19　— 無此格：5
- 把「正確的永久被動」算進可接受行為：**333 / 357**（93.3%）如預期運作，真正的缺口是 **19** 格（❌ 19 ＋ 🟡 0），另有 **0** 格 🔵 本次未量測（形態閘）。
- **閘 3 在看的那個數字**（只算版控首發名單那 49 人、扣掉「無此格」）：**323 / 342 = 94.44%**（棘輪下限 100.00%）。
- 英雄生成失敗：**0**（無）
...
| --- | --- | --- | --- | --- | --- |
| 火霧戰士 - 夏娜 | `godie-e008` | E | skillshot | 近 | no effects authored (empty effect list) |
| 種子神奇寶貝 - 妙蛙種子 | `godie-hgam` | W | ground | 近 | no effects authored (empty effect list) |
| 除魔巫女 - 桔梗 | `godie-hvwd` | PASSIVE | targeted | 遠 | no effects authored (empty effect list) |
| 黑暗福音 - 依文潔琳 | `godie-n003` | W | self | 遠 | no effects authored (empty effect list) |
| 黑暗福音 - 依文潔琳 | `godie-n003` | E | skillshot | 遠 | no effects authored (empty effect list) |
| 小叮噹 - 哆拉A夢 | `godie-n00b` | E | ground | 近 | no effects authored (empty effect list) |
| 傳說的龍騎士 - 勇者小呆 | `godie-nbbc` | Q | targeted | 近 | no effects authored (empty effect list) |
| 傲嬌電氣老鼠 - 皮卡娘 | `godie-o00k` | W | ground | 遠 | no effects authored (empty effect list) |
| 獸神官 - 傑洛士 | `godie-o00l` | Q | skillshot | 遠 | no effects authored (empty effect list) |
| 夢幻之星 - 初音 | `godie-o02p` | W | targeted | 近 | no effects authored (empty effect list) |
| 神奇寶貝兒 - 皮卡丘 | `godie-ofar` | PASSIVE | ground | 遠 | no effects authored (empty effect list) |
...
| 賽亞人 - 悟空 | `godie-ogrh` | W | ground | 近 | no effects authored (empty effect list) |
| 電車癡漢 - 臭作 | `godie-orkn` | Q | targeted | 遠 | no effects authored (empty effect list) |
| 電車癡漢 - 臭作 | `godie-orkn` | R | skillshot | 遠 | no effects authored (empty effect list) |
| 鬼畜紅王 - 鬼畜狂刀KYO | `godie-u00h` | E | skillshot | 近 | no effects authored (empty effect list) |
| 草帽小子 - 蒙其.D.魯夫 | `godie-u00n` | E | skillshot | 近 | no effects authored (empty effect list) |
| 黑手黨老大 - 基廉列克 | `godie-u00v` | W | skillshot | 近 | no effects authored (empty effect list) |
| 至尊學長 - 飛鼠先生 | `godie-udea` | Q | ground | 近 | no effects authored (empty effect list) |
...
- 每次施放後的觀察窗是**逐支算出來的**（`observationWindow`）：基礎 **26 tick**　＋　吟唱 `castWindow` = round(castTimeSec × 30) + 1　＋　位移飛行 `leapWindow`。普攻另給 **40 tick** 讓第一次揮擊落地。本次實測最長的一格是 **120 tick**（`godie-hpb1.e`）。
