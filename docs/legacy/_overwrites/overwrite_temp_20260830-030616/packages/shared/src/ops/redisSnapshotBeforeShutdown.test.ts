...
describe("停 Redis 之前一定先快照 —— 排行榜與 M幣不是快取（GH#860）", () => {
  ...
  it("★ 每一支會重建容器的腳本，都要在那一行**之前**呼叫 redis-snapshot.sh", () => {
    ...
    for (const f of scripts) {
      ...
      // ⛔⛔ 找的是**呼叫**，⛔ 不是「這個檔案有沒有提到它」。
      //   第一版沒排除函式**定義**那一行 ⇒ `redis_snapshot_before_shutdown() {`
      //   自己就滿足了條件 ⇒ ⭐ **把兩個呼叫全部刪掉，這條守衛照樣是綠的**
      //     （突變驗證當場抓到 —— 失敗形態⑥「用掃原始碼字串代替行為」）。
      const snap = lines.find(
        (l) =>
          /redis-snapshot\.sh|redis_snapshot_before_shutdown/.test(l.text) &&
          !/^\s*(?:function\s+)?[A-Za-z_][\w-]*\s*\(\)\s*\{?/.test(l.text),
      );
