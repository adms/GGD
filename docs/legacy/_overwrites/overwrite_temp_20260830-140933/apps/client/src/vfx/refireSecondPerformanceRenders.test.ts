...
describe("🎇 GH#842 出貨路徑：連續戰鬥中，**第二發之後**的特效真的還在場上 (@visual-proof)", () => {
  ...
  it("⭐ 出貨檔位（performance）—— 每一發炸開之後場上都還有粒子", () => {
    ...
    expect(r.samplesPastCap, "⛔ 上限門檻之後一發都沒量到 —— 情境沒有跑到那裡").toBeGreaterThan(4);
    expect(
      r.minActiveAfterSweep,
      "⛔ 有一發在掃描跑完之後場上一顆粒子都不剩 —— 那正是「打一打動畫就消失沒播完」",
    ).toBeGreaterThan(0);
    expect(r.stoppedAfterSweep, "⛔ 有發射器在演出中途被 stop() —— 這一發不會再有粒子").toBe(0);
  ...
  it("🔁 量尺自證：翻回 rollback 檔位（emitter）⇒ **量得到**那一發消失", () => {
    ...
    expect(r.stoppedAfterSweep, "舊行為下應該量得到發射器被 stop()").toBeGreaterThan(0);
