...
/**
 * 09 悟空的**本體**（`transform.role:"base"`）：09-04 龜派氣功住在 R、09-03 超級賽亞人住在 E
 * （`championForm to:"alternate"` → 變成 `godie-o00x`，⭐ 原作 `'O00X'` 就是超賽那一半）。
 */
...
function castR(world: SimWorld, hero: EntityId, enemy: EntityId): number {
  ...
  castUntil(world, hero, "R", FORWARD, () => {
    const hit = world.events.find(
      (e) => e.type === "damage" && e.data["target"] === enemy && String(e.data["origin"]).endsWith(".r"),
    );
...
describe("GH#1070 form 葉 —— 出貨 09-04 龜派氣功的變身增幅", () => {
  it("① 本體沒有增幅 → 變身態有 → ⭐ 變回本體同一 tick 又沒有（recentCast 的尾巴消失）", () => {
    ...
    // 09-03 超級賽亞人：出貨的 `championForm` 效果 ⇒ 效果執行器 → applyChampionForm。
    castUntil(world, hero, "E", { type: "self" }, () => championFormIndex(world, hero) === 1);
