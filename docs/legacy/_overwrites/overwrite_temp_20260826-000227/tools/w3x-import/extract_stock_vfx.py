...
def build_docs(row: dict, blob: bytes, tex: "ep.TextureResolver", notes: list[str],
    ...
    for i, em in enumerate(model.emitters2):
        # ⚠️ built with a PLACEHOLDER id; the shipping id is assigned after the
        # ordering below. Nothing inside `build_p2_doc` reads the id except to
        # write it and to label its notes.
