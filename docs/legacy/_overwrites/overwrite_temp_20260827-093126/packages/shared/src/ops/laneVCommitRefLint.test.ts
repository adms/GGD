...
describe("GH#663 · commit 訊息的票號 / lane 代號不可以互相冒充", () => {
  ...
  it("★ 離線且沒有快取 ⇒ 放行，但一定印「沒驗到」", () => {
    const r = lint("fix(#12345): x", { GGD_TICKET_CACHE: "/private/tmp/laneV-no-such-cache.txt" });
    if (r.code === 0) {
      // gh 在的機器上會直接問 GitHub；只有真的問不到才走 fail-open 那條路。
      expect(r.out.includes("沒驗到") || r.out.includes("✓")).toBe(true);
    }
