...
describe("帳本寫入端收工重生成 board:roll ＋ board:build（透過 genrun 的入口）", () => {
  ...
  it("ledger_table.py --map：對到列才叫；⛔ 沒對到（沒寫帳本）就不叫", () => {
    ...
    expect(spawnSync("python3", [LT, "--map", md, "09:59", "#1"], { encoding: "utf8", cwd: REPO, env: s.env }).status).not.toBe(0);
    expect(s.called(), "沒寫帳本卻重生成 ⇒ 白跑幾秒，而且每對錯一次跑一次").toBe("");
    const r = spawnSync("python3", [LT, "--map", md, "09:00", "#1"], { encoding: "utf8", cwd: REPO, env: s.env });
