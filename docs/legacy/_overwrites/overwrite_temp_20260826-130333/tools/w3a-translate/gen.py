...
# ───────────────────────── 翻譯來源的逐欄表 ─────────────────────────
FIELD_TO_GGD = [
    # (w3a code, 編輯器欄位, GGD 落點) —— ⭐ **欄位名與帶值支數是量出來的**,
    # 這裡只寫「它翻到 GGD 的哪一格」,而那是一個翻譯決定,不是可以推導的事實。
    ("acdn", "cooldown", "ability@1.cooldown[] ／ cooldownTier"),
    ("amcs", "mana", "ability@1.manaCost[] ／ manaCostTier"),
    ("aran", "cast_range", "ability@1.range ／ rangeTier（⚠️ WC3 世界單位,要換算）"),
    ("aare", "area", "effect.radius ／ radiusTier（⚠️ 同上）"),
    ("adur", "duration", "effect.durationSec（對雜兵）"),
    ("ahdu", "hero_duration", "⚫ **GGD 沒有這一格** —— 對英雄減時是一個缺的機制"),
    ("atar", "targets_allowed", "ability@1.targetsEnemies ＋ 效果的 filter（w3x 的集合比布林細）"),
    ("abuf", "buffs", "applyStatus.statusId ／ applyBuff（w3h 是第二條特效通道）"),
    ("acas", "castTime", "ability@1.castTimeSec"),
    ("aart", "icon", "ability@1.icon"),
    ("acat/atat/aeat/asat/amat/aaea", "art.models", "ability@1.vfxKey → content/vfx/*"),
    ("alig", "art.models.LightningEffect", "電弧家族（Lightning.slk 的列,⛔ 不是模型）"),
    ("aefs/aefa", "art.sounds", "ability@1.sfxKey（⭐ owner 2026-08-20 點名的「特效音效綁定」）"),
    ("acap/aspt/ata0..5", "art.attach", "球體綁定位置（⭐ owner 點名;vfx.bone-attachment@1）"),
    ("atp1/aub1", "（ubertip）", "ability@1.description ＋ {{cd}}/{{mp}}/{{dmg}} 佔位"),
    ("Data A–I", "data[]", "語意隨 base 而異 —— 見 §3 的逐 base 字典"),
]


def render_source_doc(w3a: dict, vocab: dict, ledger: dict, stats_count: Counter,
    ...
    A("|---|---|---:|---|")
    for code, key, landing in FIELD_TO_GGD:
        n = stats_count.get(key.split(".")[-1], None)
        if n is None:
            n = art_count.get(key, None)
        A(f"| `{code}` | `{key}` | {n if n is not None else '—'} | {landing} |")
    A("")
...
def build() -> list:
    ...
    return [
        ...
        (OUT_SRC, render_source_doc(w3a, vocab, ledger, stats_count, art_count, base_rank, data_dict)),
