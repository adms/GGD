...
/**
 * ⭐⭐ **形狀的唯一判準** —— 冷卻表要查哪一欄與形狀乘數**同一支**（⛔ 2026-09-06 之前是兩份：
 * 冷卻那一份只看帶 ratios 的節點自己 ⇒ 36 個範圍節點查到單體表；形狀那一份只看文件頂層 `radius`
 * ⇒ 15 個住在 `damageArea` 底下的節點被判成單體 —— 13-04 龍星群一支同時中兩個）。
 *
 * 由內而外找**最近**的帶形狀祖先（`damageLine` ⇒ 直線；`damageArea`／`radius` ⇒ 範圍），
 * 都沒有才退到文件頂層 `radius`，再沒有 ⇒ 單體；文件提到 `championForm` ⇒ 冷卻查變身表。
 */
export function apCoeffShapeOf(
  def: Record<string, unknown>,
  node: Readonly<Record<string, unknown>>,
  ancestors: readonly Readonly<Record<string, unknown>>[],
): { shape: ApCoeffInputs["shape"]; radiusUnits?: number | undefined; table: "單體" | "範圍" | "變身" } {
  for (const a of [node, ...[...ancestors].reverse()]) {
    if (a["kind"] === "damageLine") return { shape: "line", table: "單體" };
    if (a["kind"] === "damageArea" || typeof a["radius"] === "number") {
      const r = Number(a["radius"]) || undefined;
      return { shape: "area", ...(r !== undefined ? { radiusUnits: r } : {}), table: "範圍" };
    }
  }
  const docR = Number(def["radius"] ?? 0) || undefined;
  if (docR !== undefined) return { shape: "area", radiusUnits: docR, table: "範圍" };
  return { shape: "single", table: JSON.stringify(def).includes("championForm") ? "變身" : "單體" };
}
...
export function apCoeffCooldownFor(
  ...
  // ⭐ 形狀走 `apCoeffShapeOf`（與形狀乘數**同一支**）：以帶 ratios 的節點與它的祖先判，⛔ 不是整份文件
  //   （2026-09-06 量到 4 份混形文件 edem.w · emfr.e · emns.e · etyr.r —— 帶 AP 的是單體節點而文件另有半徑）。
  const shape = apCoeffShapeOf(def, node, ancestors).table;
