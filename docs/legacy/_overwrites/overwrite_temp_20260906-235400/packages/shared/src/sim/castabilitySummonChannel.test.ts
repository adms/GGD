 ...
 *
 * 在此之前 `snapshotChannels()` 沒有召喚那根指針：`spawnSummon` 不掛狀態／buff、
 * 不發 `EFFECT_EVENTS` 裡的任何事件，只寫 `world.summon` ⇒ 一支**只有 `summon`** 的
 * 技能在真的 SimWorld 裡召喚成功，普查卻回「cast accepted but produced no measurable
 * effect」（GH#1078 量到：`tpl-summon-agent` 0→1 具而 verdict FAIL）。
 *
 * ⛔ 一把只驗過單邊的尺不算自證過（第一守則）—— 所以這裡兩個方向一起讀：
 *   ① 純召喚（身體已註冊）⇒ PASS，channel `summon`；
 *   ② 同一個 kind、身體**未註冊**（handler 走 `summonFailed` 出口，一具都不生）⇒ ⛔ 不是 PASS。
