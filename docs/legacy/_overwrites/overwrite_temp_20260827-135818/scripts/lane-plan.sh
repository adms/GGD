...
for t in sorted(planned, key=lambda x: (x["prio"], x["n"])):
    i = 0
    while True:
        while len(batches) <= i:
            batches.append([])
        why: list[str] = []
        if any(placed.get(d, 10**6) >= i for d in t["deps"]):
            why.append("順序：相依 " + " ".join(f"#{d}" for d in sorted(t["deps"] & set(placed) | (t["deps"] - set(placed)))))
