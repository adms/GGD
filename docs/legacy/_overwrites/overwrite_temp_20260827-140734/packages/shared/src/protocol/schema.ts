...
export const ENTITY_KIND = {
  ...
  NIGHT_FLAG: 7,
  /**
   * 戰場任務的**陣營塔**（GH#752 mini dota，`systems/ObjectiveSystem.ts`）。
   * 與守護塔同樣是 transform + health + `StructureComp`，⭐ 但它**屬於一隊**，
   * 而拆掉它的那一方立即落敗 —— 所以它不可以掉進 `GUARDIAN` 那一格（那會被畫成
   * 中立灰塔，而玩家分不出「哪一座是我的」，那正是這個任務唯一要問的問題）。
   *
   * SLOT REUSE，與 REVIVE_CIRCLE / GOLD_COIN / NIGHT_FLAG 同一條慣例：
   *
   *   seatId = -1（塔不屬於任何**座位**；擁有者是一整隊，見下面的 `mana`）
   *   mana   = 擁有這座塔的 **teamId**（0..3），給客戶端上隊伍色 / 小地圖圖示。
   *            `maxMana` 保持 0，那正是這一格空著的原因：`manaPct` 算的是
   *            `maxMana > 0 ? mana / maxMana : 0`，所以沒有人會從它畫出魔力條。
   *   hp/maxHp/alive = 隨行，所以血條畫得出來（⭐ 這就是「塔快被拆了」的畫面）
   *   shield = 0 —— `shieldPct = shield / maxHp` 會被讀，非 0 會在血條上畫出
   *            一條不存在的護盾。
   */
  OBJECTIVE_TOWER: 8,
} as const;
