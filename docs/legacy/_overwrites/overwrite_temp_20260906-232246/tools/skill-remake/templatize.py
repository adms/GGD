...
PLANNED_SHAPES = {
    "blink": "#1069 tpl-blink（family `blink`，零參數）",
    "applyStatus": "#1071 tpl-apply-status（family `apply-status`，`status` 槽＋optional radius）",
    "heal": "#1072 tpl-heal（family `heal`，heal ≠ restore）",
    "damage + dot": "#1073 tpl-drain-leech.leechFlat 改 optional（清空 ⇒ 沒有 heal 節點）",
    "applyStatus + damage": "#1066 tpl-single-strike／tpl-proxy-fanout 加一格 `status`（type applyStatus）；帶 radius 且 statusId 在下拉裡的走 tpl-proxy-cast",
}
...
def m_proxy_fanout(tpl: dict, doc: dict):
    ...
    if not _damage_node(dmg):
        return None, "damage 節點多了鍵"
    if _has_status_slot(tpl):
        # ⭐ #1066 落地後：`statusId`＋`statusDurationSec` 收斂成一格 `status`（機制欄位住在節點上，任何 id 都收得下）。
        status_params, why = {"status": _status_param(st)}, None
    else:
        status_params, why = _enum_status(tpl, st, CC_MECHANIC, "proxy-fanout")
        if why:
            why += "—— #1066 的 `status` 槽落地後就收得下"
    if why:
        return None, why
    wc3 = to_wc3u(doc["radius"])
    if wc3 is None or not slot_ok(tpl, "radius", wc3):
...
#: ⚠️ 後三個（blink / apply-status / heal）的模板檔今天還沒落地 —— `plan()` 對不存在的 ref 直接跳過，
#:    技能會掉進「沒有模板發這個形狀」桶並印出票號（`PLANNED_SHAPES`）。
MATCHERS = (
...
def parity_guard(docs: dict[str, dict], proposals, allow_fix: bool, why_not: dict[str, str]):
    ...
    while changed:
        ...
        for stem in list(prop):
            ...
            if new:
                twins = ", ".join(
                    f"{s}（{blocked.get(s) or why_not.get(s, '?')}）" for s in group if s != stem
                )
                blocked[stem] = (
