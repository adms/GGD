...
def bake_emitter_quads(model, raw_bytes: bytes) -> list[dict]:
    ...
    for em in pm.emitters2[:24]:
        ...
        if tid not in mat_by_tex:
            mat_by_tex[tid] = len(model.materials)
        ...
        for corners, n in quads:
            ...
            for cx, cy, cz in corners:
                verts.append((px + cx, py + cy, pz + cz))
                norms.append(n)
            uvs += [(0.0, 0.0), (1.0, 0.0), (1.0, 1.0), (0.0, 1.0)]
            faces += [base, base + 1, base + 2, base, base + 2, base + 3]
        groups = [[em.parent_id]] if em.parent_id in model.nodes else []
        model.geosets.append(Geoset(
            vertices=verts, normals=norms, uvs=uvs, faces=faces,
            vertex_groups=[0] * len(verts), matrix_groups=groups,
            material_id=mat_by_tex[tid],
        ))
