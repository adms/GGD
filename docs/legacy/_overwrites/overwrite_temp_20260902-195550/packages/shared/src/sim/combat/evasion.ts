...
function basicEvadeSource(
  ...
  if (!(granted > 0)) return null; // 一份 grant 都沒有 ⇒ 這一次沒有主人
  /**
   * ⭐⭐ **分母要留一段給「沒有主人」的迴避。**
   *
   * ⚠️ 出貨的 `godie-u00j`（賽菲洛斯）逐字是 `base.evasion: 0.15` **＋** 一份
   * `abilityPassive:godie-u00j.passive` 的 flat 0.15 ⇒ 聚合 0.30。
   * ⛔ 如果分母只取 `Σflat`，那支被動就會被記上**每一次**迴避 —— 而其中一半
   * 根本是英雄的**基礎屬性**在閃。⭐ 那正是 Codex 禁的「猜一個來源」，
   * 只是換了個包裝。
   *
   * ⇒ 分母取 `max(Σflat, 聚合值)`：多出來的那一段 ⇒ 回傳 `null`
   *   ⇒ ⭐ 消費端維持**通用**迴避回饋（驗收②）。
   * ⚠️ 反過來 `Σflat > 聚合值`（被 `evasionCeiling` 夾住）時分母是 `Σflat`，
   *   每一段等比例縮小 —— 夾取不會偏袒任何一份來源。
   */
  const denom = Math.max(granted, evasionOf(world, target));
  // `roll < p` 已由呼叫端保證，且 `p` 是聚合值的等比例縮放 ⇒ `frac ∈ [0, 1)`。
  const cut = (roll / p) * denom;
  let cum = 0;
  for (const src of sc.sources) {
    if (!sourceActive(src, world.tick)) continue;
    const w = evasionFlatOf(src, src.stacks ?? 1);
  ...
  return null; // 落在「基礎屬性／百分比放大器」那一段
}
...
export function rollEvadeAbility(
  ...
  const { chance, by } = abilityEvasionPick(world, target, isTrue);
  const p = chance * (1 - unavoidableOf(world, source));
  if (p <= 0) return false; // THE ZERO GUARANTEE
  if (!world.rng.chance(p)) return false;
  emitEvade(world, source, target, "ability", p, by);
  return true;
}
