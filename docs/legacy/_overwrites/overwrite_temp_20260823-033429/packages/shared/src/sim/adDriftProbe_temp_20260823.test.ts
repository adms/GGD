...
function realAttacksPerSec(sheetAs: number, adGift: number | null, pinAd: number | null): number {
  const w = new SimWorld(OPEN_ARENA, 31);
  if (adGift !== null) w.baseBonus = normalizeBaseBonus({ ...DEFAULT_BASE_BONUS, ad: adGift });
  w.combatActive = true;
  ...
  for (let i = 0; i < 300; i++) {
    ...
    if (pinAd !== null) sc.final[Stat.AttackDamage] = pinAd;
...
function weaknessSwings(weakened: boolean, adGift: number): { n: number; foeAlive: boolean } {
  const world = new SimWorld(SKELETON_ARENA, 7);
  world.baseBonus = normalizeBaseBonus({ ...DEFAULT_BASE_BONUS, ad: adGift });
  ...
  return { n, foeAlive: (world.health.get(foe)?.hp ?? 0) > 0 };
}
...
describe("probe", () => {
  it("measures", () => {
    for (const gift of [0, 32] as const) {
      const a: string[] = [];
      for (const sheet of [2.0, 3.0, 4.0]) a.push(`as${sheet}=${realAttacksPerSec(sheet, gift, null)}`);
      const b: string[] = [];
      for (const sheet of [2.0, 3.0, 4.0]) b.push(`as${sheet}=${realAttacksPerSec(sheet, gift, 1)}`);
      // eslint-disable-next-line no-console
      console.log(`GIFT ${gift} rawAD: ${a.join("  ")}   || pinAD=1: ${b.join("  ")}`);
    }
    for (const gift of [0, 32] as const) {
      const nrm = weaknessSwings(false, gift);
      const wk = weaknessSwings(true, gift);
      // eslint-disable-next-line no-console
      console.log(`GIFT ${gift} weakness: normal=${nrm.n}(foeAlive=${nrm.foeAlive}) weak=${wk.n}(foeAlive=${wk.foeAlive})`);
    }
