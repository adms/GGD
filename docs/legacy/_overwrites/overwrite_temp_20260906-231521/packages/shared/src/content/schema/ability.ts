...
export const zAbilityDef = z
  .object({
     ...
     * ── ⭐⭐ 五層階梯（2026-08-31 從兩層擴成五層）────────────────────────
     * owner 2026-08-12 的優先序階梯（CLAUDE.md 第〇·六守則）逐字是**五層**：
     *
     * > 「**我給你的新版技能說明 > 編輯器產生的 JSON > JASS實際效果
     * >   > w3x技能說明 > w3x原始設定**」
     *
     * ⛔ 而這一格在此之前只有**兩層**（`owner-spec` / `w3x-import`）——
     * ⇒ ⭐ 第 2～5 層被壓成同一個值 `w3x-import`（330 支），
     *   而它們的**可信度差很多**：
     *   · JASS 是原作**真的會發生**的事（⭐ 程式不會說謊）
     *   · w3x tooltip 是**文案**（⚠️ 它與 JASS 打架時可能是刻意的隱藏機制）
     *   · w3x 原始設定的欄位**很多在 JASS 裡根本沒被讀**
     *
     * ⇒ ⭐ 分不出來的代價很具體：一份「照 tooltip 寫的」與一份「逐行讀 JASS 寫的」
     *   今天長得一模一樣，⛔ 而下一輪要判「這個數字可信嗎」時**答不出來**。
     *
     * ⚠️ ⭐ **加值是相容的**：既有 421 支一份都不必動
     * （`owner-spec` 91 · `w3x-import` 330 兩個值都還在）。
     * ⇒ 新的三個值是給**之後**的文件用的，⛔ 不是回頭去重標。
     *
     * ⚠️ ⭐ 而回頭重標**需要證據** —— 一份文件是不是「逐行讀 JASS 寫的」，
     * 只有寫它的人知道。⛔ 憑印象重標會把一個誠實的「不知道」換成一個假的「已查證」。
     */
    provenance: z
      .enum([
        /** ⭐ 第 1 層 —— owner 的新版技能說明（**設計**，贏過一切考古）。 */
        "owner-spec",
        /** ⭐ 第 2 層 —— 編輯器產生的 JSON（設計被翻成機器讀得懂的樣子）。 */
        "editor-json",
        /** ⭐ 第 3 層 —— 逐行讀 JASS 寫的（⭐ 原作**真的會發生**的事）。 */
        "jass-verified",
        /** ⭐ 第 4 層 —— 照 w3x 的**技能說明**（文案）寫的。⚠️ 它可能與 JASS 打架。 */
        "w3x-tooltip",
        /**
         * ⭐ 第 5 層 —— w3u/w3a 的**欄位值**。
         * ⚠️ 最低，因為很多欄位在 JASS 裡根本沒被讀。
         * ⭐ 而 `w3x-import`（330 支）是這一層的**歷史名字**，保留不動。
         */
        "w3x-import",
      ])
      .optional(),
    ...
    /**
     * ⭐ 2026-09-03（GH#757）—— `descriptionRoles` **整格拆掉了**。
     * ⛔ 它蓋好了、接上 UI 了、schema 開好欄位了，而**內容端是零**
     * （421 份 ability · 71 份 champion 全部沒有值），importer 的產出函式也零呼叫者。
     * ⭐ 整條鏈另存在 `docs/legacy/_retired-chains/role-markup-114.md`
     * （含 rollback 與「貼回去之前要先修的正則衝突」）。
     */
...
function refineInnate(
  ...
  if (doc.slot === "PASSIVE") {
    ...
    } else if (doc.innateKind === "active" && doc.effects.length === 0) { // MUTATION-1065
