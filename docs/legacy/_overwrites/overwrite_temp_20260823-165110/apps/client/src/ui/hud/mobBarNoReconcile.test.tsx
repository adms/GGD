 ...
 * ⇒ 每一幀 `setState` ⇒ 整棵 60 個元件 reconcile + diff + commit。
 *
...
const MOB = { id: 41, hp: 300, maxHp: 800, sx: 100, sy: 60, elite: true, boss: false };
...
function mount(): void {
  act(() => {
    root.render(
      <React.Profiler id="mob" onRender={() => (commits += 1)}>
        <MobHealthBars />
      </React.Profiler>,
    );
  });
...
describe("GH#620 殭屍血條", () => {
  ...
  it("名冊變了（多一隻）才重跑 React", () => {
    ...
    mount();
    const before = commits;
    frameBus.mobBars = [{ ...MOB }, { ...MOB, id: 42, sx: 200 }];
    frame();
