...
/** `up -d` 後面**沒有**指名服務 ⇒ 它會重建每一個服務（含 redis）。 */
const RECREATES_ALL = /docker\s+compose[^\n]*?\bup\s+-d\b(?![^\n]*\b(?:redis|caddy|edge|game|platform)\b)|docker\s+compose[^\n]*\bdown\b/;
...
describe("停 Redis 之前一定先快照 —— 排行榜與 M幣不是快取（GH#860）", () => {
  it("⭐ 量尺先自證：偵測器兩個方向都準", () => {
    ...
    expect(RECREATES_ALL.test("docker compose -f a.yaml up -d")).toBe(true);
    expect(RECREATES_ALL.test("docker compose up -d --scale caddy=0")).toBe(true);
    // ⭐ 反方向：只重建**指名的那一個**服務時**不可以**抓到
    //   （`up -d caddy` 不會碰 redis —— 誤報會逼人加假的快照）
    expect(RECREATES_ALL.test("docker compose -f a.yaml up -d caddy")).toBe(false);
    // 而且真的有腳本被掃到（⛔ 掃到 0 支 = 這條守衛在空轉）
    const hit = scripts.filter((f) =>
      codeLines(readFileSync(join(DIR, f), "utf8")).some((l) => RECREATES_ALL.test(l.text)),
    );
  ...
  it("★ 每一支會重建容器的腳本，都要在那一行**之前**呼叫 redis-snapshot.sh", () => {
    ...
    for (const f of scripts) {
      ...
      const firstRecreate = lines.find((l) => RECREATES_ALL.test(l.text));
