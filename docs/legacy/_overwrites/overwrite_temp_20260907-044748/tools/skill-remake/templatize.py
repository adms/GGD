...
def m_buff_self(tpl: dict, doc: dict):
    ...
    #    ⛔ 它與下面的 `status` 節點不是同一件事：那一格是**第二份**來源（見 expand.ts）。
    base = {"kind", "modifiers", "duration"}
    allowed = base | ({"statusId"} if _has_ref_slot(tpl, "statusId") else set())
    if not _keys_exactly(n, allowed & (set(n) | base)):
        missing_slot = "statusId" in set(n) - allowed
        return None, (
            f"{tpl['id']} 還沒有 `statusId` 槽（GH#993：一格 optional docRef）"
            if missing_slot
            else f"applyBuff 多了鍵：{sorted(set(n) - allowed)}"
        )
