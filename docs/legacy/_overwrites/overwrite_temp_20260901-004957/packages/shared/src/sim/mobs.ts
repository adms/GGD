...
export interface MobRules {
  ...
  /**
   * ⭐ GH#747 AC① —— 這一批小怪讀**誰的**屬性表（`mobWaves.mob.championId` 解析後的值）。
   * ⛔ optional：手搭的 fixture 缺席時退回今天的行為（不建 StatsComp）。
   */
  championId?: string;
  modelKey: string;
  /**
...
export interface MobProfile {
   ...
   *
   * ⚠️ ⭐ 在此之前小怪**沒有 `StatsComp`** ⇒ `recomputeStats` 的第一句就早退
   * ⇒ ⛔ **任何掛在小怪身上的 buff／debuff 一個數字都不會動**
   *（狀態掛得上去、`sources` 進得了 —— 而沒有人把它折算出來）。
   * `mobs.ts:3056` 那一段註解逐字記著這個缺口，⭐ 而這一格就是把它關掉。
   *
   * ⛔ absent ＝ 手搭的 fixture（跳過 `mobRulesFromConfig`）⇒ 不建 StatsComp，
   * 也就是**這張票之前的行為** —— 缺席永遠退回今天，⛔ 不是報錯。
   */
  championId?: string;
  maxHp: number;
  attackDamage: number;
...
function spawnMobBody(
  ...
  /**
   * ⭐⭐ 屬性欄 —— owner 2026-08-04 裁決「建 StatsComp 吧」的**第三段**（A3c，GH#747 AC①）。
   *
   * ⚠️ ⭐ 上面那一大段註解逐字記著這個缺口：「`recomputeStats` 的第一句是
   * `if (!sc || (!champ && !sm)) return;` ⇒ 【破甲】【易傷】【凋零】的**屬性**那一半
   * 對殭屍仍然無效」。⭐ 這一行 ＋ `statPipeline.ts` 那道早退放行，就是把它關掉。
   *
   * ⭐ **與召喚物完全同一個形狀**：有 `StatsComp`、⛔ 沒有 `ChampionComp`
   *（`deathSystem` 用 `world.champion.has()` 判要不要付擊殺金 ⇒ 給殭屍一份
   * ChampionComp 會讓每一隻都變成一次英雄擊殺）。
   *
   * ⚠️ `dirty: true` 是刻意的（⛔ 與王的 `false` 不同）：王沒有人會對它 `attachSource`
   * 所以留著 dirty 只是白跑；⭐ 而殭屍**正是**要吃 buff／debuff 的那一群 ——
   * 第一次重算要真的發生，⛔ 否則 `final` 停在 `zeroStats()` 而每一格都是 0。
   *
   * ⛔ 沒有 `championId`（手搭的 fixture）⇒ 不建 ⇒ **今天的行為**，⛔ 不是報錯。
   */
  // ⚠️ ⭐ `Champions.tryGet` ⛔ 不是 `has(championId)`：`recomputeStats` 裡是
  // `Champions.get(sc.championId)`，**查不到就擲例外**。⇒ 內容還沒註冊的環境
  // （單元測試夾具、內容載入失敗的 fail-open 路徑）必須退回「不建」，
  // ⛔ 而不是讓每一隻殭屍在生成的那一刻炸掉整場比賽。
  // ⭐ 這是量到的：第一版沒有這道檢查 ⇒ **35 條既有測試當場紅**，
  // 訊息全部是 `content not registered: godie-zombiex`。
  if (profile.championId !== undefined && Champions.tryGet(profile.championId as ChampionId)) {
    world.stats.set(id, {
      championId: profile.championId as ChampionId,
      final: zeroStats(),
      dirty: true,
      sources: [],
    });
