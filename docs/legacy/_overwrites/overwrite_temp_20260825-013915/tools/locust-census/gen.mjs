...
const jassRows = [...jassByRaw.values()]
  ...
  .sort((a, b) => b.trigFns - a.trigFns || b.sites - a.sites || byId(a.rawcode < b.rawcode ? { id: -1 } : { id: 1 }, { id: 0 }) || (a.rawcode < b.rawcode ? -1 : 1));
...
/** 解一行 `set VAR = …` 的 RHS 指到哪個 rawcode（在它自己的函式裡回溯）。 */
...
/** 變數回溯：同函式優先，其次全檔（全部指派同一 rawcode 才算 resolved）。 */
function resolveVar(unitExpr, callIdx, fnStart) {
  ...
  for (let k = callIdx - 1; k >= fnStart; k--) {
    if (assignRe.test(jass[k])) {
      const r = resolveAssignment(k);
      return { rawcode: r, source: r ? "var-local" : "unresolved", candidates: null };
    }
  }
  const seen = new Set();
  ...
  if (seen.size === 1) return { rawcode: [...seen][0], source: "var-global", candidates: null };
  return {
    rawcode: null,
    source: "unresolved",
    candidates: seen.size > 1 ? [...seen].sort() : null,
  };
}
...
const meta = {
  ...
  counts: {
    ...
    alphaEventUnit: runtimeAlpha.filter((e) => e.source === "event-unit").length,
...
function renderMd() {
  ...
  L.push(
    "BJ 語意：R/G/B 是 0–100%，第 4 參數是**透明度%**（100=全隱形）；alpha255 = (100−t)×2.55。" +
      "回溯不到的誠實標 `unresolved`（⛔ 不猜）；`event-unit` = 目標是事件單位（施法者/死者），不是生成的 dummy。",
  );
  L.push("");
