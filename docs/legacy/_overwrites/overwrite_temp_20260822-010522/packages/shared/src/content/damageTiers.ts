 ...
 *     ⛔ **HP 系統倍率不在裡面**（owner 2026-08-22：「不必考慮系統倍率」）——
 *        它被留給 owner 當最後的旋鈕，見 `anchorFloorFrom()` 的檔頭。
 ...
 *
 * > 「以上這些都是**不考慮系統倍率**的狀況，因為我就是要**保留系統倍率來做最後調整**，
 * >  所以你的公式**不必考慮系統倍率**」
 * > 「[KILL_CASTS_REF 20 → 35 那一列] **我改系統倍率要的就是這樣的效果**」
 *
 * ⚠️ 在此之前這裡乘了 `HP_ENV_MULT`，於是**倍率自己把自己抵銷掉了**：
 * 血條 ×N 的同時級距也 ×N，「一發極大佔血條幾 %」逐位元不變。
 * 實測（2026-08-22）：把 `maxHealth` 4.0 → 7.2，五級距自動從 600/1500/3000/4500/6000
 * 變成 1050/2625/5250/7875/10500，**佔比一格都沒動**（50.9%）——
 * 也就是那個旋鈕**根本轉不動任何東西**，而後台上它看起來完全正常。
 *
 * ⇒ 現在倍率只進**血量**那一側，所以它就是 owner 要的那支最後的旋鈕：
 * 倍率調高 = 同一發傷害佔血條的比例下降 = 要更多發才殺得死。
 */
