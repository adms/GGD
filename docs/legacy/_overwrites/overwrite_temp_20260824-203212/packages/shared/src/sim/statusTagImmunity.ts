 ...
 * 而**不問 `kind`**，所以一格 schema + 一行轉發就同時落在**四個授權面**上
 *（道具／天生技 rank／增益卡／`applyBuff` 的限時來源）。
 * ⇒ 殭屍王用的是天生技那一面（常駐），而「6 秒內免疫減速」的靴子／大招
 * 用的是 `applyBuff` 那一面（到期由那份 source 自己的 `expiresAtTick` 收掉，
