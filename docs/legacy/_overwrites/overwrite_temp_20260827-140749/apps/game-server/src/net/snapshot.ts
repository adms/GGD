  ...
  for (const [id, t] of world.transform) {
    ...
    if (proj) {
      ...
      if (structure) {
        ...
        // ⭐ GH#752 —— `world.structure` 現在有兩種住客。陣營塔走自己的 kind，
        // 而擁有者 teamId 騎在 `mana` 上（`maxMana` 留 0 ⇒ 沒有人畫魔力條），
        // 與 NIGHT_FLAG 同一條 slot-reuse 慣例。⛔ 不可以掉進 GUARDIAN：那會
        // 被畫成中立灰塔，而「哪一座是我的」正是這個任務唯一要問的問題。
        const objective = structure.kind === "objective" && structure.teamId !== undefined;
        es.kind = objective ? ENTITY_KIND.OBJECTIVE_TOWER : ENTITY_KIND.GUARDIAN;
        es.seatId = -1;
        ...
        if (hp) {
          es.hp = hp.hp;
          es.maxHp = hp.maxHp;
          es.mana = objective ? structure.teamId! : 0;
          es.maxMana = 0;
          es.alive = hp.alive;
          es.shield = 0;
        }
