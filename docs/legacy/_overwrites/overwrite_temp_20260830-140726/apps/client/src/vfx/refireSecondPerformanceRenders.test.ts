...
interface CombatReading {
  ...
  /** 跨過三秒門檻之後，還量到幾發（⛔ 母體是空的話上面那個 min 沒有意義） */
...
/**
 * 連續戰鬥 5 秒 —— **完全走出貨的那條路**（`play()` → 池化 → `update()` → 掃描）。
 */
function sustainedCombat(): CombatReading {
  ...
  for (let t = 0; t <= 5_000; t += REFIRE_MS) {
    ...
    if (t >= capMs) {
      ...
      expect(ps!.isStarted(), "發射器被停掉了 ⇒ 這一發不會再有粒子").toBeDefined();
    }
  }
