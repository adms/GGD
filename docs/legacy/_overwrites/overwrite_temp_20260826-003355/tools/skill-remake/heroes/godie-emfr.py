...
A("15-002", "15-002 敵彈吸收陣。太陰道", "self", [60], [0], 0,
  ...
  #    參數逐格：`scale 2` ＝ w3u `usca`（census u/o02Y）· `tint [1,0,0]` ＝解析後頂點色
  #    255,0,0 ÷255 · `lifeSec 3` ＝ j:34817 `RemoveUnitSP(u, 3, 1)` 的 Life_Time
  #    （helper 簽章 `RemoveUnitSP(unit, Life_Time, Die_Time)`，⛔ 不是 census 讀得到的
  ...
  #    ⚠️ 原作把它擺在 `GetSpellTargetLoc()`，而 GGD 的 15-002 是 `self` ⇒ orb 環半徑
  #    留預設（施法者腳下）。這一格差異寫進 TAIL 報告的 owner 裁決節。
  model_fx=[{"kind": "spawnModelFx", "shape": "single", "preset": "tpl-locust-orb",
             ...
             "scale": 2.0, "tint": [1.0, 0.0, 0.0], "lifeSec": 3.0}],
