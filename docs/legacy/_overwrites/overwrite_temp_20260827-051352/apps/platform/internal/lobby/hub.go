...
// SetMaxConnsPerAccount overrides the per-account socket ceiling. 0 or less
// disables the cap — which is the pre-#724 behaviour and is offered only so the
// operator has a one-setting way back if the ceiling ever turns out to bite a
// real player (see the const's doc for why it should not).
