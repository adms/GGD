...
RE_FILES = re.compile(r"^\s*(?:#{1,4}\s*|\*\*)?(?:Files\s*/?\s*modules[^\n]*|影響檔案|likely affected)", re.I)
...
# ── ③ 貪婪分批 ─────────────────────────────────────────────────────────────
# ⭐ 批次 0 先塞 `--busy`：唯讀 lane 沒有柵欄，但它**確實**在改那些檔。
batches: list[list[dict]] = []
reserved = [t for t in tickets if t["n"] in BUSY]
if reserved:
    batches.append(reserved)
placed: dict[int, int] = {t["n"]: 0 for t in reserved}
blocked: dict[int, list[str]] = {}

# ⭐ 先算**相依深度**再排，⛔ 不是照票號排 —— 不然一張被相依的低優先票還沒落地，
#   依賴它的那張就會在「等一個永遠不會來的批次」裡繞（實測會變成無窮迴圈）。
#   ⚠️ 相依成環時深度停在上限：⛔ 環不可以讓整支腳本掛掉，它只是一個**排不出順序**的事實。
...
for t in sorted(planned, key=lambda x: (depth[x["n"]], x["prio"], x["n"])):
    i = 0
    while True:
        while len(batches) <= i:
            batches.append([])
        why: list[str] = []
        late = sorted(d for d in t["deps"] if d in by_n and placed.get(d, -1) >= i)
        if late:
            why.append("順序：相依 " + " ".join(f"#{d}" for d in late) + " 還沒落地")
        if len(batches[i]) >= width:
            why.append(f"這一批已經滿了（--width {width}）")
        if t["lock"] and any(o["lock"] for o in batches[i]):
            why.append(f"🔒 全域鎖：{t['lock'][0]} 是產生器的產物 —— 一批只能有一張")
        for o in batches[i]:
            hit = sorted({f for f in t["files"] for g in o["files"] if clash(f, g)})
            if hit:
                why.append(f"撞 #{o['n']}：{' · '.join(hit[:3])}")
                break
        if not why:
            batches[i].append(t)
            placed[t["n"]] = i
            break
        blocked.setdefault(t["n"], []).extend(f"批{i + 1} ⛔ " + w for w in why[:1])
        i += 1
...
for i, b in enumerate(batches):
    if not b:
        continue
    tag = "（🚧 已經有 lane 在跑）" if i == 0 and reserved and b is reserved else ""
    print(f"\n── 批次 {i + 1}：{len(b)} 條 lane 可並行 {tag}")
    for t in sorted(b, key=lambda x: x["n"]):
        lock = " 🔒" if t["lock"] else ""
        print(f"   #{t['n']}{lock} {t['title'][:52]}")
        print(f"        柵欄：{' · '.join(sorted(t['files'])[:6])}")
        if t["n"] in blocked:
            print(f"        （{blocked[t['n']][-1]}）")
if noplan:
