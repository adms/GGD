...
describe("GH#873 觸控 gold-level 是一條，⛔ 不是一疊", () => {
  ...
  it("⭐ 條狀讓開整個叢集 —— 而翻回一疊，這把尺量得到那個已知的 88×86", async () => {
    cover("hud-bottom-cluster");
    const { applyHudScale } = await import("../hudScale");
    for (const t of ["small", "xsmall", "min", "large", "max"] as const) {
      applyHudScale(t);
      const strip = hits().filter((s) => s.includes("attack"));
      applyGoldLevelTouchLayout("column");
      const col = hits().filter((s) => s.includes("attack"));
      applyGoldLevelTouchLayout(null);
      console.log(`[PROBE tier=${t}] strip=${JSON.stringify(strip)} column=${JSON.stringify(col)}`);
    }
    applyHudScale(null);
