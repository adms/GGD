/**
 * ⭐ GH#986 —— `visual-advisory` 的**過期不是「人工批核」**，而且它的波及範圍是**一份**。
 *
 * 這一支跑**出貨的那一支產生器**（複製到 tmp 樹，⛔ 不是重寫一份它的邏輯 —— 失敗形態⑤），
 * 只問三件在 2026-09-07 之前都答錯的事：
 *  ① 一份的證據變了 ⇒ **只有那一份** stale，其餘保留原審閱（票文 AC 第 2 條）
 *  ② 分母是**推導的** ⇒ 驗收包擴編（46→47）不會讓它在每一次乾淨的 checkout 上失敗（失敗形態⑨）
 *  ③ 積木名從 `ggd-bricks.json` **推導** ⇒ 換一顆積木名仍然數得出「擋住幾支」，
 *     而一顆**已經出貨**的積木被標成 MISSING ⇒ 紅
 */
...
function run(edit: (docs: Docs) => void = () => {}): any {
  ...
  for (const [k, rel] of Object.entries(DOCS)) {
    ...
    writeFileSync(join(dir, rel), JSON.stringify(docs[k as keyof Docs]));
  }
...
describe("GH#986 Codex visual advisory", () => {
  it("⭐ 一份證據變了只作廢那一份 —— 其餘保留原審閱（⛔ 不是整包重來）", () => {
    const before = run();
    const victim = before.rows[3].id as string;
    const after = run((d) => {
      const row = d.packet.documentSources.find((r: any) => r.id === victim);
      row.sourceDigest = row.sourceDigest.replace(/^./, (c: string) => (c === "a" ? "b" : "a"));
    });
    const state = (r: any) => `${r.id}:${r.reviewState}`;
    expect(after.rows.filter((r: any) => r.reviewState === "stale").map((r: any) => r.id)).toEqual([victim]);
    // ⭐ 其餘每一列**逐列**都還在、還是 current、而且**審閱原文一個字都沒掉**。
    const kept = (o: any) => o.rows.filter((r: any) => r.id !== victim).map((r: any) => `${state(r)}|${r.note}`);
    expect(kept(after)).toEqual(kept(before));
    expect(after.summary.reviewFreshness).toEqual({ current: before.rows.length - 1, stale: 1 });
    // stale 的那一列不可以被算進任何摘要（它是對著舊證據寫的）。
    expect(after.summary.averageVisualScore).not.toBe(before.summary.averageVisualScore);
  });
  ...
  it("⭐ 積木名從清冊推導 —— 換一顆仍數得出擋住幾支；已出貨的積木被標成缺 ⇒ 紅", () => {
    const rename = (d: Docs, to: string) => {
      for (const c of d.manifest.cases) {
        for (const i of c.machineIssues ?? []) if (i.code === "MISSING_VISUAL_BRICK") i.brickId = to;
      }
      for (const e of d.source.entries) {
        e.flags = (e.flags ?? []).map((f: string) => (f.startsWith("missing-") ? `missing-${to}` : f));
      }
    };
    ...
    expect(() => run((d) => rename(d, d.bricks.bricks[0].id))).toThrow();
  });
});
