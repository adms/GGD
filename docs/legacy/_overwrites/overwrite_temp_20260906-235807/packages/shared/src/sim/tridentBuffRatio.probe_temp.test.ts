...
describe("probe: ② 在 passive 帶 statusId 之後會綠", () => {
  it("真的按 11-00 → 期間吃得到 → 到期後不吃", () => {
    ...
    dbg("after passive");
    {
      tick(world, hero);
      const ab = world.abilities.get(hero)!;
      const def = Abilities.get(ab.slots.E.abilityId);
      console.log(`[pre-E] slotE=${JSON.stringify(ab.slots.E)} castType=${def.castType} targetsEnemies=${def.targetsEnemies} teams=${JSON.stringify(world.team.get(hero))}/${JSON.stringify(world.team.get(enemy))} flower=${world.flower.has(enemy)} hpalive=${world.health.get(hero)!.alive} recovery=${JSON.stringify(ab.recovery)} approach=${JSON.stringify((ab as unknown as Record<string, unknown>)["approach"])}`);
      ab.slots.E.cooldownRemainingTicks = 0;
      world.health.get(hero)!.mana = world.health.get(hero)!.maxMana;
      const r = castAbility(world, hero, "E", { type: "entity", entityId: enemy });
      console.log(`[pre-E] castAbility=${r} enemyT=${JSON.stringify(world.transform.get(enemy))} enemyH=${JSON.stringify(world.health.get(enemy))} heroT=${JSON.stringify(world.transform.get(hero))}`);
      const r2 = castAbility(world, hero, "E", { type: "entity", entityId: enemy });
      console.log(`[pre-E] castAbility again=${r2} cast=${JSON.stringify(ab.cast)}`);
    }
