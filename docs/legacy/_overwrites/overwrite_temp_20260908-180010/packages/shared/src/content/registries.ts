...
export function registerAll(store: ContentStore, options: RegisterAllOptions = {}): void {
  ...
  for (const d of store.all<ChampionDef>("champions")) {
    // ⭐⭐ 2026-09-08 合併 PR 1118：**兩側各自重寫了這一段**,⇒ 取聯集,⛔ 不是二選一。
    //   · Codex：`verified-runtime` 短路 —— 這個表示模式在同一檔的 212／219／304／309
    //     行**已經一致地用著**（道具、增益卡、兩處 helper）;英雄不認它就是「道具認、
    //     英雄不認」的半接線缺陷（失敗形態⑧）。
    //   · main：GH#1064 的出身繼承（`roster`）＋ GH#1024 A4 的 `role` 推導。
    //   ⇒ ⭐ 先短路,再跑 main 的完整管線。
    if (options.representation === "verified-runtime") { registerChampion(d); continue; }
