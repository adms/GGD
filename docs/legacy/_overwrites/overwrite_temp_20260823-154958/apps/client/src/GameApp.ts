...
export class GameApp {
  ...
  private computeMoveSpeed(championId: string, items: string[]): number {
    ...
    // ⛔⛔ **這裡在 2026-08-23 之前只 mirror 了鏈的一半**（owner 回報「客戶端移動
    // 還是非常不順，**常常有回溯**」）。上一版寫的是 `ms *= this.combatEnv.moveSpeed`,
    // 而註解自己說「mirror **both** here or prediction diverges」——
    // ⭐ 它 mirror 了一格，而伺服器那條鏈有**兩格**：
    //
    //   `STAT_ENV_CHAIN[Stat.MoveSpeed] = [ fixed("moveSpeed"),`
    //   `                                   byAttackType(melee:"moveSpeedMelee",`
    //   `                                                ranged:"moveSpeedRanged") ]`
    //
    // 出貨值是 `moveSpeed 1` · `moveSpeedMelee 0.8` · `moveSpeedRanged 0.6`
    // ⇒ ⭐ 影子比伺服器**快 25%（近戰）／67%（遠程）**，而每一張快照都把它拉回去
    //   ＝ owner 說的「回溯」。⚠️ 遠程英雄更嚴重，正好對得上他的描述。
    //
    // ⭐ 修法⛔ **不是再抄一次那條鏈**（那就是第二個住處，而它漂掉的那天沒有東西會紅）
    //   —— 直接呼叫**伺服器用的同一支** `finalizeStat()`，連 `[2,14]` 的夾限、
    //   `base-bonus`、`per-level-bonus` 都是同一份程式。
    // ⚠️ `attackType` 讀**英雄卡**（`ChampionDef` 的必填欄位），⛔ 不是「射程 > 3」
    //   那種啟發式 —— 射程是會被道具動到的衍生值，用它反推身分等於讓一件裝備
    //   把近戰變成遠程（`statPipeline.ts:141` 逐字記著同一條）。
    return finalizeStat(ms, Stat.MoveSpeed, {
      env: this.combatEnv,
