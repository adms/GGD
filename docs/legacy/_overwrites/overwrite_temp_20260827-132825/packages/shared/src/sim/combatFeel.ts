...
export const DEFAULT_STANDSTILL: StandstillRules = Object.freeze({
  ...
  stillEps: 0.1,
  closingRatio: 0.5,
  // ⛔⛔ **出貨預設翻回 true**（2026-08-27）—— #755 的新規則造成**靈魂層回歸**。
  //
  // 量到的（⛔ 不是推測）：把這個檔換回 #755 之前的版本，`sim/facingLock.test.ts`
  // **6/6 綠**；換成新版，兩條紅，而其中一條量到 `alignAtHit = -0.73`
  // ——⭐ **攻擊朝著目標的反方向出手**，也就是 owner 抱怨過兩次的
  // 「林克倒著走路攻擊」那一族（#216 → #783 → 這裡）。
  //
  // ⭐ 根因：新規則把「有沒有在動」的門檻從 `walkEps`(0.8) 降到 `stillEps`(0.1)。
  //   於是一個**只是在飄**的單位（0.1–0.8）現在算「在走」⇒ 站定規則對它生效
  //   ⇒ 出手時機改變 ⇒ 面向還沒鎖到目標身上。
  //   ⚠️ 舊版讓 `walkEps` 一格當兩用**不是疏忽**，那個 0.8 在「算不算在動」
  //   這一側是刻意的下限。#755 只拆了欄位，⛔ 沒有替新的下限找出處。
  //
  // ⇒ 第〇·六守則：**靈魂層的守衛紅了，改動就輸**。這一格翻成 true = 走舊路徑，
  //   #755 的欄位拆分**留著**（上下界、後台可調都在），只是預設不啟用它。
  //   ⭐ 要重新啟用：後台把它改成 false，並且**先讓 facingLock 綠**。
