...
CACHE="tools/ticket-cache/issue-numbers.txt"   # ⚠️ 一行一個號碼,⛔ 不是手寫的（--refresh-cache 產生）
...
cache_is_fresh || refresh_cache >/dev/null 2>&1 || true
...
lint_one() { # $1=一行標籤（顯示用） $2=訊息全文
  ...
  for n in $(printf '%s' "$msg" | grep -oE '#[0-9]+' | tr -d '#' | sort -un); do
    ...
    if [ "$HAVE_CACHE" = 1 ] && command -v gh >/dev/null 2>&1 \
       && gh issue view "$n" --json number >/dev/null 2>&1; then
      echo "$n" >> "$CACHE"; continue        # 快取過期而票是真的 ⇒ 補一筆
    fi
    if [ "$HAVE_CACHE" = 1 ] && command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
...
[ "$rc" = 0 ] && echo "✓ commit 訊息裡的票號/lane 代號沒有互相冒充"
exit "$rc"
