...
export class MatchRoom extends Room<MatchState> implements AccountRoomHolder {
  ...
  private async buildMatch(options: MatchRoomOptions, ops: ServerOps): Promise<void> {
    ...
    //
    // ⚠️ 它刻意站在 `resolvedWhitelist` 之後、`MatchController` 之前：白名單是
    // 這一場唯一的內容 seam（選角 · 隨機池 · bot · 商店 · 掉落 · EX 全部問它），
    // ⇒ ⭐ 一刀切在這裡就覆蓋六條路，⛔ 不是在六個地方各加一個 if。
    //
    // ⭐ 「哪些 id 是社群來的」是 platform 在**發布那一刻**記進耐久覆蓋層的，
    // ⛔ 不是這一台推導的 —— 熱套用知道自己剛加了什麼，而**重啟之後那個資訊
    // 就沒了**（那會變成「重啟前只進社群房、重啟後跑進官方房」）。
    //
    // ⛔ 開關關著（`ugc.communityRoomOnly = false`）⇒ 連 HTTP 都不打。
