 ...
 *
 * `castType:"self"` 讓 `abilitySystem.ts` 把目標集合解成 `[caster]`，而 `effects/damage.ts`
 * 逐字是 `subjects = e.applyTo === "self" ? [caster] : ctx.targets` ⇒ ⭐ 一支 self 技能的
 * `damage` 不寫 `applyTo:"self"`，那一發就**打在施法者自己身上**。三條既有閘
 * （castabilitySweep / abilityCastClaims / abilityNoOpEffects）全部只問「有沒有打出來」，
 * ⛔ 沒有一條問「打在誰身上」—— 小傑 Q/W 是一顆自殺鍵而 `docs/_castability-128.md` 七格全 ✅（GH#1018）。
 *
 ...
 * 分母：`effects[]` 的直接子節點 ＋ `delayed` 子樹（它用 `wave.frozen` ＝ 原本的 `[caster]`）。
 * `damageArea` / `damageLine` / 投射物 / `onHitTargets` 自己重解受害者，⛔ 不在這一支的分母裡。
 *
 * 後台開關：`config.damage-rules@1.abilitySelfDamageGuard`（出貨 true）—— false 時只印警告不擋
 * （fail-open 沒錯，靜默才是缺陷）。⚠️ 名字與預設是我挑的（owner 2026-08-23「留後台開關可以簡易 rollback」）。
...
type Node = { kind?: string; applyTo?: string; effects?: Node[] };
type Def = { castType: string; effects?: Node[]; description?: string };
const INTENT = /自身|自己|代價|犧牲|自傷/;

/** self 施放時受害者仍是 `[caster]` 的傷害葉：頂層 ＋ `delayed` 子樹。 */
function casterBound(effects: readonly Node[] | undefined, path: string, out: { path: string; node: Node }[]): void {
  (effects ?? []).forEach((n, i) => {
    ...
    if (n.kind === "delayed") casterBound(n.effects, `${p}.effects`, out);
  });
}
