...
#   scripts/lane-plan.sh --assume-file-fence  # ⭐ 假設那幾張目錄級柵欄的票**改成逐檔列名**之後再排
#     ⚠️ 量到的（2026-08-27，71 張 open 票）：照票文原樣 ⇒ **1 條 lane**（什麼都不能並行），
#        因為 41 張票的 Files 區寫的是**目錄** ⇒ 目錄＝萬用字元 ⇒ 傳遞性地黏成一團。
#        加了這個旗標 ⇒ **14 條 lane**。⛔ 這個旗標**不是**「忽略撞車」——
#        它是「先照 CLAUDE.md ⚡④ 把柵欄改成檔案級」這件事的**預覽**。
...
BUSY=""; LIMIT=400; WIDTH=10; ASSUME=0
while [ $# -gt 0 ]; do
  case "$1" in
    --busy)  BUSY="${2:-}"; shift 2 ;;
    --assume-file-fence) ASSUME=1; shift ;;
...
def clash(a: str, b: str) -> bool:
    ...
    `docs/_daily/????-??-??.md` ⇒ 舊版只認得 `*` ⇒ 那 21 份**每日帳本產物**
...
by_n = {t["n"]: t for t in planned}
# 票在 lane 內部的順序 = 相依深度（⚠️ 成環時停住，⛔ 環不可以讓腳本掛掉）
depth = {t["n"]: 0 for t in planned}
for _ in range(len(planned) + 1):
    changed = False
    for t in planned:
        d = max((depth[x] + 1 for x in t["deps"] if x in by_n), default=0)
        if d > depth[t["n"]]:
            depth[t["n"]], changed = d, True
    if not changed:
        break
...
def _coarse(p: str) -> bool:
    if "*" in p:
        return False
    return p.endswith("/") or "." not in p.rstrip("/").rsplit("/", 1)[-1]

# ⭐ `--assume-file-fence`：把**目錄級**條目排除在撞車判斷之外，回答「票文改成逐檔列名
#   之後可以開幾條 lane」。⛔ 預設不開 —— 排除目錄＝把真的撞車藏起來。
def fence(t: dict) -> list[str]:
    return [f for f in t["files"] if not (ASSUME and _coarse(f))] or sorted(t["files"])[:1]

for a, b in combinations(planned, 2):
    if any(clash(f, g) for f in fence(a) for g in fence(b)):
        parent[find(a["n"])] = find(b["n"])
# 🔒 全域鎖 ⛔ **不是撞檔** —— 它是一個**分時共用的資源**（CLAUDE.md：「同一時間只能有
#   一條工作流跑 skills:sync」）。⇒ 它的正解是**一批只准一條 lane 帶鎖**（下面排批次時做），
#   ⛔ 不是把所有碰產物的票黏成同一條 lane。
#   ⚠️ 第一版就是黏起來的，結果 71 張票收斂成 **1 條 lane** —— 一份「排好了」的計畫，
#   而它說的是「什麼都不能並行」。
...
slot: dict[int, int] = {}
has_lock = {k: any(t["lock"] for t in v) for k, v in lanes.items()}
for k in sorted(lanes, key=lane_key):
    i = max((slot[d] + 1 for d in lane_deps[k] if d in slot), default=(1 if BUSY else 0))
    while True:
        while len(batches) <= i:
            batches.append([])
        # 🔒 一批只准**一條** lane 帶全域鎖（`skills:sync` 寫 bundle.json，同時間只能有一條）
        if len(batches[i]) < width and not (has_lock[k] and any(has_lock[j] for j in batches[i])):
            batches[i].append(k)
            slot[k] = i
            break
        i += 1

# ── ③b ⭐ **柵欄太粗**的診斷 —— 真資料上這是把並行度吃光的元兇 ─────────────────
# ⚠️ 量到的（2026-08-27，71 張 open 票）：照票文原樣算 ⇒ **1 條 lane**（＝什麼都不能並行）。
#   元兇是**目錄級**的 Files 條目（`apps/client/src` · `packages/shared/src/ops/` …）——
#   一條目錄等於一個巨大的萬用字元，它把彼此無關的票**傳遞性**地黏成一團。
# ⭐ CLAUDE.md ⚡④ 逐字說過同一件事：「柵欄用**檔案級**，⛔ 不是目錄級」，
#   而那一條講的是**排 lane 的人**；這裡量到的是**票文自己**也要逐檔列名。
# ⇒ 這一段**不偷偷把它們丟掉**（那會把真的撞車藏起來），它做的是：
#   照原樣排一份（保守、正確）＋ 再算一次「如果那幾張逐檔列名，可以開到幾條 lane」，
#   然後**逐張指名**是哪些條目。⭐ 兩個數字都印出來，⛔ 不是只印好看的那個。
def is_coarse(p: str) -> bool:
    """指向**目錄**（結尾是 / 或最後一段沒有副檔名）而且不是 glob ⇒ 它是一個萬用字元。"""
    if "*" in p:
        return False
    last = p.rstrip("/").rsplit("/", 1)[-1]
    return p.endswith("/") or "." not in last

def lanes_of(get_files) -> int:
    par = {t["n"]: t["n"] for t in planned}
    def f(x):
        while par[x] != x:
            par[x] = par[par[x]]; x = par[x]
        return x
    for a, b in combinations(planned, 2):
        if any(clash(x, y) for x in get_files(a) for y in get_files(b)):
            par[f(a["n"])] = f(b["n"])
    return len({f(t["n"]) for t in planned})

coarse_by = {t["n"]: sorted(x for x in t["files"] if is_coarse(x)) for t in planned}
coarse_by = {n: v for n, v in coarse_by.items() if v}
refined = lanes_of(lambda t: [x for x in t["files"] if not is_coarse(x)]) if coarse_by else 0

# ── ④ 報表 ────────────────────────────────────────────────────────────────
print(f"🗂️ lane-plan —— 掃了 {len(issues)} 張 {src} 票：{len(planned)} 張排得進來 · "
      f"{len(noplan)} 張沒有 Files 區 · {len(BUSY)} 張標成有人在做")
big = max((len(v) for v in lanes.values()), default=0)
print(f"⭐ 算出 **{len(lanes)} 條 lane**（撞檔的連通分量）⇒ 排成 "
      f"**{sum(1 for b in batches if b) + (1 if BUSY else 0)} 批**（--width {width}）"
      f"　·　最肥的一條 lane 有 **{big}** 張票")
if big > 8:
    # ⭐ 一條肥 lane ⛔ 不是「這些票真的相關」——它是**被幾個共用條目黏起來的**。
    #   指名那幾個條目就是一張可以動手的清單（去把票文改成逐檔列名）。
    fat = max(lanes.values(), key=len)
    seen: dict[str, int] = {}
    for t in fat:
        for f in t["files"]:
            seen[f] = seen.get(f, 0) + 1
    glue = sorted((c, f) for f, c in seen.items() if c >= 3)[::-1][:6]
    print(f"⚠️⚠️ **最肥的那條 lane 有 {big} 張票 ⇒ 它們幾乎都不能並行。**"
          " 把它們黏起來的共用條目（出現在 ≥3 張票裡）：")
    for c, f in glue:
        print(f"     {c} 張票都點名了 `{f}`")
    print("   ⇒ ⭐ 這幾個就是**下一個要拆的重災區**（CLAUDE.md 第〇·七守則觸發器②），"
          "⛔ 不是「這批票本來就該串行」。")
if BUSY:
...
if coarse_by:
    print(f"\n⚠️⚠️ **柵欄太粗 ⇒ 並行度被吃掉**（{len(coarse_by)} 張票寫了**目錄**而不是檔）")
    print(f"   照票文原樣：**{len(lanes)} 條 lane**　·　把下面這些改成逐檔列名：**{refined} 條 lane**")
    print("   ⛔ 這一段**沒有**把它們從撞車判斷裡拿掉（那會把真的撞車藏起來）——")
    print("   ⭐ 它是一張**要去改票文**的清單（CLAUDE.md ⚡④：柵欄用檔案級，⛔ 不是目錄級）：")
    for n, v in sorted(coarse_by.items(), key=lambda kv: (-len(kv[1]), kv[0]))[:12]:
        print(f"     #{n}　{' · '.join(v[:4])}" + (" …" if len(v) > 4 else ""))
if noplan:
