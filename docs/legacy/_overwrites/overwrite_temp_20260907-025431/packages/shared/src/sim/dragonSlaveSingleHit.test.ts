 ...
 *
 * ── 🧬 突變紀錄（實跑：改壞 → 🔴 → 還原）──────────────────────────────────
 * M1【承重】`content/abilities/godie-h020.e.json` 的 `onArrive.damageArea.victimCondition`
 *    整格拿掉（＝去重消失，⭐ 而畫面上跟正確的一模一樣）
 *      → 🔴 ①「落點(12,0) 吃到 2 發 —— 沿途與落點沒有互斥」
...
describe("GH#1092 · 04-03 龍破斬一次施放只結算一發", () => {
  ...
  it("★★ ② 兩段的 `amount` 逐位元相同（⭐ 原作只有一個 `udg_DragonSlaverDamage`）", () => {
    ...
    expect(
      JSON.stringify(touch?.["amount"]),
      "⛔⛔ 沿途與落點的傷害量分岔了 —— ⭐ 兩段互斥之後，一個人只會吃到其中一段，\n" +
        "  於是分岔的那一半就是「被沿途收走的人拿到一個少了 AP 項的數字」，\n" +
        "  而卡面「點選惡夢魔王碎片增幅後，可增加威力」對他就不會發生（第一·五守則）。",
    ).toBe(JSON.stringify(blast?.["amount"]));
  });
