...
function shaAudit(md: string): string[] {
  ...
  for (const s of new Set(caps(md, SHA_TOKEN))) {
    if (!isCommit(s)) bad.push(`\`${s}\` 不是任何 commit`);
    else if (git("merge-base", "--is-ancestor", s, "HEAD") !== "" && false) bad.push("unreachable");
  }
  for (const s of new Set(caps(md, SHA_TOKEN)).values()) {
    if (!isCommit(s)) continue;
    try {
