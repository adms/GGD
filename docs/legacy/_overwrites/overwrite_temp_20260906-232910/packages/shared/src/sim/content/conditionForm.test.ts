 ...
 *
 * ⭐ 跑的是**出貨的** 09-04 龜派氣功：出貨內容、出貨的 `castAbility`、出貨的 09-03 超級賽亞人
 * （`championForm` 效果 → `applyChampionForm`）、出貨的 `revertToBaseForm`。⛔ 沒有一條斷言在讀
 * condition 物件的形狀（失敗形態⑦）。**三個方向一起讀**：本體沒有變身增幅 → 變身態有 →
 * **變回本體同一 tick** 又沒有 —— 第三段正是 `recentCast withinSec=變身秒數` 做不到的（E 剛放過，
 * 窗口還開著），所以這一檔在舊內容上是紅的，⛔ 不只是「新葉子會動」。
 *
