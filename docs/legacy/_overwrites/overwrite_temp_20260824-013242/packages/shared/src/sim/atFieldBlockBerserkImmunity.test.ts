...
function arena(seed: number): { w: SimWorld; eva: EntityId; other: EntityId } {
  ...
  const other = spawn(1, 1, 30); // 另一隊、30 單位外 —— 只當施加者/傷害來源
  w.step(new Map());
  return { w, eva, other };
}
...
describe("GH#644 · AT力場格擋 + 暴走負面免疫", () => {
  it("① 物理傷害有兩種結局：原價、與被砍掉 fraction 的那一種 —— 兩種都真的發生", () => {
    const grant = Abilities.get(Champions.get(EVA)!.abilities.E as never).passive!
      .ranks[0]!.block!;
    ...
    expect(kinds[0]!).toBeCloseTo(kinds[1]! * (1 - grant.fraction), 3);
  ...
  it("② 暴走中：暈眩掛不上（免控拒絕）、詛咒落地即被拔；同一發暈眩在非暴走時掛得上", () => {
    ...
    const ctx = { world: w, caster: other, rank: 1, targets: [eva],
                  origin: "ability:test.berserk-immunity" };
