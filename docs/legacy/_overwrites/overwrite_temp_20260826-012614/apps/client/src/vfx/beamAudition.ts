...
export async function startBeamAudition(
  ...
  const [{ setAbilityVfxBindings }, { setFamilyTuning }, { Configs }] = await Promise.all([
    import("../render/vfx/abilityLayers"),
    import("../render/vfx/w3xAbilityArt"),
    import("@ggd/shared/content/registries"),
  ]);
  // ⛔ 順序照抄 `ContentDb.load()`：綁定表要在 `setFamilyTuning` **之前**。
  setAbilityVfxBindings(Configs.tryGet("ability-vfx-bindings") as never);
