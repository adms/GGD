...
export async function bootVfxScriptStudio(): Promise<void> {
  ...
  stage.addEventListener("drop", (e) => {
    ...
    // 施法者面向固定 +x（beamAuditionWorld 設的）⇒ 前後＝Δx、左右＝−Δz…
    // ⛔ 不硬抄：走與播放器同一個定義（+side＝面向的右手邊，面向 (1,0) 時右手邊是 −z 方向？
    // applyFacingOffset: x+=fz*side, z-=fx*side ⇒ facing(1,0) 時 side 加在 −z ⇒ side ＝ −Δz。
