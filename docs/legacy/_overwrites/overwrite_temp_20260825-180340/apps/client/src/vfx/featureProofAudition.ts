...
const CLASSICS: Record<string, ClassicSpec> = {
  ...
  dragonslave: {
    ...
    beats: [
      ...
      { label: "g4_blast", ticks: 1, until: { type: "explosion", nth: 1, max: 90 }, note: "落點爆炸：fx.prim.fire.explosion-lg ＋ damageArea 8 半徑 ＋ 四段 delayed 的後續" },
  ...
  endworld: {
    ...
    beats: [
      ...
      { label: "e2_resolve", ticks: 5, until: { type: "modelFxSpawn", nth: 1, max: 90 }, note: "詠唱解算：spawnProjectile（imported.wave）＋ tpl-radial-burst 十二向散開" },
      { label: "e3_burst", ticks: 12, note: "放射中：12 具沿 radial 路徑外擴（speed 6 · distance 4.5），onTouch 減速" },
      { label: "e4_arrive", ticks: 1, until: { type: "vfxSpawn", nth: 6, max: 90 }, note: "到點：fx.prim.ice.nova ＋ 12 記 screenShake ＋ 三段 delayed 的後續傷害" },
