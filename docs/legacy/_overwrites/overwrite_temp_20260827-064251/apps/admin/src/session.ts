...
export class ApiClient {
  ...
  setTokens(tokens: TokenPair | null): void {
    ...
    // ⛔ 非 cookie 模式**一個位元組都不加** —— rollback 要能還原成 #724 之前
    // 存下的那份形狀，⛔ 不是「多一格 false」。
    this.storage.save(cookieMode ? { ...tokens, refreshToken: "", rtCookie: true } : tokens);
  }
  ...
  private refreshOnce(): Promise<boolean> {
    if (!this.refreshing) {
      this.refreshing = (async () => {
        ...
        // ⛔ 兩個都沒有才是真的沒有 session。
        if (refreshToken === "") return false; // MUTANT
        try {
