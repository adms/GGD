 ...
 *
 * 這支腳本活在正式部署路徑上：docker/edge.Dockerfile COPY 進映像，
 * docker/edge-entrypoint.d/20-ggd-assert-full-assets.sh 末行 `exec … assert`，
 * assert 失敗會擋住 nginx 啟動。而在此之前它有兩個**會給錯診斷**的洞：
 *
 *   ① bytes_of() 的 `cat … 2>/dev/null | wc -c | tr` 有兩層吞：EACCES 被重導丟掉，
 *      pipeline 的離開碼來自末端的 tr ⇒ 一個**讀不到**的資產檔，最後只表現成
 *      「is 87,3xx,xxx B, manifest says 87,403,869 B」的**位元組短少**
 *      ⇒ 半夜去追一個不存在的 rsync 截斷，真相是一顆 mode bit。
 *   ② `[ "$rc" -eq 0 ] || return 1` 卡在 --deep 區塊**前面** ⇒「哪幾個檔壞了」的
 *      逐檔點名，只有在 count 與 bytes 都已經相符時才跑得到 —— 也就是**永遠不會**
 *      在最需要它的那個情況跑。
 *
 * ⚠️ 這裡**真的把腳本跑起來**（同 ggd-backup.test.ts / hostDeployScript 的形狀），
 * ⛔ 不是掃原始碼字串 —— 掃字串對「那一行還在但跑不到」是結構性失明（失敗形態⑥）。
 */
...
function newSet(): string {
  ...
  writeFileSync(join(dir, "b.bin"), "bbbbbb");
  writeFileSync(join(dir, "c.bin"), "cc");
  const m = run(["manifest", dir, "fixture"]);
  expect(m.status, `manifest 應該成功：${m.stderr}`).toBe(0);
  return dir;
}
