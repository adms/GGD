...
// ⭐ GH#1146 —— instant-blast 的六格選配（⛔ 不是新家族,見那一段的註解）。
import { zKnockback } from "../schema/effects/knockback";
import { zDispel } from "../schema/effects/dispel";
...
const FAMILIES: Readonly<Record<string, Family>> = {
  ...
  "instant-blast": (t, p) => {
    ...
    return {
      ...
      effects: [
        damageEffect(damageType(t, p, "damageType"), scaling(t, p, "damage")),
        // ⭐⭐ GH#1146 —— 六格選配。⛔ 它們**不是**六個新家族：
        //   `templatize.py` 的擋點逐條分箱之後（2026-09-09，母體 32 個形狀）：
        //     · `damageArea` 出現在 **21/32** 個形狀裡
        //     · 其餘全部是**選配的一到兩格**（applyStatus 11 · knockback 7 · spawnModelFx 7 · spawnVfx 5 …）
        //   ⇒ ⭐ 一個模板加六格選配 **解鎖 15/32（47%）**，⛔ 而做六個家族只會解鎖同樣那些。
        //
        // ⚠️ ⭐ **次序是語意，⛔ 不是排版**：傷害先結算，位移／狀態／持續傷害才跟上 ——
        //   ⛔ 把 `knockback` 排在 `damage` 之前，目標會先被推走再結算距離。
        //   ⭐ 這個次序是照那 15 支手寫技能**逐位元**的順序定的（⛔ 不是我挑的）。
        ...(has(t, p, "knockback") ? [effectNode(t, p, "knockback", "knockback", zKnockback)] : []),
        ...(has(t, p, "status") ? [statusNode(t, p, "status")] : []),
        ...(has(t, p, "dot") ? [effectNode(t, p, "dot", "dot", zDot)] : []),
        ...(has(t, p, "dispel") ? [effectNode(t, p, "dispel", "dispel", zDispel)] : []),
        // ⭐ 視覺兩格刻意**分開**（⛔ 不是一格二選一）：`spawnVfx` 是粒子腳本、
        //   `spawnModelFx` 是模型 —— 兩者的參數集合完全不同，硬併成一格會變成一個
        //   「有時候要填這幾格、有時候要填那幾格」的欄位（後台畫不出來）。
