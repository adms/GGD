...
describe("技能積木", () => {
  ...
  it("⭐ 新積木的起始值過得了它自己的 schema，而且 kind 帶上了", () => {
    for (const kind of ["damage", "heal", "applyStatus"]) {
      const fresh = newEffect(kind);
      expect(fresh.kind).toBe(kind);
      const r = (brickSchema(kind) as { safeParse: (v: unknown) => { success: boolean; error?: unknown } }).safeParse(fresh);
