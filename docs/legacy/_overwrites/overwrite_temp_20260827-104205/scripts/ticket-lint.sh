...
BAD=0
if [ "${1:-}" = "--dupes" ]; then
  RAW=$(gh issue list --state open --limit 500 --json number,title,body 2>/dev/null) || RAW=""
  [ -n "$RAW" ] || { echo "⚠️ gh 連不上 —— 接手相交**沒有被掃**(⛔ 這不是「沒有重複」)。"; exit 0; }
  printf '%s' "$RAW" > /tmp/ticket-lint-dupes.$$.json
  python3 - /tmp/ticket-lint-dupes.$$.json "tools/parallel-gates/takeover-vocab.json" <<'PY'
...
print(f"🎫 掃了 {len(issues)} 張 open 票，其中 {len(owned)} 張宣告了接手。")
if not pairs:
    print("✓ 接手相交：0 對。")
    sys.exit(0)
...
PY
  BAD=$?
  rm -f /tmp/ticket-lint-dupes.$$.json
  exit "$BAD"
fi
if [ "$1" = "--body-file" ]; then
