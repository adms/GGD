 ...
 */
const zVfxScriptEntry: z.ZodType<VfxScriptEntry, z.ZodTypeDef, unknown> = z
  .unknown()
  .superRefine((v, ctx) => {
    if (!v || typeof v !== "object" || Array.isArray(v)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: "段落要是物件（inline 段或 {call:{…}}）" });
      return;
    }
    const r = (isVfxScriptCall(v) ? zVfxScriptCall : zVfxScriptSegment).safeParse(v);
    if (!r.success) for (const issue of r.error.issues) ctx.addIssue(issue as z.IssueData);
  })
  .transform((v) => v as VfxScriptEntry);
 ...
 */
export const VFX_SCRIPT_SCHEMA_TAGS = ["vfx-script@1", "vfx-script@2"] as const;
...
/**
 * ⭐ 展開後的文件 —— **播放器唯一吃的形狀**（`VfxScriptPlayer` 的 deps 型別）。
 * 由 `content/vfxSubtypes/expand.ts::expandVfxScriptDoc` 產生；沒有 `call` 的文件展開＝原樣。
 */
export type ExpandedVfxScriptDoc = Omit<VfxScriptDoc, "segments"> & { segments: VfxScriptSegment[] };
