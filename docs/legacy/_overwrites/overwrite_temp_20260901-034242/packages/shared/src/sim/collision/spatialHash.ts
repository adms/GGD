...
export class SpatialHash {
  ...
  /**
   * ⭐⭐ GH#629 —— 每 tick 的清空**重用陣列**，⛔ 不是把它們全丟掉。
   *
   * ── ⛔ 在此之前 ──────────────────────────────────────────────────────────
   * `this.cells.clear()` 把**每一個格子的陣列**一起丟掉 ⇒ 下一 tick 的 `insert`
   * 又逐格 `arr = []` 重新配置。⭐ 一張 24×18 的競技場在 cellSize 4 下有上百格，
   * ⇒ **每 tick 上百次陣列配置**，而它們的內容每 tick 幾乎一樣。
   *
   * ── ⭐ 改成什麼 ────────────────────────────────────────────────────────────
   * 保留 Map 的鍵與陣列物件，只把 `length = 0`。
   * ⇒ ⭐ 下一 tick 的 `push` 直接寫進已配置好的空間。
   *
   * ⚠️ ⭐ 代價是「永久空掉的格子」會留在 Map 裡 —— ⛔ 而它有界（場地大小固定），
   * 且每一格只是一個空陣列的殼。⇒ 用一次配置換每 tick 一次配置，⭐ 划算。
   *
   * ⚠️ `entityBounds` **仍然真的清空**：它是逐實體的，
   * ⛔ 留著會讓一個已經離場的 id 帶著過期的 bounds（而 `queryAABB` 會讀它）。
   */
  clear(): void {
