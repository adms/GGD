...
async function runWeather(st: Stage): Promise<Record<string, unknown>> {
  ...
  const rainStep = async (
    ...
    const handle = buildRain(st.scene, root, arenaId, zones, { policy, look }, 1);
    facts[label] = { arenaId, chance, rain: look.rain, kind: look.kind, built: handle !== null };
    if (handle) {
      live = {
        dispose: (): void => {
          // ⚠️ GPUParticleSystem 在還沒送過一幀就 dispose 會在 `_releaseBuffers`
          //    擲 TypeError（Babylon 7.54）—— 先 stop 再收，收不掉就把 emitter
          //    mesh 帶走。⛔ 台子的清理失敗不可以變成「這一批量不到」。
          try {
            handle.ps.stop();
            handle.ps.dispose();
          } catch {
            (handle.ps.emitter as { dispose?: () => void } | null)?.dispose?.();
          }
        },
      };
    }
    await st.waitFrames(waitFrames);
    await st.capture(label, note);
  };
  ...
  await rainStep(
    ...
    "rainChance 1 ＋ seed 7919：clear 級的**室外**圖（羅馬大擂台）也在機率池裡 —— #676 的核心",
    "arena.colosseum",
    1,
    45,
  );
...
async function runDagger(st: Stage): Promise<Record<string, unknown>> {
  ...
  const zone = SKELETON_ARENA.zones[0]!;
  const world = new SimWorld(SKELETON_ARENA, 7); // 種子 7：第 15 tick 就擲中
  ...
  const spawn = (team: 0 | 1, dx: number): number =>
    spawnChampion(world, {
      ...
      pos: { x: zone.center.x + dx, z: zone.center.z },
      zone: 0,
    }) as unknown as number;
  const holder = spawn(0, -0.6);
  const victim = spawn(1, 0.6);
  grantItemFree(world, holder as never, DAGGER as never);
  world.nav.get(holder as never)!.attackTarget = victim as never;

  makeGround(st.scene, zone.center.x, zone.center.z, 24);
  ...
  st.camera.position.set(zone.center.x, 3.4, zone.center.z - 9);
  st.camera.setTarget(new Vector3(zone.center.x + 1.4, 1.5, zone.center.z));
  ...
  // 觸發前的安靜刀（種子 7 在第 15 tick 擲中 ⇒ 先走到 14）
  step(14);
  await st.capture("d1_quiet_swings_tick14", "普攻已經在互砍，3% 還沒擲中 —— 場上沒有血花");
  const hitTick = step(1);
  facts["triggerTick"] = hitTick;
  if (hitTick < 0) throw new Error("種子 7 在第 15 tick 沒有擲中 —— 觸發鏈或種子壞了");
  ...
  await st.capture("d2_trigger_tick15_spray_born", `3% 擲中：出貨 vfxSpawn 落地，血花池 ${pool.map((p) => p.name).join(" / ") || "無"}`);
...
async function runStuckEscape(st: Stage): Promise<Record<string, unknown>> {
  ...
  const facts: Record<string, unknown> = { startAx: posOf(a), startBx: posOf(b) };
  ...
  await st.capture(
    "s4_passed_through",
    `穿過去了：A x=${posOf(a).toFixed(2)}（>${facts["startBx"]}）、B x=${posOf(b).toFixed(2)}（<${facts["startAx"]}）`,
  );
  return facts;
}
