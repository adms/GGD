...
const WEATHER_SPEC: ConfigDocSpec = {
  ...
  fields: [
    ...
    },
    {
      path: "rainEnabled",
      zh: "🌧️ 下雨總開關（出貨**關著**）",
      note: "owner 2026-08-24：「**下雨跟起霧的天氣特效**」。霧那一半上一版就出貨了，這一格是**天上真的在掉水**的那一半。⭐ **出貨是關的** —— 這是**新功能**⛔ 不是修復，所以它不套「更新後預設啟動」那條規矩；打開它之前，畫面與上一版逐像素相同。⭐ 打開之後「哪幾張圖會下雨」**不用另外設定**：仍然是最下面那張表的級別（`rain` 與 `storm` 兩級會下雨，`clear`／`fog`／兩個 `indoor-*` 都不會）—— ⛔ 不可能出現「天氣說晴朗、天上在下雨」。⚠️ `indoor-damp`（洞窟滲水）**地面濕、有積水，而天上一滴都不掉** —— 那是 owner 的「室內不要下雨」，⭐ 滲水不是雨。",
    },
    {
