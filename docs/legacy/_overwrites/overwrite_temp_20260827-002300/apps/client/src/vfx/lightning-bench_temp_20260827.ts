...
function runPhase(
  ...
  for (let f = 0; f < frames; f++) {
    ...
    // ⭐ NullEngine 下 ParticleSystem.animate() 會因 isReady()=false 整段跳過
    //   (particleTexture/effect 永遠不 ready) ⇒ 粒子 CPU 模擬量到 0 顆。
    //   把 isReady 釘成 true、render 釘成 no-op:CPU 模擬(_update)照跑,
    //   draw 那一半本來就是 GPU 的事,NullEngine 量不到。
    for (const ps of scene.particleSystems) {
      const p = ps as unknown as { isReady: () => boolean; render: () => number; __patched?: boolean };
      if (!p.__patched) {
        p.__patched = true;
        p.isReady = () => true;
        p.render = () => 0;
...
// ⭐ JIT 暖身:同一條路先跑兩輪,丟棄結果 —— 沒有這一段,第一個 phase 會把
//   編譯成本記在自己頭上(第一輪量到尖峰單幀 66.96 ms,其實是 JIT)。
runPhase("warmup-1", [{ startMs: 50, seed: 7 }], 2400);
runPhase("warmup-2", [{ startMs: 50, seed: 8 }], 2400);
