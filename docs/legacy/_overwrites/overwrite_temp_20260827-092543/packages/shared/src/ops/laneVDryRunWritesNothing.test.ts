...
/**
 * ⚠️ 探針要 import **腳本真的會用的東西**（Pillow），⛔ 不是 `json`。
 * 這台機器上有四個 python3，其中三個的 `import json` 是好的而 `PIL` 是壞的
 * （arm64 直譯器配 x86_64 的 `_imaging.so`）—— 一個只探 `json` 的挑選器會
 * 挑中它們，然後把「環境不對」報成「這條閘紅了」。
 */
function findPython(): string | null {
  for (const c of [
    ...
    try {
      execFileSync(c, ["-c", "import json, PIL.Image"], { stdio: "pipe" });
      return c;
    } catch {
      /* next */
    }
...
const PY = findPython();
const why = PY === null ? "沒有帶 Pillow 的 python3" : !existsSync(SCRIPT) ? "找不到轉檔腳本" : "";
