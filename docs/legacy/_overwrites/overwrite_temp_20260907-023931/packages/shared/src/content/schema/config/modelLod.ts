  ...
  .object({
    ...
    phoneShortEdgePx: z
      ...
      .describe(
        ...
        "@note 觸控裝置的**短邊**（`min(視窗寬, 視窗高)`，所以橫拿直拿同一個答案）小於這個數字就判成手機，否則判成平板。⭐ 出貨 600 的來源：iPad mini 短邊 744、目前最大的手機約 430 —— 600 落在兩者中間而且離兩邊都很遠。⚠️ 往上調會開始把小平板誤判成手機（＝擋住玩家），往下調只會少說一句話。⇒ ⛔ 不確定就往下調。填 0 ＝ 沒有任何裝置會被判成手機。",
      ),
    tabletFpsCap: z
      .number()
      .int()
      ...
      .describe(
        "@zh 🎞 平板（觸控裝置）的 fps 上限\n" +
        "@note owner 2026-09-06 逐字：「支援平板**最高 30fps**」（與 2026-07-28「FPS強制都是60，除非額外調整，手機則是預設30」同一個數字）。⭐ 這是**全遊戲唯一**一份 30：`apps/client/src/render/frameCap.ts` 的 `TABLET_FPS_CAP` 從這一格推導，戰鬥、登入、中場、商店立繪四條 render loop 全部吃它。⚠️ 它是**預設**不是硬鎖 —— 玩家在設定裡選過的值永遠贏（owner 那句話的後半「除非額外調整」）。調高＝平板更順但更燙。",
      ),
    minDevice: z
      .string()
      .min(1)
