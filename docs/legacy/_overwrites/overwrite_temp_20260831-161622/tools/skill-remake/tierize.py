...
def damage_leaves(doc):
    ...
    if isinstance(tpl, dict) and tpl.get("ref") in DAMAGE_TEMPLATES:
        ...
        for key in TEMPLATE_DAMAGE_PARAMS:
            v = params.get(key)
            # ⭐ 五級距家族（tpl-periodic-field）把傷害寫成**一格級別字串**，
            #    ⛔ 不是 `zScaling` dict ⇒ `_scaling_base` 對它一律回 None。
            #    少了這兩行，那一族的技能會被判成「沒有傷害葉」⇒ `idx` 掉回極小
            #    ⇒ **冷卻級距被吸到另一格**，而 JSON 的傷害那一格看起來完全正常
            #    （檔頭 `_scaling_base` 逐字記過同一個坑的第一次）。
            b = _tier_value(v) if v in TIER_NAMES else _scaling_base(v)
            if b is not None and b > CARRIER_BASE_MAX:
