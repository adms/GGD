...
describe("GH#174 鑄技工坊的試放走玩家那條路", () => {
  ...
  it("VFX Forge 以正式反彈路徑觸發理想鄉 EX，而不是把被動偽裝成按鍵", () => {
    ...
    const c = createSimPreviewController();
    // ⭐ 2026-09-07：等級從 18 降到 6 —— AP 係數重新校準（#1035 之後 base 隨母體變動）之後，
    //   18 級的七段連擊會在第一段就打死假人 ⇒ 七個 `comboStrike` 的姿勢全部塌成同一格，
    //   而症狀是「位移沒有保存真座標」，⛔ 看起來像預覽壞了。⛔ 這不是放寬斷言：
    //   仍然要求七段之間**位置真的變過**，只是讓假人活到第七段。
    const trace = c.triggerReflectSuccess(champion, ability.id, { level: 6, rank: 1, ticks: 180 });
    const schedule = scheduleSimEvents(trace.events, ability.id);
    const cues = triggerCuesFromSim(schedule, ability);
