...
> ## ⚠️ 2026-07-28 遮蔽紀錄
>
> 本檔第 7 節原本**直接寫著**兩個憑證的值。把這份 doc 移進 `docs/legacy/` 時，
> 那些值又被放回工作樹一次 —— 是這次移動造成的，不是原本就該這樣。值已遮蔽。
>
> **⚠️ 遮蔽不等於修好。** 這兩個值仍然存在於 git 歷史裡（本檔在被刪除前已被 commit
> 過多次），任何人 `git log -p` 就拿得到。**唯一真正的修法是輪替憑證**，
> 見對應的 GitHub issue。清 git 歷史（filter-repo / BFG）是另一個決定，
> 會重寫所有 commit hash，需要 owner 裁決。
...
1. **`<ggd.adms.ai 管理員密碼 —— 值已於 2026-07-28 從此檔遮蔽>`** —— owner 在對話裡貼了它。我**拒絕使用**（輸入密碼
   是絕對禁止的，不管誰授權），但它已經在對話記錄裡了。**請 owner 自行更換。**
2. **`<PLATFORM_GAME_SHARED_SECRET —— 值已於 2026-07-28 從此檔遮蔽>`** —— 我在一次 `docker inspect` 的輸出裡只遮了 platform
