...
## ⛔⛔ 更正（2026-08-31 對抗式稽核，⭐ 我獨立複驗過）

> 本文件第一版的結論：「**每一格的消失都是 0 ⇒ 契約沒有 breaking change**」。
> ⛔ **名字層是對的，結論是錯的。**

### ⭐ 實證：拿分支的 461 份 ability 餵 main 的 `validateDoc` ⇒ **FAIL = 3**

| # | breaking change | 出處 |
|---:|---|---|
| ① | ⭐⭐ **`radiusTier` 的值域整個重定義** | `schema/ability.ts:715` `z.enum(AOE_TIER_NAMES)` |
| ② | `internalCooldown` 從**選填變條件必填** | `effects/_hook.ts:302`（main 新增的 refine，分支 grep **零命中**）|
| ③ | main 多了分支**整個沒有**的 `deprecatedFields`（`flat` · `perRank`）| 名字仍在 `effectFields` 故 −0，⛔ **語意已棄用**（GH#534）|

### ⭐⭐ 而 ① 真正危險的**不是**那 3 個失敗，是**沒失敗的那 51 個**

| | 分支 | main 今天 |
|---|---|---|
| 級距名 | `小 · 中 · 大 · 超大` | `極小 · 小 · 中 · 大 · 極大` |
| `小` | **3** | **4.5**（⭐ +50%，面積 **+125%**）|
| `中` | 4.5 | **6**（+33%）|
| `大` | 6 | **8**（+33%）|
| `超大` | 8 | ⛔ **不存在** |

⇒ ⭐ 55 次 `radiusTier` 用例裡 **51 次（93% · 28 個檔）名字存活、驗證全綠 —— 而半徑變了**。
⚠️ ⭐ **⛔ 沒有任何東西會紅。**

### ⭐ 判準（⛔ 這是方法論的洞，不只是一個算錯的數字）

> ⛔ **名字還在，不等於語意沒變。**
> ⭐ 而**這份契約結構上只有名字**（`effectFields` 等全是 `string[]`）——
> ⇒ ⛔ 值域／上下界／必填性／棄用狀態**四樣都不在契約裡**，比不出來。

⇒ ⭐ **要比語意，就得跑真的驗證**：拿一批文件餵 `validateDoc`，⛔ 不是比契約的名字集合。

### ⛔ 另一個方向相反的錯：「planned −5 是因為那 5 項被做出來了」

⭐ 實際：planned **28 → 61，離開的集合是空的**。
那 5 個 key **全部仍在 main 的 planned 裡**，而且**四個是 `partial`（⛔ 沒做完）**。

⚠️ ⭐ 這句話的危害很具體：它會讓人以為那 4 個機制可用 ⇒ **做出上線就是死的內容**（失敗形態⑧）。
...
⛔⛔ **更正（2026-08-31）**：我原本寫「`apps/editor` 不在 main 上」——
⭐ **那是假的**：`git ls-files apps/editor/` = **78 個檔**、**17 支測試**、在 pnpm workspace 裡。
⇒ ⭐ **那條覆蓋率閘今天就寫得在 main 上** —— ⛔ 把它外包出去反而製造「永遠不會紅的閘」。

⚠️ 舊的（錯的）理由：`apps/editor` 不在 main 上 ⇒ 一條寫在 main 的測試讀不到它
