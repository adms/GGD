...
| 2 | （無時序形狀） | **107** | 42 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
...
⚠️ **前搖（`castTimeSec`）不在上表**：353/421 支有它，29 個相異值連續分布在 0.067–3 秒，中位數 0.667 秒。
它是**每一支技能都有的施法動作長度**（多半是 w3x 匯進來的），⛔ 不是作者寫下的機制 ——
算進「等待」軸的話這條軸會命中 421 支裡的 362 支，於是它分不出任何一群。
