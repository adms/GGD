...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **179** | 39 | tpl-buff-self・tpl-proxy-cast | 沿用（140 支還沒接） |
| 2 | （無時序形狀） | **108** | 41 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
...
| 8 | 續效特效＋路徑 | **5** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
| 9 | 多段＋路徑 | **4** | 3 | tpl-line-sweep・tpl-orbit-array | 沿用（1 支還沒接） |
| 10 | 持續＋續效特效＋路徑 | **4** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-leap-strike・tpl-mark-stacks（另有 2 份較不貼合） |
| 11 | 等待＋續效特效 | **4** | 1 | tpl-ground-nova | 沿用（3 支還沒接） |
| 12 | 等待＋路徑 | **4** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-teleport（另有 1 份較不貼合） |
...
|---|---:|---:|---:|---|
| 等待 | 20 | 51 | **14** | 延遲結算／吟唱／飛行時間 —— 躲不躲得掉 |
| 迴圈 | 55 | 22 | **44** | 每隔 T 秒重複 —— 排程與終止條件 |
| 持續 | 144 | 251 | **39** | 有期間的狀態，到期自己收掉 |
