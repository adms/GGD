...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **158** | 31 | tpl-buff-self・tpl-proxy-cast | 沿用（127 支還沒接） |
| 2 | （無時序形狀） | **104** | 38 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
| 3 | 持續＋續效特效 | **26** | 4 | tpl-buff-self・tpl-instant-blast・tpl-proxy-cast | 沿用（22 支還沒接） |
...
| 8 | 續效特效 | **8** | 2 | tpl-single-strike | 沿用（6 支還沒接） |
| 9 | 持續＋多段 | **6** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-proxy-fanout |
| 10 | 等待＋持續＋續效特效＋路徑 | **5** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-line-blast・tpl-locust-strike・tpl-locust-travel（另有 6 份較不貼合） |
| 11 | 等待＋路徑 | **5** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-teleport（另有 1 份較不貼合） |
| 12 | 續效特效＋路徑 | **5** | 1 | tpl-single-strike | 沿用（4 支還沒接） |
...
|---|---:|---:|---:|---|
| 等待 | 21 | 50 | **14** | 延遲結算／吟唱／飛行時間 —— 躲不躲得掉 |
| 迴圈 | 55 | 26 | **40** | 每隔 T 秒重複 —— 排程與終止條件 |
| 持續 | 144 | 255 | **38** | 有期間的狀態，到期自己收掉 |
| 續效特效 | 11 | 96 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
| 多段 | 9 | 48 | **6** | 一次施放拆成多下（連段／連鎖） |
