...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **159** | 31 | tpl-buff-self・tpl-proxy-cast | 沿用（128 支還沒接） |
| 2 | （無時序形狀） | **105** | 38 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
| 3 | 持續＋續效特效 | **25** | 4 | tpl-buff-self・tpl-instant-blast・tpl-proxy-cast | 沿用（21 支還沒接） |
| 4 | 路徑 | **19** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
| 5 | 持續＋續效特效＋路徑 | **14** | 3 | tpl-buff-self・tpl-single-strike | 沿用（11 支還沒接） |
...
| 8 | 續效特效 | **7** | 2 | tpl-single-strike | 沿用（5 支還沒接） |
| 9 | 續效特效＋路徑 | **7** | 1 | tpl-single-strike | 沿用（6 支還沒接） |
...
| 13 | 持續＋續效特效＋多段＋路徑 | **4** | 1 | tpl-proxy-cast | 沿用（3 支還沒接） |
| 14 | 等待＋續效特效 | **4** | 1 | tpl-ground-nova | 沿用（3 支還沒接） |
| 15 | 等待＋迴圈＋續效特效 | **4** | 0 | — | 併進既有模板的參數 |
| 16 | 續效特效＋多段＋路徑 | **3** | 1 | tpl-line-sweep | 沿用（2 支還沒接） |
| 17 | 迴圈＋持續＋多段＋路徑 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-lock-combo（另有 5 份較不貼合） |
...
| 25 | 迴圈 | **2** | 0 | — | 併進既有模板的參數 |
| 26 | 迴圈＋持續 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-proxy-fanout |
| 27 | 迴圈＋持續＋多段 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-random-barrage（另有 1 份較不貼合） |
...
| 迴圈 | 55 | 26 | **40** | 每隔 T 秒重複 —— 排程與終止條件 |
| 持續 | 144 | 253 | **38** | 有期間的狀態，到期自己收掉 |
| 續效特效 | 11 | 94 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
| 多段 | 9 | 46 | **6** | 一次施放拆成多下（連段／連鎖） |
| 路徑 | 42 | 95 | **17** | 效果沿著空間移動 |

⇒ ⭐ **最擋人的一條軸是「迴圈」，40 支技能的說明宣稱它而 JSON 裡沒有。**

⚠️ **前搖（`castTimeSec`）不在上表**：345/421 支有它，27 個相異值連續分布在 0.067–2.267 秒，中位數 0.667 秒。
...
|---|---|---|---:|---:|---|---|
| `tpl-single-strike` | 單體斬擊 | enabled | 3 | 30 | — | （無時序形狀）×26・續效特效×2・持續＋續效特效＋路徑×1・續效特效＋路徑×1 |
