...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **169** | 35 | tpl-buff-self・tpl-proxy-cast | 沿用（134 支還沒接） |
| 2 | （無時序形狀） | **108** | 42 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
| 3 | 持續＋續效特效 | **22** | 2 | tpl-buff-self・tpl-proxy-cast | 沿用（20 支還沒接） |
| 4 | 路徑 | **20** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
| 5 | 持續＋路徑 | **11** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-leap-strike・tpl-mark-stacks（另有 2 份較不貼合） |
| 6 | 等待＋持續＋續效特效＋多段＋路徑 | **11** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-dragon-quake（另有 16 份較不貼合） |
| 7 | 續效特效＋路徑 | **9** | 2 | tpl-single-strike | 沿用（7 支還沒接） |
| 8 | 續效特效 | **7** | 1 | tpl-single-strike | 沿用（6 支還沒接） |
| 9 | 持續＋多段 | **6** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-proxy-fanout |
| 10 | 持續＋續效特效＋路徑 | **6** | 1 | tpl-buff-self | 沿用（5 支還沒接） |
...
| 16 | 續效特效＋多段＋路徑 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
...
| 21 | 等待＋續效特效＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-line-blast・tpl-locust-strike・tpl-locust-travel（另有 3 份較不貼合） |
| 22 | 等待＋迴圈 | **2** | 0 | — | 併進既有模板的參數 |
| 23 | 等待＋迴圈＋持續＋續效特效＋多段 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-combo-finisher・tpl-periodic-field（另有 2 份較不貼合） |
| 24 | 等待＋迴圈＋持續＋續效特效＋多段＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-dragon-quake（另有 20 份較不貼合） |
...
| 持續 | 144 | 246 | **39** | 有期間的狀態，到期自己收掉 |
| 續效特效 | 11 | 81 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
