...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **177** | 39 | tpl-buff-self・tpl-proxy-cast | 沿用（138 支還沒接） |
| 2 | （無時序形狀） | **109** | 42 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
...
| 持續 | 143 | 249 | **39** | 有期間的狀態，到期自己收掉 |
| 續效特效 | 9 | 64 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
| 多段 | 9 | 45 | **6** | 一次施放拆成多下（連段／連鎖） |
| 路徑 | 42 | 89 | **16** | 效果沿著空間移動 |

⇒ ⭐ **最擋人的一條軸是「迴圈」，43 支技能的說明宣稱它而 JSON 裡沒有。**

⚠️ **前搖（`castTimeSec`）不在上表**：351/421 支有它，29 個相異值連續分布在 0.067–3 秒，中位數 0.667 秒。
它是**每一支技能都有的施法動作長度**（多半是 w3x 匯進來的），⛔ 不是作者寫下的機制 ——
算進「等待」軸的話這條軸會命中 421 支裡的 360 支，於是它分不出任何一群。
