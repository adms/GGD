...
| 16 | 迴圈＋持續＋多段＋路徑 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-lock-combo（另有 5 份較不貼合） |
| 17 | 多段＋路徑 | **2** | 2 | tpl-line-sweep・tpl-orbit-array | ✅ 全部接上了 |
| 18 | 持續＋續效特效＋多段＋路徑 | **2** | 1 | tpl-proxy-cast | 沿用（1 支還沒接） |
...
⚠️ **前搖（`castTimeSec`）不在上表**：345/421 支有它，29 個相異值連續分布在 0.067–3 秒，中位數 0.767 秒。
