...
| 續效特效 | 9 | 59 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
...
`宣告形狀` = 這份模板的**參數槽**與 `requires` 加起來寫得出什麼（21 個 draft 一支技能都沒接，
