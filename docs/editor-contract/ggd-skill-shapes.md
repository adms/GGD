...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **168** | 35 | tpl-buff-self・tpl-proxy-cast | 沿用（133 支還沒接） |
| 2 | （無時序形狀） | **106** | 40 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
| 3 | 持續＋續效特效 | **24** | 3 | tpl-buff-self・tpl-proxy-cast・tpl-single-strike | 沿用（21 支還沒接） |
| 4 | 路徑 | **19** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
| 5 | 等待＋持續＋續效特效＋多段＋路徑 | **11** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-dragon-quake（另有 16 份較不貼合） |
| 6 | 持續＋路徑 | **10** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-leap-strike・tpl-mark-stacks（另有 2 份較不貼合） |
| 7 | 續效特效＋路徑 | **9** | 2 | tpl-single-strike | 沿用（7 支還沒接） |
| 8 | 持續＋續效特效＋路徑 | **8** | 1 | tpl-buff-self | 沿用（7 支還沒接） |
| 9 | 續效特效 | **8** | 2 | tpl-single-strike | 沿用（6 支還沒接） |
| 10 | 持續＋多段 | **6** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-proxy-fanout |
| 11 | 等待＋路徑 | **5** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-teleport（另有 1 份較不貼合） |
| 12 | 等待＋持續＋續效特效＋路徑 | **4** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-line-blast・tpl-locust-strike・tpl-locust-travel（另有 6 份較不貼合） |
| 13 | 等待＋續效特效 | **4** | 1 | tpl-ground-nova | 沿用（3 支還沒接） |
| 14 | 等待＋迴圈＋續效特效 | **4** | 0 | — | 併進既有模板的參數 |
| 15 | 續效特效＋多段＋路徑 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
| 16 | 迴圈＋持續＋多段 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-random-barrage（另有 1 份較不貼合） |
| 17 | 多段＋路徑 | **2** | 2 | tpl-line-sweep・tpl-orbit-array | ✅ 全部接上了 |
| 18 | 持續＋續效特效＋多段＋路徑 | **2** | 1 | tpl-line-sweep | 沿用（1 支還沒接） |
| 19 | 等待＋多段＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-teleport（另有 1 份較不貼合） |
| 20 | 等待＋持續＋續效特效 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-proxy-fanout |
| 21 | 等待＋持續＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-leap-strike・tpl-mark-stacks（另有 3 份較不貼合） |
| 22 | 等待＋續效特效＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-line-blast・tpl-locust-strike・tpl-locust-travel（另有 3 份較不貼合） |
| 23 | 等待＋迴圈 | **2** | 0 | — | 併進既有模板的參數 |
| 24 | 等待＋迴圈＋持續＋續效特效＋多段 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-combo-finisher・tpl-periodic-field（另有 2 份較不貼合） |
| 25 | 等待＋迴圈＋持續＋續效特效＋多段＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-dragon-quake（另有 20 份較不貼合） |
| 26 | 迴圈＋持續＋多段＋路徑 | **2** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-lock-combo（另有 5 份較不貼合） |
| 27 | 等待＋持續＋多段＋路徑 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-leap-strike・tpl-mark-stacks（另有 3 份較不貼合） |
| 28 | 等待＋續效特效＋多段＋路徑 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-beam-roll・tpl-dragon-serpent・tpl-locust-line（另有 9 份較不貼合） |
| 29 | 等待＋迴圈＋多段 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-random-barrage |
| 30 | 等待＋迴圈＋多段＋路徑 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-lock-combo（另有 4 份較不貼合） |
| 31 | 等待＋迴圈＋持續＋多段 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-random-barrage（另有 1 份較不貼合） |
| 32 | 等待＋迴圈＋持續＋多段＋路徑 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-lock-combo（另有 7 份較不貼合） |
| 33 | 迴圈 | **1** | 0 | — | 併進既有模板的參數 |
| 34 | 迴圈＋多段 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-random-barrage |
| 35 | 迴圈＋續效特效＋多段＋路徑 | **1** | 1 | tpl-traveling-wave | ✅ 全部接上了 |
...
| 續效特效 | 11 | 87 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
| 多段 | 9 | 43 | **6** | 一次施放拆成多下（連段／連鎖） |
| 路徑 | 42 | 88 | **17** | 效果沿著空間移動 |

⇒ ⭐ **最擋人的一條軸是「迴圈」，43 支技能的說明宣稱它而 JSON 裡沒有。**

⚠️ **前搖（`castTimeSec`）不在上表**：349/421 支有它，29 個相異值連續分布在 0.067–3 秒，中位數 0.667 秒。
...
| `godie-e00r.r` | 59-04 野戰型陽電子砲 | — | 續效特效＋多段＋路徑 |
...
|---|---|---|---:|---:|---|---|
| `tpl-single-strike` | 單體斬擊 | enabled | 3 | 32 | — | （無時序形狀）×27・續效特效×2・續效特效＋路徑×2・持續＋續效特效×1 |
| `tpl-buff-self` | 變身強化-數值面 | enabled | 3 | 30 | 持續 | 持續×28・持續＋續效特效×1・持續＋續效特效＋路徑×1 |
| `tpl-instant-blast` | 瞬發點爆 | enabled | 4 | 13 | — | （無時序形狀）×13 |
| `tpl-proxy-cast` | 代理錨點施法 | enabled | 8 | 8 | 持續＋多段 | 持續×7・持續＋續效特效×1 |
| `tpl-line-sweep` | 直線分段掃擊 | enabled | 6 | 2 | 多段＋路徑 | 持續＋續效特效＋多段＋路徑×1・多段＋路徑×1 |
