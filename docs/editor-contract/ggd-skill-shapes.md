...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **166** | 33 | tpl-buff-self・tpl-proxy-cast | 沿用（133 支還沒接） |
...
| 7 | 持續＋續效特效＋路徑 | **9** | 2 | tpl-buff-self | 沿用（7 支還沒接） |
| 8 | 續效特效＋路徑 | **9** | 2 | tpl-single-strike | 沿用（7 支還沒接） |
| 9 | 續效特效 | **8** | 2 | tpl-single-strike | 沿用（6 支還沒接） |
| 10 | 持續＋多段 | **6** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-proxy-fanout |
| 11 | 等待＋持續＋續效特效＋路徑 | **5** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-line-blast・tpl-locust-strike・tpl-locust-travel（另有 6 份較不貼合） |
| 12 | 等待＋路徑 | **5** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-charge-push・tpl-teleport（另有 1 份較不貼合） |
| 13 | 等待＋續效特效 | **4** | 1 | tpl-ground-nova | 沿用（3 支還沒接） |
| 14 | 等待＋迴圈＋續效特效 | **4** | 0 | — | 併進既有模板的參數 |
| 15 | 持續＋續效特效＋多段＋路徑 | **3** | 2 | tpl-line-sweep・tpl-proxy-cast | 沿用（1 支還沒接） |
| 16 | 續效特效＋多段＋路徑 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-blink-strike |
| 17 | 迴圈＋持續＋多段 | **3** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-random-barrage（另有 1 份較不貼合） |
| 18 | 多段＋路徑 | **2** | 2 | tpl-line-sweep・tpl-orbit-array | ✅ 全部接上了 |
...
| 27 | 等待＋續效特效＋多段＋路徑 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-beam-roll・tpl-dragon-serpent・tpl-locust-line（另有 9 份較不貼合） |
| 28 | 等待＋續效特效＋路徑 | **1** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-line-blast・tpl-locust-strike・tpl-locust-travel（另有 3 份較不貼合） |
...
| 持續 | 144 | 250 | **39** | 有期間的狀態，到期自己收掉 |
| 續效特效 | 11 | 89 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
| 多段 | 9 | 44 | **6** | 一次施放拆成多下（連段／連鎖） |
| 路徑 | 42 | 90 | **17** | 效果沿著空間移動 |

⇒ ⭐ **最擋人的一條軸是「迴圈」，43 支技能的說明宣稱它而 JSON 裡沒有。**

⚠️ **前搖（`castTimeSec`）不在上表**：350/421 支有它，29 個相異值連續分布在 0.067–3 秒，中位數 0.767 秒。
...
| `tpl-buff-self` | 變身強化-數值面 | enabled | 3 | 30 | 持續 | 持續×27・持續＋續效特效＋路徑×2・持續＋續效特效×1 |
