...
|---:|---|---:|---:|---|---|
| 1 | 持續 | **154** | 30 | tpl-buff-self・tpl-proxy-cast | 沿用（124 支還沒接） |
...
| 8 | 持續＋路徑 | **10** | 0 | — | ⚠️ 模板已在、**0 支使用**：tpl-leap-strike・tpl-mark-stacks・tpl-pull-throw（另有 1 份較不貼合） |
...
⚠️ **前搖（`castTimeSec`）不在上表**：344/421 支有它，26 個相異值連續分布在 0.067–2.267 秒，中位數 0.833 秒。
它是**每一支技能都有的施法動作長度**（多半是 w3x 匯進來的），⛔ 不是作者寫下的機制 ——
算進「等待」軸的話這條軸會命中 421 支裡的 353 支，於是它分不出任何一群。
