...
| 2 | （無時序形狀） | **85** | 29 | tpl-instant-blast・tpl-single-strike | ⛔ 逐支確認是**真的沒有**還是**沒實作**（見第 2 節差集） |
...
| 10 | 持續＋多段 | **8** | 2 | tpl-summon-agent | 沿用（6 支還沒接） |
...
| 持續 | 145 | 273 | **22** | 有期間的狀態，到期自己收掉 |
| 續效特效 | 10 | 100 | **0** | 特效自己的壽命／掛載／分層（⚠️ 宣稱側很弱，差集不計） |
| 多段 | 9 | 56 | **6** | 一次施放拆成多下（連段／連鎖） |
| 路徑 | 42 | 101 | **17** | 效果沿著空間移動 |

⇒ ⭐ **最擋人的一條軸是「持續」，22 支技能的說明宣稱它而 JSON 裡沒有。**
...
<details><summary>「持續」缺口逐支（22 支）</summary>
...
| `tpl-summon-agent` | 召喚代理 | enabled | 12 | 2 | 持續＋多段 | 持續＋多段×2 |
...
|---|---:|---|---|---|
| 持續 | **22** | `tpl-buff-self`・`tpl-periodic-field` | draft・enabled | ⭐ **模板已在、而且真的跑出這條軸** ⇒ 把這 22 支接上去（改內容，⛔ 不必動引擎） |
