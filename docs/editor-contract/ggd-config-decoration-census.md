...
|---|---:|
| 出貨 config 文件 | 101 |
| **旋鈕**（Zod 靜態鍵的葉子，⛔ 不含 record 的鍵與陣列索引） | **1951** |
...
| `authoring-rules.json` | `editor.autosaveIntervalMs` | `500` | `autosaveIntervalMs` 在出貨消費端原始碼裡**被讀** 0 次 （讀＝成員存取／解構／字串鍵；⛔ 宣告、預設值與散文不算。⛔ 後台與測試不在母體裡）⇒ 沒有任何東西讀得到它。 |
