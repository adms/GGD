...
|---|---:|
| 出貨 config 文件 | 103 |
| **旋鈕**（Zod 靜態鍵的葉子，⛔ 不含 record 的鍵與陣列索引） | **1987** |
| 消費端原始碼（⛔ 已排除後台欄位表與測試） | 2595 |
...
| `model-lod.json` | `championChannelLimit` | `500` | `championChannelLimit` 在出貨消費端原始碼裡**被讀** 0 次 （讀＝成員存取／解構／字串鍵；⛔ 宣告、預設值與散文不算。⛔ 後台與測試不在母體裡）⇒ 沒有任何東西讀得到它。 |
