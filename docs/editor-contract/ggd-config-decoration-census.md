...
|---|---:|
| 出貨 config 文件 | 104 |
| **旋鈕**（Zod 靜態鍵的葉子，⛔ 不含 record 的鍵與陣列索引） | **2024** |
| 消費端原始碼（⛔ 已排除後台欄位表與測試） | 3065 |
...
| **B** | 引擎做得到而調不到 | **11** |
| C | 正常 | 1648 |
| ⚠️ 量不到 | 鍵名不是識別字（級距標籤／列舉鍵）—— ⭐ 「零讀端」這把尺對它們**結構上是瞎的** | 335 |
...
| `model-lod.json` | `championChannelWarn` | `300` | `championChannelWarn` 在出貨消費端原始碼裡**被讀** 0 次 （讀＝成員存取／解構／字串鍵；⛔ 宣告、預設值與散文不算。⛔ 後台與測試不在母體裡）⇒ 沒有任何東西讀得到它。 |
...
### 引擎做得到而 schema 不准（0） · 11 格（B 類）

| 檔 | 路徑 | 出貨值 | 量到的 |
|---|---|---|---|
| `arena-rules.json` | `finalRound` | `"zod min 2"` | Zod 的下界是 `min(2)`，⛔ 而引擎把 0 當成一個活的分支：`if (!Number.isFinite(round) || !Number.isFinite(finalRound) || finalRound <= 0) return false;` ⇒ 一個**做得到卻調不到**的狀態。 |
...
| `arena-rules.json` | `round11.maxAliveZombies` | `"zod min 1"` | Zod 的下界是 `min(1)`，⛔ 而引擎把 0 當成一個活的分支：`maxAlivePerZone: maxAliveZombies > 0 ? maxAliveZombies : 0,` ⇒ 一個**做得到卻調不到**的狀態。 |
