...
- **editorForm**：⭐ **量值** —— Codex 的收據 `coordination/claim.editor-form-receipts-spawn-obstacle.json`（跑他們出貨的 schema walker ＋ ConditionEditor 詞彙 ＋ type-catalog 選用閘，每一列帶元件路徑）。⛔ 已經不是代理值。目前 180 顆有收據；收據裡沒有的才退回代理值。
