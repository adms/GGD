...
**指紋 `2afadee4`** —— 編輯器用它 pin base。指紋只在引擎事實真的改變時才會變。
...
| 寫在哪 | 欄位 |
|---|---|
| `ability@1` | `allowApproach` · `augment` · `castTimeSec` · `castTimeTier` · `castType` · `cooldown` · `cooldownShape` · `cooldownTier` · `description` · `effects` · `hitFeel` · `icon` · `id` · `innateActivePassive` · `innateKind` · `interruptOn` · `interruptible` · `manaCost` · `manaCostTier` · `marks` · `maxRank` · `name` · `passive` · `persistentVfx` · `provenance` · `radius` · `radiusTier` · `range` · `rangeTier` · `rangeUnlimited` · `recoveryRoots` · `recoverySec` · `requiredSummonSlot` · `requiredTargetStatus` · `rootWhileCasting` · `sfxKey` · `slot` · `statusCost` · `targetsEnemies` · `template` · `toggle` · `vfxKey` · `vfxLayers` |
