...
**指紋 `6f6d1de1`** —— 編輯器用它 pin base。指紋只在引擎事實真的改變時才會變。
...
`chance` · `distance` · `equipment` · `kind` · `learned` · `recentCast` · `stat` · `status`

**條件葉參數**（17）

`abilityId` · `appliedBy` · `is` · `itemId` · `kind` · `minStacks` · `mode` · `op` · `other` · `p` · `slot` · `stat` · `statusId` · `subject` · `tag` · `value` · `withinSec`
