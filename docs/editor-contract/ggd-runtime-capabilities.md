...
**指紋 `0a528213`** —— 編輯器用它 pin base。指紋只在引擎事實真的改變時才會變。
...
| 種類的哪一半 | 檔案 | 覆蓋 |
|---|---|---|
| 欄位與上下界 | `packages/shared/src/content/schema/effects/<種類>.ts` | 48 / 48 個種類有自己的檔 |
| 型別 | `packages/shared/src/sim/effects/variants/<種類>.ts` | 48 / 48 個種類有自己的檔 |
