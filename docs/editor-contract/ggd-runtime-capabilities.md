...
**指紋 `e37f411f`** —— 編輯器用它 pin base。指紋只在引擎事實真的改變時才會變。
...
| 種類的哪一半 | 檔案 | 覆蓋 |
|---|---|---|
| 欄位與上下界 | `packages/shared/src/content/schema/effects/<種類>.ts` | 50 / 50 個種類有自己的檔 |
| 型別 | `packages/shared/src/sim/effects/variants/<種類>.ts` | 50 / 50 個種類有自己的檔 |
...
| 寫在哪 | 欄位 |
|---|---|
| `ability@1` | `augment` · `castTimeSec` · `castTimeTier` · `castType` · `cooldown` · `cooldownShape` · `cooldownTier` · `description` · `effects` · `hitFeel` · `icon` · `id` · `innateActivePassive` · `innateKind` · `interruptOn` · `manaCost` · `manaCostTier` · `marks` · `maxRank` · `name` · `passive` · `persistentVfx` · `provenance` · `radius` · `radiusTier` · `range` · `rangeTier` · `rangeUnlimited` · `recoveryRoots` · `recoverySec` · `requiredSummonSlot` · `rootWhileCasting` · `sfxKey` · `slot` · `statusCost` · `targetsEnemies` · `template` · `toggle` · `vfxKey` · `vfxLayers` |
...
`abilitySlot` · `chance` · `chanceFrom` · `condition` · `consumeOn` · `critSource` · `damageCrit` · `damageSource` · `damageType` · `effects` · `evadeChannel` · `evadeSource` · `internalCooldown` · `internalCooldownScope` · `key` · `maxTriggers` · `observedEvent` · `on` · `onConsumed` · `oncePerCast` · `perTarget` · `reflectedDamageSource` · `reflectedDamageType` · `target` · `victim`
...
`augment` · `castTimeSec` · `castTimeTier` · `castType` · `cooldown` · `cooldownShape` · `cooldownTier` · `description` · `effects` · `hitFeel` · `icon` · `id` · `innateActivePassive` · `innateKind` · `interruptOn` · `manaCost` · `manaCostTier` · `marks` · `maxRank` · `name` · `passive` · `persistentVfx` · `provenance` · `radius` · `radiusTier` · `range` · `rangeTier` · `rangeUnlimited` · `recoveryRoots` · `recoverySec` · `requiredSummonSlot` · `rootWhileCasting` · `sfxKey` · `slot` · `statusCost` · `targetsEnemies` · `template` · `toggle` · `vfxKey` · `vfxLayers`
