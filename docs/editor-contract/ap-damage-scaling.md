...
語料：`content/abilities/*.json` **420 份**，掃到 **237** 個技能傷害 `Scaling` 節點。

| | 數量 | 佔傷害節點 |
|---|---:|---:|
| 帶法強係數（`ratios:{stat:"ap"}`） | **138** | 58.2% |
| 其中：拿掉係數之後**完全沒有屬性相依**（＝變成純固定值） | **138** | 58.2% |

法強係數的分佈：最小 **0.1** · 中位 **0.6** · 最大 **7**。

⇒ 出貨 `apRatioMode: "stack"`。理由是上面那兩列：
切成 `"replace"` 會讓那 138 個節點變成**與任何屬性都無關的常數**，
