...
⛔⛔ **`加成率` 那一格刻意不在這份契約裡**（owner 2026-08-23 · 2026-08-31，兩次）：
> 「編輯器只編輯**原始資料**（五級距），**根本不需要知道**⋯避免雙重編輯」
> 「契約中**不應該考慮**⋯（那一格的值）」

⚠️ ⭐ owner 2026-08-31 第三次指名同一件事，逐字：
> 「**系統參數⋯不應該提到 那是我人工旋鈕**」

⇒ ⭐ **所以這份文件連引用他的話時都不印那個數字** —— ⛔ 印出來它就有了第二個住處
（第〇·四守則），⭐ 而第二個住處必然過期。唯一的住處是
`content/config/ap-damage-scaling.json` 的 `rate`，⭐ 而它只有 owner 調
（閘：`content/config/owner-knobs.json` ＋ `ownerKnobs.test.ts`）。

⇒ ⭐ 編輯器要編的是**技能自己的資料**（級距、係數、目標）。
這一層是**全域的、owner 專屬的**，它在**每一發傷害之後**才乘上去 ——
⛔ 編輯器既不需要顯示它，也不可以把它算進任何預覽。
⚠️ ⭐ 把它寫進契約會造成**雙重編輯**：兩邊各有一份，而它們一定會漂開。
...
| 法強 | 0 | 25 | 50 | 100 | 150 | 200 | 250 | 300 | 400 | 500 | 611 | 1931 | 3503 | 31874 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| **乘數** | ×1 | ×1.125 | ×1.25 | ×1.5 | ×1.75 | ×2 | ×2.25 | ×2.5 | ×3 | ×3.4886 | ×4.0085 | ×9.3088 | ×14.6855 | ×41 |
...
語料：`content/abilities/*.json` **1129 份**，掃到 **819** 個技能傷害 `Scaling` 節點。

| | 數量 | 佔傷害節點 |
|---|---:|---:|
| 帶法強係數（`ratios:{stat:"ap"}`） | **145** | 17.7% |
| 其中：拿掉係數之後**完全沒有屬性相依**（＝變成純固定值） | **145** | 17.7% |

法強係數的分佈：最小 **0.1** · 中位 **0.6867** · 最大 **7**。
