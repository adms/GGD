# 歷史封存 —— 2026-07-26 深夜～07-27 凌晨

> 從 `docs/_execution-batches.md` 移出的**已完成／已定案**內容：v0.6.x 的部署紀錄、
> 以及那一夜四份交付被對抗式驗證推翻的完整驗屍報告（含所有量測數字）。
>
> **live 的任務分配在 `docs/_execution-batches.md`；重啟 session 的前情提要在
> `docs/_session-handover.md`。這一份只在需要回頭查「當時到底量到什麼」時才讀。**
...
## 📌 v0.9.9 本機驗收（2026-07-28，main `651007c4`）

> 部署協定第 3 步。**在跑起來的 localhost 上實測**,不是讀程式碼。
> lane:`game-server-272`(:2610)+`client-272`(:39572),build stamp `651007c4`。

### 驗過的（有數字）

| 項 | 怎麼驗的 | 結果 |
|---|---|---|
| **基礎加成不參與倍率** | 打一場,Saber lv3。商店屬性面板「生命 3034」、HUD 血條「3034/3034」 | ✅ `(卡891.2 + flat20)×3 + 300 = 3033.6` → 顯示 3034。v0.9.8 的讀法會是 **3633.6** |
| **面板 == 權威** | 同上兩個數字來自不同路徑(面板走 wire+content,血條走 sim) | ✅ 完全一致 |
| **客戶端讀 overlay** | network 面板看到 `GET /api/v1/content-overlay/bundle` | ✅ 有打,平台沒開 → 500 → **靜默 fail-safe**,console 零錯誤,內容照常載入 |
| **settings v3→v4 遷移** | localStorage `ggd.settings` | ✅ `version: 4`,桌機 `fpsCap: 60` |
| **協定欄位真的編碼得出去** | 新增 `matchRoomBaseBonus.test.ts`,真的走一次 Encoder/Decoder | ✅ 3 條,3 個突變全紅 |

### 只有單元測試、**沒有**在真機上驗的

- **手機 30 fps**:`__ggdForceTouch` 是 boot 前的 seam,活頁改不了。桌機路徑實測過,
  手機路徑靠 `fpsPlatform.test.ts` 6 條 + 2 個突變。**下次拿手機開一次確認**。
- **面向瞄準優先**:需要實體手把推右類比。sim 層 6 條測試 + 2 個突變,
  但「手感對不對」只有 owner 打得出來。

### 帶著上線的已知事項

1. ⚠️ **中位血量 2592 → 1992**(比 v0.9.8 之前的 2256 還低)。這是「300 真的只加 300」
   的必然結果,不是 bug。要回到 v0.9.8 的耐打度:後台 **基礎加成 → 生命上限 = 900**。
   一個欄位、不用重新部署、下一場生效。**等 owner 打過一場再決定。**
2. FPS 數字預設看不到(`showPerfOverlay: false`)。這是既有問題,不是這次造成的 ——
   `改進延遲` 計畫 1-4 已記錄:`Show ping` 開關在預設狀態下完全沒有作用。
