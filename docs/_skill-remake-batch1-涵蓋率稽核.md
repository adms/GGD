...
# §1 現況 —— **2026-08-13 01:57**（v0.14.7）

> ⚠️ **上面那份 71.5% 是 08-12 的基準線，⛔ 不是現況。** 這一節是它之後發生的事。
...
**3 個真的 engine-gap**（⛔ 今天的引擎沒有詞彙，不可以用別的格子冒充）：

| gap | 誰卡住 | 為什麼 |
|---|---|---|
| 落點錨定（以「帶某狀態的敵人」為圓心） | 44-03 火車輾過 | `damageArea` 的圓心來自施法點。⛔ 用 `victimCondition` 冒充會把規格點名要吃傷害的那群人**全部濾掉**，範圍技降成單體 |
| 條件式傷害倍率（「若被定身則傷害加倍」） | 70-04 千年練成 | 只能寫成兩顆並排 ⇒ 兩個跳字、on-hit 觸發兩次 |
| 反彈鏡射原型別 | 60-04 | 反彈封包一律以 magic 送出 ⇒ 反彈物理傷害走魔抗而不是護甲 |

**要 owner 裁的**：15 條吟唱秒數（owner 對 owner 的衝突）· 60-04 的反彈比 `1.0`
與 79-02 的施法距離 `2.0` 是**推斷值**（規格沒給）· 89-002「1/6 自己死亡」算不算擊殺。
