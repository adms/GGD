...
> # 🎯 這份文件在追什麼
>
> ## **90 個技能要 100% 實作，並且能用 JSON 就達成技能機制與效果**
>
> —— owner 2026-08-13 逐字。這是**驗收標準**，⛔ 不是願景。
>
> 它是**兩個**要求，缺一不可：
>
> | | 要求 | 怎麼驗 |
> |---|---|---|
> | **① 100% 實作** | 規格的**每一句**都要在遊戲裡真的發生 | 逐句涵蓋率（本文件 §1） |
> | **② 能用 JSON 達成** | ⛔ 不可以「為某支技能寫一段程式」—— 引擎做**機制**，JSON 做**技能** | `tag_gate` 豁免數 · engine-gap 條數 |
>
> ⭐ **② 目前成立**：197 條缺口裡只有 **2 條（1.0%）**是「引擎做不到」，
> 其餘 99% 是 JSON 沒填 / 填錯 kind / 漏逐階陣列。
> 涵蓋率最低的 5 支逐句盤點的結論是 **需要的新引擎機制 = 0**。
>
> ⚠️ **① 還沒到 100%** —— 進度看 §1。
...
> | 段 | 時間（到分鐘） | 版本 | 是什麼 |
> |---|---|---|---|
> | **§1 現況** | **2026-08-13 01:57** | **v0.14.7** | ⭐ **看這一段** |
> | §2 基準線 | **2026-08-12**（B1 開工前） | v0.14.3 | ⚠️ **修之前**的快照。⛔ 不是現況 |
>
> ⭐ 判斷過期：**§1 的時間戳比你上次改動舊，就是還沒重跑。**
> ⚠️ 每次改這份文件都要更新 §1 的時間戳到**分鐘**（owner 2026-08-13：
> 「我修改太頻繁看不出來什麼時候」）。

---

# §1 現況 —— **2026-08-13 12:08**（v0.14.7）

> ⚠️ **下面 §2 那份 71.5% 是 08-12 的基準線，⛔ 不是現況。** 這一節是它之後發生的事。

## ⭐ 12:08 這一輪：**24 支技能 · 33 條機制**落地（量到的，⛔ 不是估的）

> 量法：逐支開**出貨的 JSON**，找那條機制的關鍵鍵是否存在。33 中 33，**0 條沒中**。
> ⚠️ 這是「機制數」不是「子句數」——⛔ 不要跟下面 88.4% 那個分母混著看。

### 三支是「上架起就打錯人」，⛔ 不是「少填一格」

| 技能 | 出貨到今天實際會發生什麼 |
|---|---|
| **89-02 憤怒的菊花** | [癱瘓][詛咒] 掛在 hook 頂層 + `target:"self"` ⇒ **熊貓自己暈 1 秒、自己 5 秒 50% miss，周圍敵人什麼都沒有** |
| **60-03 三角神力** | hook **完全無條件** ⇒「每三下」變成**每一下**，輸出是規格的 **3 倍** |
| **15-03 獄炎煉我** | 兩條追打掛在**常駐 passive** ⇒ **不按 E 也永遠生效**，按了只換來移速減半 —— 一支**按了會變弱**的技能 |

### 另外六支是「機制整條不會發動」

`15-02` 雷電追打的條件是 `status rage`，而涅吉自己不掛 ⇒ **一次都不會發動** ·
`59-02`「**轉為**真傷」其實是「額外追加 50 真傷」（本體那刀照樣被護甲吃）·
`79-01`/`92-02`【破魔】是**純標記**（魔抗一點都沒掉）·
`92-01`「每秒回復」寫成**負值 dot**，而 `dotTick` 是 `if (amountPerTick > 0)` ⇒ **一滴都不回** ·
`89-002`「1/6 **自己**死亡」根本不存在（devour 沒有 applyTo，兩顆分支都打敵人）·
`60-04` 擊退只打得到**剛才那一個攻擊者**（規格是半徑 6）

### 三個引擎機制（都是「產出得了、沒有消費端」）

| 機制 | 卡住誰 | 守衛 |
|---|---|---|
| `delayed` 繼承觸發脈絡（新檔 `deferredTrigger.ts`） | 20-002 七刀反彈 | `augmentAndDeferred.test.ts` |
| 子鏈走訪 **3 → 10 條**（新檔 `effectChildChains.ts`，**型別窮盡**） | 70-002 | ⭐ 突變同時讓 **typecheck 與 vitest 都紅** |
| 強化改得到**逐階** duration（三個住處） | 77-002 | 同上 |

### ⭐ owner 2026-08-13 的兩則裁決已落地

- 「**照我的 0.06~4.00 秒**」→ 14 支寫了吟唱的技能 **14/14 全對上**；倍率／上下限進後台
- 「**70-00 follow new rule not w3a. healing friend and self**」→ 芬多精也治療自己 +
  力量+10；被取代的 w3a 考證**另存**在 `_w3x-fidelity-superseded.md`

### ⚠️ 這一輪的三個副作用（要 owner 覆核）

1. **89-002 輪盤現在吃得掉殭屍**（`devour.victim:"champion"` 那一格消失了）
2. **60-002 護盾低等級變小**（`flat 1500` → `maxHealth × 1.0`，第 1 回合約 750）
3. **20-04 + 20-002 明顯跳一階**（一次成功反彈從 1 發變 **7 發**封包）

---

## ⭐ 今天的數字（B1–B4 + 最慘 5 支 + 吟唱批修完之後**重量**的）

> # 子句涵蓋率 = **440 / 498 = 88.4%**

| 口徑 | 08-12 | 今天 | 差 |
|---|---|---|---|
| **複驗存活**（本表頭條） | ~523–528/666 = **78.5–79.3%** | **440/498 = 88.4%** | **+9.1 ~ +9.9 pt** |
| 全部缺口主張（未複驗） | 476/666 = 71.5% | 439/498 = 88.2% | +16.7 pt |

⚠️ **真實進步大約是 +9 到 +10 個百分點，不是 +17。** 兩份數字的**分母口徑不同**：
08-12 的頭條把 197 條主張全算成缺口，但它自己也寫了只有 143 條通過複驗 ——
那 54 條被推翻的主張被算進了分母。⭐ 這一列是**誠實的**那一個。

**計算基礎**：六片獨立初量 → 六位複驗者逐條開檔對抗複驗，數字是**複驗後**的。
84 支技能（90 扣掉 godie-ewar 未量的 4 支與 2 支重複計數）拆成 498 個可驗證子句。

| 第二個角度 | 08-12 | 今天 |
|---|---|---|
| **完全沒有缺口的技能** | 6 / 84 = 7.1% | **45 / 84 = 53.6%** |
| 至少缺一句 | 78 / 84 = 92.9% | 39 / 84 = 46.4% |

## ⭐ 還缺的 58 句是什麼形狀

**🔧 填值 57 條 · ⚙️ 引擎 1 條。** ⇒ ② 那個要求（能用 JSON 達成）**仍然成立**：
剩下的幾乎全是「JSON 沒填 / 填錯 statusId / 漏逐階陣列 / hook 掛錯 target」。

唯一的引擎缺口是 **70-002 樹海降臨**：`augment` 的 `damageCoeffAp` 找不到住在
`randomArea.effects[]` 裡的傷害（`abilityAugment.ts:259` 的子鏈清單只有
`onHit`/`onLand`/`branches`）—— 補一個子鏈名字就好，⛔ 不是新機制。

### 涵蓋率最低的 10 支（今天）

| # | 技能 | 涵蓋 | 還缺什麼 | 類型 |
|---:|---|---|---|---|
| 1 | **52-03 無銘斧劍** | 50.0% | 「附加[麻痺]0.6秒」四階全掛 `slow40`（減速），而 `numbness.json`（名稱「麻痺」）的描述逐字點名這一支 | 🔧 改 `statusId` |
| 2 | **70-002 樹海降臨** | 50.0% | 「[千年練成] 追加 500% AP」augment 寫了但傷害住在 `randomArea.effects[]` | ⚙️ **引擎** |
| 3 | **15-04 雷天大壯．貳式** | 53.8% | ①觸發條件「施放技能後」沒有武裝來源 ②「下一次」缺 `maxTriggers` ③雷神一擊逐階 ④缺 12 秒形態閘 | 🔧 ×6 |
| 4 | **89-02 憤怒的菊花** | 62.5% | ①3%[反彈]只是 `chance 0.03` 沒有 `incomingPct` ②[癱瘓][詛咒] 掛成 hook 頂層兄弟 + `target:"self"` ⇒ **熊貓自暈自 miss，敵人什麼都沒有** | 🔧 ×3 |
| 5 | **15-03 獄炎煉我** | 66.7% | 兩條追打**沒有 12 秒形態閘** ⇒ 不按 E 也永遠生效，按了只換來移速減半的純負面 | 🔧 ×4 |
| 6 | **15-02 疾風迅雷** | 66.7% | hook 條件是 `status rage`，而這支的 `applyBuff` **沒填 `statusId`** ⇒ **這條 hook 一次都不會發動** | 🔧 ×3 |
| 7 | **70-00 紮根** | 66.7% | 「變得無法移動」—— 替身 `godie-e010` 的 `ms` 是 5.5，比本體 5.3 還**快** | 🔧 ×2 |
| 8 | **89-03 憤怒的胸毛** | 66.7% | 第二層分支缺；「[自爆]損失現存 50% 生命」全檔沒有自傷節點 | 🔧 ×2 |
| 9 | **59-02 高週波短刀** | 66.7% | 「**轉為**真實傷害」寫成 `damage{true, flat:50}` ＝**額外追加** 50 真傷，本體那一刀照樣被護甲吃掉 | 🔧 `damageTypeOverride` |
| 10 | **60-02 鎖鏈槍** | 66.7% | 「勾住一個單位」是 `ground` + `leap{toPoint}`，玩家點的是地點不是單位 | 🔧 改 castType |

⚠️ **60-03 三角神力．勇氣**（並列 66.7%，被第 10 名切掉）值得單獨點名：
hook **完全無條件** ⇒「每三下」變成**每一下**，輸出是規格的 **3 倍**。

## ⭐ 一句話（機制面）

**197 條缺口裡只有 2 條（1.0%）是「引擎做不到」。** 其餘 195 條全部是產生器沒填、
填錯 kind、或漏掉逐階陣列 —— 也就是說 **JSON 模板系統本身沒有被證明不夠用**。

⭐ 而**涵蓋率最低的 5 支**（77-002 · 60-04 · 79-02 · 52-00 · 15-002，合計 23 條缺口）
逐句盤點的結論是：**需要的新引擎機制 0 個，全部用今天的 JSON 寫得出來。**

## 已落地的四批

| 批 | commit | 做了什麼 | 形狀 |
|---|---|---|---|
| **B1 機械批** | `ae712e02` | 兄弟酬載折進 `onHitTargets`（15 節點/10 支）· 刪 `flat=50` 注入 · `maxt` 預設拿掉 · 刪係數 clamp | ⚙️ **表格零列要動** |
| **B2 出口＋填值** | `9864bbe2` `f0ce1a30` | `res_pct=` · `buff()` 具名白名單（含五格授權欄位）· `while_form=` · 鍵序統一 ＋ **21 筆填值** | 🔌 出口 + 逐列填 |
| **B3+B4** | `c4cf1617` | **反彈**（`damage.incomingPct`，5 支）· 條件葉四個落點（9 支）· `augment` 出口 | 🔌 機制 |
| **最慘 5 支收尾** | 本次 | 見下表 | 📋 逐支 |

## 這一輪第一次真的出貨的引擎機制（採用率 0 → 有）

`onHitTargets` · `block` · `critStrike` · `attributes` · `flight` · `resourcePct` ·
`victimCondition` · `whileForm` · **`damage.incomingPct`（反彈）** ·
**`ability@1.augment`（全 repo 第一份）** · **`mark.lethal` + `perStackLost`** ·
`eventValueConversion.buff` · `modifyCooldown{hookInternalCooldown}`

## 最慘 5 支的收尾

| 技能 | 原本錯在哪 | 現在 |
|---|---|---|
| **77-002 御雷劍** | 「雷鳴劍機率上升至 50%」「ALAT 持續 30 秒」**整段沒有落點** | `augment` 兩個 target，共用「拿著御雷劍」的前提 |
| **60-04 完美盾反** | 反彈整段沒寫；回復只有一格 0.08（rank 2/3 拿不到）；擊退 `target:"self"` **推自己** | 反彈 + 回復逐階 8/16/24% + 擊退改打敵人 |
| **79-02 月牙斬擊** | `castType:"self"` ⇒ 頂層 damage **打自己**，兩條 hook 全部閘死 | 改 `targeted`；卍解追加 200% AP 用 `whileForm:"alternate"` |
| **52-00 十二道試煉** | 寫成「HP ≤ 5% 觸發 hook」—— ⛔ **那不是免死**：一發超過 5% 的傷害直接把人打死，試煉一層都不消耗 | 真正的 `mark.lethal`（consume/surviveHpPct/selfEffects/aoeEffects）＋ `perStackLost` |
| **15-002 太陰道** | 只寫了轉魔力那一半 | 反彈 + `eventValueConversion.buff` 把傷害短暫加成至 AP |

## ⚠️ 還沒關的

**owner 點名的 12 筆漏寫：關掉 9 筆，剩 3 筆**（其中 2 筆不是漏寫，是標籤誤標）。

**engine-gap：3 條 → 1 條**（兩條在 08-13 被關掉，⛔ 都不需要新機制）：

| gap | 誰卡住 | 狀態 |
|---|---|---|
| ~~落點錨定~~ | 44-03 火車輾過 | ✅ **關了**（owner「請修正為範圍技」）—— `ground` → `targeted`，`damageArea` 圓心自動落在 `ctx.targets[0]` ＝被詛咒的那個敵人。⛔ 一行 JSON，不是新機制 |
| ~~反彈鏡射原型別~~ | 60-04 | ✅ **不是缺陷**（owner 裁決：「反彈預設都是造成 AP 傷害」）＝ 設計 |
| 條件式傷害倍率（「若被定身則傷害加倍」） | 70-04 千年練成 | ⚙️ **還開著** —— 只能寫成兩顆並排 ⇒ 兩個跳字、on-hit 觸發兩次 |
| `augment` 找不到 `randomArea` 子鏈 | 70-002 | ⚙️ **新的**（補一個子鏈名字，⛔ 不是新機制） |

**owner 已裁的**：
- ✅ **吟唱秒數**：「請你照我的 **0.06~4.00 秒**」+ 倍率/上下限後台可調 → 做完，14/14 全對上
- ✅ 60-04 反彈比 `1.0` / 79-02 施法距離 `2.0` 的推斷值 → **ok**（維持）
- ✅ 89-002「1/6 自己死亡」**算擊殺**

---

# §2 基準線 —— ⚠️ **2026-08-12**（v0.14.3）的快照，**修之前**

> ⛔ **這一段的每一個數字都是舊的。** 71.5%、197 條缺口、「涵蓋率最低的技能」那張表 ——
> 全部是 B1/B2/B3/B4 **開工之前**量的。留著是因為它是**對照組**：
> 沒有它就說不出「改善了多少」。
>
> ⚠️ 下面「完整缺口表」的每一行都還標成**開著** —— ⛔ 那是 08-12 的狀態，
> ⛔ 不代表今天還開著。今天的狀態看 §1。
