...
> 本次 contentVersion：`cv_705503220e96`（下限量測基準 `cv_ecff53279fad`，2026-07-25）。
...
- **完整跑遍全 48 英雄 × 6 槽 = 288 格，無抽樣**。本測試同時是棘輪：任一格退化會讓 `pnpm --filter @ggd/shared exec vitest run` 變紅，並具名指出是哪一格（下限：PASS ≥ 281、PASS+PASSIVE ≥ 288；已知失敗集合以精確比對）。
