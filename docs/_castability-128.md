...
> **名單來源**：apps/platform/internal/curation/starter.go（51）— 本機無 data/curation/whitelist.json（正常：該檔為 gitignore 的營運狀態）。
...
- **格數**：51 英雄 × 6 槽 = **306**
- **✅ PASS：298 / 306**（97.4%）　🟣 PASSIVE：8　❌ FAIL：0
- 把「正確的永久被動」算進可接受行為：**306 / 306**（100.0%）如預期運作，只有 **0** 格是真正的缺口。
- 英雄生成失敗：**0**（無）

## 近戰 vs 遠程（attackType 維度）

- 名單：**近戰 36**、**遠程 15**。
- **普攻形態**：遠程英雄中 **15/15** 的普攻確實射出投射物（`projectileSpawn`、事件 `ranged:true`）；近戰英雄中 **36/36** 的普攻是貼身直接傷害（無投射物、`ranged:false`）。這正是遠程與近戰在普攻上的行為差異，兩邊都被本次量到。
