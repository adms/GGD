...
> 這是**診斷**：把 49 位英雄每一格 Q/W/E/R/EX + 普攻在真的 SimWorld 裡按下去，量測有沒有真的產生效果（傷害／投射物／狀態／護盾／補血／補魔／位移／特效），不修任何技能。

> **名單來源**：apps/platform/internal/curation/starter.go（48）＋ data/curation/whitelist.json 額外啟用（1）。
...
- **格數**：49 英雄 × 6 槽 = **294**
- **✅ PASS：285 / 294**（96.9%）　🟣 PASSIVE：8　❌ FAIL：1
- 把「正確的永久被動」算進可接受行為：**293 / 294**（99.7%）如預期運作，只有 **1** 格是真正的缺口。
- 英雄生成失敗：**0**（無）
