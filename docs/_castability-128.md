...
> 同時是**棘輪**：測試釘住已量測的下限（PASS ≥ 280、PASS+PASSIVE ≥ 288）與具名的已知失敗集合（目前為空），任何一格退化都會讓測試變紅並指名是哪一格。
> 本次 contentVersion：`cv_1e8298588746`（下限量測基準 `cv_1e8298588746`，2026-07-25）。
...
- **格數**：48 英雄 × 6 槽 = **288**
- **✅ PASS：280 / 288**（97.2%）　🟣 PASSIVE：8　❌ FAIL：0
- 把「正確的永久被動」算進可接受行為：**288 / 288**（100.0%）如預期運作，**沒有任何一格是缺口**。
- 英雄生成失敗：**0**（無）
...
- 每次施放後步進 **35 tick**＝**內容中最長施法前搖 27 tick**（`0.90s`）＋ 8 tick 結算餘裕；視窗由內容推導，不是寫死的常數（舊版寫死 26 tick，剛好比 0.9s 前搖短 1 tick，把 `godie-u00n` 的 R 誤判成 no-op）。普攻給 **40 tick** 讓第一次揮擊落地。
- 狀態面（護盾／狀態／buff／投射物）取**視窗內的峰值**而非結束時的值，避免短效果在視窗結束前就過期而被誤判成 no-op。
- **完整跑遍全 48 英雄 × 6 槽 = 288 格，無抽樣**。本測試同時是棘輪：任一格退化會讓 `pnpm --filter @ggd/shared exec vitest run` 變紅，並具名指出是哪一格（下限：PASS ≥ 280、PASS+PASSIVE ≥ 288；已知失敗集合以精確比對）。
