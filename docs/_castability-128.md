...
> 這是**診斷**：把 51 位英雄每一格 Q/W/E/R/EX + 普攻在真的 SimWorld 裡按下去，量測有沒有真的產生效果（傷害／投射物／狀態／護盾／補血／補魔／位移／特效），不修任何技能。

> **名單來源**：apps/platform/internal/curation/starter.go（50）＋ data/curation/whitelist.json 額外啟用（1）。
...
> 本機額外加掃（僅營運白名單開放、不在首發名單）：`godie-zombiex`。
...
- **格數**：51 英雄 × 6 槽 = **306**
- **✅ PASS：297 / 306**（97.1%）　🟣 PASSIVE：8　❌ FAIL：1
- 把「正確的永久被動」算進可接受行為：**305 / 306**（99.7%）如預期運作，只有 **1** 格是真正的缺口。
- 英雄生成失敗：**0**（無）

## 近戰 vs 遠程（attackType 維度）

- 名單：**近戰 35**、**遠程 16**。
- **普攻形態**：遠程英雄中 **16/16** 的普攻確實射出投射物（`projectileSpawn`、事件 `ranged:true`）；近戰英雄中 **35/35** 的普攻是貼身直接傷害（無投射物、`ranged:false`）。這正是遠程與近戰在普攻上的行為差異，兩邊都被本次量到。
...
| --- | --- | --- | --- | --- | --- |
| 草帽小子 - 蒙其.D.魯夫 | `godie-u00n` | R | ground | 近 | cast accepted but produced no measurable effect (no-op) |
...
- **完整跑遍全 51 英雄 × 6 槽 = 306 格，無抽樣**。
- **會變紅的三道閘**（都只看版控名單那 50 人，營運額外開放的英雄不影響）：(1) 掃描必須跑完 50×6；(2) 50 位英雄全部要能生成；(3) 可用格數（✅+🟣）不得低於 **299**（棘輪下限）。個別內容 no-op 不會使測試變紅（no-op 本身就是要回報的發現，列在下方 FAIL 清單），但既有可用的格子被改壞會。
