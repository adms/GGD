...
> 本次 contentVersion：`cv_e8451ffd51fb`（下限量測基準 `cv_1e8298588746`，2026-07-25）。
...
- **格數**：48 英雄 × 6 槽 = **294**
- **✅ PASS：286 / 294**（97.3%）　🟣 PASSIVE：8　❌ FAIL：0
- 把「正確的永久被動」算進可接受行為：**294 / 294**（100.0%）如預期運作，**沒有任何一格是缺口**。
- 英雄生成失敗：**0**（無）

## 近戰 vs 遠程（attackType 維度）

- 名單：**近戰 35**、**遠程 14**。
- **普攻形態**：遠程英雄中 **14/14** 的普攻確實射出投射物（`projectileSpawn`、事件 `ranged:true`）；近戰英雄中 **35/35** 的普攻是貼身直接傷害（無投射物、`ranged:false`）。這正是遠程與近戰在普攻上的行為差異，兩邊都被本次量到。
