...
- **格數**：61 英雄 × 6 槽 = **366**
- **✅ PASS：358 / 366**（97.8%）　🟣 PASSIVE：8　❌ FAIL：0
- 把「正確的永久被動」算進可接受行為：**366 / 366**（100.0%）如預期運作，只有 **0** 格是真正的缺口。
- 英雄生成失敗：**0**（無）

## 近戰 vs 遠程（attackType 維度）

- 名單：**近戰 43**、**遠程 18**。
- **普攻形態**：遠程英雄中 **18/18** 的普攻確實射出投射物（`projectileSpawn`、事件 `ranged:true`）；近戰英雄中 **43/43** 的普攻是貼身直接傷害（無投射物、`ranged:false`）。這正是遠程與近戰在普攻上的行為差異，兩邊都被本次量到。
- **技能投射（skillshot castType）**：本名單中 skillshot 技能格 遠程 7 格、近戰 20 格；skillshot 一律用施法方向生成投射物，與施法者是遠程或近戰無關（castType 獨立於 attackType）。
