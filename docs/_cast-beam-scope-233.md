...
|---|---:|---:|---|---|
| 1.0 s | 288 | 511 ms | `reactable` | 出現 + **下墜倒數**，光點落地＝技能生效 |
| 0.5 s | 178 | 81 ms | `marginal` | 出現 + **下墜倒數**，光點落地＝技能生效 |
| 0.3 s | 57 | -91 ms | `notice` | 出現，但**不畫下墜倒數**（沒有可閃的窗口，倒數就是騙人） |
| 0.1 s | 218 | -263 ms | `notice` | 出現，但**不畫下墜倒數**（沒有可閃的窗口，倒數就是騙人） |
| 瞬發 | 166 | — | `instant` | **不出現**（沒有施法窗口可以預告） |

## 彙總

- `reactable` — **288** 支（31.8 %）
- `marginal` — **178** 支（19.6 %）
- `notice` — **275** 支（30.3 %）
- `instant` — **166** 支（18.3 %）
