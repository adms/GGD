...
|---|---:|---:|---|---|
| 1.0 s | 279 | 511 ms | `reactable` | 出現 + **下墜倒數**，光點落地＝技能生效 |
| 0.5 s | 184 | 81 ms | `marginal` | 出現 + **下墜倒數**，光點落地＝技能生效 |
| 0.3 s | 59 | -91 ms | `notice` | 出現，但**不畫下墜倒數**（沒有可閃的窗口，倒數就是騙人） |
| 0.1 s | 219 | -263 ms | `notice` | 出現，但**不畫下墜倒數**（沒有可閃的窗口，倒數就是騙人） |
| 瞬發 | 166 | — | `instant` | **不出現**（沒有施法窗口可以預告） |

## 彙總

- `reactable` — **279** 支（30.8 %）
- `marginal` — **184** 支（20.3 %）
- `notice` — **278** 支（30.7 %）
- `instant` — **166** 支（18.3 %）
