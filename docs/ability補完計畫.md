...
## ⛔ owner 2026-08-04（第二輪）：A 組砍掉四項

| 批 | 裁決 | **必要配套**（不做這個，「不做」就變成一個會騙人的洞） |
|---|---|---|
| **A1** 火圈進佇列 | ❌ **不做** | ⚠️ 火圈**永遠不在 hook 路徑上** → 任何寫「受傷時」的卡片在火圈裡**不會觸發**；`invulnerable.blocksTrueDamage` 對火圈**永遠無效**（`FireRingSystem.ts` 檔頭 ⑤ 自承的那個 bug 不修了）。→ 這兩條要從「已知缺口」升格成**詞彙表上的永久事實**。 |
| **A2** onLevelUp | ❌ **不做** | ⛔ **`onLevelUp` 必須從編輯器下拉裡停用** —— 它今天選得到而零發射點，不接上又不停用，就是留著一個「寫了什麼都不會發生」的選項。成本 ≈ 0，但**不可以省**。 |
| **A3b/c** 殭屍屬性 | ❌ **不做** | ⚠️【破甲】【破魔】【破防】【易傷】【虛弱】【凋零】對殭屍**永久無效**。編輯器在作者選這些效果時要提示「第 3 場之後對場上大多數敵人不生效」。 |
| **A5** 增益卡授權面 | ⏸️ **等 owner 看內容判斷清單** | 見〈[A5 的內容判斷](#a5-的內容判斷-owner-要先看這個)〉 |

✅ **仍然要做**：[A4 淨化](#a4--淨化驅散) · [A6 重創](#a6--curse-更名--重創)
✅ **已出貨**：A3a（殭屍狀態欄，v0.9.39）· A6 的 curse 更名那一半（v0.9.36）

### ⚠️ 「A2 不做」有一個會回來的耦合

B4 / B5 / B6 三批**原本都靠 A2 的佇列泛化**（事件發生地不能 `import effects/hooks`，
會 close 初始化環）。A2 砍掉之後，**那三批只要有任何一批要做，佇列工作就會回來**
—— 只是改名叫「B4 的一部分」。所以 A2 的成本沒有消失，只是**延後並綁在 B4 上**。

⛔ 唯一真的省掉的是 `fireOnBulkGrant`（`grantLevels` 一次吐 40–50 級的那個防爆欄位）
—— 因為沒有 onLevelUp 就沒有東西會爆。
...
### A5 的內容判斷（owner 要先看這個）

⏸️ owner 2026-08-04：「**給我有哪些內容判斷再決定**」。

A5 的**程式**很小（6 個 schema 欄位 + `applyBuff` 一行轉發，機制全部現成）。
真正要決定的是**內容**：開放之後，三選一卡片就寫得出下面六類東西，而每一類都是
一個平衡決定。逐項給「它實際上會長什麼樣」與「風險在哪」：

| # | 開放什麼 | 卡片會長什麼樣 | ⚠️ 風險 | 我的建議 |
|---|---|---|---|---|
| 1 | **`vision`（隱身／真視）** | 「常駐隱身，出手現形」／「給全隊真視 20」 | 隱身有 8 個後台開關（擋自動索敵／擋殭屍仇恨／擋手動點選…），**預設全開＝在 2v2v2v2 裡非常強**。真視是它的解藥，但只有拿到真視卡的人有 | **只開給稜彩**（第 7–13 回合），而且**同一輪要讓真視也進池** |
| 2 | **限時 `vision` / `flight`** | 「按下 R 之後隱身 5 秒」 | 比常駐溫和得多，因為有到期 | ✅ **開**，這是最安全的一項 |
| 3 | **`flight`（飛行）** | 「無視單位與障礙移動」 | `stayInsideBoundary` 預設 true 所以不會飛出場；但**飛行在火圈裡照樣燒**，而且會讓「牆／柱子」這一族設計失效 | **開，但只給稜彩** |
| 4 | **`block`（格擋／免死）** | 「30% 機率抵擋致命一擊」 | ⛔ **多來源是 independent 疊加**（`blockRules.stacking` 出貨值）→ 兩張 30% 是 **51%**，三張是 65.7%。三選一會發重複類型的卡 | **開，但 `internalCooldown` 必填**，而且**同一場只能拿一張**（要一個 `unique` 標記，今天沒有） |
| 5 | **`critStrike`（暴擊特化）** | 「暴擊時吸血」「暴擊傷害 +X%」 | `critDamage` 是**聚合屬性** —— 加了之後這位英雄**每一次**暴擊（含普攻）都變那樣 | **開**，風險最低的一項 |
| 6 | **`damageTypeOverride`（真傷覆寫）** | 「你的普攻改判真傷」 | ⛔ **最強的一項**：無視護甲與魔抗，直接讓【破甲】【易傷】整族失去意義，也讓坦克路線失效 | ⛔ **不開**，或只開「單一技能改判」而不是整批 |
| 7 | **`auras`（靈氣）** | 「周圍隊友 +X」 | 半徑上界 40。三選一給光環＝**隊友也受益**，這會改變 2v2 的組隊價值（拿光環的人變成輔助位） | **開**，而且它是這六項裡**最能製造隊伍互動**的一項 |

### 所以要 owner 回的是三個問題

1. **這六類各要不要開？**（1 隱身 / 3 飛行 / 4 格擋 / 5 暴擊 / 6 真傷覆寫 / 7 靈氣）
2. **開的那些，限銀／金／稜彩哪一階？**（今天三階都吃同一份 schema，
   「只給稜彩」是**內容側的自律**，不是引擎擋得住的 —— 要引擎擋就得多一個 `tier` 閘）
3. **格擋要不要「同一場只能拿一張」？** 今天沒有 unique 機制，
   要的話是一個獨立的小工程（三選一發牌時排除已持有的同類）。

⛔ **在 owner 回答之前，A5 不動程式** —— 六個 schema 欄位開下去就是六個
「可以寫但沒人想過平衡」的授權，而 schema 加了之後拿掉會讓既有內容載入失敗。
...
⭐ **XS —— A4 之後幾乎免費。**
護盾不在 `Stat` enum 裡（2026-08-18 重數：**23 條**逐一對過，仍然沒有護盾），
所以 `ModOp.Override` 這條路對它**不存在**；
