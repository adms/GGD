...
| **[D4](#d4--無敵點不到)** | 無敵（點不到） | **L** ⚠️ | 【無敵】 | — ⚠️ 見九項 |
| **[D5](#d5--投射物形狀)** | 投射物形狀 chain／homing／boomerang | M | 三個玩家講得出名字的投射物行為 | — |
| **[D6](#d6--標籤預設層)** | 標籤預設層（preset） | M | 14 個 🧩 標籤變成編輯器上的一等公民 | A4 · A6 |
| **[E1](#e1--共用-shape-欄位)** | 共用 `shape` 欄位 | **L** ⚠️ | 範圍治療／範圍暈眩／錐形…全部一次到位 | 要拍板 |
| **[E2](#e2--wire-頻道)** | wire 頻道（別人看得到的狀態） | **M** ⚠️ | 敵人身上的破甲光環／隊友的冰凍外觀／頭上的標記 | 要拍板 |
...
> 這裡列的是**成本或風險超過「照做就好」的門檻**的項目。
> 每一項給：問題 · 兩到三個選項 · 我的建議 · **不決定的話會發生什麼**。
> ⛔ 前四項會**改變批次內容**，不是做完再調整。
...
⛔ **不決定的話**：我會照上面那一欄做成六個後台開關並用建議值出貨，
但這六格加起來就是「站火圈裡能不能活」的手感，**owner 打一場的體感比我的推論可信**。
...
⛔ **我的建議：A3a + A3b 現在做，A3c 等 owner 說要**。
理由：A3a+A3b 解鎖 9 個標籤而且行為中性；A3c 只多解鎖 2 個，卻要動殭屍傷害公式，
而殭屍傷害是 owner 每週在調的東西。
...
**我的建議：B（延後）**，理由是它和【附身】共用同一批接縫，
而【附身】已經在延後名單裡 —— 兩個一起做比分兩次便宜。
...
**我的建議：A**，而且**如果排不進這一輪就選 C，不要選 B。**
B 是拿一個會在幾週後才爆、爆的時候看不出根因的東西換三天。
...
⚠️ 另外：這三件道具**今天不在商店裡**（GH#261 只留能力屬性強化與傳說寶玉），
所以做完之後**玩家暫時拿不到** —— 除非它們進三選一或任務獎勵。
...
理由：回復走的是 `RegenSystem.ts:58` 一條**不同的路**（不經過 `healTarget`），
而且「中了重創連自然回血都變慢」在 LoL / Dota 都不是那個機制。
→ 做成**三格獨立倍率**，預設 `lifesteal 0.5 / healingTaken 0.5 / regen 1.0`。
...
**我的建議：A 的純加法版** —— 新 kind 一律帶 `shape`，既有三個 kind 原封不動。
這樣不用改任何一份既有技能文件，而債停止長大。⚠️ 但這是一個**架構決定**，
做了就回不去（編輯器與所有文件都會用它），所以列在這裡。
...
**我的建議：如果選 A，把這一輪所有要上線的欄位合併成一次追加**，不要各加各的。
⛔ **這一題不要替 owner 選。**
...
**我的建議：做，但排在 B3 最後一顆**，因為它是純讀、二十行、不動任何既有路徑，
而且武器道具回來的那一天它就直接可用。**不建議為了它延後 B3 其他十一顆。**
...
三格獨立倍率（見[九項⑥](#-重創到底打折哪幾種回復影響-a6)）：
`lifestealMult`（預設 0.5）· `healingTakenMult`（預設 0.5）· `regenMult`（預設 **1.0**）。
**禁療＝三格都填 0 的一份內容文件，不是第二個機制。**

**改動面**：`content/status-effects/curse.json`（一個欄位）· `sim/components.ts` 的
`StatusEffect`（三格）· `sim/effects/applyStatus.ts` · **`sim/combat/restore.ts:66-73`（唯一讀取點）** ·
`sim/combat/damage.ts:885`（吸血那一發）· `content/schema/effect.ts` ·
...
2. **吸血那一半也被打折**（不是只做治療）—— 突變：只改 `healTarget` 不改吸血讀取點 →
   吸血照舊 → 紅。⛔ 這條在防「做一半而看起來像做完」。
3. **regen 預設不受影響**：帶重創的目標每秒自動回血量必須與沒帶時**相同** ——
   突變：把 `regenMult` 預設改成 0.5 → 不同 → 紅。
   ⛔ 三條都不可以斷言 0.5 這個數字，一律從測試自己建的 status 讀。
...
見[九項⑦](#-形狀要不要抽成共用欄位e1)。建議：**純加法版** —— 新 kind 一律帶
`shape: single|circle|line|cone`，既有 `damage` / `damageArea` / `damageLine` 原封不動。
不用改任何一份既有技能文件，而債停止長大。

### E2 · wire 頻道

見[九項⑧](#-別人也看得到的狀態要不要開-wire-頻道e2)。
⛔ **如果選 A（`defineTypes` 追加），把這一輪所有要上線的欄位合併成一次追加。**
...
### A6（重創）
- **6-1 三格倍率的範圍** —— 見[九項⑥](#-重創到底打折哪幾種回復影響-a6)。
  預設 `lifesteal 0.5 / healingTaken 0.5 / regen 1.0`。
- **6-2 多來源相乘還是取 max** —— **預設取 max**（跟最接近的鄰居 `missChance` 一致）。
