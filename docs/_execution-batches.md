...
## ⚠️ 等 owner 裁決（不裁決就會卡住，或會被我自己發明數字填掉）

| # | 問題 | 我沒有自己決定的原因 |
|---|---|---|
| **1** | **`AgiMoveBonus` — 已裁定不補。** 地圖寫 `0.1`（暴雪 0），GGD 沒有敏捷→移速這根軸 | ✅ owner 2026-07-26：不補。釘成明講的決定，不是「剛好沒做」 |
| **2** | **等級上限對不起來**：地圖 `MaxHeroLevel=40`，GGD 是 **18**（`config.match.json` → `progression.levelCap`） | 18 是配合 ~3 分鐘一回合訂的；改 40 會連帶動 XP 曲線與全體成長區間，不是一行的事。**純回報** |
| **3** | **`godie-h02n` 打我阿笨蛋的成長是壞的**：`strGrowth 0`、`agiGrowth 0`、`growth.healthRegen 0`（`intGrowth 1.8` 正常）。這隻**沒有 `description`**，所以沒有可以照著補的文案 | 熊貓那隻你已經裁定「照說明成長」並且做完了；這隻沒有說明可照。**base `str=1` / `agi=1` / `ms=5.5` 是刻意的搞笑設定，不要動** |
| **4** | **克勞薩先生等級 1 只有 79 點生命** —— 後台體檢噴的「會被秒殺」是**真的** | 請看一眼這是不是你要的 |
| **5** | **⛔ 卡住批次 1-1**：節拍疊層掛在 `StatsComp`，肉鴿殭屍沒有 `StatsComp`（`effectRunner.ts:123`），所以喪標麥可的整套連段**對殭屍波完全無效** | 三個做法都會動到別的東西：(a) 給小怪 `StatsComp`（吃記憶體、動到所有 buff 路徑）；(b) 節拍改掛在攻擊者身上（語意變成「我打了幾下」而不是「這個敵人被打了幾下」）；(c) 接受它只對英雄有效，寫進說明 |
| **6** | **三條描述 ↔ JASS 衝突**（你說過 JASS 可能是刻意的隱藏機制，所以一律問） | `A0JD 浮雲-旋一閃`：敏捷係數沒實作 · `A091 及喀爾度`：缺吸引，且數字只有 1 級值（JASS 是 2×等級顆、250+100×等級）· `A0L6 死亡噴射肘擊`：缺擊退，描述寫 1000 但 JASS 是 20 步 × 40 = **800** |
| **7** | **43 位英雄要不要各自捏一具體素模型？** 目前他們共用四具（法師 18 / 騎士 10 / 蠻族 9 / 盜賊 6），靠**執行期換膚**取得辨識度 | 各捏一具＝**+2.18 MB**（現在全部只有 255 KB），是 8.5 倍回歸，而換膚已經零位元組給到辨識度。所以我沒有捏，但這是體積 vs 剪影的取捨，該由你拍板 |

### ℹ️ 順帶要你知道的一件事（不用裁決，但別被字面騙了）

**「體素貼圖自動生成」裡，真正被推導出來的比例沒有字面上那麼高。**
四層推導鏈（手寫表 → 關鍵字規則 → 技能特效元素 → id 雜湊）攤在全名冊 114 × 17 軸上是
**L1 2.1% · L2 5.2% · L3 23.1% · L4 雜湊 69.7%** —— 中位數英雄有 **12/17 軸是雜湊決定的**。

但**分軸看比這個總數好得多**，而且好的地方正好是看得見的地方：
`outfitPrimary` / `outfitSecondary` / `eye` / `accent` 是 **114 位裡 111～114 位由技能特效元素推導、零雜湊**。
純雜湊的是 `hair`（100%）、`skin`（89%）、`metal`（71%）與幾乎所有剪影／紋樣軸。

→ 也就是說：**主色是真的推出來的，髮色膚色和剪影基本上是擲骰子。**
稽核面板（體素鑄造廠的「為什麼是這個顏色」）對 L4 一律回報 `evidence: null` 並註明是雜湊，
**不會替擲骰子編一個理由** —— 那是這種頁面最容易犯、也最貴的錯。
