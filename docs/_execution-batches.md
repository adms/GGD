...
## 🗺️ owner 的六版路線圖（2026-07-28 拍板 · 逐版 push + note + deploy）

> **這是 owner 用 `/goal` 設下的長期目標，不是我自己排的。** 每一版都要走完整的
> 部署協定六步（code-cut → 無 T0 → 本機 playtest 記錄 → 重整本檔 → push+note → deploy+線上 playtest）。

| 版本 | 內容 | 狀態 |
|---|---|---|
| **v0.9.12** | 目前四項 + 其他已完成成果合併 | ✅ 已上線 `e1d80708` |
| **v0.9.13** | **殭屍規格全面改版（GH#206）** —— 傷害比例分紅（可超過總額）· 從英雄推導的數值倍率 · 隨機選英雄<br>+ **機制本體** —— Pattern A 狀態機 + 協定 + 換模型 + 補 4 份缺文件<br>→ 少數幾個英雄真的變得出來，owner 可以先玩到 | 前置調查中 |
| **v0.9.14** | 剩下 22 份文件對回 w3u + 球體附掛 + 鳳凰蛋特例 + #168 | 未開始 |
| **v0.10.1** | 鑄技工坊的編輯介面（**全部**的技能變身都可以設定） | 未開始 |
| **v0.10.2** | 鑄技工坊完整編輯器：技能/武器道具屬性 + **自由搭配複數效果與特效** | 未開始 |
| **v0.10.3** | 所有英雄技能實作**真實對照 war3** 補完 | 未開始 |

### ⚠️ 一個會貫穿 v0.9.13~v0.10.3 的、加錯就回不去的決定

變身要讓客戶端知道「這個實體現在是第二形態」。**Colyseus 的 `defineTypes` 是 APPEND-ONLY**
（照宣告順序編碼），所以協定欄位加錯位置**沒有辦法回退**。
兩條路：新增欄位（乾淨但不可逆）vs 複用 `EntityState.key`（#262 的小怪就是這樣送 kind 的，
不動 schema 但語意會擠在一個字串裡）。**v0.9.13 動工前要先定案。**

### v0.9.13 的前置：不要從零開始，但也不要照抄
#129~#135、#168 有大量既有調查，**但那些是幾天前寫的**，main 從那時起動過
v0.9.6~v0.9.11（40 位英雄改暴雪模型、體素貼圖、攻速上限、基礎加成、殭屍王…）。
**每一條結論都要在今天的 main 上重新驗證**，過期的要明說 —— 否則下一輪會拿舊結論當事實。

---

## 🚦 v0.9.12 的執行順序（owner 2026-07-28 全部拍板，一起併才 push）

> **這一節是給「下一個接手的我」看的。** owner 要求「全部做完一起併才 push + note + deploy」,
> 所以中間任何一步都**不要**自己 push。

### 波次 A — 四組平行（進行中）
| 組 | 內容 | GH |
|---|---|---|
| 經濟+攻速解鎖 | 殭屍王獎金 3000→30000 · 稜彩技能攻速×2+解鎖10 · 近戰限定傳說武器 · `itemCooldown` 獨立 1x | #187 #188 #189 |
| 殭屍身分 | 逐回合「由誰擔任」真的被吃 · 選英雄→套模型 · 染黑 0.65 · 王 ×10 體型 ×100 HP | #191 #192 |
| 戰鬥手感 | **先**擊退改百分比+減距離 → **再**打就站定 v2 | #193 #197 |
| 殭屍王 UI | 降臨橫幅 · 分紅面板 · 恐怖音效 3~5s · 慶祝音效 5~7s | #190 |

### 波次 B — 序列（每一項都跟前一項改同一個檔，不可平行）
1. **#198 近戰耐打度**（`combatEnv.ts` + 後台戰鬥系統頁）—— 必須排在 #189 的 `itemCooldown` **之後**
2. **#194 殭屍王 + 特殊殭屍會施法**（`MobSystem.ts`）—— 必須排在 #192 **之後**
3. **#199 手機不畫操作說明** —— 排在殭屍王 UI 之後，且它是 275b 合併的前提

### 波次 C — 11 條舊分支（#195，四個小波次）
盤點結論:**7 still-needed / 4 partial / 0 superseded** —— 一條都不能省。
`276`≡`276b`、`275`≡`275b` 是重複的分支**名**(同一個 commit),所以是 9 份工作。
順序見 #195。⚠️ `feat/252-zombiex-kit` 49/83 檔衝突且 leap 那半要取 main
(`leapFraming.test.ts:328` 有 negative control 釘住分支的壞 apex 值)。

### 波次 D — 收尾
全套測試 → `pnpm content:build` → **本機 playtest 打一場** → 記錄進本檔 →
push + tag + release note + deploy → **線上再 playtest** → 記錄回本檔。

### 部署成功之後的下一版
**#196 鑄技工坊 · 變身技能**（owner 指定）。⚠️ 那件事已經有大量前置調查
(#129 的 26 組配對表已就緒但模擬器端零實作、#130 的 4 個第二形態 id 指向不存在的檔、
#131~#135、#168),**動工前全部要先讀,不要從零開始**。
