...
內容匯流排復活（**線上一直沒生效的後台戰鬥設定**）· 藍水晶陳述誠實化（發放額一毛沒動）·
KayKit 退場守衛 · `/editor/` 不進正式映像 · #244 硬化殼併進 W · #246 上線燈號＋邀請碼排版 ·
熊貓照說明成長 · **版權閘退役**（owner 決定全開）· todo 新增 `withdrawn` 狀態 ·
傷害結算時機規則（施法條件在施法時判、量值與減免在命中時算）

## 驗證通過、等合併 — 這是下一個版本的內容

| 分支 | 驗證 | 內容 |
|---|---|---|
| `worktree-wf_c490a72a-ca5-1` | 通過 | #247 鏡頭：垂直軸一直用錯尺，弧線 11.00→2.40 |
| `fix/247-combo-timing` | SOUND | 蒼月潮連段在真實遊戲裡永遠算不到 |
| `fix/247-leap-followups` | 通過 | editor union、死碼 clamp、縮放通道 |
| `feat/243` + `fix/243-recovery-honesty-v2` | 3 阻擋項清 | 預演＝契約、還原誠實化、備份保留 |
| `feat/249-roster-and-form-link` | 資料層 26/26 獨立重驗 | 名單換回本體 |
| `fix/249-store-alignment` | 通過 | 商店價目表對齊＋守衛 |
| `fix/249-shared-voices` | 通過 | 變身前後共用語音，啞巴 10→0 |
| `fix/248-coefficients-and-audit` | SOUND | 八條係數改用地圖自己的 misc 表、後台體檢復活 |

## owner 本輪的裁定

1. **版權閘退役** —— 已有邀請＋審查，不需要卡版權。已合併。
2. **變身前後共用語音** —— 省掉 460 個音檔的生成。已做完。
3. **描述與 JASS 衝突要先問 owner** —— 因為 JASS 可能是刻意的隱藏機制。已寫進記憶。
4. **`AgiMoveBonus` 不補** —— 地圖有 0.1、暴雪 0、GGD 沒有這條軸；維持沒有。要釘成明講的決定，不是「剛好沒做」。
...
9. **藍水晶不砍** —— 實測 21 分鐘剛好回到原本調校點，本來就沒東西要改。

## 下一批（有相依性，同批執行）

### 批次 A — 合併與部署（無相依，可立即）
上面八條驗證過的分支依序合併 → 完整閘門 → 試玩 → push+note → **v0.6.0**。
`fix/249-*` 三條必須按 `roster-and-form-link → store-alignment → shared-voices` 的順序。

### 批次 B — 技能組忠實度（相依：批次 A 的 leap/抓取原語）
`docs/_kit-fidelity-audit-247.md` 的 114 位逐一檢查。**改用「從描述出發」而不是「從 JASS 原語出發」**——
三輪抓取系普查每一輪都被一個不符合當輪特徵的例子打臉（`A0MV 冥道殘月破` 定住 6 秒卻從不寫座標，
任何靠位移不對稱的偵測器都看不到它）。描述是有限且可窮舉的，JASS 原語不是。
**每個衝突都要列給 owner 裁定**，不可自行和解。
已知待裁定：`A0JD` 敏捷係數沒實作、`A091` 缺吸引且數字只有 1 級值、`A0L6` 缺擊退且描述誇大成 1000。

### 批次 C — 變身系統實作（相依：批次 A 的 formOf 關聯）
26 對已建立關聯與觸發語意。**地圖裡沒有任何自動施放**（91 個 order 站點、0 個變身指令），
25 支純按鈕、1 支死亡觸發（克勞薩，魔力 ≥500，10 秒後重生）。
真正的條件系統是 **EX 閘**：等級 ≥30 → 研究 `R00R` → 99 支技能解鎖，其中 8 支是變身。
回復：22 支靠 `ahdu` 時限、2 支 toggle、1 支死亡重生。**沒有任何強制提前變回**。

### 批次 D — 資產與呈現（無相依，可平行）
#245 版本徽章（保留帶不是空的，待修）· #235 煙火從來沒被看到 · #233 施法光束預告 ·
#232 回合評價 · #234 擊殺語音＋觀眾歡呼 · #238 火圈聲音的客戶端那一半

### 批次 E — 已知但未排（需要 owner 排序）
#212/#214 開放賈修＋揍敵客 · #226/#229/#231 體素 · #230 VFX 真實引用 · #178 圖示補完 ·
`tform-16` #244 改技能名後沒重新合成語音（`pnpm voice:index` 在 main 上是紅的）

---

## #248 後續 — 係數的真正來源，以及兩個 owner 要拍板的東西（2026-07-26）

### 一、八個係數裡有七個是「匯入值」，而地圖改寫了暴雪四項

#248 把三圍換算係數當成暴雪原值抄進來，但**這張地圖自己帶了一份 gameplay constants，而且已經在倉庫裡**：
`tools/w3x-import/out/GoDieEX22s-src/raw/war3mapMisc.txt`。暴雪那份是
`Units\MiscGame.txt`（已由新的 `tools/w3x-import/stock_misc_data.py` 抽成
`out/stock/STOCK_MISCGAME.json`，測試會直接讀這兩個檔）。

| 係數 | 地圖 war3mapMisc.txt | 暴雪 MiscGame.txt | 現在採用 |
|---|---|---|---|
| 力量→生命 `strToMaxHealth` | **StrHitPointBonus 23** | 25 | **23**（地圖） |
| 力量→回血 `strToHealthRegen` | **StrRegenBonus 0.04** | 0.05 | **0.04**（地圖） |
| 力量→AD `strToAttackDamage` | StrAttackBonus 1.0 | 1.0 | 1（兩邊相同） |
| 敏捷→護甲 `agiToArmor` | **AgiDefenseBonus 0.15** | 0.30 | **0.15**（地圖） |
| 敏捷→攻速 `agiToAttackSpeed` | 未覆寫 | AgiAttackSpeedBonus 0.02 | 0.02（回退暴雪） |
| 智慧→魔力 `intToMaxMana` | IntManaBonus 15 | 15 | 15（兩邊相同） |
| 智慧→回魔 `intToManaRegen` | **IntRegenBonus 0.07** | 0.05 | **0.07**（地圖） |
| 智慧→法強 `intToAbilityPower` | — | — | **1（owner 設計值，魔獸沒有這根軸）** |

`MaxHeroLevel` 地圖寫 40、暴雪寫 10 —— 這個落差就是「這張常數表確實被作者改過、不是複製品」的證據。

**原本的註解說這些數字「verified against `Units\UnitBalance.slk`」是憑空捏造的**：那個檔有 60 個欄位、
一個係數都沒有（它是 per-unit 表：STR/AGI/INT/STRplus/HP/def…）。註解掛錯來源比沒有註解更糟，
因為它會讓下一個人不去查。八個係數現在每一個都在
`packages/shared/src/sim/combatEnv.ts` 的 `ATTRIBUTE_ENV_DEFAULTS` 上標明檔名 + 欄位名，
`packages/shared/src/sim/attributeCoefficients.test.ts` 會**讀那兩個檔**，漂了就紅。

### 二、全體實測位移（112 隻，等級 1，內容單位）

| 數值 | 中位數 舊 → 新 | 變化 | 位移 >5% 的隻數 |
|---|---|---|---|
| 生命 | 600 → 564 | −6.0% | 109/112 |
| 回血 | 1.15 → 0.97 | −15.7% | 108/112 |
| **護甲** | **6.00 → 3.15** | **−47.5%** | **112/112** |
| 回魔 | 1.00 → 1.36 | **+36.0%** | 109/112 |
| AD／攻速／魔力／法強 | 不動 | 0% | 0/112 |

全體總量：生命 −5.5%、回血 −8.6%、**護甲 −46.2%**、回魔 +3.7%。
護甲砍半是最大的一項，而且是**全體同向**——如果覺得回合變太短，正確的旋鈕是
戰鬥系統頁的 `defense` ×倍率，不是把匯入係數改回暴雪值。

點名幾隻（等級 1，舊 → 新）：Saber 生命 725→679 / 護甲 4.8→2.4；藤井八雲 750→702 / 6.5→4.25；
熊貓 900→840 / 6.5→4.25。

### 三、⚠ owner 要決定：`AgiMoveBonus` 這根軸我們根本沒做

地圖寫 `AgiMoveBonus=0.1`（暴雪是 0），也就是**原作裡每 1 點敏捷 +0.1 移速**。
GGD 的模型裡沒有敏捷→移速這根軸，所以敏捷型角色少拿了一整條原作有的收益。
**沒有自己發明數字補上**——加這根軸會動到全體移速，那是平衡決定不是匯入。請 owner 決定要不要開。
（`AgiDefenseBase` 地圖是 0、暴雪是 −2；GGD 的護甲公式沒有常數項，這一項**剛好與地圖一致**，不用動。）

### 四、⚠ owner 要決定：等級上限對不起來

地圖 `MaxHeroLevel=40`，GGD 是 **18**（`content/config/config.match.json` → `progression.levelCap`）。
沒有動它：18 是配合 ~3 分鐘一回合的比賽節奏訂的，改成 40 會連帶動到 XP 曲線與全體成長區間，
不是一行的事。純回報。

### 五、⚠ owner 要決定：兩隻已上架英雄的成長是壞的（作者自己填的，跟自己的說明文字矛盾）

這兩隻不是匯入錯誤，是**原地圖作者填的值**，而且**跟他自己寫在角色介紹裡的成長值互相矛盾**。
沒有替他發明數字，等你裁決：

- **`godie-h02k` 熊貓（國寶級的畜生）** — `strGrowth / agiGrowth / intGrowth` **三個全是 0.0**，
  `growth` 區塊的 maxHealth/ad/armor 也全是 0。**這隻角色永遠停在 1 級的數值**，升級完全沒有回報。
  但他自己的角色介紹寫著 **「力量+1.0 敏捷+1.0」**。另外 `int = 2` 是全名冊最低。
- **`godie-h02n` 打我阿笨蛋（腦包英雄）** — `strGrowth 0`、`agiGrowth 0`、`hp_regen 0`。
  （他的 base str/agi = 1 與移速 1 是**刻意的搞笑設定，不是缺陷，不要動**；`intGrowth 1.8` 是正常的。）

### 六、順帶修好的：`godie-zombiex` 的 380 被係數改動打歪，已重新反解

喪標麥可的 `attributes.source` 是 `authored`（沒有 w3x 來源），那組三圍**存在的唯一目的就是重現
owner 在 #244 挑的那張表**。係數從 25 變 23 之後他會悄悄掉到 356，所以原始卡的
`baseStats.maxHealth` 由 80 反解成 **104**，`104 + 23×12` 仍然剛好是 **380**。

**沒能保住的那一項，講清楚**：舊係數下屬性層每級剛好給 +45，跟 #244 手寫的 `growth.maxHealth: 45`
數字相同（那是巧合）。新係數下屬性層是 41.4，硬湊 45 需要 `strGrowth = 1.9565…` 這種為了遷就巧合
而發明的數字，所以沒有那樣做。owner 手寫的 45 一個字沒動；**每級有效生命從 90 變成 86.4**，
等級 12 的 ×4 對照數字從 5480 變成 5321.6。

`sela` / `thorne`（skeleton 測試內容）**刻意沒有跟著反解**：那兩隻的絕對數字不帶設計意圖，
而且照著反解會需要一個負的回魔基礎值（1.6 − 0.07×26 = −0.22）去追一個沒人在看的 fixture 數字。

### 七、後台「數值體檢」曾經整頁失效（#242 的功能，被 #248 打壞）

`apps/admin/src/quickApproval.ts` 直接讀 `doc["baseStats"]["maxHealth"]`。#248 把 `baseStats`
換成 w3x 原始卡之後，**114 份文件裡有 100 份的 `baseStats.maxHealth` 就是 150**，於是：

- 同場中位數血量 480 → 150、護甲 7.5 → 0
- 血量警示 `cand < 中位數 × 0.5` 變成 `150 < 75`，**永遠不可能觸發**
- 護甲警示有 `中位數 > 0` 的守衛，中位數變 0 之後成為**死碼**
- Saber 在頁面上顯示「血量 150 · 護甲 0」，他實際是 679 / 2.4
- 克勞薩先生印出「血量 -450 … -300% — 會被秒殺」這種遊戲裡不存在的數字
- 喪標麥可原本會正確噴的「護甲 0（同場中位數 7）— 完全不減傷」**默默消失**

**428 條 admin 測試全綠**，因為那份測試從頭到尾只餵手寫的假資料、一份真的英雄文件都沒讀過。
現在頁面改走跟遊戲同一個 seam（`championStatBase`），而且測試會讀**真的英雄文件**
（健康的 Saber、故意脆的喪標麥可、負數卡的克勞薩先生）。
順帶一提：克勞薩先生真實等級 1 只有 **79 點生命**，那個「會被秒殺」是**真的**該噴，
只是要噴在真實數字上——這一隻也請 owner 看一眼。

### 八、⚠⚠ 兩隻已上架英雄的成長是壞的 —— 原文引用，等 owner 裁決

**不要替他們發明數字。** 這兩筆是原地圖作者自己填的值，而且跟他自己寫的說明文字互相矛盾。

#### `godie-h02k` 國寶級的畜生 - 熊貓

`attributes` 三個成長 **全是 0.0**（`strGrowth: 0`、`agiGrowth: 0`、`intGrowth: 0`），
`growth` 區塊的 `maxHealth / healthRegen / maxMana / manaRegen / ad / armor` 也**全是 0**
（只剩 `mr: 1.2`、`as: 0.02`）。也就是說 **這隻英雄升級以後，除了魔抗與攻速以外什麼都不會變 ——
等於永遠停在 1 級**。

他自己的角色介紹（`content/champions/godie-h02k.json` 的 `description`）卻寫著：
...
文案說力量每級 +1~4，資料說 0。另外 `attributes.int = 2` 是**全 112 隻名冊裡最低的智慧**。

#### `godie-h02n` 腦包英雄 - 打我阿笨蛋

`strGrowth: 0`、`agiGrowth: 0`、`growth.healthRegen: 0`（`intGrowth: 1.8` 是正常的）。
這隻**沒有 `description` 欄位**，所以沒有可以引用的文案矛盾 —— 但同一份 w3x 掃描報告
（`docs/_attribute-derivation-248.md` §2）把它列在「文案與抽出值不一致」之外的另一類。

**這一隻的 base `str = 1` / `agi = 1` 與 `ms = 5.5`（原始 移速 1）是刻意的搞笑設定，
不是缺陷，不要動。** 要裁決的只有那兩個 0 成長 + 0 回血。

#### 可選的做法（都需要 owner 拍板，我沒有選）
1. 照文案補：熊貓 `strGrowth 1.0 / agiGrowth 1.0 / intGrowth 0`（文案的下限值）。
2. 照原作留著不動，接受「熊貓是刻意的 1 級固定角色」這個設定，並把它寫進說明文字，
   讓玩家事先知道。
3. 把兩隻移出開放名單，直到有結論。
