# 歷史 · GGD 執行批次計畫（Execution Batches） — 封存於 2026-07-26

> 這份檔案是 `docs/_execution-batches.md` 的**歷史區**。
> 搬進來的條件只有一個：**那件事已經完整做完、而且沒有任何還沒做的事情依賴它**。
> 還在等 owner 裁決的、還有下游相依的、以及「做事的規則」本身，一律留在作戰檔裡。
>
> 搬進來的東西**不要再排工**。它們留在這裡的用途是：追溯某個決定當初為什麼那樣下、
> 以及證明某條「已完成」不是靠綠燈測試宣稱的。
>
> 封存時的 main：`2dfe4aa8`（批次 A 十條分支 + #237/#245 皆已併入）。

---

## 第一部分 · 版本部署紀錄與逐輪反省（v0.4.9 → v0.5.16）
...
## 第二部分 · 舊的批次結構（批次 0–9、相依圖、訂正區）—— 已被 A–E 批次取代

> ⚠️ 這一整段的批次編號（批次 0 到批次 9、Lane 1A/1B/1C…）**已經不再使用**。
> 2026-07-26 起改用 A–E 分組，見作戰檔。留著是因為〈訂正區〉逐條記錄了
> 「活頁說完成、實跑是假的」的實測證據，那是這個專案最貴的一課的原始材料。
...
## 第三部分 · #248 屬性係數的來源考證（已落地部分）

> 還沒拍板的四個問題（`AgiMoveBonus`、等級上限、打我阿笨蛋的 0 成長、克勞薩先生的 79 血）
> **留在作戰檔**，不在這裡。
