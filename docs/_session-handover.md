...
| ~~#274 bot 邊退邊打~~ | ⚠️ **2026-07-28 已被 owner 自己推翻** —— 見下 | 🔄 v0.9.12 實作中 |
| 取得半徑 6 vs 48 | **不放大**（「不要被拉過去打」），`MELEE_ACQUIRE_FLOOR` 維持 6 | ✅ 無程式改動 |
| 鍛造 capstone | **60~150%**（期望值 55% → 105%） | ✅ 已推 |

> ### ⚠️ #274「bot 邊退邊打 → 保留」在 2026-07-28 被推翻
>
> 原裁決寫在這張「已生效，不要再問」的表上，理由是「風箏的定義本來就是邊退邊打」。
>
> **新裁決（owner 2026-07-28）**:「遠程單位在攻擊的時候可以邊移動邊攻擊,這樣對近戰單位
> 來說是不公平的」,並指示「參考 PR #182 的概念,取其優點,幫我重新做一個」。
> 也就是**遠程不再能邊退邊打** —— 打就要站定。
>
> **這件事要帶著一個數字上線**:PR #182 的實測是遠程 bot 普攻 **1000 → 105 次(−89.5%)**,
> 3 位掛零、22/33 只剩 0–2 次。那不是削弱,是關掉。所以 v2 的硬性條件包含
> 「後台要有整條關掉的開關」與「衰退幅度要 owner 打過一場才拍板」。
> 完整的六個實測發現在 GH #197。
