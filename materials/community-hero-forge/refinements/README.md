...
本輪已更新全部 37 名／222 槽，使用既有模板完成各自核心連動與惡搞代價；完整原稿及既有模型保留。最新入口為 [逐名表](parody-review.md)、[逐槽設定與處置](parody-review.json)、[版本清單](../handoff-manifest.json)。阿薩謝爾改為極大範圍嘲諷、兩秒敵方強化與三秒賢者時間。37 名已完成最新服务 ZIP、隔離投稿／管理員發布及異帳號下載逐位元還原；畫面仍有菜月昴 W 的模型檢查失敗，以及八神庵 EX／承太郎 Q 的遮擋待處理，不能宣稱全數美術通過。依最後裁決，這類鏡頭／美術微調只記錄、不阻擋功能交付；正式部署留待 Main 審查合併。各項狀態見 [本輪驗證收據](parody-verification.json)，不沿用下列舊收據。

下方是歷史批次紀錄，數量與「最新」措辭僅代表當時狀態。


最新為炭治郎 v2（revision 12→14）：六槽已用既有 enabled 模板／機制完成行為驗證，沒有新增引擎欄位、模板定義或全域標籤。18 項角色／Editor／匯入案例、9 項資源回歸與四項有效突變已通過。原稿保留，依最新核准簡化火式額外呼吸成本，改用移速負擔；Q 冷卻不被姿態切換重置。93 槽有案例、129 槽未對應；新畫面、服務 ZIP／發布及整批品質仍未完成，#1132 保持開啟。見 [收據](tanjiro-verification.json) 與 [新版設計規則](../design-policy.json)。下列舊段落的原設計忠實度要求屬當時歷史，當前改編依新版規則。

前批空條承太郎 v1（revision 12→13）：六槽行為已登記，新增真正局部時停與正面格擋；Editor 與客戶端共用時停範圍，模擬狀態控制動畫暫停。其他 36 份作品、原文與模型不變，四項有效突變均被攔截。87 槽有案例、135 槽未對應；白金之星副模型、双模型動作、專用效果／音效、畫面驗收及當下服務 ZIP／發布仍待完成，#1132 原始範圍不變。見 [收據](jotaro-verification.json)。

前批八神庵 v2（revision 13→14）：補完其餘五槽，61 項本英雄／接續／Editor 案例及 12 項來源隔離回歸通過；四項有效突變均被攔截。紫炎由目標及施法者分開記錄，R 真正命中後才接有限連擊，完成後開同目標 EX 窗口；E 程式不變。整批 81 槽有案例、141 槽未對應、17 名通用普攻被動。模型／原文未改，原稿美術與新服務 ZIP／發布仍待完成，#1132 保持完整範圍。見 [收據](iori-verification.json)。

前批八神庵 E v1（revision 12→13）：真正三次輸入、0.2～1.2 秒窗口、逾時／控制中斷、首段成本與持續冷卻。其餘五槽待完成。模板原文與 hash 隨微調保留，伺服器歷史来源獨立核准；其他 36 份生成作品不變。整批 76 槽有案例、146 槽未對應、18 名通用普攻被動。見 [收據](iori-recast-verification.json)。
...
前批不知火舞 v1，revision 12→13：完成實際位移後的一次身法、單一穿透彈、前方弧形、沿途命中不停止的突進、R 真正命中後連段及 EX 每次施法一次／總共三次追加。31 項行為、3 項匯入／試玩、四項有效突變與 444 項集中回歸通過；Editor 可先施放 EX 再測其他槽。75 槽有案例、147 槽未對應，18 名通用普攻被動。見 [不知火舞證據](mai-verification.json)。真田幸村雙扇風格代理保留，扇彈、專用姿態、火焰弧／殘像與聲音仍待驗收；#1132 原始完整範圍不變。

前批阿薩謝爾批次微調 v1，revision 30→31：R／EX 採實際傷害輸出倍率，保留同來源 R→EX 反轉增益；E 以一次近身有效普攻／技能受擊觸發普通影子拳，不額外免傷或忽略全域傷害倍率。37 項當前配方行為、3 項匯入／試玩、9 項距離条件與四項突變通過。兩個交接入口產出的全部作品一致；舊版獨立函式及測試明列回放相容性用途。69 槽有行為案例、153 槽未對應，19 名通用普攻被動。見 [本批阿薩謝爾證據](azazel-batch-verification.json)。原作影子、白雨、能量球、反轉金光／表情及音效仍待完成，#1132 原始範圍不變。
