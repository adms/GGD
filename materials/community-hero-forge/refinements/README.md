...
目前修正 3 槽：鹿目圓 W、庫洛魔法使 W 指定友軍或自己；吉伊卡哇 EX 給附近友軍及自己護盾、三秒抗恐懼。指定盾的 castEffect 視覺改綁 target。鹿目圓希望資源、庫洛魔法使牌序資源仍未完成，不把盾的目標修好當成整槽機制完成。

`design-audit.json` 是這次實際 37 名／222 槽的原要求與編譯結果對照。30 名仍使用 tpl-on-attack 被動；沒有任何列因能編譯而自動標為原設計通過。這份資料不是發布清單。
