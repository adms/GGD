...
最新為不知火舞 v1，revision 12→13：完成實際位移後的一次身法、單一穿透彈、前方弧形、沿途命中不停止的突進、R 真正命中後連段及 EX 每次施法一次／總共三次追加。31 項行為、3 項匯入／試玩、四項有效突變與 444 項集中回歸通過；Editor 可先施放 EX 再測其他槽。75 槽有案例、147 槽未對應，18 名通用普攻被動。見 [不知火舞證據](mai-verification.json)。真田幸村雙扇風格代理保留，扇彈、專用姿態、火焰弧／殘像與聲音仍待驗收；#1132 原始完整範圍不變。
