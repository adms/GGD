...
最新為空條承太郎 v1（revision 12→13）：六槽行為已登記，新增真正局部時停與正面格擋；Editor 與客戶端共用時停範圍，模擬狀態控制動畫暫停。其他 36 份作品、原文與模型不變，四項有效突變均被攔截。87 槽有案例、135 槽未對應；白金之星副模型、双模型動作、專用效果／音效、畫面驗收及當下服務 ZIP／發布仍待完成，#1132 原始範圍不變。見 [收據](jotaro-verification.json)。
