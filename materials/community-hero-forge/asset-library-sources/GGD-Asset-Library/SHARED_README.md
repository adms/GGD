...
目前成品：**60 個 GGD 標準特效元件及全部 PNG 貼圖；完整角色包 0**。這些是現有 GGD 作者化特效，不是 MBA／300 原生特效的轉檔結果。原始下載、待轉換角色與 staging 保留在本機，另存於獨立人工許可備份區，不列入共享成品。
...
完整角色包需具備 GLB 2.0、完整 skin／貼圖、六種真實動作對應，以及 GGD 特效、事件和掛點綁定。未完成者不能進正式清單。可重用 VFX 元件按獨立類型驗證，不代表一個完整角色。
