...
def main():
 p=argparse.ArgumentParser(description=__doc__);p.add_argument('query',nargs='?',default='');p.add_argument('--pending',action='store_true');p.add_argument('--kind',choices=['character','vfx']);p.add_argument('--limit',type=int,default=20);p.add_argument('--offset',type=int,default=0);a=p.parse_args()
 ...
 if a.pending:
  ...
  for entry in read(ROOT/'catalog.json')['entries']:
   ...
   else:records.append(dict(entry,kind='character',content_root=str(base/'content')))
