...
import argparse,hashlib,json,math,re,shutil,struct,subprocess,tempfile
...
def glb(path,clipmap):
 ...
 skins=d.get('skins',[])
 if not skins:raise ValueError('Character requires a skin')
 skinned=0
 ...
 if not skinned:raise ValueError('No skinned mesh nodes')
 names={a.get('name'):a for a in d.get('animations',[])}
 if len(set(clipmap.values()))!=len(STATES):raise ValueError('Core state fallback/alias is not standardized')
 for state in STATES:
...
def validate(bundle):
 ...
 if kind not in ('character','vfx-library'):raise ValueError('Unknown profile')
 ...
 if not vfx:raise ValueError('No standardized VFX definitions')
 result={}
 if kind=='character':
  rel=resource['model'];p=local(content,rel);doc=read(p);required.add('content/'+rel);plan.append(dict(kind='model',path=str(p)))
  model=local(content,doc['glbPath']);required.add('content/'+doc['glbPath']);result=glb(model,doc['clipMap'])
  scripts=resource.get('scripts',[])
  if not scripts:raise ValueError('Missing bound VFX event scripts')
  for rel in scripts:
 ...
 policy=read(ROOT/'policy.json');repo=(ROOT/policy['contract_repo']).resolve()
