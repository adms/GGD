...
p=argparse.ArgumentParser();p.add_argument('--control',type=Path,required=True);p.add_argument('--input-intake',type=Path,required=True);p.add_argument('--output-root',type=Path,required=True);p.add_argument('--decoder',type=Path,default=Path('/private/tmp/ggd-vgmstream-r2117/cli/vgmstream-cli'));a=p.parse_args();root=a.input_intake.resolve();out=a.output_root.resolve();out.mkdir(parents=True,exist_ok=True)
...
records=[];unrecovered=[]
for dp in sorted(root/'decoded'/(n+'.ja_JP')/'decoding.json' for n in scope['allowedFreezeNames']):
