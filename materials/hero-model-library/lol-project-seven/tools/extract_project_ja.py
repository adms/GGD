...
def main():
    ...
    paths = sorted((source_dir/(n+'.ja_JP.wad.client') for n in scope['allowedFreezeNames']),key=lambda p:p.name)
    if any(not p.is_file() for p in paths): raise ValueError('All seven authorized Japanese WADs required')
    packages = []
    for path in paths:
        ...
        data = dict(schema='ggd-lol-audio-extraction@1', nativeId=name, originalPath=str(path),
                    ...
                    languageEvidence='official Riot manifest explicitly declares ja_JP and source WAD suffix matches; internal en_us paths are not listening evidence',
    ...
    # Recompute progress after all resumed packages; old receipts remain unchanged.
    summary = dict(schema='ggd-lol-audio-batch@1', localRoot=str(root), sourceDirectory=str(source_dir),
                   expectedWads=len(paths), completeWads=len(packages),
                   nativeBanks=sum(len(p['nativeAudioBanks']) for p in packages),
                   wemFiles=sum(len(p['media']) for p in packages),
