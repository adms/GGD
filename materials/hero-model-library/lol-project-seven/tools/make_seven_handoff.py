...
p=argparse.ArgumentParser();p.add_argument('--control',type=Path,required=True);p.add_argument('--metadata-root',type=Path,required=True);p.add_argument('--metadata-link-root',type=Path);a=p.parse_args()
meta=a.metadata_root.resolve();linkMeta=(a.metadata_link_root or meta).resolve();scope=load_scope(a.control);pointer=meta/'latest-seven-delivery-receipt.json';receipt=json.loads(pointer.read_text());manifest=Path(receipt['manifestPath']);assert sha(manifest)==receipt['manifestSha256'];d=json.loads(manifest.read_text());root=Path(d['localRoot']);assert d['locale']=='ja_JP' and len(d['packages'])==7
