...
def main():
    ...
    expected={n+'.ja_JP' for n in scope['allowedFreezeNames']}
    if set(batch['receipts']) != {'packages/'+n+'/extraction.json' for n in expected}: raise ValueError('Unexpected receipt path')
    if {p.parent.name for p in receipts} != expected: raise ValueError('Refusing non-seven-package decode batch')
