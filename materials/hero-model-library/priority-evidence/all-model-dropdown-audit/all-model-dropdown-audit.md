...
- `content/models` 有 1000 份 `model@1`。
- 正式 champion 的主模型與 `modelVersions` 合計 510 個不同可選 model key；`modelVersions` 有 563 列。
- Hero Forge 有 34 名英雄、38 個不同模型選項。
- 中央索引有 156 筆來源；143 筆已被 champion／Hero Forge 直接或透過 frozen version 表示。未註冊中，9 筆已有通過資格證據，4 筆只證明 Git 來源存在，後台導入驗收仍未通過。
- 中央登記旗標與實際參照不一致：0 筆。
- `componentReady` 但尚未註冊：角色／動作元件 26 筆、武器元件 2 筆。它們不是完整英雄，不能自動塞入下拉。
...
| `historical-jetragon-7bc2fa3f8` | 空渦龍 | independent-historical-model-body-component | — | no-ggd-hero-id-or-target-binding; no-runtime-model-at-1-document; component-is-not-runtime-selectable |
| `historical-kita-kita-7bc2fa3f8` | 吉他吉他老伯 | independent-historical-model-body-component | — | no-ggd-hero-id-or-target-binding; no-runtime-model-at-1-document; component-is-not-runtime-selectable |
| `historical-lord-nightmares-7bc2fa3f8` | 金色魔王／惡夢之王 | independent-historical-model-body-component | — | no-ggd-hero-id-or-target-binding; no-runtime-model-at-1-document; component-is-not-runtime-selectable |
...
目前 1000 / 1000 份 `model@1` 都沒有 `resourceRole`；`current-resources.json` 也沒有 VFX component collection。因此本報告不猜測未參照檔案是否為特效。需先在中央產生器加入 `resourceRole/assetKinds`、取得狀態與驗證狀態，才能產生完整的未使用特效清單。
