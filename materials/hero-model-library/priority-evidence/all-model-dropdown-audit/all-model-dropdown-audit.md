...
- `content/models` 有 1067 份 `model@1`。
- 正式 champion 的主模型與 `modelVersions` 合計 570 個不同可選 model key；`modelVersions` 有 604 列。
- Hero Forge 有 34 名英雄、46 個不同模型選項。
- 中央索引有 205 筆來源；191 筆已被 champion／Hero Forge 直接或透過 frozen version 表示。未註冊中，9 筆已有通過資格證據，5 筆只證明 Git 來源存在，後台導入驗收仍未通過。
- 中央登記旗標與實際參照不一致：0 筆。
...
- 其他未被中央驗證索引或英雄下拉解釋的 `model@1`：182 筆。部分已有技能／VFX 等非英雄參照，其他缺權威角色欄位；完整逐筆清單在 JSON。
...
目前 1067 / 1067 份 `model@1` 都沒有 `resourceRole`；`current-resources.json` 也沒有 VFX component collection。因此本報告不猜測未參照檔案是否為特效。需先在中央產生器加入 `resourceRole/assetKinds`、取得狀態與驗證狀態，才能產生完整的未使用特效清單。
