...
素材、原始下載、半成品與封存放 S3，本機保留。管理腳本、索引、配方、GGD 定義 JSON 與文件由 Git 保管；S3 必要的 runtime JSON／查詢索引仍可隨成品發布。原生格式解析輸出的約 3.9 萬份大型 JSON 已確定由 S3 保管，排除於 Git；這是已確認的存放分工。版本清單與 SHA-256 索引仍由 Git 管理。規則見 [STORAGE_POLICY.json](STORAGE_POLICY.json)。
...
Git 管理資料已納入 `adms/GGD` 的分支 `codex/asset-library-management`，位於 `materials/asset-library/`；發布狀態以遠端分支與 PR 為準。位置及發布狀態見 [GIT_SOURCE.json](GIT_SOURCE.json)。
