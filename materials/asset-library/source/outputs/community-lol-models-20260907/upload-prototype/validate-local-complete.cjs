...
(async () => {
  ...
  for (const name of fs.readdirSync(source).filter(name => name.endsWith('.glb')).sort()) {
    ...
    const report = await validator.validateBytes(new Uint8Array(bytes), { uri: name, format: 'glb', maxIssues: 0, writeTimestamp: false, externalResourceFunction: async () => { throw new Error('External resources are forbidden for this local upload probe'); } });
