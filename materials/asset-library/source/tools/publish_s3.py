...
def build():
 ...
 with tempfile.TemporaryDirectory(dir=stage) as t:
  payload=Path(t)
  for rel in ['hero-model-options.json','spider-identity.json','catalog.json','policy.json','query.py','tools/admit.py','tools/validate-schema.mts','BACKUP_README.md','BACKUP_POLICY.json','AGENTS.md']:
  ...
  for entry in catalog['entries']:
   ...
   else:resources.append(dict(id=entry['id'],kind='model' if entry['kind']=='model-body' else 'character',bundle_id=entry['id'],path=entry['path']))
...
def publish(dest,pointer):
 ...
 for name in ['hero-model-options.json','spider-identity.json','BACKUP_README.md','BACKUP_POLICY.json','AGENTS.md']:
 ...
 for name in ['hero-model-options.json','spider-identity.json','catalog.json','resources.json','README.md','COPY_TO_WORKFLOW.txt','DOWNLOAD.py','BACKUP_README.md','BACKUP_POLICY.json','AGENTS.md']:
 ...
 for name in ['hero-model-options.json','spider-identity.json','current.json','catalog.json','resources.json','README.md','COPY_TO_WORKFLOW.txt','DOWNLOAD.py','BACKUP_README.md','BACKUP_POLICY.json','AGENTS.md']:
  with tempfile.TemporaryDirectory() as t:
 ...
 receipt=dict(status='published_and_read_back_verified',checked_at=datetime.now().astimezone().isoformat(),role_arn=identity['Arn'],
              ...
              verified_release_files=len(checked),verified_control_files=11,archive_sha256=pointer['archive_sha256'],
