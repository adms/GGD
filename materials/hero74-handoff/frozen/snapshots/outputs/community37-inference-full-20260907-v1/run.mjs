...
if(mode==='prepare'){
 ...
 const cases=all;assert.equal(cases.length,555);assert.equal(new Set(cases.map(c=>c.heroIndex)).size,37);assert.equal(new Set(cases.map(c=>c.sourceId)).size,259);
 const keys=new Set(cases.map(c=>c.heroIndex+'.'+c.slot));
 ...
 put('PREPARATION.json',{authorizedAction:'User: 可以推論了; inference only, not training',createdAt:new Date().toISOString(),count:555,plannedCalls:1665,models:['base','r3','r7'],selection:'Entire frozen corrected corpus; prior 43-case diagnostic already exposed; not an independent unseen release test',selectedSourceUnits:[...keys],pins,trainingCallsAllowed:false,oldCancelMarkersUntouched:true,cloudGpu:false});
 ...
 assert.equal(prep.trainingCallsAllowed,false);const token=read(path.join(here,'token-budget.json'));assert(token.allFit&&token.cpuOnly&&token.count===555);assert.equal(token.requestsSha256,await hash(path.join(here,'requests.json')));
 ...
 const start=Date.now(),cutoff=start+18*60*1000;
 ...
 try{
  for(const name of ['base','r3','r7']){
   ...
   const raw=read(output);assert(raw.complete&&raw.results.length===555);assert.equal(raw.metadata.thinking,false);assert.equal(raw.metadata.modelPath,refs.r3.base);assert.equal(raw.metadata.adapter,adapter);assert(raw.metadata.peakMetalBytes<=12*1024**3);
   arms.push({name,raw});state.arms.push({name,rows:555,seconds:(Date.now()-t)/1000,peakGiB:raw.metadata.peakMetalBytes/1024**3});update();console.log(JSON.stringify(state.arms.at(-1)));
  }
 ...
 finally{clearInterval(timer);clearTimeout(killTimer);if(fs.existsSync(lock)&&read(lock).pid===process.pid)fs.unlinkSync(lock);state.workerPid=null;state.lockReleased=true;update();put('summary.json',{...state,plannedCalls:1665,completedCalls:state.arms.reduce((n,a)=>n+a.rows,0),releaseQualified:false,newDatasetTrainingRun:false,scope:'Full corrected-corpus existing-model source-entailment diagnostic; not template/generation/visual qualification'});}
}
