...
export interface DamagePacket {
  hitDelivery?: import("./hitDelivery").HitDelivery;
...
export function combatResolveSystem(world: SimWorld): void {
  ...
  for (let pass = 0; pass < DAMAGE_QUEUE_MAX_PASSES && world.damageQueue.length > 0; pass++) {
    ...
    for (const pkt of batch) {
      ...
      const triggerBase = {
        ...(pkt.hitDelivery !== undefined ? { hitDelivery: pkt.hitDelivery } : {}),
      ...
      // An active defense claims the remaining real hit before resource shields
      // or life-saving marks are spent. Legacy reflect hooks retain their phase.
      const defended = dmg > 0 && fireHooks(world, pkt.target, "onDamageTaken", pkt.source,
        undefined, { ...triggerBase, hpLost: 0, defenseDamage: dmg }, undefined,
        hook => hook.defenseWindow === true && hookNegatesDamage(hook), 1) > 0;
      const defenseCut = defended ? dmg : 0;
      if (defended) dmg = 0;
      ...
      const negated = !defended &&
        fireHooks(
          ...
          undefined,
          hook => hook.defenseWindow !== true && hookNegatesDamage(hook),
        ) > 0;
      ...
      recordDamage(world, pkt.source, pkt.target, impact, Math.max(0, dmg), mitigatedByResist + shieldAbsorbed + blockCut + defenseCut, pkt.origin);
      ...
      const blocked =
        defended || shieldAbsorbed > 1e-9 || blockCut > 1e-9 || hasDamageReductionBuff(world, pkt.target);
