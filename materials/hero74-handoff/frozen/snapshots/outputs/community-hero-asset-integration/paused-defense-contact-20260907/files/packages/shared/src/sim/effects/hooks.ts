...
import { hitDeliveryPasses } from "../combat/hitDelivery";
...
export function fireHooks(
  ...
  /** An interception is claimed by at most one successful source. */
  maxFires: number = Number.POSITIVE_INFINITY,
): number {
  let fired = 0;
  ...
  sources: for (const src of sc.sources) {
    ...
    for (let hi = 0; hi < src.hooks.length; hi++) {
      ...
      if (hook.defenseWindow === true && (incoming === undefined || !(incoming.defenseDamage! > 0) ||
          incoming.reflectDepth !== 0 || target === undefined || target === owner)) continue;
      if (hook.damageDelivery !== undefined && hook.damageDelivery !== "any" &&
          (incoming === undefined || !hitDeliveryPasses(hook.damageDelivery, incoming.hitDelivery))) continue;
      ...
      for (const creditCast of creditCasts) {
        ...
        if (hook.defenseWindow === true) {
          const pos = world.transform.get(owner)?.pos;
          world.emit("defenseSuccess", { defender: owner, attacker: target, sourceId: src.id,
            ...(src.defenseAbilityId !== undefined ? { abilityId: src.defenseAbilityId } : {}),
            amount: incoming!.defenseDamage, delivery: incoming!.hitDelivery, x: pos?.x, z: pos?.z });
        }
        fired++;
        if (fired >= maxFires) break sources;
      }
    }
  }
