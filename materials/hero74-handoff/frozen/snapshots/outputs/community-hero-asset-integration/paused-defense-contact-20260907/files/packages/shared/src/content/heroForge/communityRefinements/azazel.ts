...
export function refineAzazelProject(input: HeroProject): HeroProject {
  ...
  active("Q", "targeted", 0.2, [effect({ ...hit("physical"), contact: true }), effect({ kind: "knockback", distance: 0.4, speed: 8, subtractGap: false })],
  ...
  active("E", "self", 0, [effect({ kind: "applyBuff", applyTo: "self", duration: 0.5, modifiers: [],
    statusId: ids.counter, stackKey: ids.counter, maxStacks: 1, hooks: [zHookDef.parse({
      on: "onDamageTaken", victim: "enemy", defenseWindow: true, damageDelivery: "contact", maxTriggers: 1, onConsumed: "detachSource",
      condition: { all: [
