...
export interface TriggerDamage {
  readonly hitDelivery?: import("../combat/hitDelivery").HitDelivery;
  /** Positive damage left after ordinary block, before shield/resource spending. */
   ...
   * 封包原本的量,未經護甲/魔抗。
   *
   * ⚠️ 這是**全域傷害倍率之後**的讀數 —— 除非那一發封包免除了倍率,那時它就
   * 只是封包自己的量。這句話很重要:三個讀數全都在倍率之後,所以一發**由它們
   * 算出來**的反彈封包如果再走一次倍率,倍率就進去了兩次。
   *
   * ⭐ 免除有**兩種**(GH#929 之後 —— 在此之前這一行寫著「目前只有反彈會」,
   * 而那句話從 2026-09-02 起就是假的):
   *   · `DamagePacket.skipGlobalDamageMult` —— **整發**免除。只有[反彈]
   *     (`damage.incomingPct`)會設,理由是避免乘兩次。
   *   · `DamagePacket.unscaledFraction` —— **一部分**免除。只有「某一條血條的
   *     百分比」且 `damageType: "true"` 的那一份會設,理由是卡面寫的 X% 要真的
   *     是 X%(owner 2026-09-02:「生命百分比傷害若是[真實傷害]則不列入 AP
   *     乘數中,因為真實傷害沒有魔抗來制衡」)。
   * ⇒ 一發帶 `unscaledFraction` 的封包,它的 `raw` 是「級距那一份乘過、百分比
   *   那一份沒乘」的和。⛔ 讀這三個數去反推倍率的東西要知道這件事。
   */
  readonly raw: number;
...
export interface EffectContext {
  hitDelivery?: import("../combat/hitDelivery").HitDelivery;
