 ...
 */
import { z } from "zod";
import { HIT_DELIVERY_FILTERS } from "../../../sim/combat/hitDelivery";
...
export function refineHookDamageContext(
  hook: {
    ...
    defenseWindow?: boolean | undefined;
    damageDelivery?: string | undefined;
    maxTriggers?: number | undefined;
    onConsumed?: string | undefined;
  ...
  if (hook.defenseWindow === true) {
    if (hook.on !== "onDamageTaken" || !negates || hook.maxTriggers !== 1 ||
        hook.onConsumed !== "detachSource" || hook.perTarget === true ||
        !["direct", "contact", "projectile"].includes(hook.damageDelivery ?? "")) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["defenseWindow"], message:
        "主動防守窗口需要 onDamageTaken、直接命中類型（direct/contact/projectile）、免除原傷害、maxTriggers=1 與用完卸除來源；不能按目標分別重複反擊。" });
    }
  }
  if (hook.damageDelivery !== undefined && hook.damageDelivery !== "any" && !DAMAGE_BEARING_EVENTS.includes(hook.on)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ["damageDelivery"], message: "命中類型只適用於攜帶實際傷害封包的事件。" });
  }
...
export const zHookDefBase = z
  .object({
    defenseWindow: z.boolean().optional().describe("主動防守窗口：普通格擋後仍有有效傷害時，承接一次直接命中，再處理護盾、魔力屏障與免死。其他窗口不會重複宣告同次成功；反擊不會觸發另一防守窗口。"),
    damageDelivery: z.enum(HIT_DELIVERY_FILTERS).optional().describe("實際命中方式：contact 近身接觸、projectile 投射物、direct 前兩者、periodic 持續傷害、other 未分類。按發生路徑判定，不按角色當下攻擊射程猜測。"),
    on: zHookEvent,
    /**
