...
class Body:
    ...
    def ellipsoid(self, center, scale, bone, color, rings=12, segments=16):
        ...
        for row in range(rings):
            for col in range(segments):
                ...
                if row != 0: self.indices.extend([a, a + 1, b])
                if row != rings - 1: self.indices.extend([a + 1, b + 1, b])
    ...
    def tube(self, points, radius, bone, color):
        ...
        for i in range(len(points) - 1):
            for j in range(10):
                ...
                self.indices.extend([a, b, a + 10, b, b + 10, a + 10])
    ...
    def build(self, style):
        ...
        doc['materials'] = [{'name': 'Cartoon palette', 'doubleSided': True,
...
def main():
    ...
    source = output / 'generation-source.py'; source.write_bytes(raw)
