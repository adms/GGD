...
/**
 * 守衛**母體**的三種 op —— 與清單頁（gen.mjs 的 POSITIVE_OPS）同一份口徑。
 * ⚠️ 母體比梯子寬是刻意的：`flat` 列也是「31 列」的一員，它**必須**被豁免規則
 * 罩住而不是從母體消失 —— 消失的話 op=flat 那條豁免會被反向斷言判成過期，
 * 而「為什麼 flat 不在梯子上」這個理由就沒有任何地方受檢。
 */
export const MS_SCAN_OPS = ["flat", "pctAdd", "pctMult"] as const;
...
export const DEFAULT_MOVE_SPEED_TIERS: MoveSpeedTiers = Object.freeze({
  ...
  exemptions: Object.freeze([
    ...
    {
      id: "godie-uvng.passive",
      reason:
        "38-00 邪眼全開的**變身對子**：本體是 `pctAdd 0.16`，變身態 godie-u010 是 " +
        "`flat 0.97`（u/s，已被 op=flat 罩住）。⭐ 只級距化本體那一邊 ⇒ 變身對帳閘紅" +
        "「只動了一邊」，而玩家變身之後用的是變身態那一份。⇒ 一支技能的兩個載體" +
        "要嘛一起上梯子、要嘛一起不上；flat 那半上不了（單位不同）。" +
        "反駁方式：哪天 flat 有了自己的 u/s 梯子，兩邊一起級距化，這一條就刪。",
    },
  ]),
});
...
export function scanMsBonusNodes(file: string, doc: unknown): MsBonusNode[] {
  ...
  const visit = (node: unknown, path: string): void => {
    ...
    if (
      rec["stat"] === "ms" &&
      (MS_SCAN_OPS as readonly string[]).includes(rec["op"] as string)
    ) {
      const tier = rec[MS_BONUS_TIER_FIELD];
