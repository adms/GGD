...
export const zRankingHumanMultiplier = z
  .object({
    minHumans: int(RANKING_BOUNDS.minHumans).describe(
      "**整場**至少要有幾個真人，這個 lobby 才算「真人賽」。沒到門檻 → 倍率 1，" +
        "賽季積分與 MMR 都照原本的量走。⚠️ 數的是整場（不分陣營），⛔ 不是自己這一隊 —— " +
        "所以一個人帶朋友進來就開得起加成，⛔ 不必湊滿兩隊。調高＝更難觸發加成，" +
        "調到 1＝連單人 bot 局都算真人賽（那會讓刷 bot 局變成最有效率的爬分手段）。",
    ),
    offset: int(RANKING_BOUNDS.offset).describe(
      "倍率 = 真人數 **+ 這一格**。出貨 1 ⇒ 2 個真人 = 3 倍、12 個真人 = 13 倍（owner 的原話）。" +
        "調到 0＝倍率就等於真人數本身（2 人只有 2 倍）。⚠️ 它同時把**每一級**都往上推，" +
        "所以它動的是整條曲線的高度，不是只有起點。",
    ),
    maxMultiplier: int(RANKING_BOUNDS.maxMultiplier).describe(
      "倍率的天花板。出貨 13 ＝ 12 人滿房 +1（owner 的「最大 13 倍」）。" +
        "⚠️ 它是**保險絲**不是平衡旋鈕：房間人數上限變大時，沒有這一格就會讓一場的產出" +
        "跟著人數無限長。調低＝滿房與半房的差別被抹平。",
    ),
  })
  .strict();
...
export const zRankingShare = z
  .object({
    seasonPointsPct: int(RANKING_BOUNDS.sharePct).describe(
      "**賽季積分**（含每位英雄的積分）吃多少倍率。100（出貨值）＝ 吃滿，13 倍的 lobby " +
        "就是 13 倍的名次分；50＝只吃一半的增幅；0＝賽季積分完全不理會真人倍率。" +
...
export const zRankingRivalry = z
  .object({
    basePct: int(RANKING_BOUNDS.rivalryPct).describe(
      "勝負持平、而且**初次交手**時，打贏這個對手額外拿到的百分比。它是整條曲線的高度：" +
        "所有遞減都是從這個數字往下折。0＝整個宿敵系統關掉（一鍵 rollback）。",
    ),
    halfLife: int(RANKING_BOUNDS.rivalryHalfLife).describe(
      "**淨勝場**每增加大約這麼多，加成砍半。出貨 3 ⇒ 贏一個過去把你打爆的對手加成最高，" +
        "而重複輾壓同一個人時淨勝一路上升、加成一路掉。調大＝加成掉得慢（輾壓同一個人比較久還有賺頭）。",
    ),
    repeatHalfLife: int(RANKING_BOUNDS.rivalryHalfLife).describe(
      "⭐ **反刷分的閘**：這一對**打過的總場數**每增加大約這麼多，加成砍半 —— ⛔ 不管勝負怎麼分。" +
        "⚠️ 少了這一項，兩個帳號輪流讓對方贏就能把**淨勝永遠壓在 0**，於是上面那一項永遠是滿的、" +
        "加成也永遠是滿的。加上它之後，同一對帳號打得越多這條路的產出越接近零，" +
        "而**跟不同的人打**才拿得到加成 —— 這正是 owner 要的「獎勵大家多打真人賽」。" +
        "調大＝互餵分的窗口變寬。",
    ),
    maxPct: int(RANKING_BOUNDS.rivalryPct).describe(
      "一場之內**所有**被打敗的對手的宿敵加成加起來的上限（單一對手也吃這個上限）。" +
        "⚠️ 它擋的是「一場打贏六個宿敵」把加成疊成天文數字；MMR 那一側還要再吃一次「K 值上限」。",
    ),
  })
  .strict();
