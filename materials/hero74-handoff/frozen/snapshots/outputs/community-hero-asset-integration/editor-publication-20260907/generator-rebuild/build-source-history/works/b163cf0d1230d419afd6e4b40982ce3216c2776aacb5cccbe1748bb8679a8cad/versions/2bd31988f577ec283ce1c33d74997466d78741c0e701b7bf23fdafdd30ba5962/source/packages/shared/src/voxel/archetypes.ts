 ...
 *
 * SHARED CORE (task #229). Re-homed from `tools/voxel-gen/archetypes.ts` — see
 * the boxman.ts header. The admin studio (鑄形工坊) opens on one of these five
 * presets and edits from there, so what an operator starts from is literally
 * what the bake ships, not a look-alike default authored twice.
 */
