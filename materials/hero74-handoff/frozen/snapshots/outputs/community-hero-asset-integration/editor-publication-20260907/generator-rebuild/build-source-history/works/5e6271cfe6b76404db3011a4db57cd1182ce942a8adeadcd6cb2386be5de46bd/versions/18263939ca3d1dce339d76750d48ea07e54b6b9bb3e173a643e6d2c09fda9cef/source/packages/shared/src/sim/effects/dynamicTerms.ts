 ...
 * 500%」,在 diff 裡跟正確值長得一模一樣,而在場上是一擊必殺。
 *
 * ⚠️ 這一段以前接著寫「1.0 讓整條寫得出來,**再往上就只可能是錯的**」——
 * 而那句話 2026-08-13 被 owner 的規格證偽了(第三守則:註解會說謊):
 * 20-002 解放.約束勝利劍MAX 逐字寫著「(**[現存魔力]**+[AP])×**7倍**傷害」。
 * 那不是打錯的數字,是設計(第〇·六守則第 1 層贏過 repo 自己的政策)。
 */
export const RESOURCE_PCT_RATIO_MAX = 1;

/**
 * `subject: "self"` 時的上界 —— **刻意比 `RESOURCE_PCT_RATIO_MAX` 鬆**。
 *
 * ⭐ 為什麼可以分開,而不是把上面那個一起放寬:上面那個守衛擋的失敗**整個
 * 掛在 `subject: "target"` 上** —— 讀**對方**的條、係數打錯一個小數點,就是
 * 一擊必殺,而且愈是被你打到快沒魔力的人愈死不了(分母是他的條)。
 * 讀**自己**的條完全是另一回事:量級被我自己的池子綁死,打錯只會傷到自己的
 * 期望值,⛔ 不會變成一個對誰都成立的處決。
 *
 * 一起放寬 = 為了讓一支技能寫得出來,把那個真的很危險的形狀也一起開了。
 * 所以這是兩個常數,不是一個被調大的常數。
 */
export const RESOURCE_PCT_RATIO_SELF_MAX = 10;
