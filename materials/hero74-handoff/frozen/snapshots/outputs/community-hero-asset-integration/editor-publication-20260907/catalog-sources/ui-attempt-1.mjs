...
const before=hashes();writeFileSync(out+'/content-before.json',JSON.stringify(before));
...
try {
 ...
 await c.evaluate(`Array.from(document.querySelectorAll('button')).find(x=>x.textContent==='保存目前完整版本').click()`);
 await wait(async()=> (await c.evaluate('document.body.innerText')).includes('目前完整資料已保存。'),'Capture failed: '+await c.evaluate('document.body.innerText.slice(-1500)'));
 ...
 const expected=readFileSync(process.cwd()+'/tools/skill-remake/heroes/godie-e00s.py','utf8');assert(preview.generatorSources.every(x=>x.source===expected));
