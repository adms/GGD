 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ⚠️ `streakTimeoutSec` 缺席 = **回合內**永不逾時，⛔ 但**不再跨回合**
 *
 * owner 2026-08-18：
 *
 ...
 * >  **但的確不應該跨回合殘留**」
 *
 * ⇒ 回合內的打斷交給實際戰鬥（那是設計，不是漏洞）；跨回合殘留則由
 * `clearRoundScoped`（`sim/clearPools.ts`）在 host 的 `enterCombat()` 裡
 * **把這張表對每一個席位歸零**。⛔ 出貨刻意**不填** `streakTimeoutSec`：
 * 那是一個平衡數字，而回合邊界是結構性的事實 —— 用一個猜的秒數去模擬它
 * 只會在回合長度變動時（決賽 180 秒 vs 平時 100 秒、火圈提前收場）失準。
 *
 * ⚠️ 這一段在 2026-08-18 之前寫著「它也**跨回合**⋯⇒ 填上任何一個
 * `streakTimeoutSec` 就同時修掉這件事」，那句話現在是假的（第三守則）。
 *
