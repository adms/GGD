 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ⑥ ⭐ G10 —— 延長**也延長得到那個具名標記**，而這一支一行都沒改
 *
 * 實測過的缺陷：延長把 buff 的到期從 361 推到 573，而 52-02 讀的〔狂怒〕status
 * 停在 361 —— 玩家還在狂怒中，判定的那個閘已經關了。
 * ⛔ 修法**不是**在這裡多開一格 `extendBuff.statusId` 然後順便也去改 status
 *（那是替同一個問題做第二套機制，而且每多一個消費端就多一次串接）。修法在
 * `applyBuff.statusId`：標記騎在**同一份 `ModifierSource`** 上，所以下面那行
 * `src.expiresAtTick = …` 同時就是「標記延長了」。讀取端是 `effectCommon.ts` 的
 * `hasStatus` / `statusStacks`。
 * ⚠️ 因此 52-01 的〔狂怒〕必須用 `applyBuff{ stackKey, statusId }` 一格寫完 ——
 * 拆成 `applyStatus` + `applyBuff` 兩格的文件，這一支仍然只延長得到後者。
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ⑦ purity
 *
