...
try {
 for(const [cam,clip] of [['select','idle'],...Object.keys(model.clipMap).map(x=>['combat',x])]) {
  ...
  const boot=await c.evaluate(`import('/src/content/bootContent.ts').then(x=>x.getContentBootSnapshot())`);assert.equal(boot.result.ok,true);assert.equal(boot.result.overlayGeneration,14);assert.equal(boot.result.overlayError,undefined);assert.equal(boot.result.championCount,71);
