  ...
  passive={"name": "59-03 AT力場", "ranks": [
      {"hooks": [{"on": "onInterval", "internalCooldown": 8.0, "target": "self",
       ...
       "block": {"damageTypes": ["physical"], "chance": c, "fraction": 0.5,
                 ...
                 # ⚠️ ⭐ 為什麼**不是**一份新的六角力場面：`content/vfx/` **662 份**裡
                 #    ⛔ 零個 hex/force-field 資產（2026-09-01 逐份掃過）。
                 #    ⭐ 用既有家族是**做得到的那一半**（第一·五守則）；六角面要新美術 ⇒ 資產批。
                 #
                 # ⛔⛔ 2026-09-01 更正 —— owner **第二次**回報「格擋成功沒出現橘色光盾線條」，
                 #    ⭐ 而量下去發現機制**五段全通**（內容宣告 ✓ · rankBlock 掛得上（Q/W/E/R 都走）✓
                 #    · emitBlockVfx 發得出 ✓ · fanout 放行 ✓ · 客戶端 case 消費 ✓）。
                 # ⇒ ⭐ 真正的缺陷在**選的那顆特效**：
                 #      · 被取代的泛用火花 = 點狀 `HitSpark`，**260 ms**
                 #      · 舊的選擇 `burst.physical.s100` = `sphere` burst 48 顆，**160–460 ms**
                 #    ⇒ ⭐⭐ **取代品與被取代的東西是同一個視覺類別** ——
                 #      ⛔ 於是「機制生效」與「機制沒生效」在畫面上**長得一模一樣**，
                 #      而那正是 owner 說「修了又沒真的修」的量測答案。
                 #
                 # ⭐ 改成 `shockwave-ring`：emitter 是 **`ring`**（⛔ 不是 sphere）——
                 #    一圈**環**繞著被擊中的人，與點狀火花是**不同的類別** ⇒ 分得出來。
                 #    ⚠️ `artParams.ts:118` 的 scale 對 `ring` **也放大半徑** ⇒ 2.6 讓它真的圍住
                 #    身體（0.45 × 2.6 ≈ 1.17 世界單位），⛔ 不是原地變大的一團。
                 # ⛔ 一鍵 rollback ＝ 把這三行改回 burst ⇒ 逐位元回到舊行為。
                 "vfxId": "fx.fam.shockwave-ring.physical.s150",
                 "vfxScale": 2.6,
                 "vfxTint": [255, 140, 40]}}
