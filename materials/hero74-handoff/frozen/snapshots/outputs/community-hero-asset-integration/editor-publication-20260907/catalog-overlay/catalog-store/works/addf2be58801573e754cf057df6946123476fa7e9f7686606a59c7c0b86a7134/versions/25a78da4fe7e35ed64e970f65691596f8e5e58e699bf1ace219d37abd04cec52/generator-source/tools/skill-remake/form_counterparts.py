...
COUNTERPART_OWNER = {
    "godie-e007": (AUTHORED, (
        ...
        " ⛔⛔ **12-04 被同一天的產物重生成打回去過一次**（GH#854，2026-08-29）："
        "`ddbd65199` 對齊了說明與 `ap` 係數 2.0，而 33 分鐘後 `936cda27b` 把兩者"
        "**還原成 w3x 匯入版**（1.3 ＋「集氣每秒⋯最多 3 秒」）。⭐ 兇手量到了："
        "`tools/ap-conversion/claims.json` 還留著 `godie-e007.r` 的**換算前快照**，"
        "而 `apply.py::apply_doc()` 的第一件事就是 `out[\"description\"] = entry[\"description\"]`"
        "＋ 用快照的 `claims` 重算 `ratios` ⇒ ⭐ **那兩個欄位的作者是 apconv，⛔ 不是內容樹**。"
        "⇒ 修法是**刪掉那一筆過期的凍結條目**（記憶 `ggd-normalizer-stale-claims`："
        "「重寫/鏡射技能後 sync 又把它改回去＝claims 表過期，**刪舊條目**不是重改內容」）——"
        "那支技能今天的卡面已經是 owner 規格的 `200% [AP]`，⛔ 沒有任何屬性宣稱可以換算，"
        "一筆換算前快照留在表上只會**每跑一次 sync 就把設計覆蓋一次**。"
        " ⚠️ ⭐ 而 `castTimeSec` 是**推導**的（`deriveCastTimes.ts`）：兩形態現在都走"
        "`cls=\"authored\"`（說明裡的 `吟唱{{cast}}秒`）⇒ 同一組輸入、同一個答案 1 秒。"
        "⛔ 在此之前變身態沒有那一行 ⇒ 落進 `scored` 分支（score 0.2225 ⇒ 1.233 秒）"
        "—— ⭐ **對齊的是機制（說明＋係數），⛔ 不是手寫一個秒數**。"
    )),
    "godie-e00l": (AUTHORED, (
