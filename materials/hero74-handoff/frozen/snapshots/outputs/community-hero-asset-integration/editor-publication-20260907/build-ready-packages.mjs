...
const root='/private/tmp/ggd-community37-editor-publish/ready-packages';
...
try{
 for(const [offset,entry] of bindings.slice(start,start+count).entries()){
  ...
  try{
   ...
   let downloaded=false;for(let i=0;i<100;i++){const bytes=await readFile(out+'/'+proof.downloadFile).catch(()=>null);if(bytes&&bytes.length>=22&&bytes.readUInt32LE(bytes.length-22)===0x06054b50){downloaded=true;break;}await wait(200);}
   ...
   const response=await until(`(()=>{const t=document.querySelector('.hero-community')?.innerText??'';return /完整英雄已送審|HTTP \d{3}|Error:/.test(t)?t:null;})()`,'submission response');
