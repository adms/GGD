...
const root='/private/tmp/ggd-community37-live-match',file='/private/tmp/ggd-model-upload-acceptance/service-config-private.json';
...
assert.equal(pid,94564);
assert(execFileSync('lsof',['-a','-p',String(pid),'-d','txt','-Fn'],{encoding:'utf8'}).includes('n/private/tmp/ggd-model-upload-acceptance/platform-draft-versions-v2\n'));
...
const child=spawn(root+'/platform-replay-budget',[],{cwd:process.cwd(),env:{...process.env,...config.platform.env},detached:true,stdio:['ignore',log,log]});child.unref();
config.platform.pid=child.pid;writeFileSync(file,JSON.stringify(config,null,2));
writeFileSync(root+'/platform-service-public.json',JSON.stringify({pid:child.pid,previousPid:pid,binary:root+'/platform-replay-budget',port:8091,dataDir:config.platform.env.DATA_DIR,scope:'Existing disposable platform, unchanged account and content storage.'},null,2));
